...
ネームスペースソースを設定することで、EMQXはクライアントの接続情報からネームスペースを識別し、クライアント接続時に対応するネームスペースを自動的に作成できます。
...
1. MQTTXを使ってテナント `tenantA` をシミュレートするMQTTクライアント接続を作成します。
...
- クライアントIDにネームスペースのプレフィックスが自動的に付与され、ネームスペース間でのクライアントIDの競合を防止します。
...
1. MQTTXを使って2つのMQTTクライアント接続を作成し、2つのテナント `tenantA` と `tenantB` をシミュレートします。
...
デフォルトでは後方互換性を維持するために、認可（ACL）チェックはトピックプレフィックス（マウントポイント）を含みません。つまり、認可ルールは名前空間付きトピック（例：`tenantA/test/topic`）ではなく、元のトピック名（例：`test/topic`）に対してマッチングされます。
...
例えば、トピックプレフィックス `tenantA/` を持つリスナー経由で接続したクライアントが `test/topic` をサブスクライブしたい場合、対応する認可ルールは `tenantA/test/topic` として設定しなければなりません。

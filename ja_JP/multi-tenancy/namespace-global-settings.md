...
**When to Resolve Namespace** で **After Authentication** モードが選択されている場合、認証前のネームスペースチェックはスキップされます。明示的に作成されたネームスペースのチェックは認証完了後に実行されます。
...
- **Before Authentication**（デフォルト）: 認証チェーンが実行される前にネームスペース式を評価し、その時点で利用可能な接続メタデータ（`username`、`clientid`、`cert_common_name`など）のみを使用します。これは設定ファイルの `mqtt.client_attrs_init` で `tns` を設定することに対応します。
...
**Take Namespace From** の式はVariform構文を使用します。利用可能な関数の詳細は [Variform Expressions](../configuration/configuration.md#variform-expressions) を参照してください。
...
- `PUBLISH`、`SUBSCRIBE`、`UNSUBSCRIBE`、およびWillメッセージ処理時：

...
このページでは、EMQX 6.1 の MQTT Streams 機能の使い方を説明します。MQTTX を使ってクライアントをシミュレートし、EMQX ダッシュボードからストリームを作成・管理し、メッセージの保存、リプレイ、圧縮の動作を体験します。
...
このセクションでは、MQTT Streams がメッセージを保存し、コンシューマーが過去のデータをリプレイできることを示します。
...
3. **Enable Streams** スイッチをオンに切り替えます。
...
4. 自動作成の設定を確認し、**Regular Stream** が使用されるようにします：
   ...
   > これにより、ストリームが Last-Value Stream として自動作成されることを防ぎます。Last-Value Stream はキーごとに最新のメッセージのみを保持します。

4. 変更を行った場合は、**Save Changes** をクリックして適用します。
...
### ステップ 3: ストリームからすべてのメッセージをリプレイ

MQTTX CLI を使い、MQTT 5 のサブスクリプションユーザープロパティ `stream-offset` を設定してストリームの先頭からメッセージをリプレイします。
...
MQTT Streams では、サブスクライバーが `stream-offset` の値を指定してリプレイ開始位置を制御できます。

この例では、特定の時点以降にパブリッシュされたメッセージのみをリプレイする方法を示します。

### ステップ 1: 現在のタイムスタンプを取得

新しいメッセージをパブリッシュする前に、現在の Unix タイムスタンプ（マイクロ秒単位）を記録します。

ミリ秒単位の現在時刻は以下で取得できます：
...
この値に 1000 を掛けてマイクロ秒単位に変換し、保存します。これがリプレイ開始位置になります。
...
異なるコンシューマーはそれぞれ独立して同じストリームを異なる位置からリプレイできます：

- あるコンシューマーは先頭（`earliest`）からリプレイ
- 別のコンシューマーは特定のタイムスタンプからリプレイ
- さらに別のコンシューマーは新規メッセージのみ（`latest`）を受信

各コンシューマーのリプレイ位置は独立しており、他のサブスクライバーに影響を与えません。

これにより、MQTT Streams におけるコンシューマー制御のリプレイが示されます。

## Last-Value セマンティクスのテスト

このセクションでは、Last-Value MQTT Streams がキーごとに最新のメッセージのみを保持し、状態表現に適していることを示します。
...
キー式が `message.from` に設定されているため、同じキーのストリームでは最新のメッセージのみが保持されます。
...
ストリームキー式はメッセージメタデータからクライアント ID を抽出してキーにしているため、両メッセージは同じキーを持ち、2つ目のメッセージが1つ目を上書きします。

### ステップ 4: ストリームにサブスクライブ

ストリームにサブスクライブし、先頭からリプレイします：
...
これにより、MQTT Streams が Last-Value セマンティクスを使った状態指向のメッセージングパターンをサポートしていることが示されます。
...
6. 以下のコマンドでサブスクライブして自動作成をトリガーします：

   ```bash
   mqttx sub -h localhost -p 1883 -q 1 -t '$stream/auto_stream/demo/auto' -up "stream-offset: earliest"
   ```

   手動作成ストリームと異なり、自動作成ストリームではサブスクライブ時にトピックフィルター（この例では `demo/auto`）が必要です。

   ストリームが存在しない場合、EMQX は：

   - `auto_stream` という名前の新しいストリームを作成し、
   - トピックフィルターを `demo/auto` に設定し、
   - 設定された自動作成タイプ（レギュラーまたは Last-Value）を適用します。

...
3. 各ノード上で以下の処理が行われます：
   - tarballがEMQXのルートディレクトリ内の`plugins`サブディレクトリに保存されます（`plugins.install_dir`オプションで上書き可能）：`$EMQX_ROOT/plugins/my_emqx_plugin-1.0.0.tar.gz`
...
プラグインの状態はEMQX設定内で`enable`フラグ（`true`または`false`）のみが保存されます。プラグインの完全な設定はノード上の`$EMQX_DATA_DIR/plugins/my_emqx_plugin/config.hocon`ファイルに保持されます。
...
`on_config_changed/2`コールバック関数はEMQXクラスターの各ノードで呼び出されます。ローカルシステムの状態に依存する設定検証（例：ネットワークの可用性チェック）は避けてください。ノード間で結果が不整合になる可能性があります。代わりに`on_health_check/1`を使用してランタイムチェックを行い、リソースが利用不可の場合は不健康状態を報告してください。
...
- インストールされたプラグインパッケージ内にバンドルされているデフォルトファイル：
  - Docker環境：
...
- DashboardやAPI経由で設定が保存された後、EMQXが管理する永続化されたプラグイン設定ファイル：
  - Docker環境：
    `/opt/emqx/data/plugins/my_emqx_plugin/config.hocon`
  - deb/rpm環境：
    `/var/lib/emqx/plugins/my_emqx_plugin/config.hocon`

`priv/config.hocon`はパッケージに含まれるデフォルトテンプレートであり、`data/plugins/.../config.hocon`はEMQXが設定変更を保存した後の永続化されたプラグイン設定ファイルです。

### 起動

プラグインはDashboard、API、またはCLIを通じて手動で起動します。起動時には：

- プラグインのアプリケーションが起動されます。
- プラグインはEMQX設定の`plugins.states`に`enabled`として登録されます。

プラグインが起動している場合、その情報取得時に`on_health_check/1`コールバック関数が呼び出され、プラグインの状態が取得されます。
...
プラグインのアプリケーションは停止されますが、コードはノード上にロードされたままです。停止したプラグインでも設定は可能です。
...
プラグインがすでにビルドされ、`my_emqx_plugin-1.0.0.tar.gz`というtarballが用意されているとします。Dashboardから直接コンパイル済みプラグインパッケージをインストールする手順は以下の通りです。
...
- クラスター環境では、インストール前にすべてのノードで許可を与える必要があります。
...
1. CLIで以下のコマンドを実行し、プラグインインストールを明示的に許可します。
   ...
   このコマンドを実行後、Dashboardからインストールを進められます。
...
以前に許可したプラグインの許可を取り消すには、以下のいずれかの方法を実行します。
...
プラグインがすでにビルドされ、`my_emqx_plugin-1.0.0.tar.gz`が用意されているとします。CLIから直接コンパイル済みプラグインパッケージをインストールする手順は以下の通りです。
...
プラグインがすでにビルドされ、`my_emqx_plugin-1.0.0.tar.gz`が用意されているとします。APIを使ってプラグインをインストールする手順は以下の通りです。

1. インストールを許可します。以下のコマンドを実行してプラグインインストールを許可します。
...
3. プラグイン一覧を確認し、インストールが成功したかを検証します。
...
EMQX起動時にプラグインを即座に利用可能にしたい場合（例：カスタムDockerイメージを作成する場合）、プラグインパッケージを展開して事前にEMQXを設定することが可能です。

以下はDockerfileの例ですが、deb/rpmやベアメタルなど他のデプロイ方法でも同様の手順が適用されます。

1. プラグインtarballをpluginsディレクトリにコピーし、展開します。
...
2. EMQXのベース設定ファイルにプラグインの自動起動設定を追記します。
   ...
   `enable = true`でプラグインを自動起動し、`enable = false`でインストールのみ行い起動はしません。

...
[SDKMAN](https://sdkman.io/) を使うのが、これらのツールをインストール・管理する推奨方法です。
...
server.addTool(
    ...
    description = "パネル上のライトを制御します。明るさを変更できます。ライトを消すには明るさを0に設定してください。明るさを 'last_value' に設定すると、前回の明るさに戻せます。これはライトが消灯中に再度点灯させたい場合に便利です。",
    inputSchema = ToolSchema(
        properties = buildJsonObject {
            putJsonObject("value") {
...
現時点で Kotlin SDK は MCPクライアントの実装を提供していません。  
...
async def main():
    async with mcp_mqtt.MqttTransportClient(
        ...
        await client.start()
        while True:
            # MQTTクライアントがバックグラウンドで動作している間、他の処理をシミュレート
...
この Pythonクライアントは MCPサーバー `demo/kotlin-mcp-server` を自動検出し、`set_light_brightness` ツールを呼び出してライトの明るさを50に設定します。

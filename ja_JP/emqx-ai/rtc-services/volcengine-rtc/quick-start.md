...
| コンポーネント | 役割                   | ポート | 主な責務                                                         |
| -------------- | ---------------------- | ------ | ---------------------------------------------------------------- |
| volc-server    | Volcano Engineプロキシ | 3002   | RTCルーム・トークン管理、CustomLLMのコールバックアドレス設定    |
...
sequenceDiagram
    ...
    WebUI ->> Volc: シナリオ設定およびRTC認証情報の要求

    WebUI ->> RTC: リアルタイム音声/映像接続の確立
    RTC ->> WebUI: リアルタイム音声/映像接続の確立
    note over WebUI, RTC: ASR / TTS

    Cloud ->> App: CustomLLMのコールバック（/chat-stream、SSEストリーミング応答）
...
- MCP over MQTT：EMQX ブローカーを介したネットワーク越えのツール呼び出し。AIエージェントがデバイス機能（カメラ、表情、音量）を制御
- マルチモーダル理解：VLMを統合し、「これ何を持っている？」などの視覚ユースケースに対応
- リアルタイム音声対話：Volcano Engine RTC＋ASR/TTSによるエンドツーエンドの低レイテンシ音声認識と合成
- 並列処理アーキテクチャ：ツール呼び出しと音声合成を非同期処理し、スムーズなユーザー体験を実現
...
2. 上部に有効化の案内があればクリックして有効化（無料枠を超えたAPI利用時のみ課金）
3. 必要に応じて本人確認を完了
...
4. APIキーの横のコピーアイコンからシークレットを取得
5. 取得したAPIキーを`app/.env`の`DASHSCOPE_API_KEY`に設定

#### 他モデルサービスの利用（任意）

別のOpenAI互換モデルサービスを使う場合は`app/.env`を更新：
...
RTCコンソールでクロスサービス認可を設定しないと、エージェントがASR/TTS/LLMサービスを正しく呼び出せません。
...
本プロジェクトはCustomLLMモードを使用し、Volcano EngineがアプリのカスタムAIエージェントサービスへコールバックしてLLM応答を取得します。
...
- `VOLC_LLM_API_KEY` - カスタム認証キー。アプリの`CUSTOM_LLM_API_KEY`と完全一致させる必要あり（後述の「ステップ2: 環境変数設定」参照）

任意のモデルソース：

- Volcano Ark： [Arkコンソール](https://console.volcengine.com/ark/region:ark+cn-beijing/endpoint)で推論エンドポイントまたはアプリ作成
...
公式Volcano Engineデモを使うと設定検証が簡単です。
...
前提条件をすべて満たしたら、以下の手順で音声対話とデバイス制御を備えたAIエージェントデモを素早くセットアップします（「デバイス」はWeb UI上でシミュレーションされます）。
...
# 目的：Volcano Engineがコールバックを認証するために使用
...
- `DASHSCOPE_API_KEY`と`CUSTOM_LLM_API_KEY`の違い：
  ...
  - `CUSTOM_LLM_API_KEY`：Volcano Engineからのコールバックを認証するために使用（APIゲートウェイトークンに類似）
...
# ===== CustomLLM設定 =====
# 目的：Volcano Engineにどのエンドポイントを呼び出すか伝える

# VOLC_LLM_URL - appの/chat-streamエンドポイント
# ローカルテスト：Dockerコンテナネットワークを使用
...
3. 本番環境では`VOLC_LLM_URL`を公開可能なHTTPS URLに変更しないと、Volcano Engineがコールバックできません
...
チャットボットアバター、マイク、カメラボタンなどのUIがある仮想デバイス画面が表示されます。
...
2. 設定パネルでEMQXブローカー情報を入力：
   ...
   - パスワード：EMQX認証有効時に入力
3. 「保存」をクリック
4. 確認ダイアログで「確認」を押すとページが自動更新され、新設定が反映されてMQTT接続が自動確立されます
...
- 「私が持っているものは何？」で写真撮影＋視覚認識をトリガー
- 「音量を80％にして」や「笑顔の表情にして」でデバイス制御をテスト
...
ヒント：スムーズな対話には、Doubao-proシリーズなどの深い推論を必要としないモデルを推奨します。詳細な設定パラメータは[Volcano Engineドキュメント](https://www.volcengine.com/docs/6348/1581714)を参照してください。
...
1. 証明書ファイル（`fullchain.pem`、`privkey.pem`）を準備
   ...
   重要：単一証明書ファイルではなくフルチェーン（完全な証明書チェーン）を使用してください。Volcano Engineのコールバックはフルチェーン検証を行い、不完全だとSSLハンドシェイクに失敗します。

   - Let’s Encryptの場合：`fullchain.pem`（証明書＋中間証明書を含む）を使用
   - その他CAの場合：サーバー証明書＋中間証明書を含むフルチェーンであることを確認
...
| 問題                       | 原因例                             | 対処方法                                                     |
| -------------------------- | ---------------------------------- | ------------------------------------------------------------ |
| 「AI準備中」状態で停止     | クロスサービス認可未設定           | 1) 権限設定を完了しているか確認 2) サービス有効化と残高確認 3) パラメータの大文字小文字を確認 |
| 401/403エラー              | AK/SKまたはトークン誤り           | 1) `VOLC_ACCESS_KEY_ID`/`VOLC_SECRET_KEY`を確認 2) トークン有効期限確認 3) クロスサービス認可設定確認 |
...
| CustomLLMコールバック失敗  | 認証キー不一致           | 1) `CUSTOM_LLM_API_KEY`が両方で一致しているか確認 2) `VOLC_LLM_URL`を確認 3) volc-serverからappへの到達性確認 |
...
| カメラ写真撮影失敗         | 権限が許可されていない     | 1) ブラウザのカメラ権限を確認 2) 許可を与える 3) ページをリロード |
...
| Web UIが接続できない       | WebSocketポートが開いていない | 1) WebSocketリスナーが有効か（デフォルト8083） 2) `ws://`スキームを使用（例：`ws://localhost:8083/mqtt`） |

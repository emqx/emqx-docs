...
| API | 説明 |
|-----|-------------|
| [StartVoiceChat](#startvoicechat) | AI音声セッションを開始し、指定されたルームにAIエージェントを作成します |
...
AI音声セッションを開始し、指定されたルームにAIエージェントを作成します。
...
| パラメータ     | 型     | 必須 | 説明                                               |
| ------------- | ------ | ---- | -------------------------------------------------- |
| `AppId`       | string | はい | RTCアプリケーションID                              |
| `RoomId`      | string | はい | ルームID                                           |
| `TaskId`      | string | はい | セッション識別用のタスクID                         |
| `AgentConfig` | object | はい | エージェント設定、詳細は[AgentConfig](#agentconfig)参照 |
| `Config`      | object | はい | セッション設定（ASR、TTS、LLMパラメータ含む）、詳細は[Config](#config)参照 |
...
| パラメータ                         | 型         | 必須 | 説明                                         |
| --------------------------------- | ---------- | ---- | -------------------------------------------- |
| `TargetUserId`                    | string[]   | はい | 対象ユーザーIDリスト（クライアントユーザーID） |
| `UserId`                          | string     | はい | エージェントユーザーID（AIボット識別子）    |
| `WelcomeMessage`                  | string     | いいえ | セッション開始時に自動再生されるウェルカムメッセージ |
| `EnableConversationStateCallback` | boolean    | いいえ | 聞く・考える・話す状態の会話状態コールバックを有効化 |
| `AnsMode`                         | number     | いいえ | AIノイズリダクションモード（0:オフ、1:低、2:中、3:高、推奨3） |
| `VoicePrint`                      | object     | いいえ | ボイスプリント認識：`Mode`（0:オフ、1:オン）、`IdList`（ボイスプリントIDリスト） |
...
| パラメータ | 型     | 必須 | 説明                                          |
| --------- | ------ | ---- | --------------------------------------------- |
| `AppId`   | string | はい | RTCアプリケーションID（StartVoiceChatと同じ） |
| `RoomId`  | string | はい | ルームID（StartVoiceChatと同じ）             |
| `TaskId`  | string | はい | タスクID（StartVoiceChatと同じ）              |
...
| `AppId`         | string | はい | RTCアプリケーションID                  |
| `RoomId`        | string | はい | ルームID                              |
| `TaskId`        | string | はい | タスクID                              |
| `Command`       | string | はい | コマンドタイプ                        |
| `Message`       | string | いいえ | アナウンステキスト（最大200文字）    |
| `InterruptMode` | number | いいえ | アナウンスの優先度                    |
...
| `FunctionCallResult`   | 関数呼び出し結果の返却          |

### InterruptModeの優先度

`ExternalTextToSpeech`で使用し、アナウンスの優先度を指定します：

| 値   | 説明                                               |
| ----- | -------------------------------------------------- |
| 1     | 高優先度：現在のやり取りを停止し即時再生           |
| 2     | 中優先度：現在のやり取り終了後に再生                 |
| 3     | 低優先度：やり取り中の場合は破棄                     |
...
| パラメータ           | 型     | 必須 | 説明                                         |
| ------------------- | ------ | ---- | -------------------------------------------- |
| `Provider`          | string | はい | サービスプロバイダー、固定値 `volcano`       |
| `ProviderParams`    | object | はい | プロバイダー固有のパラメータ                 |
| `VADConfig`         | object | いいえ | 音声活動検出（VAD）設定                      |
| `VolumeGain`        | number | いいえ | 音量ゲイン（0.0～1.0）、デフォルト `0.5`     |
| `TurnDetectionMode` | number | いいえ | ターン検出モード                             |
| `InterruptConfig`   | object | いいえ | 割り込み設定                                 |
...
| パラメータ     | 型      | 説明                                         |
| ------------- | ------- | -------------------------------------------- |
| `SilenceTime` | number  | 無音継続時間閾値（ms）、デフォルト `600`    |
...
| `Provider`          | string   | はい | サービスプロバイダー、固定値 `volcano`       |
| `ProviderParams`    | object   | はい | プロバイダー固有のパラメータ                 |
| `IgnoreBracketText` | number[] | いいえ | 無視する括弧タイプ                           |
...
| `emotion_strength` | number | 感情強度（0.0～1.0） | 感情対応あり          |

::: tip TTSモード
- `standard`：標準モード、`speed_ratio`、`pitch_ratio`を使用
- `bigtts`：大規模モデルTTS、`speech_ratio`、`pitch_rate`を使用
- `bidirection`：双方向ストリーミング、`speech_rate`を使用し、`Additions`設定対応
:::
...
| パラメータ        | 型       | 必須         | 説明                                                   |
| ---------------- | -------- | ------------ | ------------------------------------------------------ |
| `Mode`           | string   | はい         | モード：`ArkV3`（Ark）または `CustomLLM`（カスタム）  |
| `Url`            | string   | CustomLLMのみ | CustomLLMコールバックURL                               |
| `APIKey`         | string   | いいえ       | API認証キー                                            |
| `EndPointId`     | string   | ArkV3のみ    | ArkモデルエンドポイントID                              |
| `ModelName`      | string   | いいえ       | モデル名                                               |
| `SystemMessages` | string[] | いいえ       | システムプロンプト                                     |
| `UserPrompts`    | object[] | いいえ       | 事前設定された会話履歴                                 |
| `Temperature`    | number   | いいえ       | サンプリング温度（0.0～1.0）、デフォルト `0.5`         |
| `TopP`           | number   | いいえ       | Top-pサンプリング（0.0～1.0）、デフォルト `0.9`         |
| `MaxTokens`      | number   | いいえ       | 最大トークン数、デフォルト `256`                       |
| `HistoryLength`  | number   | いいえ       | 保持する履歴ターン数、デフォルト `15`                   |
| `EnableRoundId`  | boolean  | いいえ       | ラウンドIDを有効化                                     |
| `VisionConfig`   | object   | いいえ       | ビジョン理解設定：`Enable`（boolean）、`SnapshotConfig`（object） |
| `Custom`         | string   | いいえ       | カスタムパラメータ（JSON文字列）、CustomLLMへ透過的に渡される |
...
| `InvalidTimestamp`       | 400            | タイムスタンプが不正または期限切れ |
| `InvalidAuthorization`   | 400            | Authorizationヘッダーが不正    |
| `InvalidCredential`      | 400            | 認証情報の形式が不正           |
| `InvalidAccessKey`       | 401            | AccessKeyが不正               |
| `SignatureDoesNotMatch`  | 401            | 署名検証に失敗                |
| `InvalidSecretToken`     | 401            | STSトークンが不正または期限切れ |
| `AccessDenied`           | 403            | IAM権限が不足している          |
| `ServiceNotFound`        | 404            | サービスが見つからない         |
| `InvalidActionOrVersion` | 404            | APIアクションまたはバージョンが不正 |
...
公式ドキュメント: [よくあるエラーコード](https://www.volcengine.com/docs/6369/68677)

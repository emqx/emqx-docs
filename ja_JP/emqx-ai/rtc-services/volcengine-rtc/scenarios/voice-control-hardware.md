...
純粋な音声対話を基盤として、音声制御ハードウェアシナリオではユーザーが音声で物理デバイスを操作できます。ユーザーは口頭でコマンドを発し、AIは理解・応答するだけでなく、実際のデバイス操作を行いタスクを完了させます。

**技術的実装**：音声伝送、ASR、TTSは純粋な音声対話シナリオと同様です。最大の違いは、AIエージェントがツール呼び出し機能を持つ点です。制御インテントを検出すると、エージェントはMCPを介してMQTTプロトコルでデバイスにツール呼び出し要求を送信します。デバイスはMCPサーバーとして動作し、カメラのオン・オフ、音量調整、表情切替、写真撮影などのハードウェア操作を実行し、実行結果をエージェントに返します。エージェントは最終的にユーザーに音声フィードバックを提供します。
...
>  **スピーカー**：「はい、寝室のカーテンは明日朝7時に自動で開きます。」

一文で複数デバイスの連携動作がトリガーされます。AIは「寝る」という文脈を理解し、事前設定された複数デバイスの操作を自動実行します。
...
>  **車載システム**：「腰部シートマッサージを起動しました。良いドライブを。」
...
非滅菌環境の機器に触れられない手術室では音声制御が不可欠であり、手術の効率と安全性を高めます。
...
>
> **ラオ・ジャン**：「検査カメラをオンにして。」
>  **システム**：「品質検査カメラを起動しリアルタイム検査を開始しました。」
>

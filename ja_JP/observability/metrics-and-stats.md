...
- メトリクスは整数型のカウンターで、送受信されたバイト数やメッセージ数など、単純な増減を計測します。

EMQXはユーザーに複数の統計およびメトリクスの閲覧方法を提供しています。最も直接的には、EMQXダッシュボードでこれらのデータを確認できます。ダッシュボードへのアクセスが困難な場合は、[REST API](#request-monitoring-status-via-rest-api)や[システムトピック](#get-monitoring-status-via-system-topics)のメッセージを通じてデータを取得できます。さらに、監視機能を独自の監視システムに簡単に統合することも可能です。詳細は[Prometheusとの統合](./prometheus.md)をご覧ください。
...
EMQXのメトリクスはカウンターとして実装されており、ノード起動以降の特定イベントの累積発生回数を記録します。これらのメトリクスは、運用者がシステムの挙動を観察し、負荷パターンを評価し、問題をトラブルシューティングするのに役立ちます。
...
| メトリクス                     | 説明                                                         |
| --------------------------- | ------------------------------------------------------------ |
| authorization.allow         | クライアント認可成功の合計数。キャッシュヒット（認可結果取得）とポリシールールにマッチした認可リクエストの合計。 |
...
これらのメトリクスは、アクティブなバンエントリによって拒否された接続試行や、フラッピング検知により作成された一時的なバンを追跡します。

| メトリクス                      | 説明                                                         |
| ---------------------------- | ------------------------------------------------------------ |
| client.banned               | クライアントがアクティブなバンエントリにマッチし接続試行が拒否された回数 |
| flapping.detected.clientid  | クライアントIDのフラッピング検知による一時的バン発生回数     |
| flapping.detected.username  | ユーザー名のフラッピング検知による一時的バン発生回数         |
| flapping.detected.peerhost  | 送信元IPアドレスのフラッピング検知による一時的バン発生回数   |

`client.banned`でカウントされた接続試行は、EMQXが非成功の`CONNACK`理由コードで接続を拒否するため、`packets.connack.error`も増加します。
...
| actions.messages | アクション実行で処理されたメッセージ数。単一アクション実行で複数メッセージを処理するため、この値は`actions.executed`以上となる。 |
...
| packets.connack.error        | 理由コードが0x00以外のCONNACKパケット送信数。`packets.connack.auth_error`以上の値となる。 |
...
APIを通じてメトリクスや統計を取得することも可能です。UIの左ナビゲーションメニューで **Metrics** をクリックすると、このAPIリクエストを実行できます。EMQX APIの利用方法は[REST API](../admin/api.md)をご参照ください。

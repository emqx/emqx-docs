...
- **ペイロード制限**：トレースファイルに出力するペイロードの最大バイト数を設定します。このオプションは **ペイロードエンコード** が `Text` または `HEX` の場合にのみ有効です。制限を超えた場合は切り詰められます。デフォルトは `1024 B` で、有効になっていますが無制限にすることも可能です。

### クライアントIDによるトレース

1. **トレース作成**ダイアログで、**タイプ**のドロップダウンリストから `Client ID` を選択します。
2. トレース対象のクライアントIDを入力します。
3. [共通トレースオプションの設定](#共通トレースオプションの設定)に従いオプションを設定します。
4. **作成**をクリックします。

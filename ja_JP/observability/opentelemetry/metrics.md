...
EMQXは、gRPC OTELプロトコルを介してメトリクスを直接OpenTelemetry Collectorにプッシュする機能を標準で備えています。Collectorはその後、データを任意のバックエンドにルーティング、フィルタリング、変換して保存および可視化できます。

本ページでは、EMQXとOpenTelemetryをダッシュボードから統合し、[Prometheus](../../observability/prometheus.md)を通じてEMQXのメトリクスを表示する方法を紹介します。

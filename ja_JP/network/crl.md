...
listeners.ssl.default {
  ...
  ssl_options {
    ...
    # クライアント証明書を必ず検証する
    verify = verify_peer
    # クライアントが空でない証明書を送信しない場合、TLS ハンドシェイクを失敗させる
...
クライアントが接続し、EMQX が初めて新しい CRL URL を検出した場合、クライアント証明書の配布ポイントから CRL を取得します。デフォルトでは、EMQX は失効情報を最新に保つためにキャッシュされた CRL を 15 分ごとに更新します。

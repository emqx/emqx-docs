...
EMQX v5.0.3 以降、EMQX は MQTT SSL リスナーに対して証明書失効リスト（CRL）チェックをサポートしています。この機能は `ssl` タイプのリスナーのみで利用可能であり、Secure WebSocket（`wss`）および QUIC リスナーは対応していません。
...
> **注意**
>
> CRL の強制適用を行うには、CRL に [RFC 3280 第5.2.5節](https://www.rfc-editor.org/rfc/rfc3280#section-5.2.5) で定義されている *Issuing Distribution Point* 拡張が含まれている必要があります。

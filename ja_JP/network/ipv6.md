...
ほとんどのOSでは、`[::]`にバインドするとデフォルトでIPv4とIPv6の両方の接続を受け入れます（デュアルスタック）。両プロトコルをサポートする環境で最も簡単な設定方法です。
...
明示的なIPアドレスなしでポートのみをバインドする場合は、`inet6`を有効にしてIPv6を明示的に有効化できます。
...
| パラメータ          | 型       | デフォルト | 説明                                                                                  |
| ------------------- | -------- | ---------- | ------------------------------------------------------------------------------------- |
| `inet6`             | boolean  | `false`    | IPv6サポートを有効化します。`false`の場合、リスナーはIPv4トラフィックのみを受け入れます。 |
| `ipv6_v6only`       | boolean  | `false`    | IPv4→IPv6マッピングを無効化します。`inet6`が`true`のときのみ有効です。                 |
...
EMQXはHTTP認証、Webhookアクション、データベース連携などの機能のために外部サービスへアウトバウンド接続を行います。
...
一部のコネクタータイプでは、設定に`ipv6_probe`トグルがあり、これを有効（HTTPコネクターのデフォルト）にするとIPv6接続を優先します。ネットワークがIPv4のみでDNSがAレコードとAAAAレコードの両方を返す場合、接続遅延を避けるためにこのプローブを無効化できます。
...
**対処法**：EMQXホストからターゲットサービスが正しいアドレスファミリーで到達可能か確認してください。HTTPコネクターは自動IPv6プローブで対応します。DNS名の場合は、DNSが正しいレコードタイプ（AまたはAAAA）を返しているか確認してください。
...
**対処法**：ダッシュボードをIPv6アドレス（`bind = "[::]:18083"`）にバインドするか、`inet6 = true`でIPv6を明示的に有効化してください。

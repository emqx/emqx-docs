...
ユーザーは設定ファイルまたは環境変数を使ってEMQXを設定できます。本節では主にEMQXの設定ファイルについて説明し、EMQXで最も一般的に使用される機能の基本的な設定方法を紹介します。詳細な設定項目と説明については、[EMQX Enterprise Configuration Manual](https://docs.emqx.com/en/enterprise/v@EE_VERSION@/hocon/)をご参照ください。
...
`etc`ディレクトリにはEMQXの初期設定を定義する設定ファイルが格納されます。これらのファイルは通常、デプロイ時やアップグレード時に変更され、実行時には安定性を確保するため読み取り専用となっています。`etc`ディレクトリの場所はインストール方法によって異なります。
...
実行時には、ダッシュボード、REST API、CLIを通じて動的に設定を変更できます。これらの変更は`data/configs`ディレクトリに保存され、セッションを跨いで永続化されます。このディレクトリの場所もインストール方法に依存します。
...
::: tip
設定の`node.data_dir`を変更するか、環境変数`EMQX_NODE__DATA_DIR`を設定することで、データディレクトリの変更が可能です。ただし、クラスター運用時はすべてのノードで同じディレクトリパスを使用する必要があります。
:::

設定ファイルの内容が重複することは推奨されませんが、重複した場合はあらかじめ定められた上書きルールに従って解決されます。詳細は[設定上書きルール](#config-override-rules)をご覧ください。

## 設定例

[スキーマ](#schema)セクションで詳細なリファレンスを提供していますが、設定例はEMQXの設定理解や適用に役立ちます。
...
`node`や`cluster`など不変の設定は`base.hocon`に設定することは**推奨されません**。詳細は[不変設定ファイル](#immutable-configuration-file)をご参照ください。
...
このファイルの設定は`base.hocon`の設定の上に適用されます。設定の上書き階層の詳細は[設定上書きルール](#config-override-rules)をご覧ください。

EMQX 5.1以降、クラスタ設定の変更時に`cluster.hocon`ファイルのバックアップが上書き前に自動で作成されます。バックアップはノードのローカル時間でタイムスタンプが付与され、最大10個まで保持されます。
...
設定の上書きに関する詳細は[設定上書きルール](#config-override-rules)をご参照ください。

## 設定パス

EMQXでは、設定値はドット区切りのパスで参照できます。これはツリー構造のように扱われ、ルート（常にStruct）から始まり、各セグメントはフィールド名またはMapのキーを指します。配列要素は1始まりのインデックスを使用します。
...
このフラット形式は以前のEMQXバージョンとの互換性がありますが、使い方が異なります。

HOCONでは文字列の両端に引用符を付けることを推奨します。特殊文字のない文字列は引用符なしでも構いません（例：`foo`、`foo_bar`）。一方、フラット形式では`=`の右側のすべてを値として扱います。
...
3. 環境変数の値はHOCON値として解析されるため、複雑なデータ型の値も渡せます。ただし、`:`や`=`などの特殊文字はダブルクォート`"`で囲む必要があります。
...
つまり、`base.hocon`の設定は最も優先度が低く、より優先度の高いファイルの設定で上書きされます。`EMQX_`で始まる環境変数が最も高い優先度を持ちます。
...
この特徴を活かし、配列の要素の値を簡単に上書きできます。例えば：
...
## 以下の方法で上書きすると、1番目の要素の`enable`以外のフィールドはすべて失われます。
...
EMQXのゾーンは設定のグルーピング概念です。ゾーンはリスナーの`zone`フィールドに設定することで関連付けられ、ゾーンに紐づくリスナーに接続するMQTTクライアントはそのゾーンの設定を継承し、グローバル設定を上書きすることがあります。
...
- `mqtt`：MQTT接続やセッション設定。特定ゾーンでのMQTTメッセージの最大パケットサイズを大きくするなど。
...
リスナーの`zone`フィールドにゾーン名を設定して関連付けます。
...
HOCONオブジェクトの型安全性を高めるため、EMQXはスキーマを導入しています。このスキーマはデータ型、フィールド名、メタデータを定義し、設定値の検証などに活用されます。
...
設定マニュアルに登場するプリミティブ型はほぼ自明であり、最小限の説明で十分です。以下に主要な型を示します。
...
列挙型で、あらかじめ定義されたシンボルのいずれかのみを取れます。例：`Enum(debug,info,warning,error)`はログレベルを定義。
...
- **三重引用符付き文字列**：`"""`で囲み、エスケープ不要（`\`を除く）で複雑な内容を記述可能。三重引用符に隣接するクォートはエスケープが必要です。
- **インデント付き三重引用符文字列**：`"""~`と`~"""`で囲み、EMQX 5.6以降で導入。設定ファイル内でインデントを保持でき、多行やフォーマット済みテキストに適します。

**無引用符文字列の注意点：**

- 禁止文字：`$`、`"`、`{`、`}`、`[`、`]`、`:`、`=`、`,`、`+`、`#`、`` ` ``、`^`、`?`、`!`、`*`、`&`、`\`、および空白。
- `//`で始まるとコメントと解釈されるため不可。
- `true`、`false`、`null`で始まる場合はブール値やnullとして誤解される恐れあり。

**三重引用符文字列のガイドライン：**

- 三重引用符に隣接するクォートはエスケープするか`~`区切りを使う。
- 複数行文字列はスペースによるインデントが可能（タブ不可）。インデントレベルは最小の先頭スペース数で決定。
...
定数文字列で、単一値の列挙型（Enum）として機能します。特定の設定やモードの静的値定義に使います。
...
EMQXのHOCON設定における複合データ型は、他の複合型やプリミティブ値を含むデータ構造を表現します。階層的かつ柔軟なデータ表現を可能にします。

#### Struct `Struct(name)`

中括弧`{}`で囲まれたフィールドを持つ構造体。`name`はその構造体のフィールド名や型を定義するスキーマの参照です。

#### Map `Map($name->Type)`

`Struct`に似ていますが、フィールド名が事前定義されていないキーと値のペアの集合です。

`$name`はドット`.`を含まない任意の文字列キーを表し、エンティティや属性名を示します。`Type`はマップ内のすべての値が同一のデータ型であることを示し、均一なデータ集合を可能にします。

#### OneOf `OneOf(Type1, Type2, ...)`

複数の型のいずれかを取るユニオン型で、1つのフィールドが複数の型のいずれかであることを示します。例として`String(infinity)`または`Duration`のどちらかを許容する場合など。
...
Mapのフィールド名が正の整数の場合、それは`Array`の別表現として解釈されます。例えば：
...
Variformは文字列操作と実行時評価のための軽量で表現力豊かな言語です。完全なプログラミング言語ではなく、EMQXの設定内に埋め込み可能な文字列操作ツールです。
...
::: tip NULL値について
Variform式では値のバインディング参照やサブ式の評価が未定義値となる場合、空文字列（`''`）として扱われます。
...
::: tip
アンエスケープ関数が必要な場合、HOCON設定で三重引用符`"""`文字列を使うと二重エスケープが不要になります。
...
EMQXはルールエンジンの文字列関数に類似した豊富な文字列、配列、乱数、ハッシュ関数を提供しています。これらは抽出データの操作や整形に使用可能です。例えば、`lower()`、`upper()`、`concat()`は文字列のフォーマット調整に、`hash()`や`hash_to_range()`はハッシュや範囲マッピングに使います。

利用可能な関数例：

- **文字列関数**：
  - [文字列操作関数](../data-integration/rule-sql-builtin-functions.md#string-operation-functions)
  - 新関数`any_to_string/1`：中間の非文字列値を文字列に変換
- **配列関数**：`nth/2`など
...
- **スキーマレスエンコード/デコード関数**：
  ...
  - `json_value(Data, Path)`：JSON文字列からドット区切りパスで値を抽出（例：`json_value(username, 'shop.floor')`）（6.0.2以降）
  - `jwt_value(Data, Path)`：JWTペイロードからクレーム値を抽出（例：`jwt_value(password, 'client_attrs.unitid')`）（6.0.2以降）
...
- **比較関数**：
  ...
  - `is_empty_var(V)`：変数が空か判定。Variformで空とは未定義値（`undefined`）、JSONの`null`（文字列`"null"`ではない）、空文字列`""`を指す
  - `not(Bool)`：`Bool`が`false`なら`true`、`true`なら`false`を返す。文字列も受け入れ、入力が文字列の場合は出力も文字列

- **システム関数**：
  - `getenv(Name)`：環境変数`Name`の値を返す。ただしOS環境変数の前に`EMQXVAR_`を付加して読み込む。例：`getenv('FOO_BAR')`は`EMQXVAR_FOO_BAR`を読み込む。読み込んだ値は不変。
...
- 未バインド変数：未定義またはスコープ外の変数参照は空文字列として評価されます。
- 実行時例外：関数の誤用や型不一致などの例外も空文字列を返します。例えば配列インデックスの範囲外など。
...
- `iif(regex_match(clientid,'^foo\.+*'),'foo','bar')`：clientidが`foo.`で始まる場合`foo`、そうでなければ`bar`を返す

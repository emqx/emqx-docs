...
ユーザーは設定ファイルまたは環境変数を用いてEMQXを設定できます。本節では主にEMQXの設定ファイルについて紹介し、EMQXで最もよく使われる機能の基本的な設定方法を説明します。詳細な設定項目と解説については、[EMQX Enterprise Configuration Manual](https://docs.emqx.com/en/enterprise/v@EE_VERSION@/hocon/)をご参照ください。

## 設定ディレクトリ

EMQXをインストールすると、設定および実行時データを管理するための一連のディレクトリが作成されます。これらのディレクトリは主に以下の2種類に分かれています。
...
実行時には、Dashboard、REST API、CLIを通じて動的な再設定が可能です。これらのツールで行われた変更は`data/configs`ディレクトリに保存され、セッションをまたいで永続化されます。このディレクトリの場所もインストール方法により異なります。
...
EMQX 5.8.4以降、`etc`ディレクトリに`base.hocon`という基本設定ファイルが存在します。このファイルにはデフォルト設定が含まれており、実行時により上位の設定ファイルで上書き可能です。

例えば、デプロイ時に基本的な認証設定で開始し、Dashboard UIからより複雑な設定で上書きするといった運用が可能です。

`node`や`cluster`のような不変設定については、デプロイ固有の値で実行時に変更しない場合、環境変数を利用することもできます。詳細は[Environment Variables](#environment-variables)および[Config Override Rules](#config-override-rules)をご参照ください。
...
`data/configs`ディレクトリの`cluster.hocon`ファイルにはクラスター全体の設定項目が含まれます。Dashboard、REST API、CLIからの設定変更はこのファイルに永続化されます。
...
EMQX 5.1以降、クラスター設定の変更時に`cluster.hocon`のバックアップが作成されます。バックアップはノードのローカル時間でタイムスタンプが付けられ、最大10ファイルまで保持されます。

## 不変設定ファイル

後方互換性のため、`emqx.conf`ファイルは`node`や`cluster`の重要なシステム設定に利用可能です。このファイルは`base.hocon`や`cluster.hocon`より優先度が高いですが、環境変数よりは低い優先度です。意図的にこの優先度を利用する場合を除き、変更は避けてください。パッケージのアップグレードでこのファイルのデフォルトが更新される可能性があります。
...
設定ファイルのほかに、環境変数を用いてEMQXを設定することも可能です。
...
3. 環境変数の値はHOCON値として解析されるため、複雑なデータ型を渡せます。`:`、`=`、`#`などHOCONの特殊文字を含む値はダブルクォート`"`で囲む必要があります。特に`#`はHOCONの行コメントを開始するため、引用符なしでは`#`以降がコメントとして無視されます。
...
パスワードなどに`#`を含む場合の注意点です。`#`はHOCONの行コメント開始文字なので、

```bash
export EMQX_DASHBOARD__DEFAULT_PASSWORD="MQtt#123"
```

はパスワードが`MQtt`として解析され、`#123`はコメントとして無視されます。リテラルとして渡すには**HOCONレベルの**ダブルクォートで囲み、パーサーに`"MQtt#123"`として認識させます。
...
EMQXは環境変数の値を`fake_key=<値>`としてHOCONパースを試みます。成功すれば解析結果を使用し、失敗すれば生の文字列を使用します。例えば、

- `EMQX_..._PASSWORD="abc#def"`は有効なHOCONで`#def`がコメント扱いされるため`abc`になる
- `EMQX_..._PASSWORD=".abc#def"`は無効なHOCONでパース失敗し、生文字列のまま`.abc#def`になる
...
つまり、`base.hocon`の設定が最も優先度が低く、上位のファイルで上書き可能です。`EMQX_`で始まる環境変数が最も優先度が高いです。

::: tip
5.8.4以前のバージョンには`base.hocon`ファイルは存在しません。優先度の順序は同じですが、`base.hocon`は含まれません。
:::
...
::: tip
1. 古いEMQXバージョン（例：5.0.2/v5.0.22以前）では`cluster-override.conf`ファイルが存在し、設定優先度は`emqx.conf < ENV < HTTP API (cluster-override.conf)`でした。
2. これらのバージョンから最新にアップグレードしても優先度は変わらず、互換性維持のため`cluster.hocon`は作成されません。
...
EMQXのゾーンは設定をグループ化する概念です。リスナーの`zone`フィールドにゾーン名を設定すると、そのゾーンに紐づくリスナーに接続したMQTTクライアントはゾーンの設定を継承し、グローバル設定を上書きする場合があります。
...
- `emqx.conf`は優先度が高いこととアップグレード影響を理解した上でのみ変更する。
- 実行時にDashboard、API、CLIから変更しない単純な上書きは環境変数で行う。

設定コードの真実の情報源として`base.hocon`を推奨します。ノード起動時に静的設定ディレクトリから読み込まれ、パッケージングやイメージビルド、構成管理、GitOpsワークフローで管理可能です。Dashboard、REST API、CLIからの実行時変更は`cluster.hocon`に永続化され、`base.hocon`の上に重ねられます。
...
`cluster.hocon`は設定コードの真実の情報源として使用しないでください。EMQXが実行時に管理し、Dashboard、REST API、CLIが書き換え、上書き前にバックアップを作成し、クラスター内でコピーされます。手動編集やマウントは実行時の更新と競合したり上書きされる恐れがあります。

`emqx.conf`は配布パッケージに同梱されるベースライン設定ファイルです。変更しなければアップグレード時に新バージョンの保守的なデフォルト変更を取り込みやすくなります。`emqx.conf`に設定項目を記述すると`base.hocon`や`cluster.hocon`より優先度が高いため、実行時変更が反映されてもノード再起動後に元に戻ることがあります。意図的にその挙動が必要な場合のみ使用してください。
...
環境変数はすべての設定ファイルを上書きするため、運用者がDashboard、API、CLIから後で調整することが想定される設定には使用を避けてください。
...
プリミティブデータ型はほぼ自明で、最小限の説明で十分です。以下に主な型を列挙します。
...
列挙型。指定されたシンボルのいずれかのみ許容。例：`Enum(debug,info,warning,error)`はログレベル。

#### String

文字列型。以下の形式をサポートします。

- **無引用**：特殊文字を含まない単純な識別子や名前に適します（詳細は後述）。
- **引用文字列**：特殊文字や空白を含む場合はダブルクォート`"`で囲み、必要に応じてバックスラッシュ`\`でエスケープします。例：`"line1\nline2"`
- **三重引用文字列**：`"""`で囲み、`\`以外のエスケープ不要で複雑な内容を簡単に記述可能。三重引用符に隣接するクォートはエスケープが必要です。
- **インデント付き三重引用文字列**：`"""~`と`~"""`で囲み、EMQX 5.6以降で導入。設定ファイル内でインデントを保持しつつ複数行や整形済みテキストを記述可能。
...
定数文字列。単一値の列挙型（Enum）として機能し、特定の固定値を定義するのに使います。
...
EMQXのHOCON設定における複合データ型は、他の複合型やプリミティブ値を含めることができるデータ構造を表現します。柔軟で階層的なデータ表現を可能にします。

#### Struct `Struct(name)`

波括弧`{}`で囲まれた構造体。`name`は構造体のフィールドと型を定義するスキーマの参照です。

#### Map `Map($name->Type)`

Structに似ていますが、フィールド名が事前定義されていないキーと値のペアの集合です。

`$name`はドット`.`を含まない任意の文字列キーを示し、`Type`はすべての値が同一データ型であることを示します。

#### OneOf `OneOf(Type1, Type2, ...)`

複数の型のいずれかを許容するユニオン型。構造体のフィールドが複数の型のいずれかを取れることを示します。例：`String(infinity)`または`Duration`のどちらか。
...
Variformは文字列操作や実行時評価のための軽量で表現力豊かな言語です。完全なプログラミング言語ではなく、EMQXの設定内に埋め込んで文字列操作を動的に行うための専用ツールです。
...
Variform式では値のバインディング参照や部分式の評価が未定義値となる場合、空文字列（`''`）で表現されます。
...
EMQXはルールエンジンの文字列関数に類似した豊富な文字列、配列、乱数、ハッシュ関数を備えています。抽出データの操作や整形に利用可能です。例：`lower()`、`upper()`、`concat()`、`hash()`、`hash_to_range()`など。
...
- **ハッシュ関数**：
  ...
  - `hash_to_range(Input, Min, Max)`：`sha256`でハッシュし、`Min`〜`Max`の範囲にマッピング（`Min <= X <= Max`）
  - `map_to_range(Input, Min, Max)`：入力を`Min`〜`Max`の範囲にマッピング
- **比較関数**：
  ...
  - `is_empty_var(V)`：変数が空か判定。Variformの空は未定義値（`undefined`）、JSONの`null`（文字列`"null"`は除く）、空文字列`""`を含む
  - `not(Bool)`：`Bool`が`false`なら`true`、`true`なら`false`を返す。文字列も受け入れ、入力が文字列なら出力も文字列

- **システム関数**：
  - `getenv(Name)`：環境変数`Name`の値を返す。以下の制約あり：
    - OS環境変数読み込み時に`EMQXVAR_`プレフィックスを付加（例：`getenv('FOO_BAR')`は`EMQXVAR_FOO_BAR`を読む）
    - OS環境変数から読み込んだ値は不変

- **データ抽出関数**：
  - `json_value(Data, Path)`：JSON文字列からドット区切りパスで値を抽出。例：`username`がJSONオブジェクトなら`json_value(username, 'shop.floor')`
  - `jwt_value(Data, Path)`：JWTトークンのペイロードからドット区切りパスでクレーム値を抽出。例：`password`がカスタムクレームを持つJWTなら`jwt_value(password, 'client_attrs.unitid')`
...
- 未定義変数：未定義またはスコープ外の変数参照は空文字列として評価。
- 実行時例外：関数の誤用や型不一致などの例外発生時も空文字列を返す。例：配列インデックスが範囲外。
...
- `iif(regex_match(clientid,'^foo\.+*'),'foo','bar')`：`clientid`が`foo.`で始まるなら`foo`、そうでなければ`bar`を返す。

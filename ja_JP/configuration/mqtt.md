...
**有効範囲：** `0`–`65535` 秒。`0` を設定するとそのセッションの keepalive チェックが無効になります。`65535` を超える値は `65535` に丸められます。クライアントのゾーンに `mqtt.server_keepalive` が設定されている場合、実効値はその最大値にも制限されます。

**ユースケース例：** 車両が駐車状態に入ると、T-Box クライアントは `$SETOPTS/mqtt/keepalive` に `300` をパブリッシュします。EMQX はそのクライアントの keepalive 許容時間を 300 秒に延長します（デフォルトの `1.5×` 乗数で実効アイドルタイムアウトは 450 秒）。これによりハートビートの頻度を下げながら、リモートコマンドを受信するための MQTT 接続を維持し続けます。
...
- すべてのクライアントが `$SETOPTS/mqtt/keepalive` にパブリッシュできるよう許可し、各デバイスが自身の keepalive を自律調整できるようにします。

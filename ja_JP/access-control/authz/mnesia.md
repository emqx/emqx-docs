...
EMQXは、組み込みデータベースを通じて低コストかつすぐに使える認可ルールの保存方法を提供しています。Dashboardや設定ファイルで組み込みデータベース（Mnesia）をデータソースとして設定し、DashboardまたはHTTP APIを通じて関連する認可チェックルールを追加できます。

::: tip 前提条件

[基本的なEMQX認可の概念](./authz.md)の理解

:::

## Dashboardで組み込みデータベース認可機能を作成する
...
## 設定ファイルで組み込みデータベース認可機能を作成する

組み込みデータベース認可機能は、`built_in_database` タイプで識別されます。
...
- **全ユーザー**：すべてのクライアント／ユーザーにルールを適用します。パターンやIP範囲で絞り込みも可能です。
...
| **IP Address Range**   | ルールが適用されるクライアントのIP範囲。CIDR表記（例：`192.168.1.0/24`）や特定IP指定可。|
| **Listener**           | ルールが適用されるリスナー。`{type}:{name}`形式（例：`tcp:default`、`ws:default`）。   |
| **Zone**               | ルールが有効となるゾーン。マルチゾーン展開時に適用。                                  |

#### スコープ別フィールド

| ルールスコープ   | フィールド                                                                                                                |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **クライアントID** | **Client ID**：（必須）このルールが適用される正確なクライアントID。<br />**Username Pattern**：（任意）このルールが有効なユーザー名の正規表現。 |
| **ユーザー名**     | **Username**：（必須）このルールが適用される正確なユーザー名。<br />**Client ID Pattern**：（任意）このルールが有効なクライアントIDの正規表現。 |
...
4. （任意）複数ルールを追加する場合は **権限を追加** をクリックし、**上へ**・**下へ** ボタンでルール実行順序を調整します。  
...
- 対応するルールの **操作** 列にある **編集** ボタンをクリックし、ルールフィールドやマッチングパターン、IP範囲設定を変更します。  
...
ルール作成前に、組み込みデータベース認可ソースを作成しておきます：

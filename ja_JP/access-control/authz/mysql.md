...
MySQLオーソライザーはほぼあらゆるストレージスキーマをサポートします。認証情報の保存方法やアクセス方法は、ビジネスニーズに応じて、1つまたは複数のテーブル、ビューなどを使用して自由に決定できます。
...
::: tip
システム内のユーザー数が多い場合は、クエリ応答時間を短縮し、EMQXの負荷を軽減するために、事前にテーブルの最適化およびインデックス作成を行ってください。
:::
...
2. 右上の **作成** をクリックし、次に **バックエンド** で **MySQL** を選択して **次へ** をクリックします。以下のように **設定** タブが表示されます。
...
3. 以下の手順に従って認可バックエンドを設定します：
   ...
   - **詳細設定**：接続プール、タイムアウト、プリペアドステートメントの動作を設定します。
     ...
     - **接続タイムアウト**（任意）：EMQXが接続試行をタイムアウトと判断するまでの待機時間を指定します。単位はミリ秒、秒、分、時間が利用可能です。デフォルトは `15` 秒です。

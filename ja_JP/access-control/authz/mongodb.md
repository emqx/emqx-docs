...
1. [EMQX Dashboard](http://127.0.0.1:18083/#/authentication)で、左側のナビゲーションツリーから **Access Control** -> **Authorization** をクリックし、**Authorization** ページに入ります。

2. 右上の **Create** をクリックし、続いて **Backend** で **MongoDB** を選択します。**Next** をクリックすると、**Configuration** タブが表示されます。
...
3. 以下の指示に従って設定を行います。
   ...
   **Advanced Settings**：
   ...
   - **Use Legacy Protocol**：MongoDBとの通信にレガシープロトコルを使用するかを選択します。`auto`、`true`、`false` の選択肢があり、デフォルトは `auto` で、新しいプロトコルのサポートを自動判定します。

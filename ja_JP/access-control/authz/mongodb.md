...
2. 右上の**作成**をクリックし、**バックエンド**として**MongoDB**を選択してから**次へ**をクリックします。**設定**タブが表示されます。

   <img src="./assets/authz-MongoDB_ee.png" alt="authz-MongoDB_ee" style="zoom:67%;" />

3. 以下の指示に従って設定を行います。

   **接続**：MongoDBへの接続に必要な情報を入力します。
   ...
   **詳細設定**：
   ...
   - **Use Legacy Protocol**：MongoDBとの通信にレガシープロトコルを使用するかどうかを選択します。`auto`、`true`、`false`のオプションがあり、デフォルトは`auto`で、新しいプロトコルのサポートを自動判別します。
   - **Record Limit**：MongoDBから取得する認可レコードの上限数を指定します。
   - **Skip**：レコードリスト取得時にスキップする認可レコード数を設定します。

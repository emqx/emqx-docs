...
このオーソライザーは、MongoDBデータベースに格納されたルールリストとパブリッシュ／サブスクライブ要求を照合することで認可チェックを実装しています。

::: tip 前提条件

[EMQX認可の基本概念](./authz.md)に関する知識
...
* `retain`（オプション）：ルールがリテインメッセージのパブリッシュを許可するかどうかを示します。値の選択肢は `0`、`1`、または `true`、`false` です。デフォルトではリテインメッセージは許可されています。
...
EMQX Dashboardを使ってMongoDBをユーザー認可に利用する方法を設定できます。
...
3. 以下の指示に従って設定を行います。
   ...
   **Connect**：MongoDB接続に必要な情報を入力します。
   ...
   - **MongoDB Mode**：MongoDBのデプロイ方式を選択します。`Single`、`Replica Set`、`Sharding` から選べます。
   ...
   **Advanced Settings**：
   ...
   - **Use Legacy Protocol**：MongoDBとの通信にレガシープロトコルを使用するかどうかを選択します。`auto`、`true`、`false` のいずれかを選べます。デフォルトは `auto` で、新しいプロトコルのサポート有無を自動判定します。
   ...
   - **Connect Timeout**（オプション）：接続タイムアウトとみなすまでの待機時間を指定します。単位はミリ秒、秒、分、時間が利用可能です。

...
* ローリングアップデートの過程では、新しいPodが起動して準備完了になるまで時間がかかるため、任意の時点でサービスを提供できるPodは最大でも_N - 1_となり、サービスの可用性が低下する恐れがあります。
...
3. EMQXクラスターの状態を確認します。
  ...
  `STATUS` が `Ready` になっていることを確認してください。完了までに時間がかかる場合があります。
...
2. アップグレードの進捗状況を確認します。
  ...
  | `sessionEvictionRate`     | このノードのMQTTセッション避難速度（秒あたりセッション数）。 |
  | `connectionEvictionRate`  | このノードのMQTT接続避難速度（秒あたり接続数）。             |
  | `initialSessions`         | このノードの初期セッション数。                               |
  | `initialConnections`      | このノードの初期接続数。                                     |

  ノード避難の進捗は、対応する[EMQXノードのステータス](../reference/v2-reference.md#emqxnode)の `connections` および `sessions` カウンターを参照することで推測できます。
...
3. アップグレード完了まで待ちます。
  ...
  `STATUS` が `Ready` であることを確認してください。MQTTクライアントやセッション数によっては、アップグレードに時間がかかる場合があります。
...
| ラベル／プレフィックス    | 説明                                                    |
|--------------------------|---------------------------------------------------------|
| Total                    | 接続の合計数。グラフの最上位のラインとして表示されます。 |
| `emqx-86f864f975`        | 古い3つのEMQXノード群の名前プレフィックス。             |
| `emqx-648c45c747`        | アップグレードされた3つのEMQXノード群の名前プレフィックス。 |

このタイムラインは、EMQX Operatorがどのようにスムーズなブルーグリーンアップグレードを実施するかを示しています。アップグレード中、接続数の合計は（移行速度、サーバー容量、クライアントの再接続戦略などの要因により）安定しており、サーバーへの過負荷を防ぎつつサービスの安定性を高めています。

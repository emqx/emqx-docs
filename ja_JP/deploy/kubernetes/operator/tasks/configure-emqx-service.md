...
* `.spec.dashboardServiceTemplate`を通じてEMQXダッシュボードのServiceを設定
* `.spec.listenersServiceTemplate`を通じてEMQXクラスターのリスナーServiceを設定
...
<<<<<<< HEAD
   デフォルトで、EMQXはポート1883でMQTT TCPリスナー `tcp-default` を、ポート18083でダッシュボードのHTTPリスナーを起動します。

   ユーザーは`.spec.config.data`を通じて新規または既存のリスナーを設定するか、EMQXダッシュボードから管理できます。

   EMQX Operatorはデフォルトのリスナー情報を自動的にServiceリソースに反映します。ユーザーが設定したServiceとEMQXが設定したリスナーに名前やポートの重複がある場合、EMQX Operatorはユーザー設定を優先します。

   :::

2. EMQXクラスターが準備完了になるまで待ちます。

   `kubectl get`でEMQXクラスターの状態を確認し、`STATUS`が`Ready`になっていることを確認してください。準備には時間がかかる場合があります。
=======
...
<<<<<<< HEAD
   - **Add Listener** をクリックし、名前を`test`、ポートを`1884`に設定して新規リスナーを追加します。以下の図を参照してください。
...
<<<<<<< HEAD
   この出力は、ポート1884で追加した新しいリスナーが`emqx-listeners` Serviceリソースに反映されていることを示しています。
=======

...
[MQTTX](https://mqttx.app)はEMQがオープンソースで提供する洗練されたクロスプラットフォームのMQTT 5.0検証ツールです。3種類のツールが含まれています。
...
1. ご使用のOSに応じて、アプリケーションストアまたは[MQTTX公式サイト](https://mqttx.app/)からインストールパッケージをダウンロードします。
2. MQTTXクライアントをインストールします。詳細な手順は[MQTTX - インストール](https://mqttx.app/docs/downloading-and-installation)をご参照ください。

以下の手順に従い、MQTTXデスクトップクライアントで簡単なテストを行います。
...
2. メッセージをパブリッシュするクライアントとして新しい接続を設定します。
   ...
   **General**セクションでクライアントの基本情報を入力します。
   ...
   - **Client ID**: デフォルトのままで構いません。クライアント接続の一意の識別子で、リフレッシュボタンを押すと自動生成されます。
   - **Host**: 使用するプロトコルを選択します。`mqtt://`または`ws://`を選択してください。`SSL/TLS`認証接続を使う場合は`mqtts://`または`wss://`を選択します。ホストIPアドレスはデフォルトで`broker.emqx.io`に設定されており、パブリックブローカーに接続します。自分のEMQXを使う場合は実際のIPに置き換えてください。
   - **Port**: 選択したプロトコルに対応するポート番号を入力します。
   - **Username**と**Password**: ブローカーでユーザー認証が有効な場合は入力し、無ければ空欄のままにします。
   - **SSL/TLS**: `SSL/TLS`認証接続を使う場合はトグルボタンで有効にします。

   他の設定はデフォルトのままにして、右上の**Connect**ボタンをクリックします。
...
5. **Connections**ペインでクライアント`Subscriber`を選択し、**+ New Subscription**をクリックします。
   ...
   **Color**: サブスクリプションを識別するための色を選択できます。
   ...
   他のオプションは空欄のままにして一般的なテストを行います。**Confirm**ボタンをクリックします。
...
これでMQTTXクライアントを使った基本的なパブリッシュ／サブスクライブ操作を試しました。詳細かつ高度な操作については[MQTTX - パブリッシュとサブスクリプション](https://mqttx.app/docs/get-started#publish-and-subscription)をご覧ください。

### MQTTX CLI

[MQTTX CLI](https://mqttx.app/cli)はEMQが提供するオープンソースのMQTT 5.0コマンドラインツールです。MQTTXのコマンドライン版であり、グラフィカルインターフェースを使わずにMQTTサービスやアプリケーションのテスト・デバッグが可能です。
...
2. コマンドラインで以下のコマンドを実行し、EMQXに接続して`testtopic/#`トピックをサブスクライブします。
   ...
   実行が成功すると、コマンドラインは受信待機状態となり、メッセージを受信すると表示します。

   詳細なパラメータは[MQTTX CLI - サブスクライブ](https://mqttx.app/docs/cli/get-started#subscribe)をご覧ください。
...
3. 新しいコマンドラインウィンドウを開き、以下のコマンドを実行してEMQXに接続し、`testtopic/#`トピックにメッセージをパブリッシュします。
   ...
   詳細なパラメータは[MQTTX CLI - パブリッシュ](https://mqttx.app/docs/cli/get-started#publish)をご覧ください。
...
[EMQX ダッシュボード](../../guides/dashboard/introduction.md)はWebSocketクライアントを提供しており、手軽かつ効果的なMQTTテストツールとして利用できます。MQTT over WebSocketを使い、EMQXへの接続、トピックのサブスクライブ、メッセージのパブリッシュをテストできます。
...
2. **Connection**セクションに接続情報を入力します。
   ...
   - **Username**と**Password**: もしあればユーザー名とパスワードを入力し、アクセス制御が無ければ空欄のままにします。

   他の設定はデフォルトのままにします。

3. **Connect**ボタンをクリックして接続を確立します。

4. **Subscription**セクションでサブスクライブする**topic**を`testtopic/#`に設定し、**Subscribe**ボタンをクリックしてサブスクリプションを完了します。`testtopic/#`トピックが下のテーブルに追加されます。

...
サブスクライブを排他的にするには、トピック名の先頭にプレフィックスを追加する必要があります。以下の表は例を示しています。
...
排他サブスクライブはデフォルトで無効化されています。設定ファイルで以下のように有効化できます。
...
3. **Connections**ペインで`Subscriber1`の接続を選択し、**New Subscription**ボタンをクリックしてサブスクライブを作成します。**Topic**テキストボックスに`$exclusive/t/1`と入力し、このトピックにサブスクライブします。**Confirm**をクリックします。

   <img src="./assets/subscribe-exclusive-topic.png" alt="排他トピックへのサブスクライブ" style="zoom:35%;" />

4. **Connections**ペインで`Subscriber2`の接続を選択し、**New Subscription**ボタンをクリックしてサブスクライブを作成します。**Topic**テキストボックスに`$exclusive/t/1`と入力し、このトピックにサブスクライブします。**Confirm**をクリックします。
...
2. ステップ1のコマンドを再度実行して、トピック`$exclusive/t/1`に別のサブスクライブを試みます。以下のように返されます。
   ...
   | 0x97 | 既に別のクライアントがこのトピックにサブスクライブしている。 |

...
- `{DelayTime}`：このMQTTメッセージのパブリッシュを遅延させる時間間隔またはタイムスタンプを秒単位で指定します。間隔の場合、最大許容間隔は42949669秒（約497日）です。タイムスタンプの場合は、現在のシステム時刻から前後42949669秒以内でなければなりません。`{DelayTime}`を整数として解析できない場合や有効範囲外の場合、メッセージは破棄されます。
...
遅延パブリッシュは、[マウントポイント](../../guides/configuration/listener.md#mountpoint)が設定されたリスナー経由で接続されたクライアントには機能しません。EMQXはマウントポイントを `$delayed/` プレフィックスのマッチング前に適用するため、メッセージは即座に通常のメッセージとしてマウントされたリテラルトピック（例：`mp/$delayed/10/t`）にルーティングされます。クライアントにはエラーは報告されません。
:::

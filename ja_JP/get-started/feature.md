...
はい。EMQXは[コマンドラインインターフェース](../guides/cli.md#clients)の`emqx_ctl clients kick <Client ID>`や[REST API](https://docs.emqx.com/en/emqx/v@CE_MINOR_VERSION@/admin/api-docs.html)の`DELETE /clients/{clientid}`を提供しており、ユーザーが手動でMQTT接続をキックできます。Dashboardのクライアントページからも操作可能です。

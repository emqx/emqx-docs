...
Hot upgradeでは、稼働中のEMQX Enterpriseノードを停止せずにパッチバージョンのアップデートを適用できます。接続はアップグレード中も維持され、ノードは次回の再起動時に新しいリリースツリーに切り替わります。

Hot upgradeはパッチバージョンの差分（バージョン番号の3番目の数字）のみ対応しています。例えば、5.10.4から5.10.5へのアップグレードはサポートされますが、5.10.xから5.11.0へのアップグレードはサポートされません。

Hot upgradeは`emqx_relup`プラグインによって実行されます。このプラグインはダッシュボードからインストールします。すべてのアップグレード操作は`emqx ctl relup` CLIを通じて行います。

::: warning 重要なお知らせ
Hot upgradeを実行する前に、`data/`、`etc/`、および`log/`のバックアップを必ず取得してください。アップグレード適用後はインプレースでのロールバックはできません。
:::
...
- プラグインが対象バージョンをサポートするアップグレードパスとして認識していること（ステップ2で確認）。

## ステップ1: emqx_relupプラグインのインストール

ダッシュボードから[ダッシュボード経由でパッケージをインストール](../../guides/extensions/plugin-management.md#install-packages-via-dashboard)の手順に従い、プラグインをインストールします。プラグインパッケージは`https://packages.emqx.io/emqx-plugins/e<EMQX-VSN>/`に公開されています。

インストール後、プラグインのステータスが`running`となっていることを確認してください。

## ステップ2: アップグレードパスのサポート確認

インストール済みプラグインが認識しているバージョンのホップ一覧を表示します。
...
- `emqx-enterprise-<TargetVsn>-<os>-<arch>.tar.gz.sha256`：タールボールのSHA256ダイジェスト。裸のダイジェスト形式と`sha256sum`の出力形式（`<digest>  <filename>`）の両方が受け入れられます。
...
`.sha256`サイドカーはタールボールと同じディレクトリに置き、同じベース名で拡張子だけ`.sha256`にしてください。
...
4. プラグインの`.relup`カタログから対応するコード変更指示を適用します。
5. アップグレード後のコールバックを実行します。
6. `<RootDir>/relup/current`に対象バージョン文字列を書き込みます。

クラスター全体への展開は運用者の責任です。ノードを1台ずつアップグレードし、それぞれを検証してから次に進んでください。
...
`relup/current`には対象バージョン文字列が含まれているはずで、`emqx ctl status`はノードが稼働中であることを示します。
...
適用済みのホップに対するインプレースのロールバックはありません。コード変更はライブVM上で実行され、アップグレード後のコールバックでディスク上のデータが変更されている可能性があるため、元に戻すことはサポートされていません。

限定的な復旧オプションは以下の2つです。

- **次回再起動前:** アップグレード後のコードに問題があるがディスク状態が旧リリースと互換性がある場合、バージョンマーカーを削除して旧ツリーで再起動します。
  ...
  これは起動パスのみを復旧します。アップグレードによるライブ状態の変更は既に反映されています。
...
| `emqx ctl relup list-supported-paths` | プラグインカタログに登録されているサポート済みの`{from, target}`バージョンホップを一覧表示します。 |

# macOSへのEMQXインストール
このページでは、zipファイルを使用してmacOSにEMQXをインストールし、起動する手順を案内します。
...
起動に成功すると、フォアグラウンドモードまたはインタラクティブモードで起動した場合に以下のメッセージが表示されます。

```bash
EMQX Enterprise @EE_VERSION@ is running now!
```

また、本番環境のオペレーター向けの警告メッセージが表示されることがありますが、ローカル環境でのテスト、実験、クライアント開発でEMQXを使用する場合は無視して問題ありません。
...
デフォルトのユーザー名とパスワードはそれぞれ`admin`と`public`です。ログイン後にパスワードの変更を求められます。

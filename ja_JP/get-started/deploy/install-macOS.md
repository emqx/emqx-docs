...
また、以下のような警告メッセージが表示される場合があります。これは本番環境の運用者向けの注意であり、テストや実験、クライアント開発のためにローカル環境でEMQXを使用する場合は無視して問題ありません。
...
* フォアグラウンドモードで起動している場合は、Ctrl+Cを押します。
* インタラクティブモードで起動している場合は、Ctrl+Cを2回押します。

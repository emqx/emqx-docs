...
本セクションでは、EMQX Operatorの環境構築、Operatorのインストール、およびそれを用いたEMQXのデプロイ手順について解説します。本セクションの手順に従うことで、EMQX Operatorを使ったEMQXの効果的なインストールと管理が可能になります。

## 環境の準備

EMQX Operatorをデプロイする前に、以下のコンポーネントが準備されていることを確認してください。

- 稼働中の[Kubernetesクラスター](https://kubernetes.io/docs/concepts/overview/)。Kubernetesのバージョンについては、[Kubernetesバージョンの選択方法](./operator.md#how-to-selector-kubernetes-version)をご参照ください。
...
1. `cert-manager`をインストールして起動します。
   ...
   ::: tip
   `cert-manager`のバージョンは`1.1.6`以上が必要です。すでに`cert-manager`がインストールされて起動している場合は、このステップをスキップしてください。
   :::
   ...
   ::: warning
   Google Kubernetes Engine（GKE）にデフォルト設定でcert-managerをインストールすると、ブートストラップの問題が発生する可能性があります。そのため、`--set global.leaderElection.namespace=cert-manager`の設定を追加し、リーダー選出時に異なるネームスペースを使用するように構成してください。詳細は[cert-managerの互換性](https://cert-manager.io/docs/installation/compatibility/)をご確認ください。
   :::
...
Operatorのインストールが完了したら、次のステップに進む準備ができています。[Deploy EMQX](#deploy-emqx)セクションでは、EMQX Operatorを使ったEMQXのデプロイ方法を解説します。
...
EMQX Operatorを使用してパブリッククラウドプラットフォーム上にEMQXをデプロイする方法については、以下のガイドをご参照ください。

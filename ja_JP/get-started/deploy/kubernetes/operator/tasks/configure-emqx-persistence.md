...
2. EMQXクラスターがReady状態になるまで待ちます。
   ...
   `kubectl get` コマンドでEMQXクラスターのステータスを確認し、`STATUS` が `Ready` であることを確認してください。完了までに時間がかかる場合があります。
...
4. EMQXクラスターがReadyになるまで待ちます。ブラウザでEMQXダッシュボードにアクセスし、以前作成したルールが残っていることを確認してください。以下の図のように表示されます：

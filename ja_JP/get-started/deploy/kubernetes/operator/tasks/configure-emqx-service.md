...
1. 以下の内容をYAMLファイルとして保存し、`kubectl apply`でデプロイします。
   ...
   デフォルトで、EMQXはポート1883でMQTT TCPリスナー `tcp-default` を起動し、ポート18083でダッシュボードHTTPリスナーを起動します。

   ユーザーは`.spec.config.data`を通じて新規または既存のリスナーを設定するか、EMQXダッシュボードから管理できます。

   EMQX Operatorはデフォルトのリスナー情報をServiceリソースに自動反映します。ユーザーが設定したServiceとEMQXが設定したリスナー（名前やポートが重複する場合）が競合した場合、EMQX Operatorはユーザー設定を優先します。

   :::

2. EMQXクラスターがReady状態になるまで待ちます。

   `kubectl get`でEMQXクラスターの状態を確認し、`STATUS`が`Ready`であることを確認してください。完了までに時間がかかる場合があります。
...
1. 新しいリスナーを追加します。
   ...
   - **Add Listener** をクリックし、名前を `test`、ポートを `1884` として新しいリスナーを追加します。以下の図を参照してください：
...
2. 新しいリスナーがServiceに反映されているか確認します。
   ...
   この出力は、新たに追加したポート1884のリスナーが`emqx-listeners` Serviceリソースに反映されていることを示しています。

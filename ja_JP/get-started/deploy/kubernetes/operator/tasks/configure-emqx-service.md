...
LoadBalancerタイプのServiceを通じてEMQXクラスターにアクセスします。
...
2. EMQXクラスターがReady状態になるまで待ちます。
   ...
   `kubectl get` コマンドでEMQXクラスターの状態を確認し、`STATUS`が`Ready`であることを確認してください。準備完了までに時間がかかる場合があります。
...
1. 新しいリスナーを追加します。
   ...
   - **Add Listener** をクリックし、名前を `test`、ポートを `1884` にして新規リスナーを追加します。以下の図を参照してください：
...
2. 新しいリスナーがServiceに反映されているか確認します。
   ...
   この出力から、ポート1884の新規リスナーが `emqx-listeners` Serviceリソースに反映されていることがわかります。

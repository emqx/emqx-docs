...
EMQX の Core-Replicant アーキテクチャの詳細については、[クラスターアーキテクチャ](../../../../../develop/cluster/mria-introduction.md) のドキュメントを参照してください。

:::tip
EMQX クラスターには最低でも1つの Core ノードが必要です。高可用性の観点から、EMQX Operator では EMQX クラスターに最低3つの Core ノードを推奨しています。
:::
...
1. 以下の内容を YAML ファイルとして保存し、`kubectl apply` でデプロイします。
   ...
   上記の例では、EMQX CR により、2つの Core ノードと3つの Replicant ノードからなる EMQX クラスターが定義されています。

   Core ノードは最低512Miのメモリが必要であり、Replicant ノードは最低1Giのメモリが必要です。これらの制約は実際の業務負荷に応じて調整可能です。一般的に Replicant ノードはすべてのクライアント要求を受け入れるため、多数の同時接続に対応するために Replicant ノードのリソースはより多く必要になる場合があります。

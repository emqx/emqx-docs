...
* ローリングアップデート中は、新旧両方のPodが対応するServiceによって選択されます。そのため、MQTTクライアントが終了処理中の古いPodに接続してしまい、頻繁な切断と再接続が発生する可能性があります。
* ローリングアップデートの過程では、新しいPodが起動して準備完了になるまでに時間がかかるため、任意の時点でサービスを提供できるPodは最大で_N - 1_となり、サービスの可用性が低下する恐れがあります。
...
アップグレード全体のプロセスは大まかに以下のステップに分かれます。

1. 更新された仕様で新しいEMQXノード群を作成します。
2. 新しいノード群が準備完了になったら、Serviceリソースを新しいノード群に切り替え、新規接続が古いノード群にルーティングされないようにします。
...
[MQTTX](https://mqttx.app/cli) は、MQTT 5.0対応のオープンソースコマンドラインクライアントツールで、自動再接続をサポートし、MQTTサービスやアプリケーションの開発・デバッグを支援します。
...
1. Podテンプレートの変更は、EMQX Operatorのアップグレード戦略をトリガーします。

  この例では、Podの `ImagePullPolicy` を変更してアップグレードをトリガーしています。
...
2. アップグレードプロセスの状態を確認します。
  ...
  | `sessionEvictionRate`       | このノードのMQTTセッション退避速度（秒あたりのセッション数）。 |
  | `connectionEvictionRate`    | このノードのMQTT接続退避速度（秒あたりの接続数）。           |
  | `initialSessions`           | このノードの初期セッション数。                               |
  | `initialConnections`        | このノードの初期接続数。                                     |

  ノード退避の進捗は、対応する[EMQXノードのステータス](../reference/v2-reference.md#emqxnode)内の `connections` と `sessions` カウンターを参照することで推測可能です。
...
以下のモニタリンググラフは、アップグレード中の接続数を10,000接続の例で示したものです。
...
このタイムラインは、EMQX Operatorがスムーズなブルーグリーンアップグレードを実行する様子を示しています。プロセス全体を通じて、接続数の合計は安定しており（移行速度、サーバー容量、クライアントの再接続戦略などの要因に依存します）、サービスの中断を最小限に抑え、サーバーの過負荷を防ぎ、全体的なサービスの安定性を向上させています。

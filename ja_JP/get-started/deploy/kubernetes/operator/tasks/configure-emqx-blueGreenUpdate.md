...
* ローリングアップデート中は、新旧両方のPodが対応するServiceにより選択されます。これにより、終了処理中の古いPodにMQTTクライアントが接続し、頻繁な切断と再接続が発生する可能性があります。
* ローリングアップデートの過程では、新しいPodが起動して準備完了になるまで時間がかかるため、任意の時点でサービスを提供できるPodは_N - 1_となり、サービスの可用性が低下する恐れがあります。
...
2. 新しいノード群が準備完了したら、Serviceリソースを新ノード群に切り替え、新規接続が古いノード群にルーティングされないようにします。
...
1. Podテンプレートに変更を加えると、EMQX Operatorのアップグレード戦略が起動します。
...
2. アップグレード進行状況を確認します。
  ...
  ノード退避の進行状況は、該当する[EMQXノードのステータス](../reference/v2-reference.md#emqxnode)の `connections` と `sessions` カウンターを参照することで推測できます。
...
| `emqx-648c45c747`          | アップグレードされたEMQXノード3台の名前プレフィックス。 |

このタイムラインは、EMQX Operatorがスムーズなブルーグリーンアップグレードを実施する様子を示しています。アップグレード全体を通じて、接続数の合計は安定しており（移行速度、サーバー能力、クライアントの再接続戦略などの要因に依存します）、サービスの中断を最小限に抑え、サーバーの過負荷を防ぎ、全体的なサービスの安定性を向上させています。

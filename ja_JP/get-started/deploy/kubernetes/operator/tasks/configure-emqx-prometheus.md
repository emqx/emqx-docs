...
[EMQX Exporter](https://github.com/emqx/emqx-exporter) をデプロイし、Prometheus と Grafana を使用して EMQX クラスターを監視します。
...
EMQX は [Prometheus 互換の HTTP API](../../../../../guides/observability/prometheus.md) を通じてさまざまなメトリクスを公開します。
...
Prometheus は EMQX ダッシュボード API からメトリクスを取得するため、ダッシュボードにサインインして [API シークレットを作成](../../../../../guides/dashboard/system.md#api-keys) する必要があります。
    ...
    spec:
      ...
      containers:
        - name: exporter
          ...
          args:
            # "emqx-dashboard-service-name" はオペレーターによって作成され、18083 ポートを公開するサービス名です
...
> 引数の `--emqx.nodes` には、オペレーターによって作成され18083 ポートを公開しているサービス名を指定してください。`kubectl get svc` コマンドでサービス名を確認できます。

上記の内容を `emqx-exporter.yaml` として保存し、`--emqx.auth-username` と `--emqx.auth-password` を新しい API シークレットに置き換えてください。以下のコマンドを実行して `emqx-exporter` をデプロイします。
...
`path` は指標収集インターフェースのパスを示します。EMQX 5 では `/api/v5/prometheus/stats` です。`selector.matchLabels` は対象 Pod のラベルを示し、`apps.emqx.io/instance: emqx` となっています。
...
Prometheus インターフェースを開き、グラフページに切り替えて `emqx` を入力すると、以下の図のように表示されます。

![](./assets/configure-emqx-prometheus/emqx-prometheus-metrics.png)

**Status** -> **Targets** ページに切り替えると、以下の図が表示され、クラスター内のすべての監視対象 EMQX Pod 情報を確認できます。

![](./assets/configure-emqx-prometheus/emqx-prometheus-target.png)

## Grafana テンプレートのインポート

ダッシュボードのすべての[テンプレート](https://github.com/emqx/emqx-exporter/tree/main/grafana-dashboard/template)をインポートしてください。メインダッシュボード **EMQX** を開いてお楽しみください！

...
公式のEMQX Helmチャートは、StatefulSet、Service、ConfigMap、Ingressルール、Gateway APIルートなど、必要なEMQXコンポーネントをすべてパッケージ化し、単一の設定可能なHelmチャートとしてKubernetesベースのデプロイを簡素化します。
...
EMQX Helmチャートは、`values.yaml` ファイルを通じて幅広い設定パラメータを提供します。以下の表は主要なパラメータとデフォルト値を示しています。
...
| `serviceAccount.name`                | 使用するサービスアカウント。設定されておらず、かつ `serviceAccount.create` が `true` の場合、full-nameテンプレートを使って名前が生成されます。 |                                                       |
| `serviceAccount.annotations`         | サービスアカウントに追加するアノテーション。           |                                                       |
| `envFromSecret`                      | 同じKubernetesネームスペース内のシークレット名。環境変数に追加される値を含みます。 | nil                                                   |
| `recreatePods`                       | アップグレード時にPodの再作成を強制します。最新の設定を常に適用したい場合に有効です。 | false                                                 |
| `podAnnotations `                    | Podに付与するアノテーション                            | `{}`                                                  |
| `podManagementPolicy`                | 既存のPVCがある状態でチャートを再デプロイする場合、デッドロックを避けるために `Parallel` に設定する必要があります。 | `Parallel`                                            |
...
フィーチャーゲートはEMQX起動時にのみ解決されます。この値を変更した場合は、EMQXのPodを再作成または再起動してください。完全な機能リストと依存関係の挙動については、[Feature Gates](../feature-gates.md)を参照してください。
...
- `httpRoute.ws` を有効にするには、`ServicePort.appProtocol` を要求しないHTTPRoute WebSocketトラフィックをサポートするGateway APIコントローラーを使用してください。EMQX HelmチャートはWebSocket Serviceポートに `appProtocol` を設定しません。
- ルートにマッチするリスナーを持つGatewayを作成してください。
- `tlsRoute.mqtts` および `tlsRoute.wss` で使用されるGateway TLSリスナーを `tls.mode: Passthrough` に設定してください。EMQXがTLS接続を終端します。

すべてのルートはデフォルトで無効です。有効にしたルートには必ず `parentRefs` を設定し、ルートをGatewayにアタッチしてください。以下の `values.yaml` の例は、サポートされているすべてのルートを有効にします：
...
Gateway名、ネームスペース、リスナーのセクション名、ホスト名は環境に合わせて置き換えてください。GatewayとHelmリリースが異なるネームスペースにある場合は、参照される各Gatewayリスナーで `allowedRoutes` を設定し、Helmリリースのネームスペースからのルートを許可してください。チャートは以下のルートを作成します：
...
アタッチされた各ルートについて、`status.parents` 内の `Accepted` と `ResolvedRefs` 条件が `True` であることを確認してください。コントローラーが `Programmed` 条件を報告している場合は、それも `True` であることを確認してください。

ルート条件はエンドツーエンドのトラフィックを検証しません。設定したダッシュボードまたはWebSocketホスト名にリクエストを送信するか、Gateway経由でMQTTSまたはWSS接続を確立し、ルートがEMQXにトラフィックを転送できることを確認してください。
...
| `httpRoute.<route>.annotations` | HTTPRouteに追加されるアノテーション。 | `{}` |
| `httpRoute.<route>.labels` | HTTPRouteに追加されるラベル。 | `{}` |
...
| `tlsRoute.<route>.annotations` | TLSRouteに追加されるアノテーション。 | `{}` |
| `tlsRoute.<route>.labels` | TLSRouteに追加されるラベル。 | `{}` |
| `tlsRoute.<route>.parentRefs` | 親GatewayおよびTLSリスナーへの参照。 | `[]` |
| `tlsRoute.<route>.hostnames` | ルートがマッチする必須のSNIホスト名。リストには少なくとも1つの有効なFQDNが必要です。 | `mqtt.emqx.local`（mqttsの場合）、`wss.emqx.local`（wssの場合） |
...
HAProxy Ingress Controllerの場合は、以下のアノテーションを追加してください：

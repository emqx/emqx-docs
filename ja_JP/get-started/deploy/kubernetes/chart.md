...
GitHubからリリース名 `my-emqx` でチャートをインストールするには、以下のコマンドを実行します：
...
公式Helmチャートリポジトリからリリース名 `my-emqx` でチャートをインストールするには、以下のコマンドを実行します：
...
| `persistence.existingClaim`          | EMQXデータの既存Persistent Volumeクレーム名。テンプレートとして評価されます。 | ""                                                    |
...
| `ingress.dashboard.ingressClassName` | EMQXダッシュボード用のIngressクラスを設定              |                                                       |
| `ingress.mqtt.enabled`               | MQTT用のIngressを有効化                                | false                                                 |

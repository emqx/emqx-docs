...
本ガイドでは、既存の HiveMQ デプロイメントを EMQX に移行する方法について説明します。TLS（ポート 8883）経由でデバイスが接続し、HiveMQ Enterprise Security Extension（ESE）で管理される X.509 クライアント証明書またはユーザー名／パスワード認証を使用する一般的なエンタープライズパターンに焦点を当てています。目的は、HOCON 設定とルールエンジンを用いて、EMQX で同等の接続性、認証、およびデータ統合の動作を再現することです。
...
2. **EMQX の設定**：HiveMQ の設定を EMQX の HOCON に変換し、キーストアを PEM に変換、リスナーとクラスター設定を再作成し、認証チェーンとルールエンジンを設定します。
3. **デバイスおよび統合の更新**：デバイスを EMQX エンドポイントにリダイレクトし、EMQX サーバー CA 証明書を展開、クライアントの識別を検証し、Kafka や Prometheus などの下流統合を移行します。

以下の表は、HiveMQ と EMQX 間の主要なアーティファクトの対応をまとめたものです：

| **パラメーター／アーティファクト** | **HiveMQ（例）** | **EMQX（例）** | **備考** |
| --- | --- | --- | --- |
| エンドポイントホスト名 | `mqtt.internal.example.com`（ロードバランサー／コントロールセンターで設定） | `mqtt.example.com`（EMQX ロードバランサー／VIP） | デバイスのファームウェアやデプロイマニフェストを更新してください。 |
...
| クラスター検出 | DNS または `extensions/*-discovery*/*.properties` | `cluster.discovery_strategy = dns`（または `static`、`etcd`、`k8s`） | 拡張ベースの検出を EMQX のネイティブ戦略に置き換えます。 |
...
3. 生成された PEM ファイルを `/etc/emqx/certs/`（またはコンテナのシークレットマウント）にコピーします。HiveMQ で信頼されているデバイス CA（`device-ca.pem`）は保持し、EMQX で mTLS 検証に再利用します。

### HiveMQ 設定ファイルのエクスポート

トレーサビリティのため、以下のアーティファクトをバージョン管理に保存してください。環境変数プレースホルダー（例：`${ENV:HIVEMQ_PORT}`）は、EMQX のダブルアンダースコア環境変数オーバーライド構文（`EMQX_LISTENERS__TCP__DEFAULT__BIND=0.0.0.0:1883`）にマッピングできるように目立たせてください。

- `conf/config.xml`：リスナー、制約、クラスタリング、パーシステンス、コントロールセンターのユーザー
- `conf/logback.xml`：ログターゲット（EMQX の `log` セクションに変換）
...
## フェーズ 2：HiveMQ のベースラインを再現するための EMQX 設定
...
その他の検出方法（etcd、Kubernetes、静的ファイルなど）については、[クラスターの作成と管理](../../guides/cluster/create-cluster.md)を参照してください。

### 認証と認可の変換

HiveMQ は Enterprise Security Extension（ESE）を通じて、**レルム**（データソース）と**パイプライン**（ロジック）を定義し、またはレガシープラグインを使用します。EMQX は **認証チェーン**（順序付けられたバックエンド）と **認可ソース**（ACL）を使用します。

| HiveMQ ESE コンポーネント | EMQX の対応 | 移行戦略 |
| :--- | :--- | :--- |
| **ファイルレルム**（`credentials.xml`） | [**組み込みデータベース**](../../guides/access-control/authn/mnesia.md) | HiveMQ ユーザーをエクスポートし、EMQX REST API でインポートします。 |
| **SQL レルム**（JDBC） | [**MySQL**](../../guides/access-control/authn/mysql.md) / [**PostgreSQL**](../../guides/access-control/authn/postgresql.md) | `mysql` または `postgresql` バックエンドでパスワード認証を設定。既存のユーザーテーブルを再利用します。 |
| **LDAP レルム** / AD | [**LDAP**](../../guides/access-control/authn/ldap.md) | LDAP バックエンドでパスワード認証を設定。HiveMQ の DN パターンを EMQX のフィルターテンプレートにマッピングします。 |
| **OAuth / JWT** | [**JWT**](../../guides/access-control/authn/jwt.md) | JWT 認証メカニズムを設定。公開鍵または JWKS エンドポイントを設定します。 |
| **HTTP / Webhooks** | [**HTTP サーバー**](../../guides/access-control/authn/http.md) | HTTP バックエンドでパスワード認証を設定し、外部認証サービスに委譲します。 |
| **X.509 証明書** | [**X.509**](../../guides/access-control/authn/x509.md) / [**mTLS**](../../guides/network/emqx-mqtt-tls.md#enable-ssl-tls-with-two-way-authentication) | `TLS` リスナーと相互（双方向）認証を使用し、既存の CA とクライアント証明書を再利用します。 |
...
2. **インポート：** EMQX REST API を使用してユーザーを作成します。EMQX は bcrypt や pbkdf2 などのパスワードハッシュを含むユーザーの一括インポートをサポートしています。ファイル形式の詳細は [ユーザーのインポート](../../guides/access-control/authn/user_management.md#importing-users) を参照してください。
...
HiveMQ は SQL レルム用に [固定のデータベーススキーマ](https://docs.hivemq.com/hivemq-enterprise-security-extension/latest/ese.html#table_users) を使用します。一方、EMQX は [独自のスキーマとクエリを定義可能](../../guides/access-control/authn/mysql.md)です。**既存の MySQL や PostgreSQL データベースを変更する必要はありません。**

以下の EMQX 設定例は、標準的な HiveMQ `users` テーブル構造に対応したクエリを使用しています。
...
HiveMQ は `enterprise-security-extension.xml`（ファイルレルム）または外部データベースでアクセスポリシーを定義します。EMQX は複数のバックエンドを同時にサポートする柔軟な **認可チェーン** を提供します（ファイル、Redis、MySQL、PostgreSQL、MongoDB、HTTP など）。
...
- [**ファイル（`acl.conf`）**](../../guides/access-control/authz/file.md)：`{allow, all, subscribe, ["device/${clientid}/#"]}.`
- [**組み込みデータベース**](../../guides/access-control/authz/mnesia.md)：クライアント ID、ユーザー名、トピックに基づきダッシュボードまたは API でルールを設定。
- [**MySQL**](../../guides/access-control/authz/mysql.md)：`SELECT action, permission, topic, ipaddress, qos, retain FROM mqtt_acl where clientid = ${clientid} and ipaddress = ${peerhost}`。
- [**PostgreSQL**](../../guides/access-control/authz/postgresql.md)：`SELECT action, permission, topic, ipaddress, qos, retain FROM mqtt_acl where clientid = ${clientid} and ipaddress = ${peerhost}`。

詳細は [認可](../../guides/access-control/authz/authz.md) ドキュメントを参照してください。

### データ統合の設定

HiveMQ は個別の拡張（例：Kafka Extension）に依存していますが、EMQX ではすべてのデータ統合が標準で組み込まれ、有効化されています。

特定の統合を設定する前に、以下のコアコンセプトを理解してください：
- [**データ統合の概要**](../../develop/data-integration/data-bridges.md)
- [**ルールエンジン**](../../develop/data-integration/rules.md)
- [**Flowデザイナー**](../../develop/flow-designer/introduction.md)
...
HiveMQ は「Prometheus Monitoring HiveMQ Extension」を使用します。EMQX はネイティブで Prometheus をサポートしています。

Prometheus がメトリクスをスクレイプするエンドポイントはデフォルトで有効です：`http://emqx-node:18083/api/v5/prometheus/stats`。

EMQX 6.3.0 以降、Prometheus スクレイプエンドポイントはデフォルトで認証が必要です。API キーとシークレットキーでスクレイパーを設定するか、認証を明示的に無効化してください。詳細は [Prometheus 連携](../../guides/observability/prometheus.md#authentication) を参照してください。

Pushgateway を使用する場合は、以下のように設定します：
...
Pushgateway 設定の詳細は [Prometheus 連携](../../guides/observability/prometheus.md#configure-push-mode-integration) を参照してください。

#### ロギング

HiveMQ は `logback.xml`（Java 標準）を使用します。EMQX は HOCON で設定可能な組み込みのログ機能を使用します。
...
ログレベル、ローテーション、フォーマッター（テキスト／JSON）の設定については [ログ](../../guides/observability/log.md) を参照してください。

#### ログトレース

HiveMQ の「Trace Recordings」は特定のクライアントセッションのデバッグに使われます。EMQX は組み込みの **ログトレース** 機能（ダッシュボードまたは CLI）を提供し、特定のクライアント ID、トピック、IP のログをリアルタイムでフィルタリング可能です。
...
詳細は [ログトレース](../../guides/observability/tracer.md) を参照してください。

## フェーズ 3：デバイスおよび統合の更新

### EMQX サーバー CA をデバイスに展開

- EMQX が内部 CA を使用する場合は、各デバイスに `device-ca.pem` をインストールします（システムの信頼ストアまたはアプリケーションバンドル）。
- EMQX が Let’s Encrypt 等の公開 CA を使用する場合は、デバイス側の対応は不要です。
...
変更点はエンドポイントホスト名とサーバー CA ファイルのみです。デバイス証明書と秘密鍵は、EMQX の `ssl_options.cacertfile` で参照される同一 CA によって署名されていれば引き続き利用可能です。

### 統合の検証

- EMQX のルールメトリクス（`emqx ctl rule show`）で Kafka トピックがメッセージを受信していることを確認。
- 監視ダッシュボードを更新して EMQX メトリクスをスクレイプ。
- アラートシステム（Splunk、ELK など）を EMQX のログフォーマットに合わせて再設定。
...
1. HiveMQ を一時的に稼働させ続ける。
2. HiveMQ で `#` をサブスクライブし、保持メッセージを EMQX に再パブリッシュするブリッジクライアントを実行。
3. QoS 1/2 のキューイングされたメッセージは、DNS 切り替え前にインフライトトランザクションを完了させる。
...
HiveMQ は静的な XML と拡張固有のリロードセマンティクスに依存しています。EMQX は動的な設定 API を提供します：
...
これは `data/configs/cluster.hocon` に書き込みます。設定を不変（`emqx.conf` のみ）にするか、環境ごとのオーバーライドを可能にする EMQX の二層モデルを採用するかを検討してください。
...
- EMQX セッションのデバイス ID が元の HiveMQ クライアント ID と一致すること。
- ACL が HiveMQ で適用していたトピックアクセスを同様に制御していること。
...
インベントリ、設定、更新の3フェーズを順に実施することで、デバイスの資格情報、トピック構造、統合フローを維持しつつ、EMQX の高並行性な Erlang ランタイムと動的設定機能を活用できます。

移行計画を慎重に立て、各リスナーと統合を検証し、自信を持ってカットオーバーを実行してください。

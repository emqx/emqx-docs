...
| :--- | :--- | :--- |
| **ファイルレルム**（`credentials.xml`） | [**組み込みデータベース**](../../guides/access-control/authn/mnesia.md) | HiveMQ のユーザーをエクスポートし、EMQX REST API でインポートします。 |
| **SQL レルム**（JDBC） | [**MySQL**](../../guides/access-control/authn/mysql.md) / [**PostgreSQL**](../../guides/access-control/authn/postgresql.md) | `mysql` または `postgresql` バックエンドを使ったパスワード認証を設定し、既存のユーザーテーブルを再利用します。 |
| **LDAP レルム** / AD | [**LDAP**](../../guides/access-control/authn/ldap.md) | LDAP バックエンドを使ったパスワード認証を設定し、HiveMQ の DN パターンを EMQX のフィルターテンプレートにマッピングします。 |
| **OAuth / JWT** | [**JWT**](../../guides/access-control/authn/jwt.md) | JWT 認証メカニズムを設定し、公開鍵または JWKS エンドポイントを設定します。 |
| **HTTP / Webhooks** | [**HTTP サーバー**](../../guides/access-control/authn/http.md) | HTTP バックエンドを使ったパスワード認証を設定し、外部認証サービスに認証情報を委譲します。 |
...
2. **インポート：** EMQX REST API を使用してユーザーを作成します。EMQX は bcrypt や pbkdf2 などのパスワードハッシュの一括インポートをサポートしています。詳細は [ユーザーのインポート](../../guides/access-control/authn/user_management.md#importing-users) を参照してください。
...
- [**組み込みデータベース**](../../guides/access-control/authz/mnesia.md)：クライアント ID、ユーザー名、トピックに基づきダッシュボードや API でルールを設定

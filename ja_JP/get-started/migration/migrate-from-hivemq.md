...
2. **EMQX の設定**：HiveMQ の設定を EMQX の HOCON に変換し、キーストアを PEM 形式に変換し、リスナーとクラスタ設定を再作成し、認証チェーンとルールエンジンをセットアップします。
...
| --- | --- | --- | --- |
| エンドポイントホスト名 | `mqtt.internal.example.com`（ロードバランサー／Control Center 設定） | `mqtt.example.com`（EMQX ロードバランサー／VIP） | デバイスのファームウェアやデプロイマニフェストを更新してください。 |
| TLS 資産 | `conf/hivemq.jks` | `/etc/emqx/certs/server-cert.pem`、`/etc/emqx/certs/server-key.pem` | `keytool` と `openssl` を使って JKS/PKCS12 から PEM に変換します。 |
...
| クライアント証明書 | デバイスフリートに PEM 形式で保存、mTLS 有効時に HiveMQ が検証 | 同じデバイス証明書、EMQX リスナーの `ssl_options.cacertfile = "device-ca.pem"` | 同一 CA を使用する場合は再プロビジョニング不要です。 |
...
以下のアーティファクトをバージョン管理に保存し、トレーサビリティを確保します。環境変数プレースホルダー（例：`${ENV:HIVEMQ_PORT}`）は EMQX のダブルアンダースコア環境変数オーバーライド構文（`EMQX_LISTENERS__TCP__DEFAULT__BIND=0.0.0.0:1883`）にマッピングできるように強調してください。
...
それぞれのパスは EMQX の特定の認証チェーンにマッピングされます。
...
HiveMQ はグローバル制限を `<restrictions>` にまとめていますが、EMQX はこれらの値をグローバルな `mqtt` セクションと各リスナーに分割しています。
...
EMQX は同一マシン上で複数ノードが稼働する場合、自動的に Erlang 分散ポートを割り当てるため、`bind-port` を手動で指定する必要はありません。

その他の検出方法（etcd、Kubernetes、静的ファイルなど）については、[クラスターの作成と管理](../../guides/cluster/create-cluster.md)を参照してください。

### 認証と認可の翻訳

HiveMQ は Enterprise Security Extension（ESE）を通じて、**レルム**（データソース）と**パイプライン**（ロジック）を管理します。一方、EMQX は**認証チェーン**（順序付けられたバックエンド）と**認可ソース**（ACL）を使用します。

| HiveMQ ESE コンポーネント | EMQX 対応 | 移行戦略 |
| :--- | :--- | :--- |
| **ファイルレルム**（`credentials.xml`） | [**組み込みデータベース**](../../guides/access-control/authn/mnesia.md) | HiveMQ ユーザーをエクスポートし、EMQX REST API でインポート。 |
| **SQL レルム**（JDBC） | [**MySQL**](../../guides/access-control/authn/mysql.md) / [**PostgreSQL**](../../guides/access-control/authn/postgresql.md) | パスワード認証を `mysql` または `postgresql` バックエンドで設定。既存のユーザーテーブルを再利用。 |
| **LDAP レルム** / AD | [**LDAP**](../../guides/access-control/authn/ldap.md) | LDAP バックエンドでパスワード認証を設定。HiveMQ の DN パターンを EMQX のフィルターテンプレートにマッピング。 |
| **OAuth / JWT** | [**JWT**](../../guides/access-control/authn/jwt.md) | JWT 認証機構を設定。公開鍵または JWKS エンドポイントを設定。 |
| **HTTP / Webhooks** | [**HTTP サーバー**](../../guides/access-control/authn/http.md) | HTTP バックエンドでパスワード認証を設定し、外部認証サービスに委譲。 |
| **X.509 証明書** | [**X.509**](../../guides/access-control/authn/x509.md) / [**mTLS**](../../guides/network/emqx-mqtt-tls.md#enable-ssl-tls-with-two-way-authentication) | `TLS` リスナーと相互（双方向）認証を使用し、既存の CA とクライアント証明書を再利用。 |
...
1. **エクスポート：** HiveMQ のファイルレルム（`credentials.xml`）からユーザーを抽出します。このファイルは通常、ハッシュ化されたパスワードとソルトを含みます。XML を解析して EMQX 用の JSON または CSV インポートファイルを生成する必要があります。
2. **インポート：** EMQX REST API を使用してユーザーを作成します。EMQX は bcrypt や pbkdf2 などのパスワードハッシュのバルクインポートをサポートしています。ファイル形式の詳細は[ユーザーのインポート](../../guides/access-control/authn/user_management.md#importing-users)を参照してください。
...
HiveMQ は SQL レルムに [固定スキーマ](https://docs.hivemq.com/hivemq-enterprise-security-extension/latest/ese.html#table_users) を使用しますが、EMQX は [独自のスキーマとクエリを定義可能](../../guides/access-control/authn/mysql.md)です。**既存の MySQL や PostgreSQL データベースを変更する必要はありません。**

以下の EMQX 設定例は、HiveMQ 標準の `users` テーブル構造に特化したクエリ（`SELECT password_hash, salt ...`）を使用しています。
...
HiveMQ は `enterprise-security-extension.xml`（ファイルレルム）や外部データベースでアクセスポリシーを定義します。EMQX は複数のバックエンドを同時にサポートする柔軟な**認可チェーン**を備えています（ファイル、Redis、MySQL、PostgreSQL、MongoDB、HTTP など）。
...
- [**組み込みデータベース**](../../guides/access-control/authz/mnesia.md)：ダッシュボードまたは API でクライアント ID、ユーザー名、トピックに基づくルールを設定。
...
HiveMQ は Kafka 拡張など個別の拡張に依存していますが、EMQX ではすべてのデータ統合が組み込みで有効化されています。

特定の統合を設定する前に、以下のコアコンセプトを理解してください。

- [**データ統合概要**](../../develop/data-integration/data-bridges.md)
...
Pushgateway を使用する場合は以下のように設定します。
...
詳細は[Prometheus との統合](../../guides/observability/prometheus.md#integrate-with-prometheus)ガイドを参照してください。

#### ロギング

HiveMQ は `logback.xml`（Java 標準）を使用しますが、EMQX は HOCON で設定可能な組み込みのロギング機能を持っています。
...
ログレベル、ローテーション、フォーマッター（テキスト／JSON）の設定は[ログ](../../guides/observability/log.md)を参照してください。
...
変更点はエンドポイントホスト名とサーバー CA ファイルのみです。デバイス証明書と秘密鍵は、EMQX の `ssl_options.cacertfile` で参照される同一 CA によって署名されていれば引き続き有効です。

### 統合の検証

- Kafka トピックがメッセージを受信していることを EMQX ルールメトリクス（`emqx ctl rule show`）で確認。
- 監視ダッシュボードを更新して EMQX メトリクスをスクレイプ。
- アラートシステム（Splunk、ELK）を EMQX ログ形式に合わせて再設定。
...
HiveMQ の `$share/group/topic` 構文は EMQX で完全サポートされています。以前 `$queue/topic` を使用していた場合は `$share/queue/topic` にマッピングしてください。`broker.shared_subscription_strategy`（例：`round_robin`、`hash_clientid`）を調整し、コンシューマーの負荷分散動作を模倣します。

### HTTP/API 駆動の設定

HiveMQ は静的 XML と拡張固有のリロードセマンティクスに依存しますが、EMQX は動的設定 API を提供します。
...
これは `data/configs/cluster.hocon` に書き込みます。設定を不変（`emqx.conf` のみ）にするか、環境ごとのオーバーライドを許容する EMQX の二層モデルを採用するか検討してください。
...
- TLS ハンドシェイクが成功し、クライアント証明書なしでは失敗すること（mTLS デバイスの場合）。
- EMQX セッションのデバイス ID が元の HiveMQ クライアント ID と一致すること。
- ACL が HiveMQ で適用していたトピックアクセス制御を適切に強制していること。
- クラスターのノードがネットワーク分断後に自動回復すること。
- Kafka 統合が変換の問題なくデータを受信していること。
...
インベントリ、設定、更新の3フェーズを踏むことで、デバイス認証情報、トピック構造、統合フローを維持しつつ、EMQX の高並行性な Erlang ランタイムと動的設定機能を活用できます。

計画的に移行を進め、各リスナーと統合を検証し、自信を持ってカットオーバーを実施してください。

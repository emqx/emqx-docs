...
本ガイドでは、既存の HiveMQ デプロイメントを EMQX に移行する方法について説明します。TLS（ポート 8883）経由でデバイスが X.509 クライアント証明書または HiveMQ Enterprise Security Extension（ESE）で管理されるユーザー名／パスワード認証情報を用いて接続する一般的なエンタープライズパターンに焦点を当てています。目的は、HOCON 設定とルールエンジンを用いて、EMQX で同等の接続性、認証、およびデータ統合の動作を再現することです。
...
2. **EMQX の設定**：HiveMQ の設定を EMQX の HOCON に変換し、キーストアを PEM に変換し、リスナーとクラスター設定を再作成し、認証チェーンとルールエンジンを設定します。
...
| **パラメータ／アーティファクト** | **HiveMQ（例）** | **EMQX（例）** | **備考** |
| --- | --- | --- | --- |
| エンドポイントホスト名 | `mqtt.internal.example.com`（ロードバランサー / Control Center 設定） | `mqtt.example.com`（EMQX ロードバランサー / VIP） | デバイスのファームウェアやデプロイメントマニフェストを更新してください。 |
...
| クライアント証明書 | デバイスフリートに PEM 形式で保存、mTLS 有効時に HiveMQ が検証 | 同じデバイス証明書、EMQX リスナー `ssl_options.cacertfile = "device-ca.pem"` | 同一 CA を使用する場合は再プロビジョニング不要です。 |
...
3. 生成された PEM ファイルを `/etc/emqx/certs/`（またはコンテナのシークレットマウント）にコピーします。HiveMQ が信頼しているデバイス CA（`device-ca.pem`）は保持してください。EMQX は mTLS 検証にこの証明書を再利用します。

### HiveMQ 設定ファイルのエクスポート

以下のアーティファクトをバージョン管理に保存し、トレーサビリティを確保します。環境変数プレースホルダー（例：`${ENV:HIVEMQ_PORT}`）は、EMQX のダブルアンダースコア環境変数オーバーライド構文（例：`EMQX_LISTENERS__TCP__DEFAULT__BIND=0.0.0.0:1883`）にマッピングできるように強調してください。
...
それぞれの方式は特定の EMQX 認証チェーンにマッピングされます。
...
キューサイズ、QoS、保持メッセージの動作などの HiveMQ 設定は、EMQX の `mqtt` セクションに直接マッピングされます。
...
EMQX は同一マシン上に複数ノードがある場合、Erlang 分散ポートを自動割り当てするため、`bind-port` の手動指定は不要です。

その他の検出方法（etcd、Kubernetes、静的ファイルなど）については、[クラスターの作成と管理](../../guides/cluster/create-cluster.md) を参照してください。

### 認証と認可の翻訳

HiveMQ は Enterprise Security Extension（ESE）を通じて、**レルム**（データソース）と**パイプライン**（ロジック）を定義し、またはレガシープラグインを利用しています。EMQX は **認証チェーン**（順序付けられたバックエンド）と **認可ソース**（ACL）を使用します。
...
| **SQL レルム**（JDBC） | [**MySQL**](../../guides/access-control/authn/mysql.md) / [**PostgreSQL**](../../guides/access-control/authn/postgresql.md) | パスワード認証を `mysql` または `postgresql` バックエンドで設定。既存のユーザーテーブルを再利用します。 |
| **LDAP レルム** / AD | [**LDAP**](../../guides/access-control/authn/ldap.md) | LDAP バックエンドでパスワード認証を設定。HiveMQ の DN パターンを EMQX のフィルターテンプレートにマッピングします。 |
| **OAuth / JWT** | [**JWT**](../../guides/access-control/authn/jwt.md) | JWT 認証機構を設定。公開鍵または JWKS エンドポイントを設定します。 |
| **HTTP / Webhooks** | [**HTTP サーバー**](../../guides/access-control/authn/http.md) | HTTP バックエンドでパスワード認証を設定し、外部認証サービスに認証情報を委譲します。 |
...
1. **エクスポート:** HiveMQ のファイルレルム（`credentials.xml`）からユーザーを抽出します。このファイルは通常、ハッシュ化されたパスワードとソルトを含みます。XML を解析し、EMQX 用の JSON または CSV インポートファイルを生成する必要があります。
2. **インポート:** EMQX REST API を使ってユーザーを作成します。EMQX は bcrypt や pbkdf2 などのパスワードハッシュを含むユーザーの一括インポートをサポートしています。ファイル形式の詳細は [ユーザーのインポート](../../guides/access-control/authn/user_management.md#importing-users) を参照してください。
...
HiveMQ は SQL レルムに対して [固定のデータベーススキーマ](https://docs.hivemq.com/hivemq-enterprise-security-extension/latest/ese.html#table_users) を使用します。一方、EMQX は [独自のスキーマとクエリを定義可能](../../guides/access-control/authn/mysql.md) です。**既存の MySQL または PostgreSQL データベースを変更する必要はありません。**

以下の EMQX 設定例は、標準的な HiveMQ `users` テーブル構造に合わせて調整されたクエリ（`SELECT password_hash, salt ...`）を使用しています。

既存の HiveMQ MySQL データベースに対してスキーマを変更せずに認証するための EMQX 設定例：
...
- [**組み込みデータベース**](../../guides/access-control/authz/mnesia.md)：クライアント ID、ユーザー名、トピックに基づいてダッシュボードや API でルールを設定。
...
HiveMQ は Kafka 拡張など個別の拡張に依存してデータ統合を行います。EMQX ではすべてのデータ統合が組み込みであり、すぐに利用可能です。
...
HiveMQ は「Prometheus Monitoring HiveMQ Extension」を使用します。EMQX はネイティブに Prometheus をサポートしています。
...
詳細は [Prometheus との統合](../../guides/observability/prometheus.md#integrate-with-prometheus) を参照してください。

#### ロギング

HiveMQ は Java 標準の `logback.xml` を使用します。EMQX は HOCON で設定可能な組み込みログ機能を使用します。
...
HiveMQ の「Trace Recordings」は特定クライアントセッションのデバッグに使われます。EMQX はダッシュボードや CLI で特定のクライアント ID、トピック、IP に対してリアルタイムにログをフィルタリングできる組み込みの **ログトレース** 機能を提供します。
...
- EMQX が内部 CA を使用している場合は、各デバイスに `device-ca.pem` をインストールします（システムの信頼ストアまたはアプリケーションバンドル）。
...
変更点はエンドポイントのホスト名とサーバー CA ファイルのみです。デバイス証明書と秘密鍵は、EMQX `ssl_options.cacertfile` で参照される同一 CA によって署名されていれば引き続き有効です。

### 統合の検証

- EMQX ルールのメトリクス（`emqx ctl rule show`）で Kafka トピックがメッセージを受信しているか確認します。
- 監視ダッシュボードを更新して EMQX メトリクスをスクレイプします。
- Splunk や ELK などのアラートシステムを EMQX のログフォーマットに合わせて再設定します。
...
2. HiveMQ に `#` をサブスクライブし、保持メッセージを EMQX に再パブリッシュするブリッジクライアントを実行します。
3. QoS 1/2 のキューイングされたメッセージについては、DNS 切り替え前にインフライトトランザクションを完了させます。

### 共有サブスクリプション

HiveMQ の `$share/group/topic` 構文は EMQX で完全サポートされています。以前 `$queue/topic` を使用していた場合は `$share/queue/topic` にマッピングしてください。`broker.shared_subscription_strategy`（例：`round_robin`, `hash_clientid`）を調整し、コンシューマーが期待するロードバランシング動作を再現します。

### HTTP/API ベースの設定

HiveMQ は静的 XML と拡張固有のリロードセマンティクスに依存します。EMQX は動的設定 API を提供します。
...
本番トラフィック切り替え前に以下を検証してください。

- すべての EMQX リスナーが `running` 状態であること（`emqx ctl listeners list`）。
- TLS ハンドシェイクが成功し、mTLS デバイスでクライアント証明書未提示時に失敗すること。
- EMQX セッションのデバイス ID が元の HiveMQ クライアント ID と一致すること。
- ACL が HiveMQ と同様のトピックアクセス制御を実施していること。
- クラスターのノードがネットワーク分断後に自動回復すること。
- Kafka 統合が変換なしでデータを受信していること。
- Prometheus にメトリクスが表示されていること。

## まとめ

HiveMQ から EMQX への移行は主に設定の翻訳作業です。Java 中心のアーティファクト（XML、JKS、拡張）を EMQX の HOCON 設定、柔軟な認証チェーン、データ統合フレームワークに変換します。

インベントリ、設定、更新の3フェーズを順に進めることで、デバイス認証情報、トピック構造、統合フローを維持しつつ、EMQX の高並行性な Erlang ランタイムと動的設定機能を活用できます。

移行計画を慎重に立て、各リスナーと統合を検証し、安心して切り替えを実行してください。

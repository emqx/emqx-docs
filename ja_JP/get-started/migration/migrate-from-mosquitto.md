...
本ガイドは、既存の Eclipse Mosquitto デプロイメントを EMQX に移行する手順を説明します。軽量な単一インスタンスのブローカーから、スケーラブルで分散型の MQTT プラットフォームへ移行したい管理者向けに設計されています。移行は EMQX の標準 MQTT プロトコル互換性を活用し、設定、認証情報、統合ロジックの移行を明確に示します。
...
2. **EMQX の設定**：Mosquitto の設定を EMQX の HOCON 形式設定ファイル（`emqx.conf`）に変換し、ユーザー認証情報をインポート、アクセス制御やデータ統合をルールエンジンで再構築します。  
3. **デバイスと統合の更新**：デバイスを EMQX クラスターにリダイレクト（ポート互換性により多くの場合シームレス）、システム動作を検証します。

| パラメータ／資産 | Mosquitto（例） | EMQX（例） | 備考 |
| :--- | :--- | :--- | :--- |
| **メイン設定** | `/etc/mosquitto/mosquitto.conf` | `/etc/emqx/emqx.conf` | EMQX は階層的な HOCON 形式を使用。 |
| **ネットワークポート** | `1883`（TCP）、`8883`（SSL） | `1883`（TCP）、`8883`（SSL） | 標準ポートは一致。通常デバイス設定変更不要。 |
...
| **パーシステンス** | `mosquitto.db` | `data/`（Mnesia + RocksDB） | EMQX はセッション永続化を自動処理。 |
...
Mosquitto はリスナーを順次定義しますが、EMQX は種類別（TCP、SSL、WebSocket）にグループ化して `emqx.conf` に記述します。
...
**注意：** Mosquitto はセッション期限をグローバルに管理しますが、EMQX（MQTT 5.0）はクライアントごとにセッション期限を設定可能です。MQTT 3.1.1 クライアント向けには、EMQX でグローバルデフォルトを設定し Mosquitto ポリシーに合わせられます。

### 認証の移行

EMQX は複数の認証バックエンドをサポートします。多くの Mosquitto 移行では、既存の認証情報を保持しつつユーザーパスワードのリセットを不要にすることが目標です。
...
**インポートコマンド：**  
`curl` でファイルをアップロードします。`type=plain` パラメータは EMQX にインポート時にパスワードをハッシュ化させる指定です。
...
* 認証方式（`password_based:built_in_database`）が存在し、設定と一致している必要があります。

#### オプション 2：Mosquitto パスワードファイルのインポート（高度）

大量ユーザーがいて、`mosquitto.passwd`（ハッシュ済みパスワードファイル）のみがある場合、Erlang スクリプトで直接 EMQX 内蔵データベースにインポート可能です。

**ステップ 1：認証設定**

データインポート前に、EMQX の [パスワードベース認証](../../guides/access-control/authn/pwoverview.md) を [内蔵データベース](../../guides/access-control/authn/mnesia.md) バックエンドで設定します。Mosquitto のデフォルトハッシュ方式に合わせるため、以下の設定を使用してください。
...
> **注意：** これらのパラメータ（101回のイテレーション、sha512）は Mosquitto のデフォルトに完全に一致します。EMQX 標準のより強力なセキュリティ設定とは異なりますが、インポートした認証情報を検証するために必要です。

**ステップ 2：パスワードファイルのコピー**

`mosquitto.passwd` ファイルを EMQX サーバー（例：`/tmp/mosquitto.passwd`）にコピーし、`emqx` ユーザーが読み取れるように権限を設定します。

**ステップ 3：インポートスクリプトの実行**

EMQX ノード上で以下のコマンドを実行します。このスクリプトはファイルを読み込み、Base64 エンコードされたソルトとハッシュをデコードし、ユーザー情報をデータベースに直接挿入します。
...
既存のユーザー管理システムと統合する企業向けには、ユーザーを外部 SQL データベース（MySQL、PostgreSQL）に移行することも可能です。EMQX は動的 SQL クエリをサポートし、多様なスキーマ形式に柔軟に対応します。

#### オプション 3：相互 TLS（mTLS）

Mosquitto で X.509 クライアント証明書（相互 TLS）認証を使用している場合、EMQX リスナーを設定してピア証明書を検証する必要があります。
...
* `use_identity_as_username` が有効だった場合、EMQX は `verify_peer` が有効なときにデフォルトで Common Name（CN）をユーザー名として使用します。
...
Mosquitto はブリッジでメッセージを転送し、外部スクリプト（Python/Node.js）でデータ処理を行うことがあります。EMQX ではこれらを内蔵の [ルールエンジン](../../develop/data-integration/rules.md) と [データ統合](../../develop/data-integration/data-bridges.md) に置き換えます。

> EMQX ルールエンジンはメッセージの選択、フィルタリング、変換を行い、コネクターを通じて外部システムに転送できます。
...
* **接続数**を確認。
  ...
  > ```  
  > またはダッシュボードの **モニタリング** -> **クライアント** で確認できます。

* **ログ**をチェックし、認証エラー（ハッシュアルゴリズムの不一致や証明書不足など）がないか確認。
...
2. Mosquitto から EMQX へすべてのメッセージを転送するブリッジを設定。
...
3. バックエンドアプリケーションを EMQX に切り替え。Mosquitto 接続と EMQX 接続の両方からデータを受信可能。  
4. デバイスを段階的に新しい EMQX エンドポイントへ移行。  
...
- **リスナー**：TCP（1883）および SSL（8883）ポートが開き接続を受け付けている。  
...
Mosquitto から EMQX への移行は、プロトコル互換性を維持しつつスケーラビリティと信頼性を大幅に向上させます。既存設定のマッピングと EMQX のルールエンジンを活用して外部スクリプトやブリッジを置換することで、アーキテクチャを簡素化し、大規模な成長に対応可能なインフラを構築できます。

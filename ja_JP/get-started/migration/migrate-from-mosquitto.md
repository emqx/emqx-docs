...
このガイドでは、既存の Eclipse Mosquitto デプロイメントを EMQX に移行する手順を説明します。軽量な単一インスタンスのブローカーからスケーラブルで分散型の MQTT プラットフォームへ移行したい管理者向けに設計されています。移行では、EMQX の標準 MQTT プロトコル互換性を活用し、設定、認証情報、および統合ロジックの移行方法を明確に示します。
...
3. **デバイスと統合の更新**：デバイスを EMQX クラスターにリダイレクト（ポート互換性により多くの場合シームレス）し、システムの動作を検証します。
...
| **ユーザー認証情報** | `/etc/mosquitto/passwd` | 内蔵データベース（Mnesia） | API 経由で既存のパスワードハッシュをインポート可能。 |
| **アクセス制御** | `/etc/mosquitto/acl_file` | `/etc/emqx/acl.conf` | Allow/Deny ルールを直接マッピング。 |
| **ブリッジ** | `connection bridge_name` | データコネクターとルール | 静的ブリッジを動的なデータルーティングに置換。 |
| **パーシステンス** | `mosquitto.db` | `data/`（Mnesia + RocksDB） | EMQX はセッションの永続化を自動で処理。 |

## フェーズ 1: Mosquitto 資産のインベントリ

### 設定ファイルと証明書の収集

主要な設定ファイルの場所を特定します。通常は `mosquitto.conf` に定義されています。
...
認証方法を確認します：
* **パスワードファイル:** 最も一般的です。これらを EMQX の内部データベースに移行します。
* **プラグイン（mosquitto-auth-plug）:** SQL や LDAP を使用している場合は、対応する EMQX 認証バックエンドを直接設定します。

## フェーズ 2: Mosquitto のベースラインを反映する EMQX の設定
...
クライアント動作の一貫性を保つため、主要なプロトコル設定を変換します。
...
**補足:** Mosquitto はセッション期限をグローバルに管理しますが、EMQX（MQTT 5.0）はクライアントごとのセッション期限をサポートします。MQTT 3.1.1 のレガシークライアント向けには、EMQX でグローバルデフォルトを設定可能です。
...
**インポートコマンド:**
`curl` でファイルをアップロードします。`type=plain` パラメーターにより EMQX がインポート時にパスワードをハッシュ化します。
...
* 認証器（`password_based:built_in_database`）が存在し設定と一致していることを確認してください。

#### オプション 2: Mosquitto パスワードファイルのインポート（上級者向け）

大量のユーザーがいて、ハッシュ化済みパスワードのみの `mosquitto.passwd` ファイルを持っている場合、Erlang スクリプトで EMQX 内蔵データベースに直接インポートできます。
...
> **注意:** これらのパラメーター（101回のイテレーション、sha512）は Mosquitto のデフォルトと完全に一致します。EMQX の標準デフォルト（より強力なセキュリティ優先）とは異なりますが、インポートした認証情報の検証には必須です。

**ステップ 2: パスワードファイルのコピー**

`mosquitto.passwd` ファイルを EMQX サーバー（例：`/tmp/mosquitto.passwd`）にコピーし、`emqx` ユーザーが読み取り可能な権限を付与してください。
...
既存のユーザー管理システムと統合が必要なエンタープライズ環境では、MySQL や PostgreSQL などの外部 SQL データベースへユーザーを移行することも可能です。EMQX は動的 SQL クエリをサポートし、様々なスキーマ形式に柔軟に対応します。
...
* `use_identity_as_username` が有効な場合、`verify_peer` が有効なときは EMQX はデフォルトで CN（Common Name）をユーザー名として使用します。
...
Mosquitto はブリッジでメッセージを転送し、Python や Node.js の外部スクリプトでデータ処理を行うことがあります。EMQX ではこれらを内蔵の [ルールエンジン](../../develop/data-integration/rules.md) と [データ統合](../../develop/data-integration/data-bridges.md) で置換します。
...
EMQX は標準 MQTT ポート（1883/8883）を使用するため、DNS 経由で接続している場合は多くのデバイスで設定変更が不要です。DNS レコードを更新して `mqtt.yourdomain.com` を EMQX クラスターのロードバランサーまたは IP に向けてください。
...
* **接続数**を確認。
  ...
  > ```
  >
  > またはダッシュボードの **監視** -> **クライアント** を参照。

* 認証エラーがないか **ログ** を確認（ハッシュアルゴリズムの不一致や証明書の不足が原因の場合があります）。
...
5. Mosquitto に接続がなくなったらブリッジを解除し、Mosquitto を停止。
...
Mosquitto から EMQX への移行は、プロトコル互換性を維持しつつスケーラビリティと信頼性を大幅に向上させます。既存の設定をマッピングし、EMQX のルールエンジンで外部スクリプトやブリッジを置換することで、アーキテクチャを簡素化し、大規模な成長に対応可能なインフラを構築できます。

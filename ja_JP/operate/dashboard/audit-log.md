...
EMQX監査ログは、[ダッシュボード](../dashboard/introduction.md)、[REST API](../admin/api.md)、および[CLI](../admin/cli.md)からの変更関連操作の記録をサポートしており、ダッシュボードユーザーのログインやクライアント、アクセス制御、データ連携の変更などが含まれます。ただし、メトリクス取得やクライアントリストの照会などの読み取り専用操作は記録されません。

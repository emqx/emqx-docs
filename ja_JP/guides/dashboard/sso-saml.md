...
その他のアイデンティティプロバイダーも統合対応中で、将来のバージョンでサポート予定です。
...
3. 設定ページで以下の情報を入力します：
   - **Dashboard Address**：ユーザーがダッシュボードの実際のアクセスアドレスにアクセスできるようにします。特定のパスを指定せず、例えば `http://localhost:18083` のように入力してください。このアドレスは自動的に連結され、IdP側設定用の **SSO Address** と **Metadata Address** が生成されます。
...
3. アプリケーション名を入力（例：`EMQX Dashboard`）、**Integrate any other application you don't find in the gallery (Non-gallery)** を選択し、**Create** をクリックします。
   ...
   <img src="./assets/entra_id_saml_app_parameters.png" alt="SAMLアプリケーションパラメータ" style="zoom:50%;" />
...
2. **SAML 2.0**カードの **Enable** ボタンをクリックします。
3. 設定ページで以下の情報を入力します：
   - **Force MFA**：このバックエンドのすべてのユーザーにログイン時のTOTP検証を必須にする場合は有効化します。デフォルトは無効です。詳細は[SSOユーザーの強制MFA](../multi-factor-authn/multi-factor-authentication.md#forced-mfa-for-sso-users)を参照してください。
   - **Dashboard Address**：ユーザーがダッシュボードの実際のアクセスアドレスにアクセスできるようにします。特定のパスを指定せず、例えば `http://localhost:18083` のように入力してください。このアドレスは自動的に連結され、IdP側設定用の **SSO Address** と **Metadata Address** が生成されます。
   - **SAML Metadata URL**：一時的に空欄のままにして、ステップ2の設定を待ちます。
...
3. **General Settings** タブでアプリケーション名を入力（例：`EMQX Dashboard`）し、**Next** をクリックします。
...
3. **Okta > Assignments** タブで、EMQXダッシュボードアプリケーションに割り当てるユーザーとグループを設定します。ここで割り当てられたユーザーのみがこのアプリケーションにログイン可能です。
...
SAML認証に成功すると、EMQXは自動的にダッシュボードユーザーを追加します。追加されたユーザーは[ユーザー管理](./system.md#users)でロールや権限を割り当てることが可能です。SAMLユーザーにログイン時のTOTP二要素認証を必須にする場合は、[SSOユーザーの強制MFA](../multi-factor-authn/multi-factor-authentication.md#forced-mfa-for-sso-users)を参照してください。

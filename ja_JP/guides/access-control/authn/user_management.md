...
組み込みデータベースに保存された認証データについては、EMQX ダッシュボードまたは HTTP API を使用して、ユーザー資格情報の作成、更新、削除、一覧表示、およびインポートが可能です。
...
ユーザー管理のエンドポイントは認証チェーンのスコープにより異なります。
...
識別子の規則やパラメータの説明については、[REST API](../../api.md) を参照してください。

## ユーザーのインポート

ユーザーのインポートは `password_based:built_in_database` 認証器のみサポートされています。

この機能により、稼働中の EMQX インスタンスに対してユーザーを一括でインポートできます。

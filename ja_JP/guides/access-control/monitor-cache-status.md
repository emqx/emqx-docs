キャッシュメトリクスを表示し、リアルタイムで使用状況を監視するには：

1. **External Resource Cache Settings** の横にある矢印をクリックし、**External Resource Cache Status** を選択します。サイドパネルが表示され、キャッシュメトリクスが確認できます。
2. ドロップダウンメニューを使って、ノード単位またはクラスター全体のメトリクスを表示します。

メトリクスには以下が含まれます：

- **メモリ使用量**：現在キャッシュが使用している合計メモリ量。
- **キャッシュエントリ数**：保存されているキャッシュ結果の総数。
- **キャッシュヒット数**：EMQXがキャッシュ内で有効な結果を見つけ、外部バックエンドへの呼び出しを回避した回数。
  - 表示されるメトリクス：現在のレート、5分間の平均、最大レート
- **キャッシュミス数**：EMQXがキャッシュ内で結果を探したが見つからず、バックエンドクエリが発生した回数。
  - 表示されるメトリクス：現在のレート、5分間の平均、最大レート
- **キャッシュ挿入数**：ミス後に新たにキャッシュに追加された結果の数。
  - 表示されるメトリクス：現在のレート、5分間の平均、最大レート

パネルの下部にはノードリストがあり、クラスター内の各ノードの**メモリ使用量**、**キャッシュエントリ数**、**キャッシュヒット数**の概要を確認できます。

パネル右上のボタンで統計情報の更新やリセットが可能です。

...
2. ドロップダウンメニューを使用して、ノード単位またはクラスター全体のメトリクスを表示します。
...
- **Cache Hits**：EMQXがキャッシュ内で有効な結果を見つけ、外部バックエンドへの呼び出しを回避した回数。
  - 表示されるメトリクス：現在のレート、5分間の平均、最大レート
- **Cache Misses**：EMQXがキャッシュ内で結果を探したが見つからず、バックエンドクエリが発生した回数。
  - 表示されるメトリクス：現在のレート、5分間の平均、最大レート
- **Cache Inserts**：ミス後に新しい結果がキャッシュに追加された回数。
  - 表示されるメトリクス：現在のレート、5分間の平均、最大レート

パネルの下部にはノードリストがあり、クラスター内の各ノードの **Memory Usage**、**Cache Entries**、**Cache Hits** の概要を確認できます。

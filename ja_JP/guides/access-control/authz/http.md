...
クライアントがサブスクライブまたはパブリッシュ操作を開始すると、HTTP認可機能は設定されたリクエストテンプレートに基づいてリクエストを構築し送信します。ユーザーは認可サービス側で認可ロジックを実装し、以下の要件に従って結果を返す必要があります。
...
- `${peerhost}`: クライアントの送信元IPアドレス。[Proxy Protocol](http://www.haproxy.org/download/1.8/doc/proxy-protocol.txt)が有効な場合は、プロキシから報告された送信元IPアドレスを使用します。
- `${peerport}`: EMQX 6.0.4以降、クライアントの送信元ポート番号（例：`51544`）。Proxy Protocolが有効な場合はプロキシから報告された送信元ポートを使用します。
- `${peername}`: EMQX 6.0.4以降、クライアントの送信元IPアドレスとポート番号。IPv4の場合は`IP:port`形式（例：`192.168.0.1:51544`）、IPv6の場合は角括弧なしで`2001:db8::1:51544`のように表記します。Proxy Protocolが有効な場合はプロキシから報告されたIPアドレスとポートを使用します。
...
::: tip
`${peerhost}`および`${peerport}`のプレースホルダーは非推奨です。後方互換性のために残されています。新しいテンプレートでは対応可能な場合`${peername}`を使用してください。
:::
...
- `200`および`204`以外のHTTPステータスコードは「無視」とみなし、例えばHTTPサービスが利用不可の場合などに該当します。
...
4.xバージョンではHTTP APIのステータスコードのみを利用し、コンテンツは破棄していました。例えば`200`は許可、`403`は拒否を意味していました。より詳細な情報を提供するため、EMQX 5.0でリクエストコンテンツの返却を追加しました。
...
1. [EMQXダッシュボード](http://127.0.0.1:18083/#/authentication)で、左のナビゲーションツリーから **アクセス制御** -> **認可** をクリックし、**認可**ページに入ります。
...
EMQX 6.0.4以降、HTTP認可機能はOAuth 2.0のクライアントクレデンシャルズグラントをサポートしています。OAuth2を有効にすると、EMQXは設定されたトークンエンドポイントからアクセストークンを取得・キャッシュ・自動更新します。外部HTTP認可サービス呼び出し時には、`Authorization: Bearer <access_token>`ヘッダーにトークンを付与し、外部サービス側でEMQXを認証可能にします。
...
- OAuth2を有効にしている場合、HTTP認可機能の`Authorization`ヘッダーを設定しないでください。EMQXは自動生成されるBearer認証ヘッダーと競合するため設定を拒否します。
...
HTTPの`POST`および`GET`リクエストをサポートしています。それぞれに特有のオプションがあります。<!--詳細は[authz:http_post](../../configuration/configuration-manual.html#authz:http_post)および[authz:http_get](../../configuration/configuration-manual.html#authz:http_get)をご覧ください。-->
...
認可サーバーがスコープを要求しない場合は`scope`を省略してください。リクエスト形式や制限事項については[OAuth2クライアント認証情報の設定](#configure-oauth2-client-credentials)をご参照ください。

...
* `retain`（オプション）：ルールが保持メッセージのパブリッシュを許可するかどうかを示します。値は `0`、`1`、または `true`、`false` です。デフォルトでは保持メッセージは許可されます。
...
3. 以下の指示に従って設定を行います。
   ...
   **Filter**：資格情報検索のためのMongoDBセレクターとして解釈されるマップです。[プレースホルダー](./authz.md#authorization-placeholders)をサポートします。

   **詳細設定**：

   - **認証ソース**：MongoDBに接続する際に使用する認証ソースを指定します。特定のデータベースやユーザー資格情報を管理するMongoDB認証データベースを指定できます。

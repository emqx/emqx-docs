...
EMQXは、バックアップとリカバリを実現するためにデータのインポートおよびエクスポート用のCLIコマンドを提供しています。EMQX 4.xのコマンドと似ていますが、エクスポートファイルの形式は4.xとは互換性がありません。
...
CLIコマンドに加え、EMQX EnterpriseではEMQXダッシュボード上にデータバックアップとリカバリのページを提供しており、そこでデータのインポートおよびエクスポートが可能です。
...
- ダッシュボードまたは`POST /api/v5/data/import`経由でデータをインポートする場合、クラスター内のすべての稼働中ノードは同じメジャーおよびマイナーバージョンを使用している必要があります。パッチバージョンは異なっても構いません。ノードのバージョンがこの要件を満たさない場合やノードのバージョンが取得できない場合、EMQXはインポート開始前にリクエストを拒否し、HTTP `400`レスポンスと`BAD_REQUEST`エラーコードを返します。エラーメッセージには各ノードとその`major.minor`バージョンが一覧表示され、バージョンが不明な場合は`unknown`と表示されます。再試行前に、すべての稼働中ノードのバージョンが取得可能で、かつ同じメジャーおよびマイナーバージョンであることを確認してください。
- [コアノード＋レプリカノード](../develop/cluster/mria-introduction.md)モードが有効な場合、データインポートはコアノードでのみ実行可能です。これは実際のインポート動作に影響はなく、データはコアノードおよびレプリカノードを含むすべてのクラスター・ノードにレプリケートされます。コアノードで操作することで正しいデータインポートが保証されます。
- データファイルの名前は変更できません。
...
EMQX 6.0.4以降、特定のネームスペースにバックアップをインポートする場合、そのネームスペースでサポートされている設定ルートのみが適用されます。バックアップ内のネームスペース設定にクラスター全体のルート（認証、認可、ExHook、リスナーなど）が含まれている場合、EMQXはそれらのルートをスキップし、グローバル設定には書き込みません。グローバルインポートではクラスター全体の設定が適用されます。

EMQXはスキップしたルートを一覧表示する`data_import_skipped_global_roots`警告をログに記録し、同じバックアップからサポートされているルートのインポートを継続します。これらのクラスター全体ルートのスキップはインポート全体の失敗を引き起こしません。CLI経由のインポートではコマンド出力にスキップされたルートが表示されます。ダッシュボードやREST API経由のインポートでは、EMQXログで警告を確認してください。

::: tip 特別な注意事項

稀に既存データとインポートデータが互換性がない場合があります。例えば、EMQXクラスターが組み込みデータベース認証を使用し、ソルトの位置を「サフィックス」に設定している一方で、インポートデータでは同じ設定が「プレフィックス」に設定されている場合などです。インポート後は新しい設定が有効になり、以前作成された古いユーザー認証情報は機能しなくなります。
...
3. データをエクスポートするには、**Create**をクリックします。グローバル管理者は**Global**ビューでのみバックアップを作成可能です。特定のネームスペースを選択している場合、**Create**は無効になります。ネームスペース管理者は割り当てられたネームスペースのバックアップを作成できます。
...
5. バックアップファイルの管理は、**Actions**列の以下のボタンで行います：
   ...
   - **Restore**：バックアップファイルを選択したスコープにインポートします。特定のネームスペースを選択している場合は、リストアの確認ダイアログで対象ネームスペースを確認してから実行してください。リストア成功後、成功メッセージに対象ネームスペースが表示されます。
...
グローバル管理者が`namespace`を省略した場合、操作は**Global**のバックアップファイルに適用されます。ネームスペース管理者が呼び出した場合、EMQXはパラメータを無視し、呼び出し元の割り当てられたネームスペースに操作を適用します。

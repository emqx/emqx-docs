...
3. 各ノードで以下が行われます：
   ...
   - Avroスキーマが存在すれば読み込まれ、検証に使用されます。
   - プラグインコードがノードにロードされますが、アプリケーションは起動されません。
   - プラグインはEMQX設定の`plugins.states`に`disabled`として登録されます。

::: tip

プラグインの状態はEMQX設定において`enable`フラグ（`true`または`false`）のみが保存されます。完全なプラグイン設定はノード上の`$EMQX_DATA_DIR/plugins/my_emqx_plugin/config.hocon`ファイルに格納されます。
...
`on_config_changed/2`コールバック関数はEMQXクラスターの各ノードで呼び出されます。ローカルシステム状態（例：ネットワークの可用性）に依存する設定検証は避けてください。ノード間で結果が不整合になる可能性があります。代わりに`on_health_check/1`を使い、リソースが利用不可の場合は非正常状態を報告してください。
...
- DashboardやAPI経由で設定保存後、EMQXが管理する永続化されたプラグイン設定ファイル：
  - Docker環境：
    `/opt/emqx/data/plugins/my_emqx_plugin/config.hocon`
  - deb/rpm環境：
    `/var/lib/emqx/plugins/my_emqx_plugin/config.hocon`

`priv/config.hocon`はパッケージ化されたデフォルトテンプレートであり、`data/plugins/.../config.hocon`はEMQXが設定変更を保存後に使用する永続化設定ファイルです。
...
プラグインのアプリケーションは停止されても、コードはノードにロードされたままです。停止中のプラグインも設定変更が可能なためです。
...
- EMQX設定からプラグインの状態（インストール済みかつ有効か）を把握します。
- 他ノードからプラグインとその実際の設定を要求します。
...
これでプラグインの起動・停止や設定が可能になります。Dashboardからプラグインをアンインストールするには、プラグイン一覧ページの**Actions**列にある**More**メニューの**Uninstall**ボタンをクリックしてください。

許可を取り消す場合は、以下のいずれかを実行してください：
...
1. EMQXノード上で、tarballをEMQXプラグインディレクトリにコピーします：
...
EMQX起動時にプラグインをすぐ利用可能にしたい場合（例：カスタムDockerイメージのビルド時）、プラグインパッケージを展開し、EMQXを事前に設定しておく方法があります。

以下はDockerfileの例ですが、deb/rpmやベアメタルなど他のデプロイ方法でも同様です：
...
2. EMQX設定にプラグインを登録し、起動時に自動的に起動するよう設定ファイルに追記します：

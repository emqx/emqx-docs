...
このページでは、EMQXにおけるプラグインのライフサイクルについて説明し、ダッシュボード、CLI、REST APIを使用してプラグインのインストール、設定、起動、停止、アンインストール、アップグレードを行う方法を解説します。
...
- **起動済み**：プラグインが動作中で、EMQXと積極的に連携している状態。
...
3. 各ノード上で以下の処理が行われます：
   ...
   - 初期設定ファイル（メインプラグインのアプリからの`config.hocon`）が`$EMQX_DATA_DIR/plugins/my_emqx_plugin/config.hocon`にコピーされます。
...
プラグインの状態はEMQX設定内の`enable`フラグ（`true`または`false`）のみが保存されます。プラグインの詳細な設定は各ノードの`$EMQX_DATA_DIR/plugins/my_emqx_plugin/config.hocon`に格納されます。

:::

### 設定

インストール後、プラグインの設定はダッシュボードまたはAPIを通じて更新可能です。
...
`on_config_changed/2`コールバック関数はEMQXクラスターの各ノードで呼び出されます。ローカルシステムの状態（例：ネットワークの可用性）に依存した設定検証は避けてください。ノード間で結果が不整合になる恐れがあります。代わりに`on_health_check/1`を使用してランタイムチェックを行い、リソースが利用できない場合は不健康状態を報告してください。
...
`priv/config.hocon`はパッケージに含まれるデフォルトテンプレートであり、`data/plugins/.../config.hocon`が設定変更後にEMQXが使用する永続化された設定ファイルの場所です。

### 起動

プラグインはダッシュボード、API、CLIから手動で起動します。起動時には以下が行われます。
...
プラグインを停止すると以下の処理が行われます。

- プラグインのアプリケーションが停止されます。
- プラグインはEMQX設定の`plugins.states`に`disabled`として登録されます。

プラグインのアプリケーションは停止されますが、コードはノード上にロードされたままです。停止中のプラグインでも設定は可能です。
...
プラグインのアンインストールはダッシュボードまたはCLIから実行可能です。詳細は[プラグインのインストールと管理](#install-and-manage-plugins)を参照してください。

### クラスター参加時の挙動

EMQXノードがクラスターに参加する際、プラグインや設定は各ノードのローカルファイルシステムに存在するため、必ずしも新規ノードにインストール・設定されているとは限りません。

新規ノードは以下の処理を行います。

- クラスター参加プロセスの一環として、グローバルなEMQX設定を取得します。
...
### ダッシュボード経由でのパッケージインストール

プラグインがすでにビルドされ、`my_emqx_plugin-1.0.0.tar.gz`のtarballが用意されているとします。ダッシュボードから直接コンパイル済みプラグインパッケージをインストールするには、以下の手順に従ってください。

::: tip 重要なセキュリティアップデート

セキュリティ上の理由から、EMQXはダッシュボード経由でのプラグインインストールに対して明示的な許可を必要とします。

- インストールを開始する前に許可を与える必要があります。
- 許可状態は一時的で、インストール完了後に自動的に取り消されます。
- クラスター環境で実行している場合は、すべてのノードで許可を与える必要があります。
...
1. CLIで以下のコマンドを実行し、プラグインインストールを明示的に許可します。
   ...
   このコマンドを実行後、ダッシュボードからインストールを進められます。
...
これでプラグインの起動・停止や設定が可能になります。ダッシュボードからプラグインパッケージをアンインストールするには、プラグイン一覧ページの**Actions**列の**More**メニューから**Uninstall**ボタンをクリックしてください。
...
プラグインがすでにビルドされ、`my_emqx_plugin-1.0.0.tar.gz`のtarballが用意されているとします。CLIから直接コンパイル済みプラグインパッケージをインストールするには、以下の手順に従ってください。
...
2. プラグインをインストールします。
   ...
   クラスター内のすべての稼働中ノードにインストールする場合は、`--cluster`オプションを追加します。
...
プラグインがすでにビルドされ、`my_emqx_plugin-1.0.0.tar.gz`のtarballが用意されているとします。APIを使用してプラグインをインストールするには、以下の手順に従ってください。
...
2. `curl`を使用してAPIにPOSTリクエストを送信し、プラグインをインストールします。
...
3. プラグイン一覧を確認してインストールが成功したかを検証します。
...
EMQX起動時にプラグインを即時利用可能にしたい場合（例：カスタムDockerイメージをビルドする場合）、プラグインパッケージを展開し、EMQXを事前に設定しておくことが可能です。

以下の手順はDockerfileの例ですが、deb/rpmやベアメタルなど他のデプロイ方法にも同様に適用できます。

1. プラグインtarballを`plugins`ディレクトリにコピーし展開します。
...
2. EMQXのベース設定ファイルにプラグインを登録し、起動時に自動的に起動するようにします。以下を`base.hocon`に追記します。
   ...
   `enable = true`に設定すると自動起動し、`enable = false`にするとインストールのみで起動しません。

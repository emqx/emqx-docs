...
**Extension Hook** は **emqx-exhook** プラグインによってサポートされています。これにより、ユーザーは他のプログラミング言語を使用して EMQX の [Hooks](./hooks.md) を処理することが可能です。
...
イベントハンドラー、すなわちユーザーが実装する gRPC サーバー側として、マウントすべきフックのリストを定義し、各イベント到着時の処理を実装します。
...
- `OnClient`、`OnSession`、`OnMessage` で始まるメソッドは [hooks](./hooks.md) のメソッドに対応し、呼び出しタイミングや引数リストもほぼ同様です。
...
2. 対応するプログラミング言語の gRPC フレームワークを使い、`exhook.proto` のサーバー側コードを生成します。

...
EMQX ネイティブのフックと同様に、計算と返却をチェーン方式で行うこともサポートしています。
...
イベントハンドラ、すなわちユーザーが実装する gRPC サーバー側は、マウントすべきフックのリストを定義し、イベントが到着した際にどのように処理するかのコールバック関数を実装します。
...
- `OnProviderLoaded`：HookProvider がロードされる際の処理を定義します。このメソッドはマウントすべきフックのリストを返します。このリストに含まれるフックのみがユーザーの HookProvider サービスにコールバックされます。
- `OnProviderUnloaded`：HookProvider が emqx からアンインストールされたことをユーザーに通知します。

フックイベントの役割：

- `OnClient`、`OnSession`、`OnMessage` で始まるメソッドは、[hooks](./hooks.md) のメソッドに対応しており、呼び出しタイミングや引数リストもほぼ同じです。
- `OnClientAuthenticate`、`OnClientAuthorize`、`OnMessagePublish` のみが EMQX に対して戻り値を返すことが許可されており、その他のコールバックは戻り値をサポートしていません。

...
- `OnClient`、`OnSession`、`OnMessage` で始まるメソッドは [hooks](./hooks.md) のメソッドに対応しており、呼び出しタイミングや引数リストが類似しています。
- `OnClientAuthenticate`、`OnClientCheckAcl`、`OnMessagePublish` のみが EMQX に返却値を返すことが許可されており、その他のコールバックは返却値をサポートしていません。

インターフェースやパラメータの詳細なデータ構造については以下を参照してください。  
...
[EMQX ダッシュボード](http://127.0.0.1:18083/#/exhook) を使って ExHook サービスの管理および監視が可能です。

...
セカンダリクラスターはプライマリクラスターに対してバックアップファイルのエクスポートを要求し、そのファイルをダウンロードしてローカルにアップロードし、インポートします。選択された設定ルートはEMQXの既存の設定インポートのセマンティクスに従ってインポートされます。選択されたMnesiaテーブルセットはスナップショットとしてインポートされるため、セカンダリクラスターにのみ存在するレコードは削除されます。選択されていないルートやテーブルセットは変更されません。
...
ルールは一般的にコネクター、アクション、ソース、およびスキーマレジストリのオブジェクトに依存します。`rule_engine`を依存関係なしで同期すると、インポートが失敗したり、実行時に不完全な動作を引き起こす可能性があります。これらの依存するルートは、セカンダリクラスターに既に存在しない限り、`sync.root_keys`に含めてください。

デフォルトでは、同期には`banned`、`builtin_authn`、`builtin_authz`のテーブルセットも含まれます。これらの選択されたテーブルセットはセカンダリクラスター上で置き換えられます。設定のみの同期が必要な場合は`sync.table_sets = []`に設定してください。サポートされている`sync.table_sets`の値は`banned`、`builtin_authn`、`builtin_authz`、`builtin_retainer`、`psk`、`mt`です。プライマリのデータバックアップAPIはAPIキーで呼び出した場合、`dashboard_users`や`api_keys`は含まれません。
...
このコマンドはローカルノード、ヘルス状態、ワーカー状態、選択されたコアノード、次回の同期予定、機密情報を含まない同期設定を表示します。
...
| 6.3.1 | 0.1.3 | [emqx_backup_sync-0.1.3.tar.gz](https://www.emqx.com/downloads/emqx-plugins/6.3.1/emqx_backup_sync-0.1.3.tar.gz) ([sha256](https://www.emqx.com/downloads/emqx-plugins/6.3.1/emqx_backup_sync-0.1.3.sha256)) |

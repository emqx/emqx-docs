...
このプラグインはACLチェック時にUnified Namespaceトピック構造を強制し、UNS Governanceで管理されているトピックにパブリッシュされるメッセージのペイロードを検証できます。
...
- 各ブートストラップモデルについて：
  ...
  - 既にデータベースに存在する場合、プラグインは読み込みをスキップし、情報レベルでログを出力します。
- バンドルされたデフォルトのブートストラップモデル：`priv/bootstrap_models/model-v1.json`

> 注意：ブートストラップモデルはクラスタ内で最初のプラグイン起動時にデータベースにロードされます。以降のプラグインやノードの再起動では再ロードされません。モデルの更新はAPIを使用してDB内のモデルストアを更新してください。
...
- トピック違反（`topic_nomatch`、`topic_invalid`、`not_endpoint`）：
  - `topic_nomatch`：アクティブなモデルのトピックフィルターがトピックにマッチしませんでした。
    （モデル固有の検証は実行されません。）
    アクティブモデルが存在せずUNS Governanceが有効な場合、トピックは
    `topic_nomatch` としてフェイルクローズされます（`exempt_topics` を除く）。
  - `topic_invalid`：選択されたモデルのフィルターはマッチしましたが、トピックが選択モデルの構造・セグメント制約に違反しました。
  - `not_endpoint`：選択されたモデルはトピックパスにマッチしましたが、対象ノードがエンドポイントではありません。
  - QoS 0：メッセージは無視されます。
...
- ペイロード違反（`payload_invalid`）：
  - UNS Governanceで管理されているトピックにメッセージがパブリッシュされ、そのトピックにペイロードスキーマがある場合、ペイロード検証はパブリッシュ処理中に実行されます。これは認可キャッシュにヒットしたパブリッシュや、ルールエンジンの再パブリッシュ、ブリッジのイングレスなどのクライアント外パブリッシュも含みます。
...
複数モデルがアクティブな場合、UNS Governanceは完全検証前にモデルを事前スクリーニングします：
...
- 事前チェックは直接的なトピック/フィルタマッチングのみを使用し、パブリッシュトピックのプレフィックス展開（例：`/#` の付加）は行いません。
- 選択されたモデルのみが完全に検証され、UNS Governanceは次のモデルには進みません。
- この事前チェックに失敗したモデルはスキップされ、モデル別ドロップカウンターにはカウントされません。
...
トップレベルカウンター：
- `messages_total`：処理したメッセージ合計（`messages_allowed + messages_dropped`）。除外トラフィックも含む。
- `messages_allowed`：許可されたメッセージおよび除外されたメッセージ。
...
- トピック／ペイロードのドロップは `messages_total`、`messages_dropped`、および該当モデルの理由別カウンターを増加させます。
- トピックフィルタ事前チェックに合格するモデルがない場合、グローバルに `topic_nomatch` が増加し、モデル別ドロップカウンターは増加しません。

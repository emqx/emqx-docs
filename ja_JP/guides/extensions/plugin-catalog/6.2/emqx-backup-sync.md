...
ルールは一般的にコネクター、アクション、ソース、およびスキーマレジストリオブジェクトに依存します。`rule_engine`を依存関係なしで同期すると、インポートに失敗したり不完全なランタイム動作を引き起こす可能性があります。これらの依存するルートは、セカンダリクラスターに既に存在しない限り`sync.root_keys`に含めてください。

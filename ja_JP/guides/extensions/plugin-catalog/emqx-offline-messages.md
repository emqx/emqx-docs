...
標準のMQTTセッションパーシステンスでは不十分なシナリオ、例えばセッションを超えてメッセージの保持が必要な場合や、他のシステムもメッセージ履歴を参照する必要がある場合に有用です。
...
プラグインには両バックエンドをカバーするデフォルトの `config.hocon` が付属しています。ダッシュボードのプラグイン詳細ページは同じスキーマを表示するため、多くの運用者はファイルを直接編集するよりUIから設定します。

共通のトップレベルフィールド：

- `topics`：プラグインが追跡すべきトピックフィルターのリスト。空リストの場合、そのバックエンドではプラグインは無効です。
...
- `init_default_schema`：`true` の場合、起動時にプラグインがデフォルトの `mqtt_msg` / `mqtt_sub` テーブルを作成します。スキーマを自分で管理する場合は `false` にしてください。
...
- `username`、`password`、`database`：認証情報と論理DB（クラスターモードでは `database` は無視されます）。
...
`init_default_schema = true` に設定すると、プラグインが初回起動時にこれらのテーブルを作成します。そうでない場合はバックエンドを有効化する前に自分で作成してください。
...
以下の[ダウンロード](#download)セクションからEMQXのバージョンに合ったtarballを取得し、ダッシュボード、REST API、またはCLIでインストールしてください。
...
プラグインを起動します（ダッシュボード、または `emqx ctl plugins start emqx_offline_messages-<version>`）。その後、ダッシュボードのプラグイン詳細ページを開き、バックエンドの設定を行います。

...
- 記録データは遅延時間の降順で表示され、最大1,000件まで保存可能です。
- リストには閾値を超えたメッセージではなく、**サブスクライバー-トピック**のデータが記録されます。
- 記録が新規の場合はリストに挿入され、既存の場合は発生時間が更新されます。いずれの場合もリスト全体が再ランキングされます。
...
- キューにメッセージが過剰に蓄積し、レイテンシが大幅に増加する（例：PUBLISHとSUBSCRIBEが同じ接続を共有し、多数のPUBLISHメッセージがキューに溜まる）
- サブスクライバーの受信速度が遅い
...
EMQXダッシュボードの左ナビゲーションメニューから**診断** -> **スローサブスクリプション**をクリックします。**スローサブスクリプション**ページで**有効化**ボタンをクリックしてください。

設定内容は以下の通りです：

- **統計閾値**：統計対象となるレイテンシの閾値。閾値を超えたメッセージ情報のみが収集されます。最小値：100ms
- **最大統計件数**：スローサブスクリプション統計記録テーブルの最大レコード数。最大値：1,000
- **記録の有効期限**：記録の有効期限。記録作成からカウントを開始し、指定期間内に再度トリガーされない記録はリストから削除されます。デフォルト：300秒
- **統計タイプ**：レイテンシ計測方法。以下から選択可能です
  - **whole**：メッセージがEMQXに到着してから送信完了まで
  - **internal**：メッセージがEMQXに到着してからEMQXが送信を開始するまで
  - **response**：EMQXが送信を開始してから送信完了まで

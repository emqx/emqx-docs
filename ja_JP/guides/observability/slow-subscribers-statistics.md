...
- フックの実行速度が遅く、ACLチェックやExHooks、ルールエンジンなどのメッセージパブリッシュ処理をブロックしている
- キューに多くのメッセージが蓄積され、レイテンシが大幅に増加している。例えば、PUBLISHとSUBSCRIBEが同じ接続を共有し、多数のPUBLISHメッセージがキューに溜まっている場合
- サブスクライバーの受信速度が遅い
...
- **Stats Threshold**：統計対象となるレイテンシの閾値。閾値を超えたメッセージ情報のみ収集されます。最小値：100ms
...
- **Client ID**：スローサブスクリプションが発生しているクライアント
- **Topic**：スローサブスクリプションが発生しているトピック
- **Duration**：メッセージのレイテンシ
- **Node**：スローサブスクリプションが発生しているノード
- **Updated**：記録の作成・更新日時

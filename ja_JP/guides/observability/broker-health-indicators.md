...
このページは、EMQXブローカーの監視に最も役立つPrometheusメトリクスの厳選リファレンスです。これらのメトリクスの公開およびスクレイピング方法については、[Prometheusとの統合](./prometheus.md)をご参照ください。
...
すべてのメトリクスはEMQXのPrometheusエンドポイント（`/api/v5/prometheus/stats`、`/api/v5/prometheus/auth`、および`/api/v5/prometheus/data_integration`）で公開されています。エンドポイントの詳細や`mode`クエリパラメーターについては、[Prometheusとの統合](./prometheus.md#configure-pull-mode-integration)をご覧ください。

::: tip コレクターのデフォルト設定について

`emqx_`で始まるメトリクスは常に有効です。より詳細なErlang VMメトリクスである`erlang_vm_`で始まるものは、上流のPrometheus Erlangエクスポーター由来であり、EMQX 6.0以降では**デフォルトで無効**になっています。プロセス数、アロケーターごとのメモリ、GCやスケジューラーの内訳を有効にするには、`prometheus.collectors.vm_system_info`、`vm_memory`、および`vm_statistics`を`enabled`に設定してください。

:::

## システム

ハードウェア層に最も近いシグナルです。ブローカーが異常な場合、通常これらのいずれかが最初に変動します。
...
| `emqx_vm_broker_pool_max_mailbox_size` | ブローカーのディスパッチプール内で最大のメールボックスサイズ。高値はサブスクライバー側のバックプレッシャーを示します。 |

### アップタイム

| メトリクス | 説明 |
|------------|-------|
| `emqx_vm_uptime_ms` | ブローカーの稼働時間（ミリ秒）。急激に小さい値になるとノードが再起動したことを示します。 |
...
| `emqx_overload_protection_delay_timeout` | 遅延試行のタイムアウト数。 |

## ブローカー

コアの運用シグナルです。メッセージ関連カウンターのレートを監視し、特に`dropped`系列に注意してください。
...
| メトリクス | 説明 |
|------------|-------|
| `emqx_license_expiry_at` | ライセンス有効期限（UNIXエポック秒）。 |
...
| `emqx_live_connections_count` | 現在接続中（TCP接続確立中）のクライアント数。 |
...
| メトリクス | 説明 |
|------------|-------|
| `emqx_retained_count` | リテインドメッセージ数。 |
| `emqx_retained_max` | 最大リテインド数。 |
| `emqx_delayed_count` | 遅延パブリッシュキューの深さ。 |
| `emqx_delayed_max` | 最大遅延キュー深さ。 |
| `emqx_banned_count` | 禁止されたクライアント／ユーザー名／IPのエントリー数。 |
...
| `emqx_messages_delivered` | サブスクライバーへの配信数（1つのパブリッシュメッセージが複数の配信を生む場合あり）。 |
| `emqx_messages_acked` | サブスクライバーからのアック（ACK）受信数。 |
| `emqx_messages_forward` | ノード間のメッセージ転送数。 |
| `emqx_messages_retained` | リテインドメッセージイベント数。 |
...
| `emqx_messages_dropped_expired` | メッセージ有効期限超過によるドロップ数。 |
...
| `emqx_messages_dropped_receive_maximum` | サブスクライバーのMQTT v5 receive-maximumクォータ超過によるドロップ数。 |
...
| メトリクス | 説明 |
|------------|-------|
| `emqx_client_connect` | 受信したCONNECT数。 |
| `emqx_client_connack` | 送信したCONNACK数。 |
| `emqx_client_connected` | `client.connected`フックが発火した回数。 |
| `emqx_client_disconnected` | `client.disconnected`フックが発火した回数。 |
| `emqx_client_disconnected_reason` | 切断理由ごとのカウント。 |
| `emqx_client_subscribe` | Subscribeフックの発火回数。 |
| `emqx_client_unsubscribe` | Unsubscribeフックの発火回数。 |
...
| `emqx_session_discarded` | 既存セッションのクリーンスタートによって破棄されたセッション数。 |
| `emqx_session_terminated` | 終了したセッション数。 |

## 認証と認可

HTTP、LDAP、またはデータベースバックエンドが認証経路にある場合に、ブローカー側かバックエンド側のどちらが遅延または障害の原因かを判断するためにこれらのメトリクスを使用します。
...
| `emqx_authorization_matched_allow` | 許可ルールがマッチして発火した回数。 |
| `emqx_authorization_matched_deny` | 拒否ルールがマッチして発火した回数。 |
...
トラフィックはルールエンジンに入り、アクションやコネクターにファンアウトし、外部システムに到達します。各レイヤーは独自のカウンターを公開しており、順に読むことでメッセージがどこで失われているかを把握できます。
...
| `emqx_rule_failed` | ルール処理失敗数。 |
| `emqx_rule_failed_exception` | ルール実行中のErlang例外。 |
...
| `emqx_action_queuing` | キューに待機中の長さ。 |
...
| `emqx_action_dropped_queue_full` | キュー上限に達した回数。 |
| `emqx_action_dropped_resource_stopped` | 対象リソースが停止している回数。 |
| `emqx_action_dropped_resource_not_found` | 対象リソースが見つからなかった回数。 |
| `emqx_action_dropped_expired` | ディスパッチ前にメッセージが期限切れとなった回数。 |
| `emqx_action_dropped_other` | その他の理由によるドロップ。 |

## 最小限の「ブローカー異常」パネル

Grafanaダッシュボードに限られた数の系列しか表示できない場合、以下の指標が最も重要です。多くの本番問題はこれらのいずれかが数秒以内に変動します。

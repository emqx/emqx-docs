...
このページは、EMQXブローカーの監視に最も役立つPrometheusメトリクスを厳選したリファレンスです。これらのメトリクスの公開およびスクレイピング方法については、[Prometheusとの統合](./prometheus.md)をご参照ください。
...
`emqx_`で始まるメトリクスは常に有効です。より詳細なErlang VMメトリクスである`erlang_vm_`プレフィックスのメトリクスは、上流のPrometheus Erlangエクスポーター由来であり、EMQX 6.0以降では**デフォルトで無効**になっています。プロセス数、アロケーター別メモリ、GCやスケジューラーの内訳を有効にするには、`prometheus.collectors.vm_system_info`、`vm_memory`、`vm_statistics`を`enabled`に設定してください。
...
| メトリクス | 説明 |
|------------|-------|
| `emqx_vm_run_queue` | 現在のスケジューラーの実行キュー長。継続的にゼロ以外の場合はCPU飽和を示します。 |
...
| メトリクス | 説明 |
|------------|-------|
| `emqx_vm_uptime_ms` | ブローカーの稼働時間（ミリ秒）。急激に小さな値になるとノードが再起動したことを示します。 |
...
| `emqx_cluster_nodes_stopped` | 停止中のクラスター・ノード数。ゼロより大きい場合はアラート対象です。 |
...
| `emqx_sessions_count` | アクティブセッション数（現在切断中の永続セッションも含む）。 |
...
| `emqx_messages_publish` | ディスパッチされたPUBLISHパケット数。 |
...
| メトリクス | 説明 |
|------------|-------|
| `emqx_messages_dropped` | ドロップされたメッセージの総数。 |
...
| `emqx_messages_dropped_receive_maximum` | サブスクライバーのMQTT v5受信最大数制限超過によるドロップ数。 |

### サブスクライバーごとの配信ドロップ

| メトリクス | 説明 |
|------------|-------|
| `emqx_delivery_dropped` | ドロップされた配信の総数。 |
...
| メトリクス | 説明 |
|------------|-------|
| `emqx_bytes_received` | 受信した総バイト数。 |
| `emqx_bytes_sent` | 送信した総バイト数。 |

### パケットレベル（プロトコルデバッグ用ダッシュボード向け）

| メトリクス | 説明 |
|------------|-------|
| `emqx_packets_received` | 受信したパケット総数。 |
| `emqx_packets_sent` | 送信したパケット総数。 |
| `emqx_packets_connect` | 受信したCONNECTパケット数。 |
| `emqx_packets_connack_sent` | 送信したCONNACKパケット数。 |
| `emqx_packets_connack_error` | 非ゼロ理由コードのCONNACK（多くのクライアント認証失敗がここに表示されます）。 |
...
| `emqx_packets_publish_error` | 受け入れられなかったPUBLISHパケット。 |
| `emqx_packets_publish_auth_error` | 認可により拒否されたPUBLISHパケット。 |
| `emqx_packets_puback_received` | 受信したPUBACKパケット（QoS 1）。 |
| `emqx_packets_puback_sent` | 送信したPUBACKパケット（QoS 1）。 |
| `emqx_packets_pubrec_received` | 受信したPUBRECパケット（QoS 2）。 |
| `emqx_packets_pubrec_sent` | 送信したPUBRECパケット（QoS 2）。 |
| `emqx_packets_pubrel_received` | 受信したPUBRELパケット（QoS 2）。 |
| `emqx_packets_pubrel_sent` | 送信したPUBRELパケット（QoS 2）。 |
| `emqx_packets_pubcomp_received` | 受信したPUBCOMPパケット（QoS 2）。 |
| `emqx_packets_pubcomp_sent` | 送信したPUBCOMPパケット（QoS 2）。 |
...
HTTP、LDAP、またはデータベースバックエンドが認証経路にある場合に、ブローカー側かバックエンド側のどちらが遅延または障害の原因かを判断するために使用します。
...
| メトリクス | 説明 |
|------------|-------|
| `emqx_authorization_allow` | 許可の判定数。 |
| `emqx_authorization_deny` | 拒否の判定数。 |
...
| `emqx_authn_users_count` | プロバイダーごとのユーザー数（パスワード、mnesia、DBバックエンドの場合）。 |
...
| `emqx_authz_rules_count` | ソースごとのルール数（ファイル、mnesia、DBバックエンド）。 |
...
トラフィックはルールエンジンに入り、アクションやコネクターに分岐し、外部システムへ到達します。各層で独自のカウンターが公開されており、順番に読むことでメッセージがどこで失われているかがわかります。
...
| `emqx_action_success` | アクション呼び出し成功回数。 |
| `emqx_action_failed` | アクション呼び出し失敗回数。 |
...
これらの系列のいずれかで非ゼロのレートがある場合、下流システムが不健康かアクションの設定ミスを示します。

| メトリクス | 説明 |
|------------|-------|
| `emqx_action_dropped` | アクション層でドロップされた総数。 |
| `emqx_action_dropped_queue_full` | キューの上限に達した回数。 |
| `emqx_action_dropped_resource_stopped` | 対象リソースが停止しているためのドロップ。 |
| `emqx_action_dropped_resource_not_found` | 対象リソースが存在しないためのドロップ。 |
| `emqx_action_dropped_expired` | ディスパッチ前にメッセージが期限切れとなった回数。 |
| `emqx_action_dropped_other` | その他の理由によるドロップ。 |

## 最小限の「ブローカー異常」パネル

Grafanaダッシュボードに表示できる系列が限られている場合、以下の指標が最も重要です。ほとんどの本番問題は、イベント発生から数秒以内にこれらのいずれかが動きます。

- `rate(emqx_messages_dropped[1m])`：非ゼロはブローカーが処理を拒否または失っていることを示します。
- `rate(emqx_action_dropped[1m])`：統合層で処理が失われています。
...
- `emqx_vm_process_messages_in_queues`：大きい値はプロセスメールボックスのバックログを示します。

...
`emqx_`で始まるメトリクスは常に有効です。より詳細なErlang VMメトリクスである`erlang_vm_`プレフィックスのメトリクスは、上流のPrometheus Erlangエクスポーター由来であり、EMQX 6.0以降では**デフォルトで無効**になっています。プロセス数、アロケータ別メモリ、GCやスケジューラの内訳を有効にするには、`prometheus.collectors.vm_system_info`、`vm_memory`、および`vm_statistics`を`enabled`に設定してください。

:::

## システム

ハードウェア層に最も近いシグナルです。ブローカーが不調になると、通常これらのどれかが最初に変動します。
...
| `erlang_vm_memory_processes` | アロケータ別メモリ：プロセス（`vm_memory`コレクター有効時）。 |
| `erlang_vm_memory_atom` | アロケータ別メモリ：アトム。 |
| `erlang_vm_memory_binary` | アロケータ別メモリ：バイナリ。 |
| `erlang_vm_memory_ets` | アロケータ別メモリ：ETSテーブル。 |
| `erlang_vm_memory_code` | アロケータ別メモリ：ロード済みコード。 |
| `erlang_vm_memory_system` | アロケータ別メモリ：システムオーバーヘッド。 |
...
| メトリクス | 説明 |
|------------|-------|
| `emqx_vm_run_queue` | 現在のスケジューラのランキュー長。継続的にゼロ以外の場合はCPU飽和を示します。 |
...
| `emqx_overload_protection_delay_ok` | 遅延適用に成功した回数。 |
...
| `emqx_sessions_count` | アクティブセッション数（現在切断中のクライアントの永続セッションも含む）。 |
...
| `emqx_delivery_dropped_queue_full` | サブスクライバーのメッセージキューが満杯のためのドロップ数。 |
| `emqx_delivery_dropped_too_large` | サブスクライバーの最大パケットサイズを超えたためのドロップ数。 |
...
HTTP、LDAP、またはデータベースバックエンドが認証パスにある場合に、ブローカー側かバックエンド側のどちらが遅延または障害の原因かを判断するためにこれらのメトリクスを使用します。
...
トラフィックはルールエンジンに入り、アクションやコネクターにファンアウトし、外部システムへ到達します。各レイヤーは独自のカウンターを公開しています。順に読むことでメッセージがどこで失われているかがわかります。
...
| メトリクス | 説明 |
|------------|-------|
| `emqx_rule_actions_total` | ルールから呼び出されたアクション数。 |
...
| `emqx_action_received` | アクションキューに到達した数。 |
...
これらの系列のいずれかに非ゼロのレートがある場合、下流システムが不健康かアクションの設定に問題があります。
...
Grafanaダッシュボードに数系列しか表示できない場合は、以下が最も重要です。ほとんどの本番問題は、イベント発生から数秒以内にこれらのいずれかが動きます。

- `rate(emqx_messages_dropped[1m])`：非ゼロはブローカーが処理を拒否または失っていることを示します。
- `rate(emqx_action_dropped[1m])`：統合レイヤーが処理を失っていることを示します。
- `emqx_cluster_nodes_stopped`：0より大きい場合、メンバーが失われています。

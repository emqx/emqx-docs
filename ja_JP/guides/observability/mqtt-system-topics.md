...
`$SYS`システムメッセージのパブリッシュ間隔は、ダッシュボードの「Management/MQTT Setting/System Topic」で設定できます。
...
| トピック                    | 説明                                                       |
| --------------------------- | ---------------------------------------------------------- |
| ${clientid}/connected       | オンラインイベント。クライアントがオンラインになった際にパブリッシュされます |
| ${clientid}/disconnected    | オフラインイベント。クライアントがオフラインになった際にパブリッシュされます |
...
| トピック                    | 説明                                                      |
| --------------------------- | --------------------------------------------------------- |
| ${clientid}/subscribed      | サブスクライブイベント。クライアントがトピックをサブスクライブした際にパブリッシュされます |
| ${clientid}/unsubscribed    | アンソブスクライブイベント。クライアントがトピックのサブスクライブを解除した際にパブリッシュされます |

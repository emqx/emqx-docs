...
`$SYS`トピックのパスは`$SYS/brokers/{node}/`で始まり、`{node}`はイベントやメッセージが生成されたノード名を表します。例えば：
...
パブリッシュおよびサブスクライブのACLルールを変更する場合は、[ファイル認可](../access-control/authz/file.md)をご参照ください。
...
| $SYS/brokers/\${node}/datetime   | EMQXの現在時刻      |
...
`connected`イベントメッセージのペイロードはJSON形式で解析できます：
...
`disconnected`イベントメッセージのペイロードはJSON形式で解析できます：
...
| ${clientid}/unsubscribed    | アンソブスクライブイベント。クライアントがトピックのサブスクライブを解除した際にパブリッシュされるメッセージ |

`subscribed`イベントメッセージのペイロードはJSON形式で解析できます：
...
`unsubscribed`イベントメッセージのペイロードはJSON形式で解析できます：
...
| packets/publish/error        | PUBLISHパケット処理エラーの累積数                  |
| packets/publish/auth_error   | PUBLISHパケット拒否の累積数                         |
| packets/publish/dropped      | PUBLISHパケット破棄の累積数                         |
| packets/puback/received      | 累積受信PUBACKパケット数                           |
| packets/puback/sent          | 累積送信PUBACKパケット数                           |
| packets/puback/inuse         | 累積破棄PUBACKパケット数                           |
| packets/puback/missed        | 累積見逃しPUBACKパケット数                         |
| packets/pubrec/received      | 累積受信PUBRECパケット数                           |
| packets/pubrec/sent          | 累積送信PUBRECパケット数                           |
| packets/pubrec/inuse         | 累積破棄PUBRECパケット数                           |
...
| packets/pubcomp/inuse        | 累積破棄PUBCOMPパケット数                          |
| packets/pubcomp/missed       | 累積見逃しPUBCOMPパケット数                        |
| packets/subscribe/received   | 累積受信SUBSCRIBEパケット数                        |
| packets/subscribe/error      | SUBSCRIBEパケット処理エラーの累積数                |
| packets/subscribe/auth_error | SUBSCRIBEパケット拒否の累積数                       |
...
| messages/dropped              | 破棄されたメッセージの総数                          |
| messages/dropped/expired      | 期限切れによる破棄メッセージの総数                  |
| messages/dropped/no_subscribers | サブスクライバーなしによる破棄メッセージの総数      |

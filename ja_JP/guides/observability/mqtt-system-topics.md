...
EMQXは、稼働状況、メッセージ統計、クライアントのオンライン・オフラインイベントを定期的に`$SYS/`で始まるシステムトピックにパブリッシュします。
...
| packets/pubcomp/missed        | 累積ミスされたPUBCOMPパケット数                |
...
| messages/forward              | ノードによってフォワードされたメッセージの総数  |
| messages/retained             | 累積リテインドメッセージ数                      |

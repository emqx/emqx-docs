...
MQTTはステートフルな長時間接続アクセスプロトコルであり、一度確立された接続は簡単に切断されません。そのため、クラスターのノードのアップグレード、メンテナンス、スケールアウトはより困難になります。EMQXは、ユーザーのクラスター運用・保守を支援するために、ノード退避とクラスター負荷再分散機能を提供しています。

## ノード退避

クラスター内のノードをメンテナンスやアップグレードする必要がある場合、ノードを直接シャットダウンすると接続やセッションが失われ、データ損失が発生します。また、この操作により多数のデバイスが一時的にオフラインになり再接続が発生し、サーバー負荷が増加し、全体のビジネスに影響を与える可能性があります。

そこでEMQXは、ノード退避機能を提供し、シャットダウン前に該当ノードのすべての接続およびセッションデータをクラスター内の他のノードへ移行することで、全体のビジネスへの影響を軽減します。

### 動作の仕組み

ノード退避は以下の順序で動作します。

1. 退避対象ノードは新規接続の受け入れを停止します。
2. 退避対象ノードは設定されたレート（`conn-evict-rate`で指定）で現在のクライアントを段階的に切断します。切断されたクライアントは再接続機構を使ってクラスター内の他ノード（ターゲットノード）に接続します。再接続機構はプロトコルバージョンにより異なります。
...
4. 再接続待機時間経過後、退避対象ノードに残る未引き継ぎのセッションはターゲットノードへ移行されます。
...
CLIコマンドを使ってノード退避の開始、退避状態の取得、退避の停止が可能です。
...
| パラメータ               | 型               | 説明                                                                                   |
| ------------------------ | ---------------- | -------------------------------------------------------------------------------------- |
| `--wait-health-check`    | 正の整数         | ノードがロードバランサー（LB）からアクティブなバックエンドノードリストから除外されるまで待機する時間（秒、デフォルト60秒）。この待機時間経過後に退避処理が開始され、新規接続の受け入れを拒否します。 |
...
ノード`emqx@127.0.0.1`上のクライアントを`emqx2@127.0.0.1`および`emqx3@127.0.0.1`のノードに移行したい場合、`emqx@127.0.0.1`ノード上で以下のコマンドを実行します。
...
このコマンドは既存クライアントを1秒あたり30接続のレートで切断し、すべての接続が切断された後に200秒間待機してクライアントセッションを再接続ノードに移行します。その後、残りのセッションを1秒あたり30セッションのレートで`emqx2@127.0.0.1`と`emqx3@127.0.0.1`ノードに移行します。

#### 退避状態の取得

以下のCLIコマンドで退避状態を取得できます。
...
HTTP APIを使ってノード退避の開始・停止も可能です。退避対象ノードをパラメータで指定する必要があります。詳細は[APIドキュメント](https://docs.emqx.com/en/enterprise/v5.1/admin/api-docs.html)を参照してください。

## 負荷再分散

MQTTがステートフルな長時間接続プロトコルであるため、接続確立後は簡単に切断されません。ノードをスケールアウトしても既存接続は自動的に新規ノードに移動しません。そのため新規クライアント接続が少ない場合、追加ノードが長期間低負荷のままになることがあります。このような場合、高負荷ノードから低負荷ノードへ接続を手動で移行し、クラスターの負荷バランスを取る必要があります。
...
任意のノードでクラスター負荷再分散タスクを開始できます。EMQXは各ノードの現在の接続負荷に基づいて接続移行計画を自動計算し、高負荷ノードから低負荷ノードへ対応する数の接続およびセッションを移行し、ノード間の負荷バランスを実現します。ワークフローは以下の通りです。

1. 移行計画を計算し、再分散に関与するノード（`--nodes`で指定）を送信元ノードと受信先ノードに分割します。
   - 送信元ノード：高負荷ノード
   - 受信先ノード：低負荷ノード
2. 送信元ノードで新規接続の受け入れを停止します。
3. 一定期間（`wait-health-check`で指定）待機し、ロードバランサー（LB）が送信元ノードをアクティブバックエンドノードリストから除外するのを待ちます。
4. 送信元ノードの接続クライアントを段階的に切断し、平均接続数が受信先ノードと同等になるまで続けます。
5. 受信先ノードがクライアントと再接続し、セッションを引き継ぐのを待ちます（`wait-takeover`で指定）。
6. 再接続待機時間経過後、送信元ノードは残る未引き継ぎセッションを受信先ノードへ、`sess-evict-rate`で指定したレートで移行します。
...
CLIコマンドを使って負荷再分散の開始、状態取得、停止が可能です。
...
| フィールド               | 型               | 説明                                                                                   |
| ------------------------ | ---------------- | -------------------------------------------------------------------------------------- |
| `--nodes`               | 文字列           | 再分散に参加するノードのスペースまたはカンマ区切りリスト。コマンドを実行するコーディネーター（ノード）を含む場合も含まない場合もあります。 |
| `--wait-health-check`   | 正の整数         | ノードがロードバランサー（LB）からアクティブバックエンドノードリストから除外されるまで待機する時間（秒、デフォルト60秒）。この待機時間経過後に負荷再分散処理が開始されます。 |
| `--conn-evict-rate`     | 正の整数         | 送信元ノードでのクライアント切断レート。デフォルトは1秒あたり500接続。               |
...
| `--sess-evict-rate`     | 正の整数         | 送信元ノードでのセッション退避レート。デフォルトは1秒あたり500セッション。           |
...
CLIで可能なすべての操作はAPIでも可能です。開始・停止コマンドはノードをパラメータとして指定する必要があります。詳細は[APIドキュメント](https://docs.emqx.com/en/enterprise/v5.1/admin/api-docs.html)を参照してください。

## ロードバランサーの統合

ユーザーはロードバランサーを統合して退避や負荷再分散を実行できます。切断されたクライアントが再接続を試みる際、ロードバランサーはバックエンドノードの現在の状態に基づいて受信先ノードへリダイレクトします。ユーザーはロードバランサー統合のためにヘルスチェックパラメータを設定する必要があります。設定しないと切断が過剰に発生する可能性があります。EMQXはこの設定を支援するためにヘルスチェック用REST APIを提供しています。

`GET /api/v5/load_rebalance/availability_check`

ヘルスチェックは、送信元または退避中ノードに対してはHTTPコード503を返し、正常稼働中で接続を受け入れているノードに対してはHTTPコード200を返します。

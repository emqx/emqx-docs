...
[Prometheus](https://prometheus.io/) は SoundCloud によってオープンソース化された監視ソリューションで、多次元データモデルのサポート、柔軟なクエリ言語、強力なアラーム管理機能を特徴としています。EMQX はサードパーティ監視システムとして Prometheus との統合をサポートしています。この機能の詳細については、[Prometheus との統合](../observability/prometheus.md) をご参照ください。
...
- `push_gateway_server` は Prometheus の Pushgateway サーバーの URL を設定します。これはメトリクスを Prometheus にプッシュするために使用され、デフォルトは `http://127.0.0.1:9091` です。
...
左側のナビゲーションメニューから **Management** -> **Monitoring** -> **Integration** をクリックすることで、ダッシュボードからも Prometheus とのプッシュモード統合を設定できます。

ダッシュボードで統合を設定すると、その設定は設定ファイル内の同じ項目を上書きします。  
設定ファイルから Prometheus を設定したい場合は、`emqx.conf` ではなく `base.hocon` を使用することを推奨します。  

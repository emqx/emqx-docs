...
ユーザーは設定ファイルまたは環境変数を用いてEMQXを設定できます。本節では主にEMQXの設定ファイルについて紹介し、EMQXで最もよく使われる機能の基本的な設定方法を説明します。詳細な設定項目や解説については、[EMQX Enterprise Configuration Manual](https://docs.emqx.com/en/enterprise/v@EE_VERSION@/hocon/)をご参照ください。
...
`etc`ディレクトリにはEMQXの初期設定を定義する設定ファイルが格納されます。これらのファイルは通常、デプロイ時やアップグレード時に変更され、実行時は安定性を保つために読み取り専用となっています。`etc`ディレクトリの場所はインストール方法によって異なります。
...
例えば、基本的な認証設定でデプロイを開始し、その後ダッシュボードUIからより複雑な設定に上書きすることが可能です。

`node`や`cluster`のような不変の設定は、`base.hocon`に設定することは**推奨されません**。詳細は[Immutable Configurations File](#immutable-configuration-file)をご参照ください。
...
EMQX 5.1以降、クラスター設定の変更時に`cluster.hocon`ファイルのバックアップが作成されます。バックアップはノードのローカル時間でタイムスタンプが付与され、最大10個まで保持されます。
...
EMQXでは、設定値はドット区切りのパスで参照できます。ルート（常にStruct）から始まり、各セグメントはフィールド名またはMapのキーを指します。配列要素は1始まりのインデックスを使用します。
...
HOCONは人間に読みやすいデータフォーマットであり、JSONのスーパーセットです。継承や結合、引用符などの機能により設定作業をさらに簡素化します。

**HOCON構文例：**

JSONのようなオブジェクト表現が可能です。
...
このフラット形式は従来のEMQXバージョンとの互換性を保ちつつ、使い分けられています。
...
2. 他の環境変数と区別するため、EMQXは環境変数名の先頭に`EMQX_`を付加します。
3. 環境変数の値はHOCON値として解析されるため、複雑なデータ型も渡せます。`:`、`=`、`#`などのHOCON特殊文字を含む値は、ダブルクォート`"`で囲む必要があります。特に`#`はHOCONの行コメントを開始するため、クォートなしでは`#`以降がコメントとして無視されます。
...
パスワードなどに`#`を含む場合、`#`はHOCONの行コメントを開始するため、以下のように設定するとパスワードが途中で切れてしまいます。

```bash
export EMQX_DASHBOARD__DEFAULT_PASSWORD="MQtt#123"
```

この場合、パスワードは`MQtt`として解釈され、`#123`はコメントとして無視されます。文字列として正しく渡すには、**HOCONレベルの**ダブルクォートで囲み、パーサーに文字列として認識させます。
...
EMQXは環境変数の値を`fake_key=<値>`としてHOCONパースを試みます。成功すれば解析結果を使い、失敗すれば生の文字列を使います。

そのため、`EMQX_..._PASSWORD="abc#def"`は有効なHOCONで`#def`がコメント扱いされ`abc`となりますが、`EMQX_..._PASSWORD=".abc#def"`は無効なHOCONで生文字列のまま保持されます。HOCONクォートで囲むことで動作が明確になります。
...
既知のルートパスに対して未知のフィールド名が設定された場合、起動時に`warning`ログを出力します。例えば`enable`を誤って`enabled`と設定すると以下のように表示されます。
...
つまり、`base.hocon`の設定は最も低い優先度であり、より優先度の高いファイルで上書き可能です。`EMQX_`で始まる環境変数が最も優先されます。

::: tip
バージョン5.8.4以前は`base.hocon`ファイルが存在しませんでした。優先順位は同じですが、`base.hocon`は含まれません。
:::

EMQXダッシュボードUI、HTTP API、CLIからの変更は実行時に`cluster.hocon`に永続化され、即座に反映されます。ただし、`emqx.conf`や環境変数で同じ設定項目が異なる値に設定されている場合、ノード再起動後に変更が元に戻ることがあります。
...
2. 5.0.2/v5.0.22以前から最新バージョンにアップグレードする際も優先順位は変わらず、互換性維持のため`cluster.hocon`は作成されません。
3. `cluster-override.conf`の仕組みはバージョン5.1で廃止されました。
:::
...
## 以下の方法では、最初の要素の`enable`以外のフィールドはすべて失われます。
...
EMQXのゾーンは設定をグループ化する概念です。ゾーンはリスナーの`zone`フィールドにゾーン名を設定することで関連付けられます。ゾーンに関連付けられたリスナーに接続したMQTTクライアントは、そのゾーンの設定を継承し、グローバル設定を上書きする場合があります。

::: tip
デフォルトではリスナーは`default`という名前のゾーンにリンクされています。`default`ゾーンは論理的なグループであり、設定ファイルには存在しません。
:::

ゾーンレベルで上書き可能な設定項目は以下の通りです。

- `mqtt`：MQTT接続およびセッション設定。特定ゾーンでMQTTメッセージの最大パケットサイズを大きく許可するなど。
- `force_shutdown`：強制シャットダウンポリシー。
- `force_gc`：Erlangプロセスのガベージコレクションの微調整。
- `flapping_detect`：クライアントのフラッピング検出。
- `durable_sessions`：セッション永続化設定。特定ゾーンでMQTTセッションの耐久ストレージを有効化するなど。

EMQXバージョン5のデフォルト設定ファイルにはゾーンは含まれておらず、バージョン4の`internal`および`external`の2つのデフォルトゾーンとは異なります。
...
::: tip
ゾーンの設定スキーマは各グループで共通のため、設定マニュアルには含まれていません。例：`zones.my_zone1.mqtt {...}`は`mqtt {...}`と同じスキーマです。
:::

### プリミティブデータ型

設定マニュアルのプリミティブ型はほぼ自明であり、最小限の説明で十分です。以下にすべてのプリミティブ型の一覧を示します。
...
指定範囲内の整数。例：`1..+inf`は1から正の無限大までの整数を意味し、正の整数のみ許容されます。

#### Enum(symbol1, symbol2, ...)

列挙型で、定義されたシンボルのいずれかのみを取ります。例：`Enum(debug,info,warning,error)`はログレベルの許容値です。

#### String

文字列型で、複数のフォーマットをサポートします。

- **無引用符**：特殊文字を含まない簡単な識別子や名前に適します（詳細は後述）。
...
EMQX独自のインデント付き三重引用符文字列の詳細は[emqx/hocon.git README](https://github.com/emqx/hocon?tab=readme-ov-file#divergence-from-spec-and-caveats)をご覧ください。

#### String("constant")

定数文字列値で、単一値列挙型（Enum）として機能します。特定の設定やモードの静的値を定義する際に使います。
...
パスワードやトークンなどの機密情報用の型。用途と重要性の説明。

### 複合データ型

EMQXのHOCON設定における複合型は、他の複合型やプリミティブ値を含むデータ構造を表現します。柔軟で階層的なデータ表現を可能にします。
...
複数の型のいずれかを取るユニオン型。あるフィールドが複数の型のいずれかを許容する場合に使います。例：`String(infinity)`または`Duration`。
...
Variformは文字列操作や実行時評価のための軽量で表現力豊かな言語です。完全なプログラミング言語ではなく、EMQXの設定内に埋め込んで動的に文字列操作を行うための専用ツールです。

::: tip
Variform式は特定の設定項目にのみ適用可能です。明示されていない限り使用しないでください。
:::

::: tip NULL値について
Variform式では、値バインディング参照や部分式の評価結果が未定義値となる場合、空文字列（`''`）として扱われます。
...
EMQXはルールエンジンの文字列関数に似た豊富な文字列、配列、乱数、ハッシュ関数を提供しています。これらは抽出データの操作や整形に使えます。例：`lower()`、`upper()`、`concat()`、`hash()`、`hash_to_range()`など。
...
- **比較関数**：
  - `num_eq(A, B)`：2つの数値が等しい場合`true`、それ以外は`false`
  - `num_neq(A, B)`：2つの数値が異なる場合`true`、それ以外は`false`
  ...
  - `str_eq(A, B)`：2つの文字列が等しい場合`true`、それ以外は`false`
  - `str_neq(A, B)`：2つの文字列が異なる場合`true`、それ以外は`false`
...
- **システム関数**：
  - `getenv(Name)`：環境変数`Name`の値を返す。OS環境変数の前に`EMQXVAR_`が付加される。例：`getenv('FOO_BAR')`は`EMQXVAR_FOO_BAR`を読み込む。読み込み後は値は不変。

- **データ抽出関数**：
  - `json_value(Data, Path)`：JSON文字列からドット区切りパスで値を抽出。例：`username`がJSONオブジェクトの場合、`json_value(username, 'shop.floor')`でフィールドを取得。
  - `jwt_value(Data, Path)`：JWTトークンのペイロードからドット区切りパスでクレーム値を抽出。例：`password`がカスタムクレームを含むJWTの場合、`jwt_value(password, 'client_attrs.unitid')`でネスト値を取得。
...
- 実行時例外：関数の誤用や型不一致、配列の範囲外アクセスなどの例外は空文字列を返します。

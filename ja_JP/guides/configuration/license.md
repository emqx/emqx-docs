...
さらに、ライセンスの接続クォータ使用状況に関する設定を設定ファイルで以下のように構成できます。
...
- `connection_high_watermark` はライセンス接続クォータ使用率のアラームが発動される閾値を設定します。デフォルトは `"80%"` です。

実行後、`emqx ctl license info` を実行して新しいライセンスファイルが適用されていることを確認できます。

::: tip

ライセンスファイルはEMQXダッシュボードからも設定可能です。ダッシュボードでのライセンス設定方法については、[Work with License](../../get-started/deploy/license.md) を参照してください。ダッシュボードで設定した内容は、設定ファイル内の同じ設定項目より優先されます。

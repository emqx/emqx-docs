...
SSL/TLS証明書はEMQXのセキュリティアーキテクチャの中核をなす要素です。これにより、ネットワーク通信の認証、暗号化、およびデータの完全性が提供されます。EMQXでは、以下のシナリオで安全な接続を確立するためにSSL/TLS証明書が必要です。
...
2. 管理証明書（EMQX 6.1以降）：再利用可能なリソースとして管理され、名前で参照される方式

どちらの方式も標準のPEM形式ファイルを使用し、EMQXのSSL/TLS実装と完全に互換性があります。
...
- **自己署名証明書**
    ...
    独自の認証局（CA）が発行する証明書です。デフォルトでは信頼されないため、テストや管理された環境での利用が推奨されます。

- **信頼されたCAが発行する証明書の申請または購入**
...
<!--EMQXはACMEプロトコル（例：Let's Encrypt）を用いてサーバー証明書の自動取得および更新をサポートします。-->
...
1. 以下のコマンドを実行し、鍵ペアを生成します。コマンド実行時に鍵を保護するパスワードの入力が求められます。このパスワードは証明書の生成、発行、検証時に必要となるため、安全に保管してください。

   ```bash
   openssl genrsa -des3 -out rootCA.key 2048
   ```

2. 以下のコマンドを実行し、鍵ペアの秘密鍵を用いてCA証明書を生成します。コマンド実行時に証明書の識別名（DN）の設定が求められます。
...
先ほど作成したCA証明書を用いて、EMQXのリスナーがクライアントに自身の正当性を証明するためのサーバー証明書を発行します。サーバー証明書は通常、ホスト名、サーバー名、またはドメイン名（例：[www.emqx.com](https://www.emqx.com/en)）に発行されます。サーバー証明書の生成には、CAの秘密鍵（rootCA.key）、CA証明書（rootCA.crt）、およびサーバー証明書署名要求（CSR）（server.csr）が必要です。
...
2. サーバー鍵ペアを用いてCSRを作成します。CSRはCAの秘密鍵で署名されることで証明書が発行されます。コマンド実行時に証明書の識別名（DN）の設定が求められます。

   ```bash
   openssl req -new -key server.key -out server.csr
   ```

   以下のような入力プロンプトが表示されます。各項目の意味は以下の通りです。
...
3. 以下のコマンドを実行し、サーバーCSRを用いてサーバー証明書を生成します。有効期間はここでは365日に設定しています。
...
- 証明書は同じCA証明書で署名されます。したがって、クライアント証明書も前述のCA証明書で署名可能です。
...
EMQX 6.1以降、管理証明書というTLS証明書ファイルを集中管理し、複数コンポーネントで再利用可能な仕組みが導入されました。
...
EMQXの管理証明書は従来のTLS証明書展開方法と同様にPEM形式のファイルを使用します。

PEMファイルを使用する利点：

- **可視性**：`openssl`などの標準ツールで証明書ファイルの検査や検証が容易
...
同一リスナーに複数の管理証明書バンドルが設定されている場合、EMQXはServer Name Indication（SNI）に基づいて動的に証明書を選択します。

SNIはTLS拡張機能で、クライアントがTLSハンドシェイク中に接続先のホスト名を指定する仕組みです。
...
- TLSハンドシェイク時に以下が行われます。
  1. クライアントがSNIホスト名を送信
  2. EMQXが設定済みの証明書エントリとSNIを照合
  3. 一致する証明書バンドルがあればそれを使用
  4. 一致しなければリストの最初の証明書をデフォルトとして使用

この仕組みにより、単一リスナーで同一IP・ポート上に複数ドメインやテナント向けの異なるTLS証明書を安全に提供可能です。
...
ここではダッシュボードおよびREST APIを用いた管理証明書バンドルの作成と管理方法を説明します。作成後、証明書バンドルは選択可能となり、複数のリスナーやコネクターで再利用できます。
...
3. **管理証明書の作成**パネルで以下の情報を入力します。
   ...
   - **ネームスペース**：管理証明書バンドルをグローバルネームスペースか特定のテナントネームスペースに作成するかを制御します。
     ...
     - グローバル管理者は`global`ネームスペースおよび任意のテナント（非グローバル）ネームスペースにバンドルを作成可能です。
     - ネームスペーススコープのユーザーは自身のネームスペース内でのみ作成可能です。

   - **TLS証明書**（必須）：PEM形式のサーバー証明書。証明書内容を直接貼り付けるか、**ファイル選択**でアップロード可能です。必要に応じて完全な証明書チェーンを含めてください。

   - **TLS秘密鍵**（必須）：サーバー証明書に対応するPEM形式の秘密鍵。内容を直接貼り付けるかファイルアップロード可能です。

   - **鍵パスワード**：秘密鍵が暗号化されている場合のパスフレーズ（任意）。

   - **CA証明書**：PEM形式の認証局証明書（任意）。双方向TLS認証やCA証明書を必要とするコネクターでの再利用時に必要です。
...
- 画面上部のネームスペースドロップダウンでグローバルネームスペースと特定のテナント（非グローバル）ネームスペースを切り替えられます。選択に応じて一覧が自動更新されます。
...
EMQX 6.1.4以降、証明書バンドルが何らかの設定で参照されている場合は削除できません。

1. **削除**をクリックすると、**削除の確認**ダイアログが表示されます。
2. **キャンセル**でダイアログを閉じ、削除を中止できます。**確認**をクリックすると削除処理を進めます。
3. **確認**クリック後、以下のいずれかが発生します。
   - 依存関係がなければ証明書バンドルが削除されます。
   - 依存する設定がある場合は削除が拒否され、**使用中のバンドル**ダイアログに参照している設定が一覧表示されます。**ページへ移動**をクリックし、該当設定を更新または削除してから再度削除を試みるか、**キャンセル**でダイアログを閉じます。
...
ダッシュボードに加え、EMQXはTLS証明書ファイル管理用のREST APIを提供しています。APIで作成した管理証明書はダッシュボードと同様にリスナーやコネクターから参照可能です。
...
EMQX 6.1.4以降、全ネームスペースの設定で参照されている管理証明書バンドルや個別ファイルは削除できません。リクエストは`400 BAD_REQUEST`エラーで拒否されます。
...
バンドル内の個別ファイルを削除する場合も同じエンドポイントを使用し、`kind`クエリパラメータでファイル種別を指定します。依存関係チェックはバンドル全体および個別ファイルに適用されます。

`force_delete`クエリパラメータはAPIスキーマから削除されており、依存関係チェックを回避できません。既存クライアントが`force_delete=true`を送信した場合、EMQXは無視して通常の削除処理を行います。

削除がブロックされた場合、`400`レスポンスの本文に`referencing_configs`フィールドが含まれ、参照している設定がネームスペース別に示されます。例：
...
EMQXはACMEプロトコル（例：Let's Encrypt）を用いたサーバー側TLS証明書の自動発行および更新をサポートします。この機能は、クライアントが公開ドメイン名を使って直接EMQXにTLS接続する展開向けのオプション機能です。ロードバランサーでTLS終了する場合やプライベート・内部ホスト名のみを使用する場合には適しません。

ACMEはEMQX管理者が設定ファイルで有効化・設定し、ダッシュボードやネームスペーススコープユーザーからは利用できません。
...
ACME発行証明書は手動アップロードの管理証明書と同様に動作します。ユーザーはリスナー設定時に生成済みの管理証明書バンドルを選択・参照するだけでよく、手動アップロードは不要です。
...
- 管理証明書を参照するすべてのリスナーやTLS対応コンポーネントは自動的に更新後の証明書を使用します。
- リスナーやEMQXの再起動は不要です。

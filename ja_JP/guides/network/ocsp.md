...
注意：QUICリスナーのSecure WebSocketは現在サポートされていません。
...
EMQXでは、ダッシュボードおよび設定ファイルの両方からSSLリスナーに対してOCSPを有効化できます。
...
EMQXダッシュボードで、**Management** -> **Listeners** に移動し、**Listener** ページを開きます。デフォルトのSSLリスナーに対してOCSP機能を有効化するには、その名前をクリックして **Edit Listener** ページを開きます。右側に表示されるダイアログの下部までスクロールし、**Enable OCSP Stapling** のトグルスイッチを見つけてください。
...
- **OCSP Issuser Certificate**：SSL/TLS証明書を発行した認証局（CA）の証明書を設定します。EMQXはこの証明書を用いてOCSPレスポンスの真正性を検証します。

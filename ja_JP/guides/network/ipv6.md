...
EMQXはクライアント接続、ダッシュボード、ノード間クラスタリング、および外部サービスへのアウトバウンド接続に対してIPv6を完全にサポートしています。このページでは、シングルスタック（IPv6のみ）からデュアルスタック展開まで、IPv6環境向けのEMQX設定方法を説明します。
...
同じポートでIPv4およびIPv6接続の両方を受け入れるには、`[::]`にバインドします：
...
ほとんどのオペレーティングシステムでは、`[::]`へのバインドはデフォルトでIPv4およびIPv6接続の両方を受け入れます（デュアルスタック）。両方のプロトコルをサポートする環境において最も簡単な設定です。
...
EMQXクラスターのノード間通信がIPv6ネットワークを使用する場合、設定が必要なコンポーネントは2つあります：クラスタ調整に使用されるErlang分散プロトコルと、ノード間のデータ転送に使用されるGen RPCチャネルです。
...
- MQTTコネクターでは、IPv6のみのブローカーやホスト名がIPv6の`AAAA`レコードのみを返すブローカーにも接続可能です。MQTTブローカーアドレスは、`[::1]:1883`や`mqtt://[::1]:1883`のような括弧付きIPv6リテラルも使用できます。

### 手動オーバーライド

一部のコネクタータイプでは設定に`ipv6_probe`トグルがあり、有効（HTTPコネクターのデフォルト）にするとEMQXはまずIPv6接続を試みます。ネットワークがIPv4のみでDNSがAレコードとAAAAレコードの両方を返す場合、接続遅延を避けるためにプローブを無効化できます：
...
**対処法**：EMQXホストからターゲットサービスが正しいアドレスファミリーで到達可能か確認してください。HTTPコネクターは自動IPv6プローブで対応します。DNS名の場合はDNSが正しいレコードタイプ（IPv4はA、IPv6はAAAA）を返しているか確認してください。

### IPv6環境でダッシュボードにアクセスできない

**症状**：IPv6のみ環境でEMQXを起動するとダッシュボードにアクセスできない。

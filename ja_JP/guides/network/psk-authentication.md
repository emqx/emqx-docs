...
事前共有鍵（PSK）認証は、事前に共有された鍵を用いて本人確認を行う認証方式です。PSK認証方式を使用すると、クライアントとEMQXはセキュアな接続を確立する前に同じ鍵を共有している必要があります。この事前共有鍵は、クライアントとEMQX間のTLS接続の確立およびその後の通信においてデータの暗号化と復号に使用されます。PSK認証を有効にすると、クライアントとEMQXは証明書や証明書機関を必要とせずに相互認証を行い、安全な接続を確立できます。

本ページでは、EMQXでPSK認証を有効にする方法を紹介します。

1. 任意のディレクトリに `data/psk_file.txt` ファイルを作成し、事前共有鍵のID（identity）と秘密値を記述します。

   ::: tip

   秘密値は任意の文字列で構いませんが、選択した暗号スイートに対応した長さである必要があります。例えば、暗号スイートが `TLS_PSK_WITH_AES_128_CBC_SHA` の場合、秘密値は128ビットの長さでなければなりません。

   :::

   ```bash
   # 1行につき1つのデータ、フォーマットは PSKIdentity:SharedSecret

...
Pre-Shared Key（PSK）認証は、事前に共有されたキーを用いて本人確認を行う認証方式です。PSK認証方式では、クライアントとEMQXの両者がセキュアな接続を確立する前に同じキーを事前に共有しておく必要があります。共有されたキーは、クライアントとEMQX間のTLS接続の確立およびその後の通信においてデータの暗号化・復号に使用されます。PSK認証を有効にすると、クライアントとEMQXは証明書や認証局を必要とせずに相互認証を行い、安全な接続を確立できます。
...
1. 任意のディレクトリに `data/psk_file.txt` ファイルを作成し、事前共有キーのIDと秘密値を記述します。
   ...
   ```bash
   # 1行に1つのデータを、PSKIdentity:SharedSecret の形式で記述
...
3. 設定ファイルでSSLリスナーを設定します。`listeners.ssl.default` グループを修正し、以下のオプションを追加してください。
   ...
   - `ssl_options.versions`: `tlsv1.3` のサポートを削除します。`tlsv1.3` はPSK暗号を無効化するためです。

...
EMQXをシングルモードまたはEMQXクラスターとしてデプロイした後、システムの処理能力や負荷時の挙動を評価するためにパフォーマンステストを実施できます。本セクションでは、パフォーマンステストを行うための[eMQTT-Bench](https://www.emqx.com/en/try?product=emqtt-bench)のインストール方法と使用方法について説明します。
...
各`emqtt-bench`リリースの詳細は[Releases](https://github.com/emqx/emqtt-bench/releases)をご覧ください。
...
コンパイル後、カレントディレクトリに`emqtt_bench`という実行可能スクリプトが生成されます。以下のコマンドを実行して正常に動作することを確認してください。
...
例えば、10接続を開始し、1秒間に100件のQoS0メッセージをトピック`t`にパブリッシュし、各メッセージのペイロードサイズを16バイトにする場合は以下のように実行します。
...
`./emqtt_bench sub --help`を実行すると、このサブコマンドで利用可能なパラメーターがすべて表示されます。説明は上記の表に含まれているためここでは省略します。
...
`./emqtt_bench conn --help`を実行すると、このサブコマンドで利用可能なパラメーターがすべて表示されます。説明は上記の表に含まれているためここでは省略します。
...
`emqtt-bench`はSSLによるセキュアな接続を確立し、テストを行うことが可能です。
...
ここでは、典型的な2つのシナリオでストレステストを行う方法を説明します：接続数とスループットです。
...
合計3台の8コア16GBメモリのサーバーを用意し、1台をEMQX用、2台をクライアント負荷用に使用します。

- **システム**: `CentOS Linux release 7.7.1908 (Core)`
- **CPU**: `Intel Xeon Processor (Skylake)` メインクロック：`2693.670 MHZ`
- **サーバー**: `emqx-centos7-v4.0.2.zip`
- **負荷ツール**: `emqtt-bench v0.3.1`。各負荷ツールは10枚のネットワークカードを設定し、接続テストで多数のMQTTクライアント接続を確立するために使用します。
...
クライアント負荷ツールとサーバーの両方でシステムパラメーターのチューニングが必要です。詳細は[Tuning guide](./tune.md)を参照してください。
...
すべての接続が確立された後、`./bin/emqx ctl listeners`を実行すると、EMQXの接続数に関する以下の情報が確認できます。

...
EMQXをシングルモードまたはクラスターとしてデプロイした後、システムの容量や負荷時の挙動を理解するためにパフォーマンス評価を行うことができます。本節では、パフォーマンステストを実施するための[eMQTT-Bench](https://www.emqx.com/en/try?product=emqtt-bench)のインストール方法と使用方法について説明します。
...
eMQTT-BenchはErlangで書かれており、ビルドには[Erlang/OTP](https://www.erlang.org/) 22.3以上が必要です。Erlang/OTPのインストール手順は割愛しますが、オンラインのインストールチュートリアルをご参照ください。
...
例えば、10接続を開始し、1秒ごとにQos0メッセージを100件、トピック`t`にペイロードサイズ16バイトで送信する場合は以下のように実行します。
...
`./emqtt_bench sub --help`を実行すると、このサブコマンドで利用可能なパラメータが表示されます。説明は上記の表と重複するため省略します。
...
`./emqtt_bench conn --help`を実行すると、このサブコマンドで利用可能なパラメータが表示されます。説明は上記の表と重複するため省略します。
...
本節では、接続数とスループットの2つの典型的なシナリオにおけるストレステストの実施方法について説明します。
...
2. スループット：`emqtt-bench`を使用してEMQXに`100k/s Qos 0`のメッセージスループットを発生させる。

### デバイスとデプロイトポロジー

合計で3台の8コア16GBサーバーを用意します。1台はEMQX用、2台はクライアント負荷用です。

- **システム**：`CentOS Linux release 7.7.1908 (Core)`
- **CPU**：`Intel Xeon Processor (Skylake)` メイン周波数：`2693.670 MHZ`
- **サーバー**：`emqx-centos7-v4.0.2.zip`
- **負荷用**：`emqtt-bench v0.3.1`、各負荷マシンは10枚のネットワークカードを持ち、接続テストで多数のMQTTクライアント接続を確立するために使用
...
クライアント負荷用とサーバーの両方でシステムパラメータのチューニングが必要です。詳細は[Tuning guide](./tune.md)を参照してください。

...
ネームスペースソースを設定することで、EMQXはクライアント接続情報からネームスペースを識別し、クライアントが接続した際に対応するネームスペースを自動的に作成します。
...
4. **操作** 列の **クライアント** をクリックし、このネームスペースに接続されているクライアントを表示します。
...
- クライアントIDにネームスペースのプレフィックスが自動的に付与され、ネームスペース間でのクライアントID競合を防止します。
...
デフォルトでは、後方互換性を維持するため、認可（ACL）チェックはトピックプレフィックス（マウントポイント）を含みません。つまり、認可ルールは名前空間付きトピック（例：`tenantA/test/topic`）ではなく、元のトピック名（例：`test/topic`）に対してマッチングされます。
...
例えば、クライアントがトピックプレフィックス `tenantA/` が付与されたリスナー経由で接続し、`test/topic` をサブスクライブしたい場合、対応する認可ルールは `tenantA/test/topic` として設定しなければなりません。

...
デバイスがProtobufでエンコードされたバイナリメッセージをパブリッシュし、そのメッセージをルールエンジンでマッチングし、`name`フィールドに関連付けられたトピックに再パブリッシュする必要があります。トピックの形式は `person/${name}` です。
...
以下のコードはPython言語を使用し、ユーザーメッセージを作成してバイナリデータとしてエンコードし、`t/1` トピックに送信します。詳細は[フルコード](https://gist.github.com/thalesmg/3c5fdbae2843d63c2380886e69d6123c)を参照してください。
...
デバイスが `protobuf_out` トピックをサブスクライブし、Protobufでエンコードされたバイナリメッセージを受信することを期待しています。ルールエンジンはそのようなメッセージをエンコードし、関連トピックにパブリッシュするために使用されます。
...
以下のコードはPython言語を使用し、ユーザーメッセージを作成してバイナリデータとしてエンコードし、`protobuf_in` トピックに送信します。詳細は[フルコード](https://gist.github.com/thalesmg/c5f03f99f982401d16ef6583e30144fa)を参照してください。

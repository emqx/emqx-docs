...
サポートされていないキーワードをスキーマが使用している場合、検証はキーワードを無視するのではなくエラーを返します。

## JSON Schema の作成

整数がちょうど2つ含まれる配列を受け入れるスキーマを作成します：
...
`prefixItems` 配列は各位置のスキーマを定義します。ローカルの `$ref` により、2番目の要素は1番目の要素と同じ整数スキーマを使用します。`minItems` と `maxItems` により、ペイロードはちょうど2つの要素を含む必要があります。
...
2. 別のターミナルで、スキーマに準拠したペイロードをパブリッシュします：
...
ルール内で `schema_check` を使うとルールの実行をフィルタリングできますが、元の MQTT メッセージは拒否されません。非準拠メッセージを拒否または破棄するには、[スキーマ検証](./schema-validation.md) をご利用ください。

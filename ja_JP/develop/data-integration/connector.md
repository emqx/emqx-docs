...
コネクターは外部データシステムへの接続のみに特化しています。ユーザーは異なる外部データシステムごとに複数のコネクターを作成でき、1つのコネクターで複数のSink/Sourceへの接続を提供できます。
...
- 接続設定をデータ処理およびマッピング設定から分離することで、データ処理フローの設計がよりモジュール化され柔軟になります。
...
4. コネクター設定情報ページで、コネクター名、説明、接続パラメータなど基本情報を入力します。ここでの内容は各Sink/Sourceの使用ドキュメント内のコネクターパラメータ節を参照しており、詳細は省略します。

   ![EMQX Connector Configuration](./assets/config-connector.png)

## コネクターの表示と管理

コネクター作成後は、Connectorsページで基本情報を確認できます。コネクターの有効化・無効化や設定編集、ルール作成、複製、削除などの管理操作は、コネクター一覧の **Actions** 列から行えます。
...
- **Connecting**：ヘルスチェック実施前の初期状態で、コネクターは外部データシステムへの接続を試みています。
- **Connected**：コネクターが外部データシステムに正常に接続されています。この状態でヘルスチェックが失敗すると、障害の深刻度に応じて「connecting」または「disconnected」状態に切り替わることがあります。
- **Disconnected**：ヘルスチェックに失敗し、非正常状態です。設定により定期的に自動再接続を試みる場合があります。
- **Inconsistent**：クラスター内のノード間でコネクターの状態が不整合です。例えば、一部のノードで接続済み、一部のノードで切断状態となっている場合などです。
...
例えば、3ノードのEMQXクラスターでコネクションプールサイズを8に設定した場合、EMQXは3 × 8 = 24接続を作成します。

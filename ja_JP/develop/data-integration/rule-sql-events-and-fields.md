...
[ルールエンジン構文](./rule-sql-syntax.md)の節で説明したように、`FROM`句でデータソースを指定し、対応するフィールドを`SELECT`句や`where`句で参照できます。本節では、[MQTTメッセージ](#mqtt-message)、[MQTTイベント](#mqtt-events)、および[データブリッジ](#data-bridges)のフィールドについて紹介します。
...
EMQXのルールを使ってイベントトピックからデータを抽出し、クライアントのオンライン・オフラインやサブスクリプションなどのイベント通知を受け取れます。イベントトピックは`"$events/"`で始まり、例えば`"$events/client/connected"`のように`FROM`句で指定します。
...
| [$events/message/dropped](#message-dropped-when-routing-event-events-message-dropped) | ルーティング中のメッセージ破棄 |
| [$events/message/delivery_dropped](#message-dropped-when-delivering-event-events-delivery-dropped) | 配信中のメッセージ破棄       |
...
| [$events/session/unsubscribed](#unsubscribe-event-events-session-unsubscribed) | アン・サブスクライブ         |
...
EMQX 5.10.0以降、イベントトピックにネームスペースが導入され、論理的で階層的な構造に再編成されました。これにより、イベントトピックの明確化、フィルタリング、管理が改善されています。

後方互換性のため旧イベントトピックもサポートされていますが、新規設定ではネームスペース付きの新しいトピックを使用することを推奨します。以下の表は旧トピックと新トピックの対応表です。
...
### ルーティング中のメッセージ破棄イベント ("$events/message/dropped")
...
### 配信中のメッセージ破棄イベント ("$events/message/delivery_dropped")
...
| `peername`          | 端末のIPアドレスとポート <!--全てポート含む？-->        |
...
| フィールド           | 説明                                                   |
| :------------------ | :----------------------------------------------------- |
| `reason`            | 切断理由：<br/><br/>`normal`：クライアントが意図的に切断<br/><br/>`kicked`：REST API経由でEMQXが強制切断<br/><br/>`keepalive_timeout`：指定されたキープアライブ期間が経過<br/><br/>`not_authorized`：認可失敗<br/><br/>`tcp_closed`：ピアがネットワーク接続を閉じた<br/><br/>`discarded`：`clean_start`が`true`の別クライアントが同じClientIDで接続し、前の接続が切断<br/><br/>`takenover`：`clean_start`が`false`の別クライアントが同じClientIDで接続し、前の接続を引き継ぎ<br/><br/>`internal_error`：不正なメッセージ形式やその他不明なエラーによる切断 |
| `clientid`          | クライアントID                                        |
| `username`          | クライアントのユーザー名                              |
| `peername`          | IPアドレスとポート番号                                |
| `sockname`          | EMQXがリッスンしているIPアドレスとポート              |
| `connected_at`      | クライアントの接続開始時刻（単位：ミリ秒）。このタイムスタンプは現在のセッションが確立した時刻を示し、どの接続セッションの切断イベントかを判別するために使用されます。遅延した切断イベントが新しい接続状態を上書きしないようにします。 |
...
### アン・サブスクライブイベント ("$events/session/unsubscribed")

このイベントトピックは、端末のサブスクリプション解除が成功した際にルールをトリガーできます。
...
ルールは、データブリッジによってトリガーされたメッセージやイベントを、`$bridges/`で始まるトピックを使って扱います。形式は以下の通りです。

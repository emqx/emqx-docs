...
GreptimeDB によって動作する EMQX Tables は、EMQX ブローカーとシームレスに統合され、InfluxDB Line Protocol をサポートしているため、テレメトリーデータの効率的な保存、クエリ、および可視化が可能です。詳細は [EMQX Tables 概要](https://docs.emqx.com/en/cloud/latest/emqx_tables/emqx_tables_overview.html) をご覧ください。

EMQX Enterprise 6.1 以降では、EMQX Tables コネクターと Sink が提供されており、オンプレミスの EMQX Enterprise デプロイメントから EMQX Cloud 上の EMQX Tables へ MQTT データを安全に書き込み、集中管理されたクエリや処理を行うことができます。

![enterprise_tables_integration](./assets/enterprise_tables_integration.png)

本ページでは、EMQX Enterprise から EMQX Cloud の EMQX Tables へ MQTT データを取り込む手順を以下の流れで解説します。
...
データを書き込む前に、EMQX Enterprise デプロイメント内で EMQX Tables へのコネクターを作成します。
...
3. **コネクター作成** ページで以下の設定を行います。
   ...
   - **説明**（任意）：識別用の簡単な説明を入力します。
   
   - **サーバーホスト**：`<host>:<port>` 形式で EMQX Tables サービスのアドレスを入力します。例：`tables.example.emqx.com:4001`
   
   - **データベース**：EMQX Tables 内の対象データベース名を指定します。例：`public`
   
     ::: tip
     
     EMQX Tables デプロイメント作成時にデフォルトで `public` データベースが作成されます。カスタムデータベースを作成したい場合は、[カスタムデータベースの作成](https://docs.emqx.com/en/cloud/latest/emqx_tables/emqx_tables_quick_start.html#create-a-custom-database) を参照してください。
   ...
   - **TLS を有効にする**：EMQX Tables への接続時に TLS 暗号化を使用する場合は有効にします。運用環境では TLS の利用を推奨します。
...
3. **SQL エディター**でルールのロジックを定義します。例として、クライアントが `temp_hum/emqx` トピックに温度と湿度のデータをパブリッシュしたときにトリガーされるルールは以下の通りです。
   ...
   EMQX ルールが初めての場合は、**Try It Out** をクリックして SQL ルールをインタラクティブに学習・テストできます。
...
3. 以下の項目を設定します。
   ...
   - **説明**（任意）：このアクションの説明を入力します。

   - **書き込み構文**：EMQX Tables にデータを書き込むための InfluxDB Line Protocol 形式を定義します。

     書き込み構文内のプレースホルダー（例：`${location}`, `${temp}`）は、SQL ルールで選択したフィールドに対応している必要があります。ルールがトリガーされると、EMQX はこれらのプレースホルダーを SQL クエリの結果の値に置き換えます。

     行プロトコルの先頭にあるメジャーメントがテーブル名を決定します。データが初めて正常に書き込まれるとテーブルが自動作成されます。
     ...
     - `temp_hum` がメジャーメントでテーブル名として使用されます。
     ...
     > - 値が負の可能性があるか符号付き整数として保存する必要がある場合は `i` を使用し、非負値で符号なし整数として保存したい場合（カウンター、ID、単調増加メトリクスなど）は `u` を使用してください。

   - **時間精度**：タイムスタンプの時間精度を選択します。デフォルトは `millisecond` です。

   - **フォールバックアクション**（任意）：このアクションが失敗した場合に実行されるフォールバックアクションを設定できます。デフォルトでは設定されていません。詳細は [フォールバックアクション](./data-bridges.md#fallback-actions) を参照してください。
...
温度と湿度のデータ送信をシミュレートするために、[MQTTX](https://mqttx.app/) や他のクライアントツールの使用を推奨します。簡単なデモとして、ダッシュボード内の組み込み診断ツールを利用することも可能です。

### Websocket クライアントでテストデータをパブリッシュ
...
ルールおよび関連する EMQX Tables アクションの成功・失敗回数などの実行統計を確認できます。

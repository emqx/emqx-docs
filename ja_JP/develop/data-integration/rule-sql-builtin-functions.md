...
ルールエンジンはさまざまな組み込み関数を提供しています。これらの関数はSQL内で利用でき、以下のような基本的なデータ処理を実現できます。
...
例えば、`acos(X: integer | float) -> float` は引数 `X` のデータ型が整数または浮動小数点数であり、戻り値のデータ型が浮動小数点数であることを示します。

引数が指定範囲を超える場合やサポートされていないデータ型を使用した場合、現在のSQL実行は失敗し、失敗回数が1増加しますのでご注意ください。

:::tip

1. 一部のエスケープシーケンスは使用時にアンエスケープする必要があります。詳細は[unescape関数](#unescapestring-string---string)を参照してください。  
2. EMQX 5.0以降、複雑なデータ変換に[jq構文](https://stedolan.github.io/jq/manual/)の使用もサポートしています。詳しくは[jq関数](./rule-sql-jq.md)をご覧ください。
...
`X` の双曲線アークコサインをラジアンで返します。`X` は1以上である必要があります。例：
...
`X` の常用対数（底10の対数）を返します。`X` は0より大きい必要があります。例：
...
`is_null_var` の逆で、変数 `Term` が定義されていてかつ `null` でないかを判定します。
...
`Term` が文字列の場合、指数表記も使用可能です（例：`float('3.14e4')`）。浮動小数点数は最大16桁の有効数字をサポートします。文字列 `Term` が表す浮動小数点数の有効数字が16桁を超える場合、変換時に丸め誤差が発生する可能性があります。
...
浮動小数点数 `Float` を文字列に変換します。小数点以下最大 `Decimals` 桁を含み、末尾のゼロは切り捨てられます。`Decimals` の範囲は `[0, 253]` です。`Float` の有効数字が16桁を超える場合、変換時に丸め誤差が発生する可能性があります。
...
- `Term` が文字列の場合、少なくとも1つの数字を含み、`+` または `-` の単一の接頭辞を許容し、先頭のゼロは無視されます。数学的表記もサポートします。
...
- `Term` が浮動小数点数の場合、末尾のゼロを切り捨てた対応する文字列を返します。返される文字列は小数点以下最大10桁を保持します。より多くの小数点以下桁数を返すには `float2str` 関数を使用してください。
...
このエンコードはMicrosoft SQL Serverなど、リトルエンディアンUTF-16を使用するシステムで一般的に使用されます。
...
文字列の大文字小文字変換、空白除去、部分文字列抽出、置換、エスケープ／アンエスケープなどに利用できます。
...
`String` 内で部分文字列 `SearchPattern` を検索し、`SearchPattern` より前の部分を削除して残りの文字列を返します。`SearchPattern` が見つからない場合は空文字列を返します。`find(String, SearchPattern, 'leading')` と同等です。
...
`find/2` と同じですが、`Direction` で検索方向を指定できます。
...
`pad/2` と同様ですが、`Direction` で埋める方向を指定できます。  
`leading` は先頭にスペース埋め、`trailing` は末尾にスペース埋め、`both` は両端にスペース埋めです。

`both` 指定で埋めるスペース数が奇数の場合、最後の1つは末尾に埋められます。
...
`pad/3` と同様ですが、指定したグラフェムクラスタ `Char` で埋められます。

ルールエンジンは `Char` が合法なグラフェムクラスタかどうかをチェックしないため、`Char` の文字数に関わらず1文字として扱われます。例：
...
正規表現 `Expression` にマッチする部分を文字列 `Replacement` で置換します。マッチしない場合は元の文字列を返します。例：
...
正規表現のキャプチャグループを含むパターンを文字列内で非グローバル検索し、完全一致部分を除くキャプチャグループのリストを返します。
...
連続する複数の区切り文字は1つとして扱われず、空文字列が含まれる場合があります。`split/2` は出力結果をトリムし空文字列を除外します。空文字列を残したい場合は `split(String, Separator, 'notrim')` を使用してください。
...
- `notrim`: 文字列内のすべての区切り文字を処理し、空文字列も返す可能性あり
- `leading`: 先頭の区切り文字のみ処理し、空文字列は返さない
- `leading_notrim`: 先頭の区切り文字のみ処理し、空文字列を返す可能性あり
- `trailing`: 末尾の区切り文字のみ処理し、空文字列は返さない
- `trailing_notrim`: 末尾の区切り文字のみ処理し、空文字列を返す可能性あり
...
`Format` に従ってフォーマットされた文字列を返します。`Format` 文字列は通常文字と制御シーケンスで構成されます。
...
`String` を `SeparatorList` に含まれる文字で分割し、空文字列を含まないリストを返します。連続する区切り文字は1つとして扱われます。
...
`String` の先頭と末尾から空白文字（スペース、タブ、改ページ、改行など）を削除します。Unicode標準では `\r\n` は1つのグラフェムクラスタとして扱われるため、まとめて削除されます。例：
...
[EMQX Mapping Tablesプラグイン](../../guides/extensions/plugin-catalog/6.1/emqx-maptabs.md)で管理されるマッピングテーブルから行を検索します。テーブルやキーが存在しない、またはキーの型が一致しない場合は `undefined` を返します。
...
`Map` に `Key` と対応する `Value` を挿入し、更新されたマップを返します。元の `Map` に `Key` が存在する場合は値を上書きします。例：
...
`Map` の指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。配列で複数キーを指定するとネストしたマップから関連値を取得できます。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新されたマップを返します。元の `Map` に `Key` が存在する場合は値を上書きします。配列で複数キーを指定するとネストしたマップに挿入できます。例：
...
`Array` のN番目の要素を返します。`N` は配列長以下でなければなりません。例：
...
配列 `Array` の1番目から最大 `Length` 要素の部分配列を返します。`Length` が配列長を超える場合は全配列を返します。例：
...
`sublist/2` と同様ですが、`Start` で開始位置を指定できます。`Start` + `Length` が配列長を超える場合は全配列を返します。例：
...
`binary` 型はバイト列を表し、1バイトは8ビットなのでビット数は8の倍数でなければなりません。  
`bitstring` 型は任意長のビット列を表します。
...
バイト列 `Bin` の `Start` 位置（1始まり）から長さ `BitNum` のビットを取得し、ビッグエンディアンの符号なし整数に変換します。`subbits(Bytes, Start, BitNum, 'integer', 'unsigned', 'big')` と同等です。
...
バイト列 `Bin` の `Start` 位置（1始まり）から長さ `BitNum` のビットを取得し、`OutputType` で指定した型に、`Signedness`（符号属性）、`Endianness`（バイトオーダー）に従って変換します。
...
オプションを指定してbase64デコードします。

**オプション：**

- `no_padding`：パディング文字（`=`）なしでデコード。パディング省略のBase64文字列に有用。
- `urlsafe`：URLセーフBase64（`+` と `/` の代わりに `-` と `_`）でデコード。

複数オプションの組み合わせも可能で、順序は影響しません。
...
オプションを指定してbase64エンコードします。

**オプション：**

- `no_padding`：パディング文字（`=`）なしでエンコード。
- `urlsafe`：URLセーフBase64（`+` と `/` の代わりに `-` と `_`）でエンコード。
...
EMQXは `schema_encode` と `schema_decode` 関数で、指定したスキーマに従い [Protobuf (Protocol Buffers)](https://developers.google.com/protocol-buffers) や [Avro](https://avro.apache.org/) データのエンコード・デコードをサポートしています。詳細は[スキーマレジストリ](./schema-registry.md)を参照してください。
...
現在時刻をMongoDBのISODate型または文字列で返します。MongoDB関連アクションとSQLテストでのみサポートされます。SQLテストでは文字列（例：`ISODate("2024-02-23T15:00:00.123Z")`）を返します。その他の関数への入力としては現在サポートされていません。
...
指定したUnixタイムスタンプをMongoDB ISODate型または文字列に変換します。`Unit` でタイムスタンプの単位を指定できます。その他は `mongo_date/0` と同様です。

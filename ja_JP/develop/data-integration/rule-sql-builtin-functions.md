...
引数が指定範囲を超える場合やサポートされていないデータ型を使用した場合、現在のSQL実行は失敗し、失敗回数が1増加しますのでご注意ください。

:::tip

1. 一部のエスケープシーケンスは使用時にアンエスケープが必要です。詳細は[unescape関数](#unescapestring-string---string)を参照してください。
2. EMQX 5.0以降、複雑なデータ変換のために[jq構文](https://stedolan.github.io/jq/manual/)もサポートしています。詳細は[jq関数](./rule-sql-jq.md)をご覧ください。
...
:::tip
浮動小数点数の絶対値には、`ceil` または `floor` 関数を使用してください。
:::
...
指定したフィールドのデータ型を判定し、指定した型に合致するかどうかを真偽値で返します。
...
変数 `Term` が未定義かどうかを判定します。この関数は変数に値が割り当てられているかを判定するもので、値がJSONの `null` であっても未定義とはみなしません。
...
コンピューター上で浮動小数点数は正確に保存できないため、`Decimals` が `Float` の小数点以下の桁数（先行ゼロを含む）より大きい場合、`float2str` は `Float` の2進近似の10進表現を返すことがあります。
...
- `Term` がブール型の場合、`true` は1に、`false` は0に変換されます。
- `Term` が浮動小数点型の場合、`Term` 以下の最大の整数に切り下げられます。
- `Term` が文字列の場合、少なくとも1つの数字を含み、先頭に `+` または `-` の1文字の接頭辞が付くことができ、先行ゼロは無視されます。数学的表記もサポートされます。
- `Term` が整数型の場合、そのまま返されます。
...
- `Term` がマップまたは配列の場合、`str` 関数は `Term` をJSONでエンコードしようとします。
- `Term` が浮動小数点数の場合、末尾のゼロを切り捨てた対応する文字列を返します。返される文字列は小数点以下最大10桁まで保持します。より多くの小数桁を返したい場合は `float2str` 関数を使用してください。
...
UTF-16リトルエンディアンエンコード文字列はJSONオブジェクト内で正しく表示されない場合があります。EMQXでは通常バイナリデータとして扱われます。16進数文字列として読みやすく変換するには `bin2hexstr` 関数を使用してください。
...
文字 `Char` のASCIIコードを返します。`Char` が複数文字の場合、最初の文字のコードのみ返します。例：
...
`pad/2` と同様ですが、`Direction` でパディング方向を指定できます。`leading` は先頭に、`trailing` は末尾に、`both` は両端にスペースを埋めます。
...
ルールエンジンは `Char` が合法なグラフェムクラスタかをチェックしないため、`Char` の文字数に関わらず1文字長として処理されます。例：
...
正規表現パターン（キャプチャグループ付き）を文字列に非グローバル検索し、キャプチャされた部分文字列のリストを返します。完全一致部分は含みません。
...
連続する複数の区切り文字は1つとして扱われず、空文字列が含まれることがあります。`split/2` はデフォルトで結果をトリムし、空文字列を除外します。空文字列を残したい場合は `split(String, Separator, 'notrim')` を使用してください。
...
`String` の `Start` 位置から最大長 `Length` の部分文字列を返します。添字は0始まりです。例：
...
`String` を `SeparatorList` のいずれかの文字で分割し、部分文字列のリストを返します。
...
認識できないエスケープシーケンスや無効なUnicode文字の場合、例外がスローされます。
...
マップをRedisの `HSET`（または `HMSET`）コマンドのフィールド名と値のリストに変換します。

例：`SELECT map_to_redis_hset_args(payload.value) as hset_fields FROM t/1` のように使用し、Redisアクションのテンプレートで `HMSET name1 ${hset_fields}` と統合できます。

例えば `payload.value` が `{"a" : 1, "b": 2}` の場合、生成されるコマンドは `HMSET name1 b 2 a 1` となります。マップ内のフィールド順序は非決定的です。
...
`Map` 内の指定した `Key` の値を返します。存在しない場合は `undefined` を返します。配列で複数キーを指定すると、ネストしたマップから対応する値を取得できます。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新されたマップを返します。既存のキーは上書きされます。配列で複数キーを指定すると、ネストしたマップにデータを挿入できます。例：
...
任意長の文字列 `String` の128ビットMD5ハッシュ値を計算し、32桁の16進小文字文字列で返します。例：
...
任意長の文字列 `String` の160ビットSHA-1ハッシュ値を計算し、40桁の16進小文字文字列で返します。例：
...
任意長の文字列 `String` の256ビットSHA-2ハッシュ値を計算し、64桁の16進小文字文字列で返します。例：
...
注：バイナリデータは直接JSONエンコードできないため、16進文字列に変換するには `bin2hexstr` 関数を使用してください。
...
DEFLATEアルゴリズムで `Data` を圧縮し、zlibヘッダーとチェックサムを含む圧縮結果を返します。例：
...
`Num1` と `Num2` のビットAND演算結果を返します。入力と出力は符号付き整数です。例：
...
`Num` のビット否定（NOT）演算結果を返します。入力と出力は符号付き整数です。例：
...
ルールエンジンはビット列操作用の関数を提供します。例えば `subbits` はビット列から指定長のビットを抽出し、指定データ型に変換します。

:::tip

`binary` 型はバイト列を表し、各バイトは8ビットで構成されるため、ビット数は8の倍数でなければなりません。`bitstring` 型は任意長のビット列を表します。

簡単に言うと、すべての `binary` は `bitstring` ですが、逆は必ずしも真ではありません。

`bitstring` は長さが8の倍数でない場合、JSONなど外部形式に直接シリアライズできません。通常は整数などに変換する前の中間値として使います。
...
Microsoft SQL Serverの `CONVERT` 関数と組み合わせて、UTF-8非対応のSQL ServerバージョンにUTF-16リトルエンディアンエンコードされたUnicode文字列を書き込む際に使用できます。
...
EMQXは `schema_encode` と `schema_decode` 関数を使い、指定したスキーマに基づいて [Protobuf (Protocol Buffers)](https://developers.google.com/protocol-buffers) や [Avro](https://avro.apache.org/) データのエンコード・デコードをサポートしています。詳細は[スキーマレジストリ](./schema-registry.md)をご覧ください。
...
EMQXはSparkplug Bメッセージのデコード・エンコード用に特別な関数 `spb_decode` と `spb_encode` を備えています。詳細は[Sparkplug B](./sparkplug.md)をご覧ください。
...
`DateTimeString` にタイムゾーンオフセットが含まれない場合、`Offset` で手動指定できます。その他の動作は `date_to_unix_ts/3` と同様です。`Offset` は文字列または秒数を表す整数で指定可能です。
...
`date_to_unix_ts/3, 4` の `Unit`, `Offset`, `FormatString` と同様の値を取ります。
...
現在時刻をMongoDBのISODate型または文字列で返します。MongoDB関連アクションとSQLテストでのみサポートされ、SQLテストでは文字列（例：`ISODate("2024-02-23T15:00:00.123Z")`）を返します。文字列以外の戻り値は他の関数の入力としては現時点でサポートされていません。
...
指定したミリ秒単位のUnixタイムスタンプをMongoDB ISODate型または文字列に変換します。その他は `mongo_date/0` と同様です。
...
指定したUnixタイムスタンプをMongoDB ISODate型または文字列に変換します。`Unit` で入力タイムスタンプの単位を指定できます。その他は `mongo_date/0` と同様です。
...
- OS環境変数から読み取る際、接頭辞 `EMQXVAR_` が自動的に付加されます。例えば、`getenv('FOO_BAR')` は `EMQXVAR_FOO_BAR` を読み取ります。
...
データフィールドがnullかどうかをチェックし、デフォルト値で置き換えたい場合に便利です。

例えば、`coalesce(payload.value, 0)` は `payload.value` がnullでなければその値を、nullなら0を返します。

SQLの `CASE WHEN is_null(payload.value) THEN 0 ELSE payload.value END` と同等ですが簡潔です。

...
1. 一部のエスケープシーケンスは使用時にアンエスケープが必要です。詳細は[unescape関数](#unescapestring-string---string)をご覧ください。  
2. EMQX 5.0以降、複雑なデータ変換に[jq構文](https://stedolan.github.io/jq/manual/)もサポートしています。詳細は[jq関数](./rule-sql-jq.md)をご参照ください。
...
:::tip
浮動小数点数の絶対値を求める場合は、`ceil` または `floor` 関数を使用してください。
:::
...
変数 `Term` が未定義かどうかを判定します。この関数は変数に値が割り当てられているかを判定しますが、値がJSONの `null` である場合は未定義とはみなしません。
...
`Term` を小数点以下最大 `Decimals` 桁の浮動小数点数に変換します。`Decimals` の範囲は `(0, 253]` です。その他の動作は `float/1` と同じです。例：
...
浮動小数点数はコンピュータ上で正確に格納できないため、`Decimals` が `Float` の小数点以下の桁数（先頭のゼロも含む）より大きい場合、`float2str` は `Float` の2進近似の10進表現を返すことがあります。
...
- `Term` が文字列の場合、少なくとも1つの数値文字を含み、先頭に `+` または `-` のいずれか1文字の接頭辞を持つことができ、先頭のゼロは無視されます。数学的表記もサポートされます。
...
UTF-16リトルエンディアンエンコードされた文字列はJSONオブジェクト内で正しく表示されない場合があります。EMQXでは通常バイナリデータとして扱われます。これを16進数の読みやすい文字列に変換するには `bin2hexstr` 関数を使用してください。  
...
文字 `Char` のASCIIコードを返します。`Char` に複数文字が含まれる場合は先頭の1文字のコードのみ返します。例：
...
`String` 内で部分文字列 `SearchPattern` を検索し、`SearchPattern` より前の部分を削除して残りの文字列を返します。`SearchPattern` が見つからない場合は空文字列を返します。`find(String, SearchPattern)` は `find(String, SearchPattern, 'leading')` と同等です。
...
配列 `Array` の要素を区切り文字 `Sep` で連結し、1つの文字列にします。例：
...
`pad/2` と同様ですが、`Direction` で埋める方向を指定できます。`leading` は先頭、`trailing` は末尾、`both` は両端にスペースを埋めます。
...
ルールエンジンは `Char` が合法なグラフェムクラスタかをチェックしないため、`Char` の文字数に関わらず1文字分として処理されます。例：
...
正規表現パターン（キャプチャグループ付き）を文字列内で非グローバルに検索し、マッチしたキャプチャグループのリストを返します。完全マッチ部分は含みません。
...
連続する区切り文字は1つとして扱われず、空文字列を含む場合があります。`split/2` はデフォルトで結果をトリムし、空文字列を除外します。空文字列を残したい場合は `split(String, Separator, 'notrim')` を使用してください。

`Separator` は複数文字でも可能ですが、1つのまとまりとして扱われます。複数の区切り文字を指定したい場合は `tokens` 関数を使用してください。
...
`Format` に従い文字列をフォーマットして返します。`Format` 文字列は通常文字とフォーマット用の制御シーケンスを含みます。

制御シーケンスの形式は一般に `~F.P.PadModC` です。

`C` は制御シーケンスのタイプを決定し、必須です。`F`、`P`、`Pad`、`Mod` は任意です。詳細は https://www.erlang.org/doc/apps/stdlib/io.html#fwrite-1 をご参照ください。
...
`tokens/2` と同様ですが、`NoCRLF` に `nocrlf` を指定すると改行コード（CRおよびLF）も分割対象に含めます。例：
...
- 16進エスケープコード：
  ...
  - `\xH...`：`H...` は1文字以上の16進数（0-9, A-F, a-f）で、任意のutf32文字をエンコード可能。

認識されないエスケープシーケンスや不正なUnicode文字の場合、例外が発生します。
...
`Map` 内の指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。例：
...
[EMQX Mapping Tablesプラグイン](../../guides/extensions/plugin-catalog/6.1/emqx-maptabs.md)で管理されるマッピングテーブルから行を検索します。テーブルが存在しない、キーが存在しない、またはキーの型が一致しない場合は `undefined` を返します。

キーの一致は型変換なしの厳密な等価性で判定されます。例えば整数キー `50` と文字列キー `'50'` は異なるキーです。
...
`maptab_lookup/2` と同様ですが、検索に失敗した場合は指定した `DefaultRow` を返します。`DefaultRow` は `map_new()`, `map_put(...)`, `json_decode('{...}')` で作成します。

返された行を他の関数に渡す場合は、その関数が必要とするすべてのフィールドを含むようにしてください。空のデフォルト行（例：`map_new()`）はフィールドがすべて `undefined` のままです。
...
マッチした行の指定フィールドを検索します。テーブル、キー、フィールドが存在しない、またはキーの型が一致しない場合は `undefined` を返します。
...
`maptab_lookup(Table, Key, Field)` と同様ですが、検索に失敗した場合は指定した `Default` を返します。
...
`FOREACH` 内で `maptab_lookup` を使用する場合、欠損する可能性がある行のフィールドアクセスはガードしてください。例えば、`maptab_lookup('signals', item_id)` が `undefined` を返す場合、`sig.start_bit` を `subbits` に渡すとそのメッセージのSQL実行全体が失敗します。`CASE WHEN is_map(sig)` を使うか完全なデフォルト行を用意してください。
...
例：`SELECT map_to_redis_hset_args(payload.value) as hset_fields FROM t/1` のように指定し、Redisアクションのテンプレートで `HMSET name1 ${hset_fields}` と組み合わせて使用します。

例えば、`payload.value` が `{"a" : 1, "b": 2}` のマップの場合、生成されるコマンドは `HMSET name1 b 2 a 1` のようになります。マップのフィールド順序は非決定的です。
...
`Map` 内の指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。配列で複数のキーを指定し、ネストしたマップから値を取得することも可能です。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新されたマップを返します。`Key` が既に存在する場合は値を上書きします。配列で複数のキーを指定し、ネストしたマップにデータを挿入することも可能です。例：
...
配列 `Array` の `N` 番目の要素を返します。`N` は配列の長さを超えてはいけません。例：
...
配列 `Array` の先頭から最大 `Length` 要素の部分配列を返します。`Length` が配列の長さを超える場合は全配列を返します。例：
...
`sublist/2` と同様ですが、`Start` で開始位置を指定できます。`Start` + `Length` が配列の長さを超える場合は全配列を返します。例：
...
DEFLATEアルゴリズムで `Data` を圧縮し、返される圧縮結果はgzヘッダーと末尾のチェックサムを含みます。例：
...
DEFLATEアルゴリズムで `Data` を圧縮し、返される圧縮結果はzlibヘッダーと末尾のチェックサムを含みません。例：
...
`Num` を右に `Shift` ビットシフトし、左端は符号ビット（正数は0、負数は1）で埋めます。例：
...
ルールエンジンはビット列操作用の関数を提供します。例えば `subbits` はビット列から指定した長さのビットを抽出し、指定型に変換します。
...
`bitstring` は長さが8の倍数でない場合、JSONなどの外部形式に直接シリアライズできません。通常は整数など他の型に変換する前の中間値として使います。
...
バイト列 `Bin` の先頭から長さ `BitNum` のビットを取得し、ビッグエンディアンの符号なし整数に変換します。これは `subbits(Bytes, 1, BitNum, 'integer', 'unsigned', 'big')` と同等です。
...
バイト列 `Bin` の `Start` 位置（1始まり）から長さ `BitNum` のビットを取得し、ビッグエンディアンの符号なし整数に変換します。これは `subbits(Bytes, Start, BitNum, 'integer', 'unsigned', 'big')` と同等です。
...
バイト列 `Bin` の `Start` 位置（1始まり）から長さ `BitNum` のビットを取得し、指定したバイト順 `Endianness` と符号属性 `Signedness` に従い、指定型 `OutputType` に変換します。
...
- `no_padding`：パディング文字（`=`）なしでデコード。パディングが省略されたBase64文字列に有効。
- `urlsafe`：URLセーフBase64形式（`+` と `/` の代わりに `-` と `_`）でデコード。
...
- `urlsafe`：URLセーフBase64形式でエンコード（`+` と `/` の代わりに `-` と `_` を使用）。
...
任意のバイナリデータをMicrosoft SQL Serverのバイナリ型（`0x` プレフィックス付きのHEX文字列）に変換します。

::: tip

この関数はMicrosoft SQL Serverの `CONVERT` 関数と組み合わせて、UTF-8非対応のSQL ServerバージョンにUTF-16リトルエンディアンエンコードされたUnicode文字列を書き込む際に使用できます。
...
EMQXは指定したスキーマに従い、[Protobuf (Protocol Buffers)](https://developers.google.com/protocol-buffers) および [Avro](https://avro.apache.org/) データのエンコード・デコードを行う `schema_encode` と `schema_decode` 関数もサポートしています。詳細は[スキーマレジストリ](./schema-registry.md)をご覧ください。
...
EMQXはSparkplug Bメッセージのデコード・エンコード用の特殊関数（`spb_decode` と `spb_encode`）も備えています。詳細は[Sparkplug B](./sparkplug.md)をご覧ください。
...
`DateTimeString` にタイムゾーンオフセットが含まれない場合、`Offset` で手動指定できます。その他の動作は `date_to_unix_ts/3` と同じです。`Offset` は文字列または秒数の整数で指定可能です。
...
`date_to_unix_ts/3, 4` の `Unit`、`Offset`、`FormatString` と同じ値が使用できます。
...
`rfc3339_to_unix_ts/1` と同様ですが、`Unit` で返却するUnixタイムスタンプの単位を指定できます。`second`, `millisecond`, `microsecond`, `nanosecond` をサポートします。例：
...
現在時刻をMongoDBのISODate型または文字列で返します。MongoDB関連のアクションやSQLテストでのみサポートされ、SQLテストでは文字列（例：`ISODate("2024-02-23T15:00:00.123Z")`）を返します。その他の関数の入力としては文字列以外の戻り値は現在サポートされていません。
...
指定したミリ秒単位UnixタイムスタンプをMongoDB ISODate型または文字列に変換します。その他の動作は `mongo_date/0` と同じです。
...
指定したUnixタイムスタンプをMongoDB ISODate型または文字列に変換します。`Unit` で入力タイムスタンプの単位を指定できます。その他の動作は `mongo_date/0` と同じです。
...
SQL式の `CASE WHEN is_null(payload.value) THEN 0 ELSE payload.value END` と同等ですが、より簡潔です。

...
jq 関数が初めての方は、[リファレンス](#references) セクションを参照して素早く学習を始めてください。

::: tip

jq 関数は、ルールの SQL 言語だけでは難しい変換処理に便利です。
...
2. `<JSON_input>`: jq プログラムへの入力となる JSON エンコード済みの文字列またはオブジェクト。
...
この例では、`jq` を使った JSON データの操作方法（アクセス、変換、値の計算など）を示しています。
...
例えば、以下の JSON オブジェクトは日付と複数のセンサー情報を持ち、それぞれのセンサーには名前とデータポイントの配列が含まれています。これは特定の日付のセンサー読み取り値を表します。

...
MQTT over QUIC はデフォルトで無効化されています。以下の手順で手動でリスナーを有効化してください。
...
この設定は、ポート `14567` で QUIC リスナーを有効にすることを示しています。変更を保存し、EMQX を再起動して設定を反映させてください。
...
- [NanoSDK](https://github.com/nanomq/NanoSDK/)：EMQ NanoMQ チームがリリースした C 言語の MQTT SDK。WebSocket や nanomsg/SP などのプロトコルもサポートしています。
...
クライアントライブラリのほかに、EMQ はエッジコンピューティング製品である NanoMQ を使った MQTT over QUIC ブリッジ機能も提供しています。NanoMQ を利用すれば、エッジデータを QUIC 経由でクラウドにブリッジできるため、MQTT over QUIC リスナーを開発や統合の負担なく活用できます。

## ネットワークフェイルオーバー

QUIC は UDP プロトコルをベースとしているため、多くの通信事業者は UDP パケットに対して特別なルーティング戦略を持っており、QUIC 接続の失敗やパケットロスが発生しやすいです。

そのため、MQTT over QUIC クライアントはフォールバック機能を備えています。API 層は統一的な操作でサービスを書き込み、トランスポート層はネットワーク状況に応じてリアルタイムに切り替えます。QUIC が利用できない場合は自動的に TCP/TLS 1.2 に切り替わり、さまざまなネットワーク環境下でサービスを確保します。

## 例 1：NanoSDK を使った MQTT over QUIC

[NanoSDK](https://github.com/nanomq/NanoSDK/) は MsQuic をベースにした、C 言語で MQTT over QUIC を実装した初の SDK であり、EMQX 5.0 とシームレスに互換性があります。完全非同期 IO 設計を採用し、QUIC ストリームと MQTT 接続のマッピングをバインドし、0 RTT の高速ハンドシェイク再接続機能を内蔵し、マルチコアのタスク並列処理をサポートしています。

NanoSDK の API は MQTT over TCP とほぼ同様に動作します。以下のコマンド一つで QUIC ベースの MQTT クライアントを作成できます。
...
NanoSDK は Java および Python のバインディングも提供しています。
...
[NanoMQ](https://nanomq.io/) は IoT エッジ向けの超軽量かつ高速なサービ スで、クロスプラットフォーム対応、マルチスレッド対応、MQTT over QUIC ブリッジをサポートしています。

従来の MQTT クライアントからのデータを QUIC パケットに変換し、クラウド上の EMQX に送信できます。これにより、統合が難しい、または適切な MQTT over QUIC SDK がないエンド側 IoT デバイスでも QUIC プロトコルを利用可能にします。
...
2. インストール後、設定ファイル `/etc/nanomq.conf` にて MQTT over QUIC ブリッジ機能および関連トピックを設定します。URL プレフィックスの `mqtt-quic` は、MQTT 送信層として QUIC を使用していることを示します。
...
NanoMQ はテストツール `nanomq_cli` も提供しており、MQTT over QUIC クライアントツールを含むため、EMQX 5.0 の MQTT over QUIC 機能をテストできます。

...
このセクションでは、実際のユースケースを通じてFlowデザイナーでLLMベースのFlowを素早く作成し、テストする方法を説明します。
...
デバイスがMQTTトピック `sensors/temp_humid` に温度と湿度の読み取り値を報告すると仮定します。各メッセージにはJSON形式の生センサーデータが含まれています。EMQX Flowは以下の処理を行います。

- **データ処理**：デバイスIDとセンサー値を抽出する。
- **LLMベース処理**：OpenAIモデルを使ってセンサーの読み取り値を要約する。
...
3. **Data Processing**ノードを追加します。
   ...
   - 以下のマッピングを追加します：
     - `payload.device_id` をエイリアス `device_id` に
     - `payload.temperature` をエイリアス `temperature` に
     - `payload.humidity` をエイリアス `humidity` に
   - **Save**をクリックします。
...
3. 結果を確認します。
   ...
   - **OpenAI**ノードの実行統計やメトリクスを確認するには、編集ページを閉じてノードをクリックし、編集パネルの**Overview**タブを開きます。

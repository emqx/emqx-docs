...
このデモでは、MQTTトピックからセンサーのデータを受信し、LLM（例：OpenAI GPT）を使ってデータを解釈し、その意味を自然言語で要約するワークフローを構築します。生成された要約は、新しいトピック `ai/summary` に再パブリッシュされ、下流で利用されます。
...
2. テストを開始します。
   ...
   - **編集**をクリックし、次に**テスト開始**をクリックして画面下部にテストパネルを開きます。
...
3. 結果を確認します。
   ...
   - **OpenAI**ノードの実行統計やメトリクスを確認するには、編集ページを閉じてノードをクリックし、編集パネルの**概要**タブをクリックしてください。

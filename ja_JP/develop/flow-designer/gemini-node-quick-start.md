...
このFlowはGeminiを活用し、振動、温度、圧力など複数のフィールドの全体的な文脈を分析し、潜在的な機械的故障を示唆する複雑な異常を検出します。例えば、振動と温度が同時に高い場合、Geminiはより深刻なリスク（例：ベアリングの過負荷）を推論し、正確で説明可能なアラートを出力します。

- **データ処理**：ペイロードからデバイスの読み取り値を抽出し、後続で利用できるよう`clientid`（例：`device_1`）を公開します。
...
2. **Messages**ノードを追加します。
   ...
   - Sourceパネルから**Messages**ノードをドラッグします。
...
3. 結果を確認します。
   ...
   - **Gemini**ノードの稼働状況やメトリクスを確認するには、編集ページを閉じ、ノードをクリックして編集パネルを開き、**Overview**タブをクリックしてください。

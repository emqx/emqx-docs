# クイックスタート：Anthropicノードを使ったフローの作成

このページでは、Claude 3 Sonnetを使用してフォールト分類を行い、受信したテレメトリに基づいて是正推奨を生成する方法を示します。これは、スマートファクトリーやスマートビルディングなどのIoTシステムがデバイスからのステータスメッセージを受信し、その問題を自動かつインテリジェントに解釈する実際のシナリオをシミュレートしています。

## シナリオの説明

多くの産業用やスマートビルディングのシナリオでは、IoTデバイスが単一のMQTTメッセージ内で複数のメトリクスを報告します。例えば、電力監視デバイスは複数の回路における電力消費を一つのペイロードで送信することがあります。
...
この例では、LLM（Claude 3 Sonnet）を使ってメッセージ内のすべての数値を合計（すなわち回路全体の総電力消費）し、その数値結果のみを下流処理や課金のために再パブリッシュすることを目標とします。
...
5. 右上の**Save**をクリックしてフローを保存します。

   ![anthropic_node_flow](./assets/anthropic_node_flow.png)

6. フローとフォームルールは相互運用可能です。ルールページでSQLや関連するルール設定も確認できます。
...
1. MQTTクライアントをEMQXに接続します。
   ...
   フローを素早くテストするには、ダッシュボードの**Diagnostic Tools** -> **WebSocket Client**を使ってMQTTクライアントをシミュレートできます。あるいは、[MQTTX](https://mqttx.app/)ツールや実際のMQTTクライアントも使用可能です：
...
2. テスト開始。
   ...
   - **Edit**をクリックし、続けて**Start Test**をクリックすると画面下部にテストパネルが開きます。
...
3. 結果とノードの処理メトリクスを確認します。
   ...
   - **WebSocket Client**ページに戻ると、AI生成のサマリーとして以下のような結果を受信できます：

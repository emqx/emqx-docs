...
このページでは、EMQXのMQTTファイル転送機能をすばやく始めるためのステップバイステップガイドを提供します。ローカルディスクを使用する場合とS3バケットを使用する場合の2つのシナリオをカバーしています。以下の手順に従って、必要な設定を行い、ファイルをアップロードし、提供されるAPIを使ってアクセスしてください。
...
4. S3コマンドラインツールを使って、以下のコマンドでアップロード済みファイルを手動で一覧表示します。
...
6. 提供されたURIを使って、S3ストレージから直接ファイルをダウンロードします。

   ::: tip

   S3エクスポーターの場合、ダウンロードリンクはEMQXではなく直接S3ストレージを指しているため、ファイルはEMQXのローカルには保存されていません。

...
このページでは、EMQXのMQTTファイル転送機能をすばやく始めるためのステップバイステップガイドを提供します。ローカルディスクを使用する場合とS3バケットを使用する場合の2つのシナリオをカバーしています。以下の手順に従い、必要な設定を行い、ファイルをアップロードし、提供されるAPIを使ってアクセスしてください。
...
6. 以下のコマンドを使って提供されたURIから直接S3ストレージ上のファイルをダウンロードします。
   ...
   S3エクスポーターのシナリオでは、ダウンロードリンクはEMQXではなく直接S3ストレージを指しているため、ファイルはEMQXにローカル保存されていません。

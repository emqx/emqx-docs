...
4. 以下のコマンドでファイル保存ディレクトリに移動し、アップロードされたファイルを手動で一覧表示します。
...
6. 以下のコマンドで提供されたAPIエンドポイントからファイルをダウンロードします。
...
ファイル転送機能は、Amazon S3やMinoなどのS3互換オブジェクトストレージシステムへのアップロードファイルのエクスポートをサポートしています。S3バケットを使用する場合、EMQXはファイル転送リストのみを保存し、ファイル自体は保存しません。

::: tip 前提条件

開始前に、`s3cmd` が正しくインストールおよび設定されていることを確認してください。詳細は[公式s3cmdリポジトリ](https://github.com/s3tools/s3cmd)をご参照ください。
...
1. 設定ファイルでファイル転送機能を有効にし、S3バケットを設定してEMQXを起動します。
   ...
   file_transfer {
      ...
      storage.local.exporter.s3 {
         ...
         # S3とのHTTP(S)接続設定。安全なファイルアップロードとコネクションプール管理を可能にします。
         transport_options {
            ssl.enable = true
            connect_timeout = 15s
         }
...
4. 以下のコマンドを使ってS3コマンドラインツールでアップロードされたファイルを手動で一覧表示します。

   ::: tip

   `s3cmd` が正しくインストールおよび設定されていることを必ず確認してください。詳細は[公式s3cmdリポジトリ](https://github.com/s3tools/s3cmd)をご参照ください。
...
6. 以下のコマンドで提供されたURIから直接S3ストレージ上のファイルをダウンロードします。

   ::: tip

   S3エクスポーターのシナリオでは、ダウンロードリンクはEMQXではなく直接S3ストレージを指しているため、ファイルはEMQX上にローカル保存されていません。

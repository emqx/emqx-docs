...
このページでは、EMQXのMQTTファイル転送機能をすばやく始めるためのステップバイステップガイドを提供します。ローカルディスクを使用する場合とS3バケットを使用する場合の2つのシナリオをカバーしています。以下の手順に従って必要な設定を行い、ファイルをアップロードし、提供されるAPIを使ってアクセスしてください。

::: warning 重要なお知らせ

ネームスペース付きダッシュボードユーザーおよびAPIキーは、役割やスコープに関わらず、ローカルに保存されたファイルの一覧取得やダウンロードのためにファイル転送エンドポイントを使用できません。グローバルダッシュボードユーザーまたはグローバルAPIキーで認証してください。S3ストレージの場合、EMQXの認証はファイル一覧取得時のみ必要で、S3は返される署名付きURLを通じてアクセス制御を行います。
...
3. `emqx-ft` コマンドラインツールを使用して、以下のコマンドでファイルをアップロードします。
   ...
   | `--segment-size` | ファイルのセグメントサイズ（バイト単位）。大きなファイルを小さな断片に分割してアップロードするために使用します。 |
...
ファイル転送機能は、Amazon S3やMinoなどのS3互換オブジェクトストレージシステムへのアップロードファイルのエクスポートをサポートしています。S3バケットを使用する場合、EMQXはファイル自体ではなくファイル転送リストのみを管理します。

::: tip 前提条件

開始する前に、`s3cmd` が正しくインストールおよび設定されていることを確認してください。詳細は[公式s3cmdリポジトリ](https://github.com/s3tools/s3cmd)を参照してください。
...
4. S3コマンドラインツールを使って以下のコマンドを実行し、アップロード済みファイルを手動で一覧表示します。

   ::: tip

   `s3cmd` が正しくインストールおよび設定されていることを確認してください。詳細は[公式s3cmdリポジトリ](https://github.com/s3tools/s3cmd)を参照してください。
...
6. 以下のコマンドを使って提供されたURIから直接S3ストレージ上のファイルをダウンロードします。
   ...
   S3エクスポーターのシナリオでは、提供されるダウンロードリンクはEMQXではなく直接S3ストレージへのリンクとなるため、ファイルはEMQX上にローカル保存されていません。

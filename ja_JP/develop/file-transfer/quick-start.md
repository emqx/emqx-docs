...
3. `emqx-ft`コマンドラインツールを使って以下のコマンドを実行し、ファイルをアップロードします。
   ...
   | `--client-id`      | ファイルアップロードを行うクライアントを識別するためのクライアントID。 |
...
5. 以下のコマンドを実行してHTTP API経由でアップロード済みファイルの一覧を取得します。
   ...
   レスポンスにはファイル名、サイズ、タイムスタンプなどの詳細情報が含まれます。
...
開始する前に、`s3cmd`がインストールされ正しく設定されていることを確認してください。詳細は[公式s3cmdリポジトリ](https://github.com/s3tools/s3cmd)をご参照ください。
...
4. 以下のコマンドを使ってS3コマンドラインツールでアップロード済みファイルを手動で一覧表示します。
...
6. 以下のコマンドを使って提供されたURIから直接S3ストレージ上のファイルをダウンロードします。
   ...
   S3エクスポーターのシナリオでは、ダウンロードリンクはEMQXではなく直接S3ストレージを指すため、ファイルはEMQX上にローカル保存されていません。

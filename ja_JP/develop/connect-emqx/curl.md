...
curlはデータ転送や自動化に広く使われるコマンドラインツールです。2020年以降、curlはMQTTプロトコルをサポートしており、curl 8.19.0（2026年3月予定）以降ではMQTTS（TLS上のMQTT）もサポートされます。

curlを使うことで、開発者は言語固有のMQTTクライアントSDKをインストールせずに、コマンドラインから直接EMQXに接続し、メッセージをパブリッシュ／サブスクライブできます。これにより、簡単なテストやスクリプト作成、IoTプロトタイピングに便利なツールとなっています。

このページでは、curlを使ったEMQXとのMQTTおよびMQTTS通信の接続、パブリッシュ／サブスクライブ、認証、TLS設定、一般的なトラブルシューティングについて説明します。
...
**Protocols**リストに`mqtt`（およびcurl ≥ 8.19.0の場合は`mqtts`）が表示されていることを確認してください。
...
接続先のMQTTブローカーが必要です。このガイドではMQTTとMQTTSの両方をサポートするEMQXを使用します。

### EMQXパブリックブローカー（テスト用）

独自のブローカーを用意せずにすぐにテストしたい場合は、EMQXのパブリックブローカーを利用できます。
...
curlコマンドを作成する際は、EMQX Enterpriseのリスナー設定、認証、TLS設定を参照してください。
...
> 自社管理のEMQX Enterpriseに加え、完全マネージドMQTTサービスである[EMQX Cloud](https://docs.emqx.com/en/cloud/latest/overview.html)（ServerlessまたはDedicated）へのcurl接続も可能です。
>
...
curlをEMQX Enterpriseで使う場合、サブスクライブやパブリッシュコマンドを実行すると、自動的に接続が確立されます。接続にはEnterpriseブローカーのアドレスや認証・TLS設定が反映されます。
...
- `[:port]`：省略可能なポート番号。指定しない場合はデフォルトが使われます。
  - `mqtt://`は`1883`
  - `mqtts://`は`8883`
...
トピックをサブスクライブすると、curlはMQTTメッセージの生データを以下の形式で出力します：
...
この出力はトピック名とペイロードが連結されたバイナリ形式であり、パースしないと読みづらい形式です。

出力はバイナリであり、デフォルトでは人間が読みやすい形ではありません。以下の[MQTTメッセージのパース](#parse-mqtt-messages)を参照して変換例をご覧ください。
...
### MQTTSによるセキュアなサブスクライブ（curl ≥ 8.19.0）
...
トピックをサブスクライブすると、curlはMQTTメッセージを**バイナリ形式**で出力します。整形されたテキスト形式ではありません。
...
curl -sN mqtt://broker.emqx.io/curl/test | \
  while IFS= read -r -d $'\0' d; do
    ...
    # このループはNUL(0x00)を区切り文字に使い、LSBを先に読み、MSB=0と仮定（トピック長0～255バイト対応）。
    lsb=$(printf "%d" "'${d:0:1}")
    topic_len=$((lsb))
...
MQTTメッセージのバイナリ構造を理解するため、サブスクライブの生出力をファイルに保存し、手動で確認できます。
...
これにより、トピック長のプレフィックスやトピックバイト、ペイロードの配置が明確に見えます。
...
> 本番利用や複雑なパースが必要な場合は、[MQTTX CLI](https://mqttx.app/cli)の利用を検討してください。整形済み出力、MQTT 5.0完全対応、QoS処理、ワイルドカードサブスクライブなどが可能です。
...
### MQTTSによるセキュアなパブリッシュ（curl ≥ 8.19.0）
...
以下は本ドキュメントで使われるcurlコマンドラインオプションのまとめです。
...
| `-v`             | 詳細出力（MQTTハンドシェイクの表示など）     | トラブルシューティング |
...
> curlのオプション一覧は公式ドキュメントを参照してください。
...
> 証明書検証をスキップする場合は`-k`を使いますが、本番環境では推奨されません。
...
curlはテストやスクリプトには便利ですが、以下の制限があります：
...
出力に`mqtt`（curl ≥ 8.19.0の場合は`mqtts`も）が含まれていれば、MQTTサポート付きのcurlビルドです。含まれていない場合は：

- curlを7.70.0以上にアップグレードする
- MQTT対応ビルドをインストールする
- `--enable-mqtt`オプション付きでcurlをソースからビルドする
...
- ブローカーアドレスまたはポートが誤っている
- ネットワークファイアウォールがMQTT/MQTTSポートをブロックしている
...
**Protocols**に`mqtt`（およびTLS対応なら`mqtts`）があるか確認。

- curlをアップグレードするか、MQTT対応ビルドをインストールする。

### TLSハンドシェイクまたは証明書エラー（MQTTS）
...
curlを使ったMQTTおよびMQTTSの詳細なステップバイステップ解説、背景説明、追加例、利用上の注意点については、ブログ記事「[Using curl for MQTT: Connect, Publish, and Subscribe with Secure IoT Communication](https://www.emqx.com/en/blog/using-curl-for-mqtt)」をご覧ください。

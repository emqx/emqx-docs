...
この機能は、直接の TCP 接続が利用できない、または制限されている環境、例えばウェブブラウザや特定の企業ネットワークで特に有用です。
...
MQTT over WebSocket を使用するには、WebSocket リスナーを有効にする必要があります。
...
MQTT over TCP/TLS と比較して、MQTT over WebSocket は以下の点があります：

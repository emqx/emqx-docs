...
`capabilities`、`securitySchemes`、`supportedInterfaces`、拡張パラメータを含む完全なエージェントカードスキーマは、[A2A仕様](https://a2a-protocol.org/latest/specification/)をご参照ください。
...
各パターンの詳細な仕様は、[A2A over MQTT トランスポート仕様](https://www.emqx.com/mqtt-for-ai/a2a-over-mqtt/specification/0.1/basic/mqtt_transport.html)をご覧ください。
...
sequenceDiagram
    ...
    M->>E: エージェントカードをパブリッシュ（リテインド）
    R->>E: エージェントカードをパブリッシュ（リテインド）

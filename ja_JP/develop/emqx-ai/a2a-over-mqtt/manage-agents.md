...
このページでは、A2Aレジストリの有効化方法と、ダッシュボードUI、CLI、またはMQTTを使用してエージェントの登録、表示、削除を行う方法について説明します。
...
| `trusted_jkus` | Array | `[]` | 空でない場合、Agent Card内の`jwksUri`はリスト内のいずれかのプレフィックスと一致する必要があります。空リストはJKU検証を無効にします（許容モード）。 |
...
2. 識別フィールドを入力します：
   ...
   - **Organization ID**：エージェントが所属する組織またはトラストドメイン（例：`com.example`）。デプロイ間での一意性を保つためにリバースDNS表記を使用してください。
   - **Unit ID**：組織内の区分（例：事業部門やデプロイ環境、例：`factory-a`）。
   - **Agent ID**：組織およびユニット内で一意のエージェント識別子（例：`iot-ops-agent-001`）。

   これら3つの値はすべて英数字、ハイフン、アンダースコア、ピリオド（`^[A-Za-z0-9._-]+$`）のみを含み、`/`、`+`、`#`、空白は含められません。これらはエージェントの完全なアドレス `{org_id}/{unit_id}/{agent_id}` を形成します。

3. Agent CardのJSONをエディタに貼り付けます。必要なフィールドやテンプレートは **Help** ボタンで確認できます。
...
- Retainフラグ有効、QoS 1。
- ペイロードは`name`、`description`、`version`、`url`、`skills`を最低限含むAgent CardのJSON。
...
2. 接続情報を入力します：
   ...
   - **Host**：EMQXブローカーのアドレス
   - **Port**：`1883`（または適切なポート）
   - **Client ID**：`com.example/factory-a/iot-ops-agent-001` に設定
   - **MQTT Version**：`5.0`
...
**Validate Schema** が有効な場合、EMQXはペイロードを登録前に検証します。無効なカードはPUBACKの理由コードで拒否されます。
...
登録済みエージェントはダッシュボードで閲覧・検査でき、CLIでクエリも可能です。
...
エージェントを削除するとA2Aレジストリから登録解除され、保持されたAgent Cardがクリアされて検出不可になります。

### ダッシュボードから

エージェント一覧で削除したいエージェントの削除アクションをクリックします。確認画面で完全な `{org_id}/{unit_id}/{agent_id}` を入力して確定します。

### MQTTから

エージェントのディスカバリートピックに空の保持メッセージをパブリッシュします。これにより保持されたカードがクリアされ、レジストリから削除されます。
...
クライアントエージェントはワイルドカードを使ってディスカバリートピックをサブスクライブし、利用可能なエージェントを検出します。カードは保持されているため、サブスクライブ時に即座に配信されます。

**MQTTX Desktopを使用する場合：**

1. EMQXブローカーに接続します。
2. **+ New Subscription** をクリックし、ワイルドカードトピックを入力します。例：`$a2a/v1/discovery/com.example/+/+`（組織内のすべてのエージェントを検出）。
3. **Confirm** をクリックします。保持されたAgent Cardがメッセージペインに即時表示されます。
...
| `a2a-status-source` | `broker` | EMQXが接続状態に基づき設定したステータス。 |
| `a2a-status-source` | `agent` | エージェント自身が積極的にパブリッシュしたステータス（例：正常なオフライン）。 |
| `a2a-status-source` | `lwt` | Last Will and Testamentによる異常切断を反映したステータス。 |

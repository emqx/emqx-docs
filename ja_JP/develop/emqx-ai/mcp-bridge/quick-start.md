...
3. プラグインを設定します：
   ...
   ブラウザで http://localhost:18083/#/plugins/ にアクセスし、MCP ブリッジプラグインをクリックして設定ページを開きます。ここでリッスンアドレスや証明書などの設定を変更できます。**保存**をクリックすると設定が自動的に適用され、プラグインの手動再起動は不要です。

   リッスンアドレスを `https://your-hostname:9909/mcp` に設定した場合、MCP プラグインは指定ポート上に以下の 2 つの HTTP エンドポイントを起動します：
   ...
   また、一部のモデルや AI エージェントでは MCP サーバーへの HTTPS アクセスが必要になる場合があります。その場合は、MCP ブリッジプラグインに有効かつ信頼された SSL 証明書を設定し、URL が公開アクセス可能であることを確認してください。

   **Target MQTT Client ID acquisition method** を **Tool Parameter** に設定してください。これにより、MCP クライアントは接続確立時に HTTP ヘッダーで固定の Client ID を指定する代わりに、ツール呼び出し時にデバイスの MQTT クライアント ID をパラメーターとして渡せるようになります。

   ![MCP ブリッジプラグイン設定](./assets/mcp-bridge-config.png)

## MCP over MQTT SDK を使用したデバイスのシミュレーション
...
上記の Python コードは MCP over MQTT プロトコルを使用して MCP サーバーを起動し、スマートライトデバイスをシミュレートします。ライトのオン・オフや明るさ調整の MCP ツールを公開しています。サーバー名は `devices/light` と指定しています。

次に、2 つの端末で以下のコマンドを実行し、それぞれデバイス ID が `abc123` と `abc456` の MCP サーバーを起動します：
...
3. **MCP** ページで以下の設定で MCP サーバーを追加します：
   ...
   Cherry Studio は HTTP と SSE の両方のプロトコルをサポートしています。ローカルテストの場合は `http://localhost:9909/mcp` を利用できます。

   ![MCP サーバーの追加](./assets/cherry-studio-mcp-config-mcp-bridge.png)

4. 「Device Assistant」という名前の新しいアシスタントを作成し、その中に「MQTT Device Control」という会話トピックを作成します。アシスタントおよび会話トピックのシステムプロンプトを以下のように設定します：
...
5. 最後に、ツール呼び出しに対応したモデル（例：`qwen-flash`）を選択し、チャットボックスに自然言語でデバイス制御コマンドを入力してテストします：

   ```
   リビングルームのライトをオンにして。
   ベッドルームのライトの明るさを 75% に設定して。
   ```

   システムプロンプトに基づき、デバイスアシスタントが正しいデバイス ID を識別し、対応する MCP ツールを呼び出してデバイスを制御する様子が確認できます。

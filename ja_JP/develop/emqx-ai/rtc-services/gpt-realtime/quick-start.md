...
このコードではWebRTCの音声チャネルを作成するだけでなく、GPT-Realtimeモデルとのイベント送受信用にデータチャネルも作成しています。受信したすべてのイベントはコンソールにログ出力されます。テスト時に音声が出ないなどの問題があれば、コンソールのエラーメッセージを確認してください。

## MCPを使ったデバイス制御

1. EMQXを起動し、MCPブリッジプラグインをインストール・設定します。

2. スマートライトをシミュレートするMCPサーバーを起動します。詳細な手順は[EMQX MCPブリッジでIoTデバイスにアクセスする](../../mcp-bridge/quick-start.md)を参照してください。

   なお、EMQXはパブリックネットワーク環境にデプロイし、MCPブリッジプラグインには有効なSSL証明書を設定しておく必要があります。これによりGPT-RealtimeはHTTPS経由でMCPサービスにアクセス可能となります。

3. フロントエンドコードをMCPツール対応に修正します。

   MCPツールを有効にするには、GPT-Realtimeのイベントを処理する関数 `handle_event()` を追加します。
   ...
   この関数内で `session.created` イベントを処理し、セッション作成時にMCPツールを有効化するための `session.update` イベントを送信します。MCPサーバーのアドレスは `https://your-emqx-host:port/mcp` に設定してください。
       ...
       if (event.type === "session.created") {
           ...
           const session_update_event = {
               ...
               session: {
                   ...
                   tool_choice: "auto",
                   // セッションフィールドを直接設定可能。プロンプトフィールドより優先されます:
...
- MCPプラグインに有効で自己署名でないSSL証明書が設定されていること
...
前述のコードでは、システム指示を使ってデバイスのクライアントIDを事前にモデルに伝えていました。

...
- フレーム単位の Voice Activity Detection（VAD）による正確な発話開始・終了検出
...
| 機能                     | 説明                                                         |
| ------------------------ | ------------------------------------------------------------ |
| インテリジェント割り込み | フルデュプレックス通信により、ユーザーはいつでも AI を割り込み可能で、より自然な対話を実現 |
| ビジュアル理解           | 画像や動画入力をサポートし、AI が視覚コンテンツを「見る」ことと理解を可能にする |
| 関数呼び出し             | LLM がユーザーの意図を特定し、外部関数を呼び出すことを可能にする |
...
Volcano Engine Speech Services は使用量に基づいて課金されます。各課金項目には無料トライアル枠が含まれます。詳細は

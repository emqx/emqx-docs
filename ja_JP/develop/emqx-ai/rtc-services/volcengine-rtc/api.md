...
| パラメータ                 | 型       | 説明                                          |
| ------------------------- | -------- | --------------------------------------------- |
| `InterruptSpeechDuration` | number   | 割り込み発話継続時間（ms）、デフォルト `400`  |
...
- `bidirection`: 双方向ストリーミング、`speech_rate`を使用し、`Additions`設定に対応
:::
...
詳細音声リスト: [Volcano Engine TTS音声一覧](https://www.volcengine.com/docs/6561)
...
- **Signature**: AppKeyを使ったHMAC-SHA256署名
...
トークン生成ライブラリについては[インストールとテスト - RTCトークン生成](./installation-and-testing.md#generating-an-rtc-token)をご参照ください。
...
| `InvalidAuthorization`   | 400            | 認証ヘッダーが無効             |
...
| `FlowLimitExceeded`      | 429            | レート制限を超過している       |

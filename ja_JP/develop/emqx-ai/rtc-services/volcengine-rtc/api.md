...
| `Config`      | object   | 必須     | セッション設定。ASR、TTS、LLMパラメータを含む。詳細は[Config](#config)参照 |
...
| `EnableConversationStateCallback` | boolean    | 任意     | 聞き取り／思考／発話状態のコールバックを有効化 |
...
成功時は `Result` が空オブジェクトとなります。失敗時は `ResponseMetadata.Error` にエラー情報が含まれます。
...
進行中の音声セッションを更新します。割り込み、関数呼び出し、カスタムアナウンスをサポートします。
...
| 値   | 説明                                                       |
| ----- | ---------------------------------------------------------- |
| 1     | 高優先度：現在のインタラクションを停止して即時再生         |
| 2     | 中優先度：現在のインタラクション終了後に再生               |
| 3     | 低優先度：インタラクション中の場合は破棄                   |
...
| 音声ID             | 説明         |
| ------------------ | ------------ |
| `BV033_streaming`  | 女性、やさしい |
| `BV001_streaming`  | 男性、魅力的   |
| `BV700_streaming`  | 女性、甘い     |
| `BV406_streaming`  | 男性、落ち着いた |
...
| `EndPointId`     | string     | ArkV3のみ      | ArkモデルのエンドポイントID                                   |
...
公式ドキュメント: [一般的なエラーコード](https://www.volcengine.com/docs/6369/68677)

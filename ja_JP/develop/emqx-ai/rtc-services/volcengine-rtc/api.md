...
| `Url`            | string     | CustomLLMのみ必須 | CustomLLMコールバックURL                           |
| `APIKey`         | string     | 任意             | API認証キー                                        |
| `EndPointId`     | string     | ArkV3のみ必須     | ArkモデルのエンドポイントID                        |
...
トークン生成ライブラリの詳細は[インストールとテスト - RTCトークンの生成](./installation-and-testing.md#generating-an-rtc-token)をご参照ください。

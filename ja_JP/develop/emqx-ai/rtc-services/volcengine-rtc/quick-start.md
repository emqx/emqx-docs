...
本ドキュメントでは、Docker Composeを使用してAIエージェントのデモをデプロイする方法を説明します。このデモでは、ブラウザ上のインテリジェントドールをスマートデバイスのシミュレーションとして利用し、[Volcano Engine RTC](https://www.volcengine.com/product/veRTC/ConversationalAI)を活用した低レイテンシ音声対話、MQTTプロトコル経由のMCPによるデバイス側機能（写真撮影、表情切り替え、音量調整など）の呼び出し、そしてVolcano Engineの`CustomLLM`モードを用いたカスタムAIエージェントサービスの統合によるマルチターン会話とツール呼び出しを実現します。音声対話からデバイス制御までの一連のワークフローを紹介します。

全体の動作イメージは[デモ動画](https://www.bilibili.com/video/BV1P2WTzBEu4/)をご覧ください。
...
| app            | MCPクライアント＋AIエージェント | 8081   | `/chat-stream`エンドポイント提供、LLM/VLM推論とMCPツール呼び出し処理 |
...
- マルチモーダル理解：VLMを統合し、「今何を持っている？」などの視覚的質問に対応
- リアルタイム音声対話：Volcano Engine RTC＋ASR/TTSによる低レイテンシな音声認識と合成をエンドツーエンドで実現
...
- マネージドサービス：[EMQX Cloud](https://docs.emqx.com/zh/cloud/latest/)利用
...
本プロジェクトはVolcano Engine CustomLLMモードを介してカスタムAIエージェントを統合します。デフォルトではAlibaba Cloud Bailianの`qwen-flash`モデルを使用します。

#### Alibaba Cloud Bailianの有効化

1. [Alibaba Cloud Bailianコンソール](https://bailian.console.aliyun.com)にアクセス
2. 上部に有効化案内があればクリックして有効化（無料枠内は無料、超過分のみ課金）
3. 必要に応じて本人確認を完了

#### APIキーの作成

1. [API-KEY管理](https://bailian.console.aliyun.com/#/api-key)へアクセス
2. API-Keyタブで「API-KEY作成」をクリック
3. アカウントとワークスペース（通常はデフォルト）を選択し説明を入力して作成
4. 作成したAPIキーのコピーアイコンをクリックしてシークレットを取得
5. `app/.env`の`DASHSCOPE_API_KEY`に設定

#### 他モデルサービスの利用（任意）

別のOpenAI互換モデルサービスを使う場合は`app/.env`を以下のように変更：
...
詳細：[リアルタイム会話AI前提条件](https://www.volcengine.com/docs/6348/1315561)
...
- `VOLC_LLM_URL` - アプリの`/chat-stream`エンドポイントを指す
  ...
  - 本番環境：`https://your-domain.com/chat-stream`（公開HTTPS必須）
...
- 本番環境（推奨）：公開HTTPS URL（例：`https://your-domain.com/chat-stream`）にデプロイし、SSEストリームが`data: [DONE]`で正しく終了することを確認
- ローカルテスト：非公開環境ではLLM推論やMCP over MQTTツール呼び出しのAPIテストのみ可能で、Volcano Engine音声対話は完全には体験できません

## クイックチュートリアル：10分で音声対話＋デバイス制御デモを動かす

前提条件を満たした後、以下の手順で音声対話とデバイス制御（Web UI上でデバイスをシミュレート）を備えたAIエージェントデモを素早くセットアップします。
...
最も重要なステップです。前提条件で取得した認証情報を3つのサービスの設定ファイルに正確に記入してください。各項目の説明と取得元をよく確認してください。
...
# 目的：Volcano Engineがコールバック時に認証するため
...
# 目的：EMQXブローカーに接続しMCP over MQTT通信を行うため
MQTT_BROKER_HOST=localhost        # EMQXブローカーのホスト
MQTT_BROKER_PORT=1883             # MQTTポート
...
# 本番環境：公開HTTPS URL（Volcano Engineコールバック用）
...
webサービスはビルド時環境変数を利用します。ローカル開発時のデフォルト設定で通常は問題ありません。
...
- volc-serverのポートを3002以外に変更している場合
...
1. `CUSTOM_LLM_API_KEY`は自分で生成し、`app/.env`と`volc-server/.env`で完全に一致させる必要があります。
...
チャットボットアバター、マイク、カメラボタンなどを備えた仮想デバイス画面が表示されます。
...
2. 設定パネルでEMQXブローカー情報を入力：
   ...
   - パスワード：EMQX認証有効時に入力
3. 「保存」をクリック
4. 確認ダイアログで「確認」をクリックするとページが自動更新され、新設定が反映されてMQTT接続が自動的に確立されます
...
画面下部のマイクボタンをクリックし、マイク使用許可を与えます。RTC接続が自動で確立され、成功するとマイクボタンが紫色に変わり、発話可能になります。

推奨テスト例：

- 「こんにちは」や「物語を聞かせて」と話しかけて基本会話を試す
- 「今何を持っている？」で写真撮影と視覚認識をトリガー
- 「音量を80%にして」や「嬉しい表情にして」でデバイス制御を試す

#### 4.4 成功条件

- 音声対話：ASRの文字起こしが正確で、LLMが応答をストリーミングし、TTS再生が正常に動作する
- MCPツール呼び出し：写真撮影、表情切替、音量調整が正しく反映される
...
3. `volc-server/src/config.ts`でLLM設定を編集：
   ...
   llm: {
     ...
     systemMessages: [
       { role: 'system', content: 'あなたは親切な音声アシスタントです' }
     ],
     historyLength: 5,                 // コンテキスト履歴ターン数
   }
   ```

4. volc-serverサービスを再起動し、ArkプラットフォームLLMを利用開始

ヒント：スムーズな対話には、Doubao-proシリーズなどの軽量モデルを推奨します。詳細な設定パラメータは[Volcano Engineドキュメント](https://www.volcengine.com/docs/6348/1581714)を参照してください。
...
注意：volc-serverポートを変更した場合は、`VITE_AIGC_PROXY_HOST`環境変数も合わせて更新してください。

#### HTTPS有効化（本番環境）

1. 証明書ファイル（`fullchain.pem`、`privkey.pem`）を用意

   重要：単一証明書ファイルではなく、完全な証明書チェーン（フルチェーン）を使用してください。Volcano Engineのコールバックはフルチェーン検証を行い、これがないとSSLハンドシェイクに失敗します。

   - Let’s Encryptの場合：`fullchain.pem`（証明書＋中間証明書含む）を使用
   - その他CAの場合：サーバー証明書＋中間証明書を含むファイルを用意

2. プロジェクト内の適切な場所（例：`certs/`）に配置
...
4. `volc-server/.env`の`VOLC_LLM_URL`をHTTPSアドレス（例：`https://your-domain.com:8081`）に変更
...
| 環境変数が反映されない           | `.env`ファイル未読み込み | 1) `.env`が正しい場所にあるか確認 2) ファイル権限確認 3) イメージ再ビルド |
...
| CustomLLMコールバック失敗 | 認証キー不一致           | 1) `CUSTOM_LLM_API_KEY`が両者で一致しているか確認 2) `VOLC_LLM_URL`確認 3) volc-serverからappへの接続確認 |
| HTTPSコールバック失敗      | 証明書チェーン不完全     | フルチェーン証明書を使用（`APP_SSL_CERTFILE`は`fullchain.pem`を指す）必要あり。SSLハンドシェイク失敗防止のため必須。 |

#### MCPツール呼び出し問題

| 問題                       | 原因例                     | 対処法                                                     |
| -------------------------- | -------------------------- | ---------------------------------------------------------- |
| ツールが利用できない       | MQTT接続やdevice_id問題    | 1) ブラウザコンソールでMQTT状態確認 2) Device IDが一致しているか確認 3) `MCP_TOOLS_WAIT_SECONDS=10`に増加 |
| カメラ撮影が失敗           | 権限未許可                 | 1) ブラウザのカメラ権限確認 2) 許可をクリック 3) ページをリロード |

#### MQTT接続問題

| 問題                       | 原因例                     | 対処法                                                     |
| -------------------------- | -------------------------- | ---------------------------------------------------------- |
| MQTT接続失敗               | ブローカー設定誤り         | 1) EMQXブローカー起動確認 2) `MQTT_BROKER_HOST`/`PORT`確認 3) 認証情報確認 4) ネットワーク疎通確認 |
...
- 音声品質：`volc-server/src/config.ts`でASRのVAD閾値やTTS音声を調整
- ツール呼び出し遅延：appとweb間のネットワーク品質を確保し、MQTTレイテンシを低減（同一LANや低レイテンシ環境での展開推奨）

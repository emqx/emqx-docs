...
ユーザー起動型の音声制御や対話型シナリオとは異なり、ここでのインタラクションはデバイスによって開始されます。デバイスが環境変化やイベントを検知すると、AI生成の音声メッセージを能動的にトリガーし、ユーザーに再生します。これにより「人がデバイスを探す」から「デバイスが人を探す」へとシフトします。
...
>
> *(異常振動検知)*
>  **システム**：「注意：モーター＃5で異常振動を検知しました。ベアリング摩耗の可能性があります。データは記録され、メンテナンチケットが発行されました。」

即時の音声アラートにより点検負荷を軽減し、事故防止に貢献します。

### 子育て — 安全と寄り添い

母親がキッチンで料理中、3歳児がリビングで遊んでいる状況：

> *(カメラが子供がコンセントに近づくのを検知)*
>  **スピーカー**：「お子さま、コンセントは危ないので触らないでね。代わりにブロック遊びをしようか？」
>
> *(泣き声を検知)*
>  **スピーカー**：「どうしたの？転んじゃった？お母さんがすぐに行くよ。」
>  *(母親のスマホにも通知が送信される)*
>
...
AIエージェントは最もカスタマイズ可能なコアコンポーネントです。開発者は以下のようにシナリオに合わせて調整できます：

- **アナウンススタイル**：産業用の厳格な警告、子育て向けの優しい寄り添い、医療用の専門的リマインダー

...
このガイドでは、[MCP over MQTT C SDK for ESP32](https://github.com/mqtt-ai/esp-mcp-over-mqtt) を使用して、シンプルな MCP over MQTT サーバーを作成する方法を説明します。現在、MCPサーバーのみがサポートされています。MCPクライアントはPython SDKを使用してインタラクションを行うことができます。

...
これらのツールのインストールおよび管理には、[SDKMAN](https://sdkman.io/) の使用を推奨します：
...
現在、Kotlin SDK には MCPクライアントの実装は含まれていません。テスト用に Python SDK を使ってシンプルな MCPクライアントを作成できます。
...
この Pythonクライアントは、自動的に `demo/kotlin-mcp-server` という名前の MCPサーバーを検出し、`set_light_brightness` ツールを呼び出してライトの明るさを50に設定します：

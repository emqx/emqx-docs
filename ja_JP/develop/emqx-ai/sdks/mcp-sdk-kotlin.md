...
[SDKMAN](https://sdkman.io/) を使用するのが、これらのツールをインストールおよび管理する推奨方法です。
...
server.addTool(
    ...
    description = "パネル上のライトを制御します。明るさを変更できます。ライトを消すには明るさを0に設定します。明るさを 'last_value' に設定すると、前回の明るさを復元します。これはライトが消灯中に再度点灯させたい場合に便利です。",
    inputSchema = ToolSchema(
        properties = buildJsonObject {
            putJsonObject("value") {
...
現時点で Kotlin SDK は MCP クライアントの実装を提供していません。テストには Python SDK を使用してシンプルな MCP クライアントを作成できます。
    ...
    if capabilities.tools:
        ...
        for tool in tools:
            ...
            if tool.name == "set_light_brightness":
                ...
                logger.info(
                    f"ツールを呼び出しました: set_light_brightness(value=50), 結果: {result}"
                )
...
async def main():
    async with mcp_mqtt.MqttTransportClient(
        ...
        await client.start()
        while True:
            # MQTT クライアントがバックグラウンドで動作中に他の処理をシミュレート
...
```bash
INFO 2025-12-19 13:07:44,445 - ツールを呼び出しました: set_light_brightness(value=50), 結果: meta=None

...
[Getting Started](../../../get-started/get-started.md) ガイドに従って、EMQX ブローカーをインストールし起動してください。
...
server.addTool(
    ...
    description = "パネル上のライトを制御します。明るさを変更できます。ライトをオフにするには明るさを0に設定します。'last_value' を設定すると前回の明るさに戻せます。これはライトがオフのときに再度オンにしたい場合に便利です。",
    inputSchema = ToolSchema(
        properties = buildJsonObject {
            putJsonObject("value") {
...
現時点で Kotlin SDK は MCP クライアントの実装を提供していません。テストには Python SDK を使ってシンプルな MCP クライアントを作成できます。
...
この Python クライアントは、`demo/kotlin-mcp-server` という MCP サーバーを自動検出し、`set_light_brightness` ツールを呼び出してライトの明るさを 50 に設定します：

...
このガイドでは、[MCP over MQTT C SDK with Paho MQTT](https://github.com/mqtt-ai/paho-mcp-over-mqtt) を使用して、シンプルな **MCP over MQTTサーバー** を作成する方法を説明します。現在、MCPサーバーのみがサポートされています。Python SDKを使用して、対話用のMCPクライアントを作成することが可能です。
...
## Python SDKでMCPクライアントの作成と実行

上記で作成したMCPサーバーに接続し、`get_temperature` ツールを呼び出すMCPクライアントの作成については、[Python SDK](./mcp-sdk-python.md) のドキュメントを参照してください。

## CMakeでMCPサーバーのビルドとコンパイル

[paho-mcp-over-mqttリポジトリ](https://github.com/mqtt-ai/paho-mcp-over-mqtt) にある **CMakeの例** を参考にビルドを行ってください。

...
このガイドでは、[Paho MQTTを使ったMCP over MQTT C SDK](https://github.com/mqtt-ai/paho-mcp-over-mqtt)を用いて、シンプルな**MCP over MQTTサーバー**を作成する方法を説明します。現在、MCPサーバーのみがサポートされています。Python SDKを使ってMCPクライアントを作成し、相互にやり取りすることが可能です。
...
## Python SDKでMCPクライアントを作成・実行

上記で作成したMCPサーバーに接続し、`get_temperature`ツールを呼び出すMCPクライアントの作成方法については、[Python SDK](./mcp-sdk-python.md)のドキュメントを参照してください。

## CMakeでMCPサーバーをビルド・コンパイル

[paho-mcp-over-mqttリポジトリ](https://github.com/mqtt-ai/paho-mcp-over-mqtt)にある**CMakeの例**に従ってビルドを行うことができます。

...
MCP over MQTTアーキテクチャでは、メッセージルーターとして中央集約型のMQTTブローカーが導入されており、その他のコンポーネント（Host、Client、Server）は標準MCP設計と一貫しています。
...
主な違いは、ClientとServerが直接通信するのではなく、MQTTブローカーを介して通信する点です。ブローカーの導入により、ClientとServer間の関係は1対1から多対多へと変わります。
...
スケーラビリティとロードバランシングを実現するために、MCP Serverは複数のインスタンス（プロセス）を起動できます。各インスタンスは一意の `server-id` をMQTTクライアントIDとしてブローカーに接続し、すべてのインスタンスは同じ `server-name` を共有します。

**Clientのインタラクションフロー：**

1. Clientはサービス検出トピックをサブスクライブし、対象の `server-name` 配下にあるすべての利用可能な `server-id` を取得します。
...
- **スケールアップ時**には、既存のMCPクライアントは古いサーバーインスタンスに接続し続け、新規クライアントは新たに追加されたインスタンスで初期化できます。
- **スケールダウン時**には、MCPクライアントは再初期化して他の利用可能なサーバーインスタンスに接続できます。

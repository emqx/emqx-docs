...
- **TLS相互認証（mTLS）**：ピアクラスターは管理下のCA発行のクライアント証明書を提示し、リスナーは`verify = verify_peer`および`fail_if_no_peer_cert = true`で検証します。これはトランスポート層でピアのIDを固定し、認証チェックを兼ねるため最も強力です。パスワード認証は不要になります。[X.509証明書認証](../../guides/access-control/authn/x509.md)を参照してください。

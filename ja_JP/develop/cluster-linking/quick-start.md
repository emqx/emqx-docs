...
それぞれ異なるリージョンにコンピュートリソースを用意し、各リージョンにEMQXクラスターをホスティングしていることを確認してください。例として、`us-east` と `eu-west` リージョンを使用し、クラスター名をそれぞれ `cluster-us-east` と `cluster-eu-west` とします。
...
クラスターリンクでは、各クラスターのMQTTリスナーが相手クラスターのネットワークから到達可能である必要があります。トラフィックの均等分散のため、これらのMQTTリスナーはロードバランサーの背後に配置することを推奨します。セキュリティ面では、パブリックインターネットを利用する場合、クラスター間通信は[暗号化（TLS）](./configuration.md)および厳格な[TLSまたはMQTTクライアント認証](../../guides/access-control/authn/authn.md)で保護してください。
...
最初のクラスター `cluster-us-east` は、以下の設定スニペットをクラスターの設定ファイルに追加してセットアップします。
...
2番目のクラスター `cluster-eu-west` は、以下の設定スニペットをクラスターの設定ファイルに追加してセットアップします。
...
この設定は最初のクラスターの設定と対称的です。両方の設定が適用され、クラスターが起動すると、2つのクラスター間に対称的で双方向のクラスターリンクが確立されます。非対称リンクの作成も可能であり、後述します。
...
2. `cluster-eu-west` からメッセージをパブリッシュ：
   ...
   [6/4/2024] [3:53:35 PM] › …  メッセージパブリッシュ中...
...
4. 逆方向でも同様に実施：
   ...
   - `cluster-us-east` からメッセージをパブリッシュ：
     ...
     [6/4/2024] [3:54:15 PM] › …  メッセージパブリッシュ中...

...
- **PUB および HPUB メッセージはパブリッシュ操作として扱われます**：
  ...
  - クライアントが `CONNECT verbose=1` で接続している場合、変換後の MQTT メッセージは QoS 1 を使用し、それ以外は QoS 0 となります。
- **SUB メッセージはサブスクリプション要求として扱われます**：
  ...
  - キューグループをサポートし、SUB メッセージのキューグループ値は MQTT の共有サブスクリプションのグループ名に変換されます。
- **UNSUB メッセージはサブスクリプション解除要求として扱われ**、サブスクリプション ID（sid）で解除対象を特定します。

::: tip

NATS ゲートウェイはパブリッシュ／サブスクライブ操作に対する独自のアクセス制御を実装していません。トピックの権限管理は統合された [認可設定](../../guides/access-control/authz/authz.md) を使用してください。
...
クラスター環境では、ダッシュボードまたは REST API で行った設定は自動的に全ノードに適用されます。特定ノードのみに設定を反映したい場合は、そのノードの `base.hocon` 設定ファイルを使用してください。
...
NATS ゲートウェイは TCP、SSL、WS、WSS タイプのリスナーをサポートします。設定可能なパラメータの完全な一覧は、[EMQX Enterprise 設定マニュアル](https://docs.emqx.com/en/enterprise/v@EE_VERSION@/hocon/) のゲートウェイ設定 - リスナーのセクションを参照してください。

## NATS ゲートウェイのカスタマイズ

デフォルト設定に加えて、EMQX はビジネスニーズに合わせた多様な設定オプションを提供しています。本節では **ゲートウェイ** ページで利用可能な設定項目を詳述します。

### 基本設定

1. **ゲートウェイ** ページで **NATS** を探し、**操作** 列の **設定** ボタンをクリックします。

2. **設定** タブでは、ゲートウェイの接続パラメータ、マウントポイントプレフィックス、クライアント識別情報の上書きを設定できます。

   - **サーバー名**：ゲートウェイの内部参照用の一意識別子。デフォルトは `emq_nats_gateway`。
   - **マウントポイント**：ゲートウェイを通過するすべてのトピックに自動的に付加される文字列プレフィックス。プロトコル間のトピック分離に役立ちます。例：`nats/` を使うことで、クライアントが手動でプレフィックスを付けることなくクロスプロトコルルーティングが可能になります。
   ...
   - **統計収集の有効化**：このゲートウェイの統計収集およびレポートを有効にするかどうか。デフォルトは有効。
   - **クライアント情報の上書き**：`CONNECT` パケットから認証情報を抽出する方法を定義します。

     ::: tip

     認証が有効な場合は、`username` と `password` の正しいフィールドマッピングを必ず設定し、適切に認証情報が処理されるようにしてください。

     :::

     - **ユーザー名**：`CONNECT` パケットの `user` フィールドにマッピング。
     - **パスワード**：`CONNECT` パケットの `pass` フィールドにマッピング。
     - **クライアント ID**：`${generated}` を設定すると自動生成されます。カスタムロジックでの設定も可能です。
...
MQTT プロトコルとは異なり、NATS ゲートウェイは認証器のリスト（チェーン）ではなく単一の認証器のみをサポートします。認証器が有効化されていない場合、すべての NATS クライアントは認証なしで接続可能です。
...
1. NATS ゲートウェイの設定画面で **認証** タブに移動します。
2. **+ 認証作成** をクリックし、機構として **パスワードベース** を選択、データソースに **HTTP サーバー** を選び、**次へ** をクリックします。
3. 設定パラメータを入力します。各項目の詳細は [HTTP パスワード認証](../../guides/access-control/authn/http.md) を参照してください。
...
その他の認証方式については、[EMQX 認証器](../../guides/access-control/authn/authn.md#emqx-authenticators) のドキュメントを参照してください。
...
NATS プロトコルゲートウェイおよびそのユースケースの詳細については、ブログ記事をご覧ください： [EMQX NATS ゲートウェイ：MQTT-NATS 双方向相互運用の実現](https://www.emqx.com/en/blog/emqx-nats-gateway)。

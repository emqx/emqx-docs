...
ゲートウェイは**gbt32960-2016**および**gbt32960-2025**の両プロトコルバージョンをサポートします。プロトコルバージョンはフレームヘッダーから自動検出されます：
...
各レポートは`Infos`配列に複数の情報項目を含めることができます。`Type`フィールドで情報の種類を区別します。
...
| `Mode`              | 整数     | 運転モード：`1`=純電気、`2`=ハイブリッド、`3`=燃料、`254`=エラー、`255`=無効                               |
...
| `ProbeNum`            | 整数     | **gbt32960-2016のみ**：燃料電池プローブ数。有効範囲0～65531                                |
| `ProbeTemps`          | 配列     | **gbt32960-2016のみ**：各燃料電池プローブの温度値                                           |
| `H_MaxTemp`           | 整数     | 水素システム最高温度。有効範囲0～2400（オフセット40℃、-40℃～+200℃）、単位：0.1℃          |
| `H_TempProbeCode`     | 整数     | 最高水素温度のプローブコード。有効範囲1～252                                                |
| `H_MaxConc`           | 整数     | 最高水素濃度。有効範囲0～60000（0～50000 mg/kg）、単位：1 mg/kg                            |
| `H_ConcSensorCode`    | 整数     | 最高水素濃度のセンサーコード。有効範囲1～252                                                |
| `H_MaxPress`          | 整数     | 最高水素圧力。有効範囲0～1000（0～100 MPa）、単位：0.1 MPa                                 |
| `H_PressSensorCode`   | 整数     | 最高水素圧力のセンサーコード。有効範囲1～252                                               |
| `DCStatus`            | 整数     | 高電圧DC/DC状態：`1`=動作中、`2`=切断                                                     |
| `RemainingH2`         | 整数     | **gbt32960-2025のみ**：水素システム内の残存水素量、単位：1 kg                              |
...
| `Status`            | 整数     | **gbt32960-2016のみ**：エンジン状態。`1`=稼働中、`2`=停止中                           |
...
| `Status`            | 整数     | 位置情報状態。全ステータスビットの整数値（プロトコル表15参照）                          |
| `CoordinateSystem`  | 整数     | **gbt32960-2025のみ**：座標系。`1`=WGS-84、`2`=GCJ-02                                  |
| `Longitude`         | 整数     | 経度（度×10^6）、小数点以下6桁までの精度                                               |
| `Latitude`          | 整数     | 緯度（度×10^6）、小数点以下6桁までの精度                                               |
...
| `FrameCellsIndex`     | 整数     | 本フレームで報告するセルの開始インデックス（1始まり）。有効範囲1～65531。セル数が200を超える場合はフレーム分割されます。 |
| `FrameCellsCount`     | 整数     | 本フレームのセル数。有効範囲1～200                                                                   |
...
| `StackProbeTemp`    | 配列     | スタック温度プローブの測定値                                                           |
...
| `MaxVoltageManagerSysNo`  | 整数     | 最高電圧の管理システム番号                                                             |
| `MaxVoltageCellCode`      | 整数     | 最高電圧のセルコード                                                                   |
| `MaxVoltageCellValue`     | 整数     | 最高電圧値。単位：1 mV                                                                 |
| `MinVoltageManagerSysNo`  | 整数     | 最低電圧の管理システム番号                                                             |
| `MinVoltageCellCode`      | 整数     | 最低電圧のセルコード                                                                   |
| `MinVoltageCellValue`     | 整数     | 最低電圧値。単位：1 mV                                                                 |
| `MaxTempManagerSysNo`     | 整数     | 最高温度の管理システム番号                                                             |
| `MaxTempProbeCode`        | 整数     | 最高温度のプローブコード                                                               |
| `MaxTempValue`            | 整数     | 最高温度値                                                                             |
| `MinTempManagerSysNo`     | 整数     | 最低温度の管理システム番号                                                             |
| `MinTempProbeCode`        | 整数     | 最低温度のプローブコード                                                               |
| `MinTempValue`            | 整数     | 最低温度値                                                                             |
...
| フィールド  | 型       | 説明                                                                                     |
|------------|----------|------------------------------------------------------------------------------------------|
| `Action`   | 文字列   | コマンドタイプ。クエリ操作は`Query`                                                    |
| `Total`    | 整数     | クエリするパラメータの総数                                                              |
...
| フィールド  | 型       | 説明                                                                                     |
|------------|----------|------------------------------------------------------------------------------------------|
| `Action`   | 文字列   | コマンドタイプ。設定操作は`Setting`                                                    |
...
コマンドによりパラメータが異なります。パラメータなしのコマンドは`Param`フィールドを省略します。
...
| フィールド   | 型       | 説明                                                                                  |
|--------------|----------|---------------------------------------------------------------------------------------|
| `Action`     | 文字列   | コマンドタイプ。制御操作は`Control`                                                 |

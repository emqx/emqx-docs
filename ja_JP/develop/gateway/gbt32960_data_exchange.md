...
ゲートウェイは**gbt32960-2016**および**gbt32960-2025**の両方のプロトコルバージョンをサポートしています。プロトコルバージョンはフレームヘッダーから自動検出されます：
...
- ペイロードはJSON形式で構成されます。
- JSONキー名はUpperCamelCase（パスカルケース）を使用します。
...
| `Mileage`           | 整数     | 走行距離累計、有効範囲0〜9,999,999（0〜999,999.9 km）、単位：0.1 km                                         |
...
| `DC`                | 整数     | DC/DC状態：`1`=稼働中、`2`=切断、`254`=エラー、`255`=無効                                                  |
| `Gear`              | 整数     | ギア位置。プロトコルの表A.1にあるギア表の整数表現                                                         |
...
| `DCStatus`            | 整数     | 高圧DC/DC状態：`1`=稼働中、`2`=切断                                                       |
| `RemainingH2`         | 整数     | **gbt32960-2025のみ**：水素システム内の残水素量、単位：1 kg                                |
| `DCDCTemp`            | 整数     | **gbt32960-2025のみ**：高圧DCDC温度、オフセット40℃、-40℃〜+210℃                          |
...
| `Status`            | 整数     | 位置状態。すべての状態ビットの整数値（プロトコル表15参照）                            |
| `CoordinateSystem`  | 整数     | **gbt32960-2025のみ**：座標系。`1`=WGS-84、`2`=GCJ-02                                |
| `Longitude`         | 整数     | 経度（度×10^6）、百万分の一度の精度                                                   |
| `Latitude`          | 整数     | 緯度（度×10^6）、百万分の一度の精度                                                   |
...
| `MaxBatteryVoltage`       | 整数     | 最高セル電圧。有効範囲0〜15000（0〜15 V）、単位：0.001 V                                  |
| `MinVoltageBatterySubsysNo` | 整数     | 最低電圧のバッテリーサブシステム番号。有効範囲1〜250                                      |
| `MinVoltageBatteryCode`   | 整数     | 最低電圧のバッテリーセルコード。有効範囲1〜250                                            |
| `MinBatteryVoltage`       | 整数     | 最低セル電圧。有効範囲0〜15000（0〜15 V）、単位：0.001 V                                  |
...
| `MaxAlarmLevel`           | 整数     | 最高アラームレベル。**gbt32960-2016**：有効範囲0〜3（"0"=故障なし、"1"=レベル1故障）。<br>**gbt32960-2025**：有効範囲0〜4、新値"4"=熱イベント故障 |
...
| `FaultDriveMotorNum`      | 整数     | 駆動モーターの故障総数。有効範囲0〜252                                                                                     |
| `FaultDriveMotorList`     | 配列     | 駆動モーター故障コードリスト                                                                                              |
| `FaultEngineNum`          | 整数     | エンジンの故障総数。有効範囲0〜252                                                                                        |
| `FaultEngineList`         | 配列     | エンジン故障コードリスト                                                                                                  |
| `FaultOthersNum`          | 整数     | その他の故障総数                                                                                                          |
...
| `FrameCellsIndex`     | 整数     | このフレームで報告されるセルの開始インデックス（1始まり）。有効範囲1〜65531。セル数が200を超える場合はフレーム分割される。 |
| `FrameCellsCount`     | 整数     | このフレーム内のセル数。有効範囲1〜200                                                                        |
...
コマンドごとに異なるパラメータを持ち、パラメータがない場合は`Param`フィールドを省略します。

...
EMQX がクラスターで稼働している場合、ダッシュボードや HTTP API での設定はクラスター全体に反映されます。単一ノードのみを設定したい場合は、[`base.hocon`](../../operate/configuration/configuration.md) でゲートウェイを設定してください。
...
- 詳細な HTTP API インターフェースは以下を参照してください：[HTTP API - Gateway](../../operate/api.md)

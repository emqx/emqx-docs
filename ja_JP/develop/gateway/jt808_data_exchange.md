...
| フィールド名       | JSONキー   | 値の型   | JSONでの型   |
| :----------------: | :--------: | :-------:| :----------: |
| 総フラグメント数   | frag_total | word     | integer     |
| フラグメント番号   | frag_sn    | word     | integer     |
...
#### サブパケット再送要求 `"msg_id": 32771` (0x8003)
...
**注意**: 2019年版では再送パケット総数の型が BYTE から WORD に変更されています。
...
**注意**: 2019年版では製造商ID（5→11バイト）、端末モデル（20→30バイト）、端末ID（7→30バイト）が拡張されています。
...
**注意**: 2019年版では認証コード長、端末IMEI、ソフトウェアバージョン情報が追加されています。
...
| パラメータ値           | value    | [パラメータ値型マッピング参照](#parameter-value-type-mapping) | [パラメータ値型マッピング参照](#parameter-value-type-mapping) |
...
- 2019年版では製造商ID（5→11バイト）、端末モデル（20→30バイト）、端末ID（7→30バイト）が拡張されています。
...
| フィールド名       | JSONキー   | 値の型 | JSONでの型 |
| :----------------: | :--------: | :-----:| :--------: |
| 深睡眠状態         | deep_sleep | 1 bit  | integer   |
| 睡眠状態           | sleep      | 1 bit  | integer   |
...
**注意**: 2019年版では新たにテキストタイプフィールドが追加され、flagのbit0–1の意味が変更されています。
...
| 制御種別リスト         | controls | list   | 制御種別のリスト         |
| 制御種別ID             | type_id  | byte   | integer                 |
| 制御種別パラメータ     | param    | binary | string(base64エンコード) |

**注意**: 2019年版ではメッセージ構造が大幅に変更され、制御種別数と制御種別リストで構成されます。
...
**注意**: 2019年版では区間夜間最大速度、ルート名長、ルート名が追加されています。
...
| 再送パケット総数           | length     | byte   | integer         |
| 再送パケットIDリスト       | retx_ids   | list   | 再送IDのリスト   |

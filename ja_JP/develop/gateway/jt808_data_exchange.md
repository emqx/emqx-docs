...
プロトコルバージョンはメッセージヘッダーのバージョンフラグにより自動判定されます。
...
**注意**: 2019年版では認証コード長さ、端末IMEI、ソフトウェアバージョン情報が追加されています。
...
| 0x0110–0x01FF             | byte[8]  | string (base64)     | CANバスID独立取得パラメータ             |
| その他不明パラメータ       | byte[n]  | string (base64)     | 予約パラメータまたはベンダー定義パラメータ |
...
| パラメータ値           | value    | --     | [0x8103 パラメータ値の型マッピング](#parameter-value-type-mapping)参照 |
...
- 端末ハードウェアバージョン番号およびファームウェア番号の長さはバイナリメッセージ解析用であり、公開されません。
...
| ルート走行時間過短・過長警報追加情報 | path_time_alarm | -      | map                      |
...
- 出入区域・ルート警報追加情報は `in_out_alarm` マップ内。
...
- ルート走行時間過短・過長警報追加情報は `path_time_alarm` マップ内。
...
**注意**: 2019年版ではテキストタイプフィールドが追加され、flagのbit0–1の意味が変更されています。
...
| 制御タイプリスト       | controls | list   | 制御リスト             |
| 制御タイプID           | type_id  | byte   | integer                |
| 制御タイプパラメータ   | param    | binary | string(base64エンコード) |

**注意**: 2019年版ではメッセージ構造が制御タイプ数＋制御タイプリストに完全に再設計されています。
...
| 地域名長さ             | name_len         | word   | integer    |
| 地域名                 | name             | string | string     |

**注意**: 2019年版では地域項目に夜間最大速度、地域名長さ、地域名のフィールドが追加されています。
...
**注意**: 2019年版では夜間最大速度、地域名長さ、地域名のフィールドが追加されています。
...
**注意**: 2019年版では区間夜間最大速度、ルート名長さ、ルート名のフィールドが追加されています。
...
**注意**: 2019年版では解像度フィールドに新しい値 `0x00`（最低画質圧縮）および `0xFF`（最高画質圧縮）が追加されています。
...
**注意**: 2019年版では位置情報報告のメッセージ本文が固定長28バイトから可変長に変更されています。

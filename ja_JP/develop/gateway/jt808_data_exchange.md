...
**補足**: 2019年版では再送パッケージ総数の型が BYTE から WORD に変更されました。
...
- 端末ハードウェアバージョン番号とファームウェア番号の長さはバイナリメッセージ解析用であり、外部には公開されません。
...
- 追加の位置情報項目は `extra` に含まれます。

|       フィールド名（追加情報説明）       |    JSON キー    | 値の型 |        JSON 内の型         |
| :-------------------------------------: | :-------------: | :----: | :-----------------------: |
| 走行距離                               |    mileage      | dword  |         integer           |
| 燃料計                                 |   fuel_meter    | word   |         integer           |
| 運行記録機能による速度                 |     speed       | word   |         integer           |
| 手動確認が必要なアラームイベントID     |    alarm_id     | word   |         integer           |
| タイヤ空気圧（2019年追加）             |  tire_pressure  |   -    | string (base64 encoded)   |
| 車内温度（2019年追加）                 |  carriage_temp  | word   |         integer           |
| 速度超過アラーム追加情報（長さ1または5） | overspeed_alarm |   -    |           map             |
| 進入・退出エリア／ルートアラーム追加情報 |  in_out_alarm   |   -    |           map             |
...
| 続くカスタム情報の長さ                 |    custome     |   -    | string(base64 encoded)    |

- 速度超過アラーム追加情報（長さ1または5）は `overspeed_alarm` マップ内。
...
- 進入・退出エリア／ルートアラーム追加情報は `in_out_alarm` マップ内。
...
**補足**: 2019年版では新たにテキストタイプフィールドが追加され、flag のビット0–1の意味が変更されました。
...
| 制御タイプパラメータ         |  param    | binary | string (base64 encoded) |
...
**補足**: 2019年版では地域項目に夜間最大速度、地域名長、地域名のフィールドが追加されました。
...
**補足**: 2019年版では夜間最大速度、地域名長、地域名のフィールドが追加されました。
...
|             フィールド名             |    JSON キー     | 値の型 | JSON 内の型 |
| :---------------------------------: | :--------------: | :----: | :---------: |
| 区間の夜間最大速度                  | night_max_speed  |  word  |  integer    |
| ルート名長                        |    name_len      |  word  |  integer    |
| ルート名                          |      name        | string |  string     |

**補足**: 2019年版では区間の夜間最大速度、ルート名長、ルート名のフィールドが追加されました。
...
| エリア／ルート項目リスト    |  items    | binary | string (base64 encoded)   |
...
| 電子運行票内容                 |   data    | string(base64 encoded) |  string     |
...
| データ収集方式                |  data_method  | 1 bit    |          integer            |
...
**補足**: 2019年版では解像度フィールドに `0x00`（最低画質圧縮）および `0xFF`（最高画質圧縮）の値が追加されました。

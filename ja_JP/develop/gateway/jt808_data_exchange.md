...
プロトコルバージョンはメッセージヘッダー内のバージョンフラグに基づいて自動判別されます。
...
| パラメータ項目リスト     | params   | list                 | idと値のリスト `[{"id":ID, "value":VAL}, ...]` |
...
パラメータ値のJSON型はパラメータIDに対応するデータ型により決定されます。
...
| 経路走行時間過短・過長警報追加情報 | path_time_alarm | -      | map                       |
...
- 速度超過警報追加情報（長さ1または5）は `overspeed_alarm` マップ内。
...
- 出入区域・経路警報追加情報は `in_out_alarm` マップ内。
...
- 経路走行時間過短・過長警報追加情報は `path_time_alarm` マップ内。
...
#### サーバー発端の端末リンク検出要求 `"msg_id": 33284` 0x8204 (2019年追加)
...
**補足**: 2019年版では新たにテキストタイプフィールドが追加され、flagのビット0–1の意味が変更されています。
...
| 回答候補リスト       | answers  | list   | 回答リスト `[{"id":ID, "len":LEN, "answer":ANS}, ...]` |
...
**補足**: 2019年版では区域項目に夜間最高速度、区域名長、区域名のフィールドが追加されています。
...
**補足**: 2019年版では夜間最高速度、区域名長、区域名のフィールドが追加されています。
...
**補足**: 2019年版では区間夜間最高速度、経路名長、経路名のフィールドが追加されています。

#### 経路削除 `"msg_id": 34311` 0x8607

| フィールド名       | JSONキー | 値の型 | JSON内の型     |
| :----------------: | :------: | :----: | :------------: |
| 経路数             | length   | byte   | integer        |
...
#### 運転者情報アップロード要求 `"msg_id": 34562` 0x8702

空のJSON

#### 運転者情報収集報告 `"msg_id": 1794` 0x0702
...
**補足**: 2019年版では運転者IDカード番号フィールドが追加されています。
...
| 位置報告データ項目リスト | location | list   | 位置情報のリスト |
...
| CANバスデータ項目リスト | can_data   | list            | CANデータのリスト        |
...
**補足**: 2019年版では解像度フィールドに新たな値 `0x00`（最低画質圧縮）および `0xFF`（最高画質圧縮）が追加されています。
...
ベンダー定義のメッセージ。メッセージ本文フォーマットはベンダーが定義。

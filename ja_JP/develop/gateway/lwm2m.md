...
::: tip
`base.hocon` でゲートウェイを設定する場合はノード単位の変更が必要ですが、ダッシュボードや REST API で設定するとクラスター全体に反映されます。
:::
...
**Update** メッセージは LwM2M クライアントが LwM2M サーバーに対して登録情報を更新するために送信します。  
Register メッセージに似ていますが、初回登録後に送信されます。  
Update メッセージには IP アドレスの変更や LwM2M オブジェクトによってモデル化されたデータの更新など、クライアントの機能や状態の変更が含まれます。  
Update メッセージはクライアントの登録期間を延長する役割も持ち、クライアントがまだ利用可能でアクティブであることをサーバーに知らせる手段です。
...
```
変数：
- `{?ResourcePath}`: 文字列、要求されたリソースパス。以下の3つのパターンがあります：
...
- `{?ReadResponseData}`: JSON 配列、リクエストに対するリソースの値の結果。
...
この操作は特定のオブジェクトインスタンスにどのリソースが存在するかを探索するために使われます。  
...
リクエストコマンドで **MsgType** が `"write"` の場合、**RequestData** は2つの構造があります。
...
- `{?ValueType}`: 文字列、以下のいずれか： "Time", "String", "Integer", "Float", "Boolean", "Opaque", "Objlnk"
...
このインターフェースは LwM2M サーバーが登録済み LwM2M クライアントのリソースの変化を監視し、新しい値が利用可能になると通知を受け取るために使用します。  
監視関係はオブジェクト、オブジェクトインスタンス、リソースに対する "Observe" 操作を LwM2M クライアントに送信することで開始されます。  
...
"Notify" 操作は LwM2M クライアントから LwM2M サーバーへ、有効な監視中にオブジェクトインスタンスまたはリソースの新しい値を送信するために送られます。  
...
LwM2M プロトコルはトランスポート層に CoAP を使用します。CoAP は UDP 上で動作するため、単一のデータグラムサイズはネットワーク MTU（通常約1500バイト）に制限されます。  
送信データがこの制限を超える場合、単一の CoAP パケットでの送信ができません。  
例えば、数百KBや数MBに及ぶファームウェアパッケージをデバイスにプッシュする場合や、多数のリソースを含むオブジェクトを読み取る場合に発生します。
...
- **Block1 (サーバー -> デバイス)**
  ...
  例えば、256バイトのファームウェアペイロードを16バイトのブロックサイズで分割すると、16個のブロックに分かれて順次送信されます。

- **Block2 (デバイス -> サーバー)**

  デバイスが単一パケットのサイズ制限を超えるレスポンスを生成する際（例：デバイスオブジェクト `/3/0` の読み取り）、デバイスは複数の Block2 セグメントでレスポンスを送信します。  
...
ブロック転送に関連する設定項目は以下の通りです：

| 設定項目                      | タイプ       | デフォルト | 説明                                                         |
| ----------------------------- | ------------ | ---------- | ------------------------------------------------------------ |
| `blockwise.enable`            | ブール値    | `true`     | ブロック転送を有効にするかどうか。                          |
...
| `blockwise.exchange_lifetime` | 期間        | `"247s"`   | ブロック転送交換状態の有効期間。                             |

適切に設定すると、ブロック転送は UDP 上での大容量ペイロードの信頼性ある送信を保証し、MQTT アプリケーションには完全に透過的に動作します。

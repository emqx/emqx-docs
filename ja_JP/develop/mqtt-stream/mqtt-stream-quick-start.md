...
このクイックスタートでは、EMQX MQTT Streams が以下のことを実現できることを示します。
...
MQTT Streams では、サブスクライバーが `stream-offset` の値を指定することで、再生開始位置を制御できます。
...
この値に 1000 を掛けてマイクロ秒単位に変換し、保存します。これが再生開始位置として使われます。

### ステップ 2: 新しいメッセージのパブリッシュ
...
これにより、MQTT Streams におけるコンシューマー制御の再生が実証されます。
...
ストリームキー式がメッセージメタデータのクライアント ID をキーとして抽出するため、両メッセージは同じキーを共有し、2つ目のメッセージが1つ目を上書きします。

### ステップ 4: ストリームのサブスクライブ
...
このセクションでは、自動作成を有効にしてテストする方法を説明します。
...
7. ダッシュボードの **Streams** ページで自動作成が確認できます。ストリームリストに新しい `auto_stream` が表示されます。

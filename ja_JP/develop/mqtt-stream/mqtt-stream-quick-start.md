...
- `$stream/` サブスクリプションを通じてストリームを自動作成する
...
開始前に、MQTT Streams 機能が有効であり、自動作成設定が本例に影響しないことを確認してください。
...
5. 変更を加えた場合は、**Save Changes** をクリックして適用します。
...
- メッセージは圧縮や上書きされていないこと

## 異なる位置からのメッセージ再生

MQTT Streams は、サブスクライバーが `stream-offset` 値を指定して再生開始位置を制御できます。
...
この値に 1000 を掛けてマイクロ秒に変換し、保存します。これが再生開始位置として使われます。
...
異なる消費者は同じストリームを別々の位置から独立して再生できます。

- ある消費者は最初から（`earliest`）再生
- 別の消費者は特定のタイムスタンプから再生
- さらに別の消費者は新しいメッセージのみ（`latest`）受信

各消費者の再生位置は独立しており、他のサブスクライバーに影響しません。

これにより、MQTT Streams の消費者制御による再生が実証されます。
...
これにより、MQTT Streams が Last-Value セマンティクスを使って状態指向メッセージングパターンをサポートしていることが示されます。

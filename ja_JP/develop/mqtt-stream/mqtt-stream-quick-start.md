...
- 状態指向メッセージングのための Last-Value セマンティクスを有効にする
...
- [MQTTX](https://mqttx.app/)（または MQTT 5.0 対応の任意のクライアント）
...
4. 自動作成の設定を確認し、**Regular Stream** が使用されるようにします：
   ...
   - **Auto Create Stream Type** が **Regular Stream** に設定されている。

   > これにより、ストリームが Last-Value Stream として自動作成されるのを防ぎます。Last-Value Stream はキーごとに最新のメッセージのみを保持します。

5. 変更を行った場合は、**Save Changes** をクリックして適用します。
...
1. MQTTX CLI がインストールされていることを確認してください。詳細は [インストール](https://mqttx.app/docs/cli/downloading-and-installation) を参照してください。
...
MQTTX CLI を使い、MQTT 5 のサブスクリプションユーザープロパティ `stream-offset` を設定して、ストリームの最初からメッセージを再生します。
...
- メッセージが圧縮または上書きされていないこと

## 異なる位置からのメッセージ再生

MQTT Streams では、サブスクライバーがサブスクライブ時に `stream-offset` を指定することで、メッセージ再生の開始位置を制御できます。
...
この値に 1000 を掛けてマイクロ秒に変換し、再生開始位置として保存します。
...
この時刻以降にパブリッシュされたメッセージのみがサブスクライバーに届きます：
...
異なるコンシューマーは同じストリームを別々の位置から独立して再生できます：

- あるコンシューマーは最初から（`earliest`）再生
- 別のコンシューマーは特定のタイムスタンプから再生
- さらに別のコンシューマーは新しいメッセージのみ（`latest`）受信

各コンシューマーの再生位置は独立しており、他のサブスクライバーに影響しません。

これにより、MQTT Streams のコンシューマー制御による再生が示されます。
...
ストリームキー式がメッセージのメタデータからクライアント ID を抽出してキーにしているため、両メッセージは同じストリームキーを持ちます。2つ目のメッセージが1つ目を上書きします。
...
これにより、MQTT Streams が Last-Value セマンティクスを用いた状態指向メッセージングパターンをサポートしていることが示されます。

## ストリームの自動作成

EMQX の MQTT Streams は、クライアントが `$stream/` プレフィックス付きトピックにサブスクライブすると自動的にストリームを作成できます。これにより、ダッシュボードで手動作成せずにストリームを動的にプロビジョニングできます。
...
6. 以下のコマンドでサブスクライブし、自動作成をトリガーします：
   ...
   手動作成ストリームとは異なり、自動作成ストリームはサブスクライブ時にトピックフィルター（この例では `demo/auto`）が必要です。
...
7. ダッシュボードの **Streams** ページで自動作成が確認できます。ストリーム一覧に "auto_stream" が表示されているはずです。

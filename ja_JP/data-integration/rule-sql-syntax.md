...
また、SQL ステートメントは式の中に [jq プログラム](https://stedolan.github.io/jq/) を埋め込むことをサポートしており、必要に応じて複雑なデータ変換を行うことができます。式は `SELECT` と `FOREACH` ステートメント内に埋め込むことができます。`SELECT` と `FOREACH` ステートメントで参照可能なフィールドについては、[データソースとフィールド](./rule-sql-events-and-fields.md)を参照してください。
...
- `SELECT` 句は出力に含めるフィールドを指定します。
  ...
  - `payload.clientid`: メッセージペイロード内のクライアントID。ペイロード内のすべてのフィールドは `payload` の下に格納されています。
    ...
    - `as` 構文は `payload.clientid` フィールドを `myclientid` として名前変更します。
...
`WHERE` 句はオプションで、`FROM` 句で指定したトピックやイベントのフィルタに加え、メッセージが満たすべき追加条件を指定してメッセージの絞り込みを行います。

例えば、トピック `t/#` 配下のメッセージのうち、ユーザー名が `eric` のものだけをフィルタリングする場合は以下のように記述します。
...
[式](#expressions-and-operations)は `SELECT` や `WHERE` 句内でデータ変換に使用できます。例えば、以下の SQL 文は `clientid` フィールドの値を大文字に変換し、サフィックスを追加して、出力メッセージのフィールド名を `cid` としています。
...
ドット記法を使って複雑な構造のペイロード内のフィールドにアクセスすることもできます（ペイロードが JSON 形式であることが前提です）。
...
`FOREACH` ステートメントは `SELECT` ステートメントのより一般的な形と見なせます。各入力メッセージに対してゼロ個以上の出力メッセージを生成できます。特定条件に基づくデータのフィルタリングや、結果の MQTT トピックやデータブリッジへの出力に使います。
...
`FOREACH` ステートメントは、`FOREACH` 句で入力メッセージから配列を作成します。`FROM` と `WHERE` 句は `SELECT` ステートメントの同名句と同じ目的で動作します。`FOREACH` ステートメントにはさらに2つのオプション句があります。
...
`FOREACH` 句以外はすべて `SELECT` ステートメントの対応する句と同じであるため、`FOREACH` ステートメントは前述の通り `SELECT` ステートメントの一般化と見なせます。以下の2つのステートメントは等価です（`jq('.', payload)` はペイロードを配列でラップしています）。
...
`FOREACH` 句の `as` 構文は配列要素に名前を付けるために使い、`DO` 句内で「現在の」要素を参照しやすくします。`as name` 部分を省略すると、デフォルト名は `item` になります。

以下は `FOREACH` ステートメントで2つの値を出力する例です。両方の値は `value` という単一フィールドのみを持ち、その値はそれぞれメッセージの `field_1` と `field_2` の値です。
...
例えば、トピック `t/#` にパブリッシュされたメッセージで、センサーの idx が 1 以上のときにタイムスタンプ、クライアントID、センサー名、インデックスを出力したい場合、以下のように記述します。
...
- `DO` 句は出力に含めるフィールドを指定しています。
  - `timestamp` は入力メッセージのメタデータからのタイムスタンプ
  - `clientid` は入力メッセージのメタデータからのクライアントID
...
- `INCASE` 句は追加のフィルタ条件を指定し、`idx` フィールドの値が 1 以上のセンサーだけを取得します。
- `FROM` 句はトピックパターン `t/#` にマッチするメッセージを対象としています。

ルールを作成したら、本番環境に投入する前に必ずテストすることを推奨します。ダッシュボードの UI にはサンプルメッセージでルールをテストできる機能があります。SQL ステートメントのテスト方法の詳細は [ルールのテスト](./rule-get-started.md#test-the-rule) を参照してください。上記のルールは以下の JSON 形式のペイロードを入力としてテストできます。
...
EMQX のルール構文では、データ変換やメッセージのフィルタリングのために式を使うことができ、`SELECT`、`FOREACH`、`DO`、`INCASE`、`WHERE` などの句で利用可能です。このセクションでは式の使い方について詳述します。以下は式を構成するために使える演算子の一覧です。また、[組み込み関数](./rule-sql-builtin-functions.md)も多数利用可能です。
...
`CASE` 式は条件付きの処理を行うために使います。`CASE` 式は他言語の if-then-else 文に相当します。以下の例で使い方を示します。
...
- ペイロードに username フィールドがあり、その値が 'Steven' のメッセージから username フィールドを抽出（`FROM` 句でのトピックフィルター `#` の使用は推奨されません。これはすべてのメッセージに対してルールがチェックされるためです）:

    ```sql
    SELECT username FROM "#" WHERE username='Steven'
    ```

- 入力メッセージのペイロードから `x` フィールドを抽出し、出力メッセージでは `y` と名前変更。`y` は `WHERE` 句でも使用可能。この SQL 文はペイロードが `{"x": 1}` のメッセージにマッチし、`{"x": 2}` にはマッチしません。

    ```sql
    SELECT payload.x as x FROM "tests/test_topic_1" WHERE y = 1
    ```

- ペイロードが `{"x": {"y": 1}}` のメッセージにマッチ（例: `{"x": {"y": 1}, "other": "field"}` も含む）:

    ```sql
    SELECT * FROM "#" WHERE payload.x.y = 1
    ```

- clientid = 'c1' の MQTT クライアントが接続した場合、そのソース IP アドレスとポート番号を抽出:
...
- ペイロードの内部フィールドにアクセスするにはピリオド（`.`）を使います。例: ネストされた JSON 構造の場合、`payload.outer_field.inner_field` で `outer_field` の中の `inner_field` にアクセス可能です。
...
`sensors` 配列内の各オブジェクトを、オブジェクトの `idx` を使ったトピック `sensors/${idx}` に、`name` の内容 `${name}` で再パブリッシュしたい場合。上記の入力例ではルールエンジンは以下の3つのメッセージを発行します。
...
この SQL 文では、`FOREACH` 句で配列 `sensors` を指定しています。`FOREACH` ステートメントは結果の配列の各オブジェクトに対して「メッセージ再パブリッシュ」アクションを実行するため、3回実行されます。

#### 例2

`sensors` 配列内の `idx` フィールドが 1 以上のオブジェクトだけを、トピック `sensors/${idx}` に再パブリッシュし、内容は `clientid=${clientid},name=${name},date=${date}` としたい場合。上記の入力例では、`idx` が 0 の要素は除外され、2つのメッセージが発行されます。
...
この SQL 文では、`FOREACH` 句で配列 `sensors` を指定し、`DO` 句で各操作に必要なフィールドを選択しています。`clientid` はメッセージのメタデータから、`name` と `idx` は現在のセンサーオブジェクトから選択されます。`item` は `sensors` 配列内の現在のオブジェクトを表します。`INCASE` 句は配列オブジェクトのフィルタ条件を指定し、条件に合わないオブジェクトは無視されます。

`DO` と `INCASE` 句内では、`item` で現在のオブジェクトにアクセスできますが、`FOREACH` 句の `as` 構文で変数名をカスタマイズすることも可能です。したがって、この例の SQL 文は以下のようにも書けます。
...
例2を拡張し、`clientid` フィールドの `c_steve` の `c_` プレフィックスを削除したい場合。

ルールエンジンには `FOREACH`、`DO`、`INCASE` 句内で呼び出せる組み込み関数が多数あります。`c_steve` を `steve` に変換したい場合、例2の SQL を以下のように変更します。
...
複数の式を `FOREACH` 句に記述可能で、最後の式が配列を指定していれば問題ありません。

例えば、入力メッセージのペイロードが以下のような構造の場合、

...
[jq](https://jqlang.org) は、主に [JSON](https://www.json.org/json-en.html) 形式でエンコードされたデータの変換およびクエリに特化した強力なコマンドラインツール兼プログラミング言語です。
...
`jq` 関数は、指定された入力に対して与えられた jq プログラムが生成するオブジェクトのリストを返します。実行がタイムアウト前に終了しない場合や jq プログラムが例外を発生させた場合は、エラーがスローされます。

## 利用例

以下は、簡単な `jq` 関数の呼び出し例とその結果です。

### JSON データの操作

この例は、JSON データのアクセス、変換、値の計算など、`jq` を使った様々な操作方法を示しています。
...
以下の JSON オブジェクトは日付と複数のセンサーを含み、それぞれに名前とデータポイントの配列があり、特定の日付のセンサー読み取り値を表しています。
...
- `FROM` 節は、[トピックフィルター](./rule-sql-events-and-fields.md) `car/measurements` にマッチするメッセージにルールを適用します。

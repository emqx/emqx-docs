...
`jq` 関数は、指定された入力に対して与えられた jq プログラムを実行し、生成されたオブジェクトのリストを返します。タイムアウト前に実行が終了しない場合や、jq プログラムが例外を発生させた場合は、関数はエラーをスローします。

## 利用例

以下は、簡単な `jq` 関数呼び出しとその結果の例です。
...
以下の JSON オブジェクトは、日付と複数のセンサーを含み、それぞれのセンサーには名前とデータポイントの配列があり、特定の日付のセンサーの測定値を表しています。
...
`jq` 関数を `FOREACH` ステートメントと組み合わせて使用し、jq の出力オブジェクトを複数のメッセージに分割できます。各メッセージには日付フィールドと、外れ値を除いたセンサーのデータフィールドの平均値が含まれます。
...
この例では、複数のセンサー測定値を含む入力メッセージを処理し、センサーの種類ごとに別々のメッセージに分割します。処理の流れは以下の通りです。

- `FOREACH` は jq 関数を使って、入力メッセージを `sensor_type` と `value` フィールドを持つオブジェクトの配列に変換します。
- `DO` 節では、出力メッセージに必要なフィールドを選択します。
...
この例は、複数のセンサー測定値を含む入力メッセージを、センサーの種類ごとに別々のメッセージに分割する別の方法を示しています。

`FOREACH` 節内の jq 関数は、入力とすべてのセンサー種類を保存し、各センサー種類ごとに関連フィールドを持つオブジェクトを出力します。

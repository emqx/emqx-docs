...
[jq](https://jqlang.org) は、主に [JSON](https://www.json.org/json-ja.html) でエンコードされたデータの変換やクエリに特化した強力なコマンドラインツールおよびプログラミング言語です。
...
以下の JSON オブジェクトは、日付と複数のセンサーを含み、各センサーは名前とデータポイントの配列を持ち、特定の日付のセンサーの測定値を表しています。
...
`jq` 関数を `FOREACH` ステートメントと組み合わせて使用すると、jq の出力オブジェクトを複数のメッセージに分割できます。各メッセージには日付フィールドと、外れ値を除いたセンサーのデータフィールドの平均値が含まれます。

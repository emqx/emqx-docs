...
例えば、`abs(X: integer | float) -> integer | float` は引数 `X` のデータ型が整数または浮動小数点数のいずれかであり、戻り値のデータ型もそれに対応して整数または浮動小数点数となることを意味します。

引数が規定の範囲を超えるか、サポートされていないデータ型を使用した場合、現在のSQL実行は失敗し、失敗回数が1増加しますのでご注意ください。
...
データ型判定関数は指定したフィールドのデータ型をチェックし、指定のデータ型に合致するかどうかを真偽値で返します。
...
変数 `Term` が未定義かどうかを判定します。この関数は変数に値が割り当てられているかを判定するために使い、値がJSONの `null` であっても未定義とはみなしません。
...
`Term` をブール型に変換します。`Term` はブール型、0または1の整数型、もしくは文字列型で `true` または `false` のみ許容されます。
...
`Term` が文字列型の場合、科学的記数法も使用可能です（例：`float('3.14e4')`）。浮動小数点数は最大16桁の有効数字をサポートします。文字列 `Term` が表す浮動小数点数の有効数字が16桁を超えると、変換時に丸め誤差が発生する可能性があります。
...
浮動小数点数 `Float` を文字列に変換します。小数点以下最大 `Decimals` 桁まで含み、末尾のゼロは切り捨てられます。`Decimals` の範囲は `[0, 253]` です。`Float` の有効数字が16桁を超えると変換時に丸め誤差が発生する可能性があります。
...
- `Term` が文字列の場合、少なくとも1つの数字を含み、先頭に単一の `+` または `-` の接頭辞を持つことができ、先行ゼロは無視されます。数学的表記もサポートします。
...
このエンコードはMicrosoft SQL ServerのようなリトルエンディアンUTF-16エンコードを利用するシステムで一般的に使用されます。
...
文字列関数は大文字・小文字変換、空白除去、部分文字列抽出、置換、エスケープ・アンエスケープなどの操作に使用できます。
...
`String` 内で部分文字列 `SearchPattern` を検索し、`SearchPattern` より前の内容をすべて削除して残りを返します。`SearchPattern` が見つからない場合は空文字列を返します。この関数は `find(String, SearchPattern, 'leading')` と同等です。
...
`String` の末尾にスペースを追加して指定長さにパディングします。例：
...
ルールエンジンは `Char` が合法なグラフェムクラスタかどうかのチェックを行わず、`Char` の文字数に関わらず1文字長として処理します。例：
...
正規表現のキャプチャグループを用いて文字列から部分抽出を行います。完全一致部分は含まず、キャプチャされたグループのみをリストで返します。マッチがなければ空リストを返します。
...
区切り文字は複数文字でも構い、それらは一つのまとまりとして扱われます。複数の区切り文字を同時に指定したい場合は `tokens` 関数を使用してください。
...
`Format` に従ってフォーマットされた文字列を返します。`Format` は通常の文字とフォーマット用の制御シーケンスを含みます。

制御シーケンスの形式は一般的に `~F.P.PadModC` です。

文字 `C` は制御シーケンスの種類を決定し、唯一必須のフィールドです。`F`, `P`, `Pad`, `Mod` はすべて任意です。詳細は https://www.erlang.org/doc/man/io.html#fwrite-1 を参照してください。
...
エスケープシーケンスを対応する文字に戻します。SQLでエスケープシーケンスを使う場合は、正しく処理するためにこの関数でアンエスケープしてください。
...
`Map` の指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。配列で複数のキーを指定すると、ネストされたマップから対応する値を取得できます。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新されたマップを返します。`Key` が既に存在する場合は値を上書きします。配列で複数のキーを指定すると、ネストされたマップにデータを挿入できます。例：
...
`Data` を解凍します。`Data` はgzヘッダと末尾にチェックサムを含む必要があります。例：
...
バイト列 `Bin` の位置 `Start`（1始まり）から長さ `BitNum` のビットを抽出し、指定のバイト順 `Endianness` と符号属性 `Signedness` に従い、指定のデータ型 `OutputType` に変換します。
...
EMQXは `schema_encode` と `schema_decode` 関数を使い、指定したスキーマに基づいて[Protobuf (Protocol Buffers)](https://developers.google.com/protocol-buffers)や[Avro](https://avro.apache.org/)データのデコード・エンコードをサポートしています。詳細は[スキーマレジストリ](./schema-registry.md)をご覧ください。
...
`FormatString` で使えるプレースホルダは以下の通りです：
...
`rfc3339_to_unix_ts/1` と同様ですが、返すUnixタイムスタンプの単位を `Unit` で指定できます。例：
...
タイムゾーンオフセット文字列を秒単位の整数に変換します。サポートされる表現は以下の通り：

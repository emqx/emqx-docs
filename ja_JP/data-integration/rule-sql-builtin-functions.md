...
ルールエンジンはさまざまな組み込み関数を提供しています。これらの関数はSQL内で利用でき、以下の基本的なデータ処理を実現できます。
...
例えば、`acos(X: integer | float) -> float` は引数 `X` のデータ型が整数または浮動小数点数であり、戻り値のデータ型は浮動小数点数であることを意味します。

引数が指定範囲を超えるか、サポートされていないデータ型の場合、現在のSQL実行は失敗し、失敗回数が1増加しますのでご注意ください。
...
:::tip
浮動小数点数の絶対値には、代わりに `ceil` または `floor` 関数を使用してください。
:::
...
変数 `Term` が未定義かどうかを判定します。この関数は変数に値が割り当てられているかどうかを判定しますが、値がJSONの `null` であっても未定義とはみなしません。
...
`Term` が整数型または浮動小数点数型かどうかを判定します。例：
...
`Term` をブール型に変換します。`Term` はブール型、0または1の整数型、または文字列型の `true` / `false` のみ許容されます。
...
`Term` が文字列型の場合、科学的記数法も使用可能です（例：`float('3.14e4')`）。浮動小数点数は最大16桁の有効数字をサポートします。文字列で表現された浮動小数点数の有効数字が16桁を超える場合、変換時に丸め誤差が発生する可能性があります。
...
浮動小数点数はコンピュータ上で正確に保持できないため、`Decimals` が `Float` の小数点以下の桁数（先行ゼロ含む）より大きい場合、`float2str` は `Float` の2進近似値の10進表現を返すことがあります。
...
- `Term` が文字列の場合、少なくとも1つの数字文字を含み、先頭に `+` または `-` の1文字の接頭辞が付くことができ、先行ゼロは無視されます。数学的表記もサポートされます。
...
UTF-16リトルエンディアンエンコードされた文字列はJSONオブジェクト内で正しく表示されないことがあります。EMQXでは通常バイナリデータとして扱われます。16進数文字列として読みやすく変換するには `bin2hexstr` 関数を使用してください。
...
文字列の大文字・小文字変換、空白除去、部分文字列抽出、置換、エスケープ・アンエスケープなどに使用できます。

### ascii(Char: string) -> integer

文字 `Char` に対応するASCIIコードを返します。`Char` に複数文字が含まれる場合は最初の1文字のコードのみ返します。例：
...
`String` 内で部分文字列 `SearchPattern` を検索し、`SearchPattern` より前の内容を削除して残りの文字列を返します。`SearchPattern` が見つからない場合は空文字列を返します。この関数は `find(String, SearchPattern, 'leading')` と同等です。
...
`find/2` と同様ですが、`Direction` により検索方向を指定できます。例：
...
`pad/2` と同様ですが、`Direction` によりパディング方向を指定できます。`leading` は前方にスペースを埋め、`trailing` は後方に、`both` は前後両方に埋めます。

`both` 指定時、埋めるスペース数が奇数の場合は最後のスペースが後方に入ります。
...
`pad/3` と同様ですが、指定したグラフェムクラスター `Char` でパディングします。

ルールエンジンは `Char` が合法なグラフェムクラスターかどうかをチェックしないため、`Char` が複数文字でも1文字分として処理されます。例：
...
正規表現 `Expression` にマッチした部分を文字列 `Replacement` に置換します。マッチがない場合は元の文字列を返します。例：
...
正規表現パターン（キャプチャグループ付き）を文字列に対して非グローバルに検索し、完全一致部分を除くキャプチャグループのリストを返します。

マッチがあればすべてのキャプチャグループをリストで返し、マッチなしまたはグループなしの場合は空リストを返します。
...
隣接する複数の区切り文字は1つとはみなされず、空文字列が結果に含まれることがあります。`split/2` はデフォルトで結果をトリムし空文字列を除外します。空文字列も保持したい場合は `split(String, Separator, 'notrim')` を使用してください。

区切り文字は複数文字でも構い、全体として扱われます。複数の区切り文字を同時に指定したい場合は `tokens` 関数を使用してください。
...
`split/2` と同様ですが、`Option` により区切り文字の処理位置や空文字列の返却を指定できます。
...
`C` は制御シーケンスの種類を決定し必須です。`F`, `P`, `Pad`, `Mod` はすべて任意です。詳細は https://www.erlang.org/doc/apps/stdlib/io.html#fwrite-1 を参照してください。
...
`String` の `Start` 位置（0始まり）から最大長さ `Length` の部分文字列を返します。例：
...
`tokens/2` と同様ですが、`NoCRLF` に `nocrlf` を指定するとキャリッジリターンと改行文字も同時に区切り文字として扱います。例：
...
- 16進エスケープコード：
  ...
  - `\xH...` （`H...` は1文字以上の16進数、UTF-32文字の任意エンコードを可能にします）

認識されないエスケープシーケンスや無効なUnicode文字の16進エスケープは例外をスローします。
...
例：`SELECT map_to_redis_hset_args(payload.value) as hset_fields FROM t/1` のように使用し、Redisアクションのテンプレートで `HMSET name1 ${hset_fields}` と組み合わせて利用します。
...
`Map` の指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。配列で複数キーを指定すると、ネストされたマップから対応する値を取得できます。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新されたマップを返します。`Key` が既に存在する場合は値を上書きします。配列で複数キーを指定すると、ネストされたマップにデータを挿入できます。例：
...
`Array` のN番目の要素を返します。`N` は `Array` の長さを超えてはなりません。例：
...
配列 `Array` の最初の要素から最大長さ `Length` の部分配列を返します。`Length` が `Array` の長さを超える場合は全体を返します。例：
...
`sublist/2` と同様ですが、`Start` で返し始める要素位置を指定できます。`Start` + `Length` が `Array` の長さを超える場合は全体を返します。例：
...
任意長の文字列 `String` の128ビット固定長MD5ハッシュ値を計算します。ハッシュ値は32桁の16進数文字列（小文字）で返します。
...
任意長の文字列 `String` の160ビット固定長SHA-1ハッシュ値を計算します。ハッシュ値は40桁の16進数文字列（小文字）で返します。
...
任意長の文字列 `String` の256ビット固定長SHA-2ハッシュ値を計算します。ハッシュ値は64桁の16進数文字列（小文字）で返します。
...
SHA-256で `Value` をハッシュ化し、ハッシュ値を `[Min, Max]` の範囲内の整数にマッピングします。`Min` は `Max` 以下である必要があります。

メッセージフィールドから安定したバケットやシャード番号を取得する際に便利です。例えば、トピックのセグメントでデバイスを複数のルールやアクションに分散する場合など。
...
注意：バイナリデータはJSONに直接エンコードできないため、16進数文字列に変換するには `bin2hexstr` 関数を使用してください。
...
`Num1` と `Num2` のビットごとのAND結果を返します。入力・出力は符号付き整数です。例：
...
`Num` のビットごとの否定結果を返します。入力・出力は符号付き整数です。例：
...
`Num1` と `Num2` のビットごとのOR結果を返します。例：
...
`Num1` と `Num2` のビットごとのXOR結果を返します。例：
...
`binary` 型はバイト列を表し、1バイトは8ビットなので、バイナリのビット数は8の倍数でなければなりません。`bitstring` 型は任意長のビット列を表します。

簡単に言うと、すべての `binary` は `bitstring` ですが、逆は必ずしも真ではありません。

`bitstring` は長さが8の倍数でない場合、JSONなどの外部形式に直接シリアライズできません。

通常は整数や他の適切な型に変換する前の中間値として使われます。
...
バイト列 `Bin` の先頭から長さ `BitNum` のビットを取得し、ビッグエンディアンで符号なし整数に変換します。これは `subbits(Bytes, 1, BitNum, 'integer', 'unsigned', 'big')` と同等です。
...
バイト列 `Bin` の位置 `Start`（1始まり）から長さ `BitNum` のビットを取得し、ビッグエンディアンで符号なし整数に変換します。これは `subbits(Bytes, Start, BitNum, 'integer', 'unsigned', 'big')` と同等です。
...
バイト列 `Bin` の位置 `Start`（1始まり）から長さ `BitNum` のビットを取得し、指定されたバイト順 `Endianness` と符号属性 `Signedness` に従い、指定型 `OutputType` に変換します。
...
- `no_padding`: パディング文字 `=` を期待せずにデコードします。パディングなしbase64に便利です。
- `urlsafe`: URLセーフなbase64（`+` と `/` の代わりに `-` と `_`）としてデコードします。

複数オプションは個別または組み合わせて使用でき、順序は結果に影響しません。
...
- `urlsafe`: URLセーフなbase64形式でエンコードします（`+` と `/` の代わりに `-` と `_` を使用）。
...
`DateTimeString` にタイムゾーンオフセットが含まれない場合、`Offset` で手動指定できます。その他の挙動は `date_to_unix_ts/3` と同様です。`Offset` は文字列または秒数を表す整数で指定可能です。
...
`date_to_unix_ts/3, 4` の `Unit`, `Offset`, `FormatString` と同様の値が使えます。
...
現在のシステム時刻を秒単位のRFC3339日時文字列として返します。例：
...
現在のシステム時刻を秒単位のUnixタイムスタンプとして返します。例：
...
`unix_ts_to_rfc3339/1` と同様ですが、`Unit` で時間単位を指定できます。例：
...
現在時刻をMongoDBのISODate型または文字列で返します。MongoDB関連のアクションやSQLテストでのみサポートされ、SQLテストでは文字列（例：`ISODate("2024-02-23T15:00:00.123Z")`）を返します。文字列以外は他の関数の入力としては現在サポートされていません。
...
例えば、`coalesce(payload.value, 0)` は `payload.value` がnullでなければその値を、nullであれば0を返します。

...
例えば、`abs(X: integer | float) -> integer | float` は、引数 `X` のデータ型が整数または浮動小数点数であり、戻り値の型もそれに対応して整数または浮動小数点数であることを意味します。

指定された引数が範囲外であったり、サポートされていないデータ型の場合は、現在のSQL実行が失敗し、失敗回数が1増加しますのでご注意ください。

:::tip

1. 一部のエスケープシーケンスは使用時にアンエスケープが必要です。詳細は[unescape関数](#unescapestring-string---string)を参照してください。  
2. EMQX 5.0以降では、複雑なデータ変換に[jq構文](https://stedolan.github.io/jq/manual/)もサポートしています。詳細は[jq関数](./rule-sql-jq.md)を参照してください。
...
指定したフィールドのデータ型を判定し、指定のデータ型に合致するかどうかを真偽値で示します。
...
この関数は変数に値が割り当てられているかを判定するために使いますが、値がJSONの `null` である場合は未定義とはみなしません。
...
`Term` が文字列の場合、指数表記も使用可能です（例：`float('3.14e4')`）。浮動小数点数は最大16桁の有効数字をサポートします。文字列で表される浮動小数点数の有効数字が16桁を超える場合、変換時に丸め誤差が生じる可能性があります。
...
浮動小数点数はコンピュータ上で正確に保存できないため、`Decimals` が `Float` の小数点以下の桁数（先行ゼロ含む）より大きい場合、`float2str` は `Float` の2進近似値の10進表現を返すことがあります。
...
- `Term` が文字列の場合、少なくとも1つの数字文字を含み、先頭に `+` または `-` の1文字の接頭辞があってもよく、先行ゼロは無視されます。数学的表記もサポートされます。  
...
このエンコードはMicrosoft SQL Serverなど、リトルエンディアンUTF-16エンコードを使用するシステムで一般的に使われます。
...
文字 `Char` のASCIIコードを返します。`Char` に複数文字が含まれている場合は最初の1文字のコードのみ返します。例：
...
`String` 内で部分文字列 `SearchPattern` を検索し、`SearchPattern` より前の内容をすべて削除して残りの文字列を返します。`SearchPattern` が見つからなければ空文字列を返します。この関数は `find(String, SearchPattern, 'leading')` と同等です。
...
`both` 指定時、詰める空白数が奇数の場合は末尾に1つ多く詰めます。
...
本関数はEMQX v5.7.1以降で導入されました。

:::

正規表現のキャプチャグループを用いて、文字列から部分文字列を非グローバルに検索し抽出します。完全一致部分は含みません。

マッチがあればキャプチャされたすべてのグループのリストを返し、マッチがなければ空リストを返します。
...
2つ以上連続する区切り文字は1つとして扱われません。そのため分割結果に空文字列が含まれる場合があります。`split/2` はデフォルトで結果をトリムし空文字列を除外します。空文字列を残したい場合は `split(String, Separator, 'notrim')` を使ってください。

`Separator` は複数文字でも構い、それらは一体として扱われます。複数の区切り文字を同時に指定したい場合は `tokens` 関数を使ってください。
...
`Format` に従い文字列をフォーマットして返します。`Format` 文字列は通常文字とフォーマット制御シーケンスを含みます。
...
`String` の位置 `Start` から長さ `Length` の部分文字列を返します。添字は0始まりです。例：
...
2つ以上連続する区切り文字は1つとして扱われ、空文字列は発生しません。
...
エスケープシーケンスを元の文字に戻します。SQL内でエスケープシーケンスを使う場合は、処理のためにまずこの関数でアンエスケープしてください。

::: tip

本関数はEMQX v5.7.0以降で導入されました。
...
- 16進エスケープコード：
  ...
  - `\xH...`（`H...` は1文字以上の16進数文字（0-9, A-F, a-f））、任意のUTF-32文字をエンコード可能

認識できないエスケープシーケンスや、16進エスケープが有効なUnicode文字にならない場合は例外をスローします。
...
`Map` に `Key` と対応する `Value` を挿入し、更新後のマップを返します。`Key` が既に存在する場合は古い値を新しい値で置き換えます。例：
...
例：`SELECT map_to_redis_hset_args(payload.value) as hset_fields FROM t/1` のように使い、`hset_fields` をRedisアクションのコマンドテンプレート `HMSET name1 ${hset_fields}` に組み込みます。
...
`Map` の指定した `Key` の値を返します。`Key` が存在しなければ `undefined` を返します。配列で複数のキーを指定すると、ネストされたマップから対応する値を取得できます。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新後のマップを返します。`Key` が既に存在する場合は古い値を新しい値で置き換えます。配列で複数のキーを指定すると、ネストされたマップにデータを挿入できます。例：
...
`sublist/2` と同様ですが、`Start` で返し始める要素の位置を指定できます。`Start` + `Length` が配列長を超える場合は全体を返します。例：
...
`Num` を左に `Shift` ビットシフトし、右端に0を詰めます。例：
...
ルールエンジンはビット列操作用の関数を提供します。例えば `subbits` はビット列から指定長のビットを抽出し、指定のデータ型に変換します。
...
バイト列 `Bin` の位置 `Start`（1始まり）から `BitNum` ビットを抽出し、ビッグエンディアンの符号なし整数に変換します。これは `subbits(Bytes, Start, BitNum, 'integer', 'unsigned', 'big')` と同等です。
...
バイト列 `Bin` の位置 `Start`（1始まり）から `BitNum` ビットを抽出し、指定のバイト順 `Endianness` と符号属性 `Signedness` に従い、指定の型 `OutputType` に変換します。
...
任意のバイナリデータをMicrosoft SQL Serverのバイナリ型に変換します。すなわち、`0x` プレフィックス付きのHEXエンコード文字列です。

::: tip

本関数はMicrosoft SQL Serverの `CONVERT` 関数と組み合わせて、UTF-8非対応のSQL ServerバージョンにUTF-16リトルエンディアンエンコードのUnicode文字列を書き込む際に利用できます。
...
EMQXは `schema_encode` と `schema_decode` 関数を使い、指定したスキーマに従い [Protobuf (Protocol Buffers)](https://developers.google.com/protocol-buffers) や [Avro](https://avro.apache.org/) データのデコード・エンコードをサポートしています。詳細は[スキーマレジストリ](./schema-registry.md)を参照してください。
...
EMQXはSparkplug Bメッセージのデコード・エンコード用に特別な関数（`sparkplug_decode` と `sparkplug_encode`）も備えています。詳細は[Sparkplug B](./sparkplug.md)を参照してください。
...
`DateTimeString` にタイムゾーンオフセットが含まれない場合、`Offset` で手動指定できます。その他の挙動は `date_to_unix_ts/3` と同じです。`Offset` は文字列または秒数を表す整数です。
...
`rfc3339_to_unix_ts/1` と同様ですが、返すUnixタイムスタンプの単位を `Unit` で指定できます。`second`, `millisecond`, `microsecond`, `nanosecond` をサポート。例：
...
秒単位UnixタイムスタンプをシステムのローカルタイムゾーンのRFC3339準拠日時文字列に変換します。例：
...
現在時刻をMongoDBのISODate型または文字列で返します。MongoDB関連アクションとSQLテストでのみサポートされます。SQLテストでは文字列（例：`ISODate("2024-02-23T15:00:00.123Z")`）を返します。文字列以外の戻り値は他の関数の入力としては現在サポートされていません。
...
- OS環境変数を読み込む際に `EMQXVAR_` プレフィックスが付加されます。例えば、`getenv('FOO_BAR')` は `EMQXVAR_FOO_BAR` を読み込みます。  
- OS環境から読み込んだ値は不変です。
...
例えば、`coalesce(payload.value, 0)` は `payload.value` がnullでなければその値を返し、nullなら0を返します。SQL式 `CASE WHEN is_null(payload.value) THEN 0 ELSE payload.value END` と同等ですが簡潔です。

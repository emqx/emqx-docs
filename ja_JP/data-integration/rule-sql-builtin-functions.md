...
ルールエンジンはさまざまな組み込み関数を提供しています。これらの関数はSQL内で利用でき、基本的なデータ処理を実現できます。以下のカテゴリがあります：
...
データ型判定関数は指定したフィールドのデータ型をチェックし、そのフィールドが指定したデータ型に合致するかどうかを真偽値で返します。
...
この関数は変数に値が割り当てられているかどうかを判定するもので、値がJSONの `null` であっても未定義とはみなしません。
...
`Term` が整数型または浮動小数点型かどうかを判定します。例：
...
`Term` が文字列の場合、指数表記も使用可能です（例：`float('3.14e4')`）。浮動小数点数は最大16桁の有効数字をサポートします。文字列で表現された浮動小数点数の有効数字が16桁を超える場合、変換時に丸め誤差が発生することがあります。
...
浮動小数点数 `Float` を文字列に変換します。小数点以下最大 `Decimals` 桁まで含み、末尾のゼロは切り捨てられます。`Decimals` の範囲は `[0, 253]` です。`Float` の有効数字が16桁を超える場合、変換時に丸め誤差が発生することがあります。

浮動小数点数はコンピュータ上で正確に保存できないため、`Decimals` が `Float` の小数点以下の桁数（先頭のゼロも含む）より大きい場合、`float2str` は `Float` の2進近似値の10進表現を返すことがあります。
...
- `Term` がブール型の場合、true は 1 に、false は 0 に変換されます。
- `Term` が浮動小数点型の場合、`Term` 以下の最大の整数に切り捨てられます。
- `Term` が文字列の場合、少なくとも1つの数字を含み、先頭に単一の `+` または `-` 記号を付けられます。先頭のゼロは無視されます。数学的表記もサポートします。
...
UTF-16リトルエンディアンエンコードされた文字列はJSONオブジェクト内で正しく表示されない場合があります。EMQXでは通常バイナリデータとして扱われます。読みやすい16進数文字列に変換するには `bin2hexstr` 関数を使用してください。  
このエンコードはMicrosoft SQL Serverなど、リトルエンディアンUTF-16を使うシステムで一般的に使用されます。
...
文字列関数は大文字・小文字変換、空白除去、部分文字列抽出、置換、エスケープ／アンエスケープなどに利用できます。

### ascii(Char: string) -> integer

文字 `Char` のASCIIコードを返します。`Char` が複数文字の場合は最初の1文字のコードのみ返します。例：
...
`String` 内で部分文字列 `SearchPattern` を検索し、`SearchPattern` より前の部分を削除して残りを返します。`SearchPattern` が見つからない場合は空文字列を返します。`find(String, SearchPattern)` は `find(String, SearchPattern, 'leading')` と同等です。
...
`String` の末尾にスペースを追加して長さを `Length` に揃えます。例：
...
`pad/2` と同様ですが、`Direction` でパディングの方向を指定できます。`leading` は先頭にスペースを追加、`trailing` は末尾に追加、`both` は両端に追加します。
...
ルールエンジンは `Char` が合法なグラフェムクラスタかどうかをチェックしないため、`Char` の文字数に関わらず1文字として処理されます。例：
...
正規表現のキャプチャグループを使って文字列から部分抽出します。完全一致部分は除外されます。

マッチがあればキャプチャされたすべてのグループをリストで返し、マッチなしまたはグループなしの場合は空リストを返します。
...
2つ以上連続した区切り文字は1つとして扱われず、空文字列が結果に含まれる場合があります。`split/2` は出力結果をトリムし空文字列を除外します。空文字列を残したい場合は `split(String, Separator, 'notrim')` を使ってください。

`Separator` は複数文字で構成可能ですが、全体で1つの区切り文字として扱われます。複数の区切り文字を同時に指定したい場合は `tokens` 関数を使ってください。
...
`C` は制御シーケンスの種類を決定し、必須です。`F`, `P`, `Pad`, `Mod` はすべて任意です。詳細は https://www.erlang.org/doc/apps/stdlib/io.html#fwrite-1 を参照してください。
...
`Map` から指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。例：
...
`Map` に `Key` と関連付けられた `Value` を挿入し、更新されたマップを返します。既存の `Key` があれば値を上書きします。例：
...
例：`SELECT map_to_redis_hset_args(payload.value) as hset_fields FROM t/1` のように使用し、Redisアクションコマンドテンプレートで `HMSET name1 ${hset_fields}` として利用します。

例えば、`payload.value` が `{"a" : 1, "b": 2}` の場合、生成されるコマンドは `HMSET name1 b 2 a 1` となります。マップの順序は非決定的です。
...
`Map` から指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。配列を指定してネストしたマップから複数のキーを一度に取得可能です。例：
...
`Map` に `Key` と関連付けられた `Value` を挿入し、更新されたマップを返します。既存の `Key` があれば値を上書きします。配列を指定してネストしたマップに一度に複数のキーを挿入可能です。例：
...
配列 `Array` の `N` 番目の要素を返します。`N` は配列の長さを超えてはなりません。例：
...
配列 `Array` の先頭から最大 `Length` 要素の部分配列を返します。`Length` が配列長を超える場合は配列全体を返します。例：
...
注：バイナリデータは直接JSONエンコードできないため、16進数文字列に変換するには `bin2hexstr` 関数を呼び出す必要があります。

### gunzip(Data: binary) -> binary | string

`Data` を解凍します。`Data` はgzヘッダーと末尾にチェックサムを含む必要があります。例：
...
DEFLATEアルゴリズムで `Data` を圧縮し、返り値はzlibヘッダーと末尾のチェックサムを含みます。例：
...
ルールエンジンはビット列操作用の関数を提供しています。例えば `subbits` はビット列から指定した長さのビットを抽出し、指定したデータ型に変換します。

:::tip

`binary` 型はバイト列を表し、各バイトは8ビットで構成されるため、`binary` のビット数は8の倍数でなければなりません。`bitstring` 型は任意のビット数のビット列を表します。

つまり、すべての `binary` は `bitstring` ですが、逆は必ずしも真ではありません。

`bitstring` は長さが8の倍数でない場合、JSONなどの外部形式に直接シリアライズできません。通常は整数など適切な型に変換する前の中間値として使われます。
...
バイト列 `Bin` の位置 `Start`（1始まり）から長さ `BitNum` のビットを取得し、ビッグエンディアンの符号なし整数に変換します。これは `subbits(Bytes, Start, BitNum, 'integer', 'unsigned', 'big')` と同等です。
...
バイト列 `Bin` の位置 `Start`（1始まり）から長さ `BitNum` のビットを取得し、指定したバイト順 `Endianness` と符号属性 `Signedness` に従い、指定した型 `OutputType` のデータに変換します。
...
オプション付きで `Data` をBase64形式にエンコードします。
...
任意のバイナリデータをMicrosoft SQL Serverのバイナリ型（`0x` プレフィックス付き16進数文字列）に変換します。

::: tip

この関数はMicrosoft SQL Serverの `CONVERT` 関数と組み合わせて使用し、UTF-16リトルエンディアンエンコードされたUnicode文字列をUTF-8非対応のSQL Serverバージョンに書き込む際に利用できます。
...
EMQXはSparkplug Bメッセージのデコード・エンコード用に特別な関数（`spb_decode` と `spb_encode`）も用意しています。詳細は [Sparkplug B](./sparkplug.md) をご覧ください。

## 日時変換関数

### date_to_unix_ts(Unit: string, FormatString: string, DateTimeString: string) -> integer

日時文字列 `DateTimeString` をフォーマット文字列 `FormatString` に従って解析し、指定した単位 `Unit` のUnix時間に変換します。
...
Unix時間 `Time` を指定のフォーマット文字列 `FormatString` の日時文字列に変換します。`Unit` はUnix時間の単位、`Offset` は出力日時のタイムゾーンオフセットを表します。
...
`rfc3339_to_unix_ts/1` と同様ですが、`Unit` で返すUnixタイムスタンプの単位を指定できます。`second`, `millisecond`, `microsecond`, `nanosecond` をサポート。例：
...
指定したミリ秒単位のUnixタイムスタンプをMongoDBのISODate型または文字列に変換します。その他の動作は `mongo_date/0` と同じです。
...
指定したUnixタイムスタンプをMongoDBのISODate型または文字列に変換します。`Unit` で入力タイムスタンプの単位を指定できます。その他の動作は `mongo_date/0` と同じです。
...
例えば、`coalesce(payload.value, 0)` は `payload.value` がnullでなければその値を、nullなら0を返します。SQL式の `CASE WHEN is_null(payload.value) THEN 0 ELSE payload.value END` と同等ですが簡潔です。

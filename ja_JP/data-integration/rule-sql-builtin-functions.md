...
- [日付・時刻変換関数](#date-and-time-conversion-functions)
...
例えば、`acos(X: integer | float) -> float` は引数 `X` のデータ型が整数または浮動小数点数であり、返り値の型は浮動小数点数であることを示します。
...
変数 `Term` が未定義か判定します。値がJSONの `null` であっても値が割り当てられているとみなします。
...
`Term` が文字列の場合、科学的記数法が使用可能です（例：`float('3.14e4')`）。浮動小数点数は最大16桁の有効数字をサポートします。文字列で表現された浮動小数点数の有効数字が16桁を超えると、変換時に丸め誤差が発生する可能性があります。
...
- ブール型の場合、trueは1、falseは0に変換されます。  
- 浮動小数点型の場合、`Term` 以下の最大の整数に切り捨てられます。  
- 文字列の場合、少なくとも1つの数字を含み、先頭に単一の `+` または `-` が付くことができ、先行ゼロは無視されます。数学的表記もサポートします。  
- 整数の場合、そのまま返されます。
...
- `Term` が浮動小数点数の場合、末尾のゼロを切り捨てた対応する文字列を返します。返される文字列は小数点以下最大10桁まで保持します。より多くの小数点以下桁数を返すには `float2str` を使用してください。
...
`String` 内で部分文字列 `SearchPattern` を検索し、`SearchPattern` までの前半を削除して残りを返します。`SearchPattern` が見つからない場合は空文字列を返します。`find(String, SearchPattern)` は `find(String, SearchPattern, 'leading')` と同義です。
...
`String` の末尾に空白を追加して指定長 `Length` にパディングします。例：
...
`pad/2` と同様ですが、`Direction` でパディング方向を指定できます。`leading` は先頭に空白、`trailing` は末尾に空白、`both` は両端に空白を追加します。

`both` 指定時、パディング数が奇数の場合は最後の空白が末尾に追加されます。
...
ルールエンジンは `Char` が合法なグラフェムクラスタかをチェックしないため、`Char` の長さに関わらず1文字として処理されます。例：
...
正規表現パターン（キャプチャグループ付き）を文字列から非グローバルに検索し、キャプチャした部分を配列で返します。完全一致部分は含みません。
...
`String` の先頭にある `Prefix` を削除します。`Prefix` が先頭にない場合は元の文字列を返します。例：
...
連続する区切り文字は1つとして扱われず、空文字列が結果に含まれる場合があります。`split/2` はデフォルトで結果をトリムし空文字列を除去します。空文字列を残したい場合は `split(String, Separator, 'notrim')` を使います。

区切り文字は複数文字でも可能ですが一塊として扱われます。複数の区切り文字を指定したい場合は `tokens` 関数を使ってください。
...
`Format` に従ってフォーマットした文字列を返します。`Format` は通常文字と制御シーケンスで構成されます。

制御シーケンスの形式は一般に `~F.P.PadModC` です。

`C` は制御シーケンスの種類を決定し、必須です。`F`, `P`, `Pad`, `Mod` は省略可能です。詳細は https://www.erlang.org/doc/apps/stdlib/io.html#fwrite-1 を参照してください。
...
`String` の位置 `Start` から最大 `Length` 文字の部分文字列を返します。添字は0始まりです。例：
...
エスケープシーケンスを元の文字に戻します。SQLでエスケープシーケンスを使う場合はこの関数でアンエスケープしてから処理してください。
...
`unescape` 関数でアンエスケープすれば期待通りの結果が得られます：
...
- 16進エスケープコード：
  ...
  - `\xH...`（`H...` は1つ以上の16進数字（0-9, A-F, a-f）で任意のUTF-32文字をエンコード可能）
...
`Map` に `Key` と対応する `Value` を挿入し、更新済みのマップを返します。`Key` が既に存在する場合は旧値を新しい `Value` に置き換えます。例：
...
例：`SELECT map_to_redis_hset_args(payload.value) as hset_fields FROM t/1` のように使用し、Redisアクションコマンドテンプレートで `HMSET name1 ${hset_fields}` のように利用します。

例えば、`payload.value` が `{"a" : 1, "b": 2}` の場合、コマンドは `HMSET name1 b 2 a 1` となります。マップのフィールド順序は非決定的です。
...
`Map` の指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。配列で複数キーを指定するとネストしたマップから値を取得できます。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新済みのマップを返します。`Key` が既に存在する場合は旧値を新しい値に置き換えます。配列で複数キーを指定するとネストしたマップにデータを挿入できます。例：
...
注：バイナリデータは直接JSONエンコードできません。`bin2hexstr` 関数で16進文字列に変換してください。
...
`binary` 型はバイト列を表し、1バイトは8ビットなので長さは8の倍数です。`bitstring` 型は任意長のビット列を表します。

簡単に言うと、すべての `binary` は `bitstring` ですが、逆は必ずしも真ではありません。

`bitstring` は長さが8の倍数でない場合、JSONなど外部フォーマットに直接シリアライズできません。

通常は整数など適切な型に変換する前の中間値として使います。
...
バイト列 `Bin` の先頭から `BitNum` ビットを抽出し、ビッグエンディアンの符号なし整数に変換します。`subbits(Bytes, 1, BitNum, 'integer', 'unsigned', 'big')` と同義です。
...
バイト列 `Bin` の位置 `Start`（1始まり）から `BitNum` ビットを抽出し、ビッグエンディアンの符号なし整数に変換します。`subbits(Bytes, Start, BitNum, 'integer', 'unsigned', 'big')` と同義です。
...
バイト列 `Bin` の位置 `Start`（1始まり）から `BitNum` ビットを抽出し、指定したバイト順 `Endianness` と符号属性 `Signedness` に従い、指定型 `OutputType` に変換します。
...
- `no_padding`：パディング文字 `=` を期待せずにデコードします。パディング省略のBase64文字列に有効です。  
...
- `no_padding`：パディング文字 `=` を付加せずにエンコードします。  
- `urlsafe`：URLセーフBase64形式でエンコードします（`+` と `/` の代わりに `-` と `_` を使用し、URLに埋め込みやすくなります）。
...
この関数はMicrosoft SQL Serverの `CONVERT` 関数と組み合わせて、UTF-8非対応のSQL ServerバージョンにUTF-16リトルエンディアンエンコードされたUnicode文字列を書き込む際に利用可能です。
...
EMQXは指定したスキーマに従い、[Protobuf (Protocol Buffers)](https://developers.google.com/protocol-buffers) および [Avro](https://avro.apache.org/) データをデコード・エンコードする `schema_encode` と `schema_decode` 関数もサポートしています。詳細は[スキーマレジストリ](./schema-registry.md)をご覧ください。
...
指定したUnixタイムスタンプをMongoDBのISODate型または文字列に変換します。`Unit` で入力タイムスタンプの単位を指定可能です。その他は `mongo_date/0` と同様です。
...
- OS環境変数を読む際は `EMQXVAR_` プレフィックスが付加されます。例えば、`getenv('FOO_BAR')` は `EMQXVAR_FOO_BAR` を読みます。  
- OS環境から読み込んだ値は不変です。
...
`Value1` がnullの場合に `Value2` を返します。データフィールドがnullかどうかを判定してデフォルト値に置き換える際に便利です。

...
ルールエンジンは多様な組み込み関数を提供しています。これらの関数はSQL内で利用でき、基本的なデータ処理を実現します。主に以下のカテゴリがあります：
...
例えば、`abs(X: integer | float) -> integer | float` は、引数 `X` のデータ型が整数または浮動小数点数のいずれかであり、戻り値の型も対応して整数または浮動小数点数であることを示します。
...
2. EMQX 5.0以降、複雑なデータ変換に [jq構文](https://stedolan.github.io/jq/manual/) の使用もサポートしています。詳細は [jq関数](./rule-sql-jq.md) セクションをご覧ください。

:::

## 数学関数

EMQXは以下の幅広い数学関数をサポートしています：
...
データ型判定関数は指定したフィールドのデータ型を判定し、指定のデータ型に合致するかどうかを真偽値で返します。
...
`Term` が文字列の場合、指数表記も使用可能です（例：`float('3.14e4')`）。浮動小数点型は最大16桁の有効数字をサポートします。有効数字が16桁を超える場合、変換時に丸め誤差が発生する可能性があります。
...
浮動小数点数はコンピューター上で正確に保存できないため、`Decimals` が `Float` の小数点以下桁数（先行ゼロ含む）より大きい場合、`float2str` は `Float` の2進近似値の10進表現を返すことがあります。
...
- `Term` が文字列の場合、少なくとも1つの数字を含み、先頭に単一の `+` または `-` が付くことができ、先行ゼロは無視されます。数学的表記もサポートされます。
...
UTF-16リトルエンディアンエンコード文字列はJSONオブジェクト内で正しく表示されない場合があります。EMQXでは通常バイナリデータとして扱われます。これを16進数文字列に変換するには `bin2hexstr` 関数を使用してください。  
...
文字 `Char` に対応するASCIIコードを返します。`Char` に複数文字が含まれる場合は最初の文字のコードのみ返します。例：
...
`String` 内で部分文字列 `SearchPattern` を検索し、`SearchPattern` より前の内容をすべて削除して残りの文字列を返します。`SearchPattern` が見つからない場合は空文字列を返します。`find(String, SearchPattern)` は `find(String, SearchPattern, 'leading')` と同義です。
...
`trim/1` と同様ですが、`String` の先頭の空白文字のみを削除します。例：
...
`pad/2` と同様ですが、`Direction` でパディング方向を指定できます。`leading` は前方にスペースを埋め、`trailing` は後方に埋め、`both` は前後両方に埋めます。

`both` 指定時、埋めるスペース数が奇数の場合、最後のスペースは後方に埋められます。
...
正規表現パターンにキャプチャグループが含まれる場合に、非グローバル検索で文字列から抽出します。完全一致部分は除外し、キャプチャされたグループのリストを返します。マッチしない場合やキャプチャがない場合は空リストを返します。
...
- `all`: すべての `SearchPattern` を置換（`replace/3` と同義）
...
`trim/1` と同様ですが、`String` の末尾の空白文字のみを削除します。例：
...
隣接する複数の区切り文字は1つとはみなされず、空文字列が結果に含まれることがあります。`split/2` は出力結果のトリムと空文字列の除去をデフォルトで行います。空文字列を保持したい場合は `split(String, Separator, 'notrim')` を使用してください。

区切り文字は複数文字でもよいですが、1つのまとまりとして扱われます。複数の区切り文字を同時に指定したい場合は `tokens` 関数を使用してください。
...
`Format` に従ってフォーマットされた文字列を返します。`Format` 文字列は通常文字と制御シーケンスを含みます。

制御シーケンスの形式は一般に `~F.P.PadModC` です。

文字 `C` は制御シーケンスの種類を決定し、必須項目です。`F`、`P`、`Pad`、`Mod` は任意です。詳細は https://www.erlang.org/doc/man/io.html#fwrite-1 を参照してください。
...
`String` の位置 `Start` から末尾までの部分文字列を返します。文字列のインデックスは0始まりです。例：
...
`String` の位置 `Start` から最大長 `Length` の部分文字列を返します。インデックスは0始まりです。例：
...
`tokens/2` と同様ですが、`NoCRLF` に `nocrlf` を指定すると改行コード（CR, LF）も同時に分割します。例：
...
`String` の先頭と末尾から空白文字（スペース、タブ、フォームフィード、改行文字など）を削除します。`\r\n` はUnicode標準でグラフェムクラスタとみなされるため、まとめて削除されます。例：
...
- 16進エスケープコード：
  ...
  - `\xH...` （`H...` は1文字以上の16進数、任意のUTF-32文字をエンコード可能）
...
`Map` の指定した `Key` に対応する値を返します。`Key` が存在しない場合は `undefined` を返します。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新されたマップを返します。`Key` が既に存在する場合は古い値を新しい値で置き換えます。例：
...
例：`SELECT map_to_redis_hset_args(payload.value) as hset_fields FROM t/1` のように使用し、Redisアクションのテンプレートで `HMSET name1 ${hset_fields}` として利用します。

例えば、`payload.value` が `{"a" : 1, "b": 2}` のマップの場合、`HMSET name1 b 2 a 1` のようなコマンドになります。マップのフィールド順序は非決定的です。
...
`Map` の指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。配列で複数キーを指定するとネストされたマップから値を取得できます。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新されたマップを返します。`Key` が既に存在する場合は古い値を新しい値で置き換えます。配列で複数キーを指定するとネストされたマップにデータを挿入できます。例：
...
配列 `Array` の先頭から最大長 `Length` の部分配列を返します。`Length` が `Array` の長さより大きい場合は配列全体を返します。例：
...
`sublist/2` と同様ですが、`Start` で返却開始位置を指定できます。`Start` + `Length` が `Array` の長さを超える場合は配列全体を返します。例：
...
任意長の文字列 `String` の128ビット固定長MD5ハッシュ値を計算します。ハッシュ値は32桁の16進数文字列で返され、小文字（a〜f）固定です。例：
...
任意長の文字列 `String` の160ビット固定長SHA-1ハッシュ値を計算します。ハッシュ値は40桁の16進数文字列で返され、小文字（a〜f）固定です。例：
...
任意長の文字列 `String` の256ビット固定長SHA-2ハッシュ値を計算します。ハッシュ値は64桁の16進数文字列で返され、小文字（a〜f）固定です。例：
...
注：バイナリデータは直接JSONエンコードできないため、16進数文字列に変換するには `bin2hexstr` 関数を呼び出してください。
...
DEFLATEアルゴリズムで `Data` を圧縮し、返却値はgzヘッダーと末尾のチェックサムを含みます。例：
...
DEFLATEアルゴリズムで `Data` を圧縮し、返却値はzlibヘッダーと末尾のチェックサムを含みません。例：
...
DEFLATEアルゴリズムで `Data` を圧縮します。返却値はzlibヘッダーと末尾のチェックサムを含みます。例：
...
`Num1` と `Num2` のビット単位AND演算結果を返します。入力も出力も符号付き整数です。例：
...
`Num` のビット単位否定演算結果を返します。入力も出力も符号付き整数です。例：
...
`Num` をビット単位で左に `Shift` ビットシフトし、右端は0で埋めます。例：
...
`Num` をビット単位で右に `Shift` ビットシフトし、左端は符号ビット（正数は0、負数は1）で埋めます。例：
...
`Num1` と `Num2` のビット単位OR演算結果を返します。例：
...
`Num1` と `Num2` のビット単位XOR演算結果を返します。例：
...
ルールエンジンはビット列を操作する関数を提供します。例えば `subbits` はビット列から指定長のビットを抽出し、指定型に変換します。
...
EMQXはSparkplug Bメッセージのデコード・エンコード用の専用関数（`sparkplug_decode` と `sparkplug_encode`）も備えています。詳細は [Sparkplug B](./sparkplug.md) を参照してください。
...
Unix時間 `Time` を指定フォーマットの日時文字列に変換します。`Unit` はUnix時間の単位、`Offset` は出力日時のタイムゾーンオフセット、`FormatString` は出力日時フォーマットを表します。
...
`rfc3339_to_unix_ts/1` と同様ですが、返却するUnixタイムスタンプの単位を `Unit` で指定できます。`second`、`millisecond`、`microsecond`、`nanosecond` をサポートします。例：
...
現在時刻をMongoDBのISODate型または文字列で返します。MongoDB関連のアクションやSQLテストでのみサポートされ、SQLテストでは文字列（例：`ISODate("2024-02-23T15:00:00.123Z")`）を返します。その他の関数の入力としては現在文字列以外はサポートされていません。
...
指定したミリ秒単位UnixタイムスタンプをMongoDBのISODate型または文字列に変換します。その他の動作は `mongo_date/0` と同様です。
...
- OS環境変数から読み取る際、`EMQXVAR_` プレフィックスが付加されます。例えば `getenv('FOO_BAR')` は `EMQXVAR_FOO_BAR` を読み取ります。
- OS環境から読み込まれた値は不変です。

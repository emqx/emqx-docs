...
例えば、`abs(X: integer | float) -> integer | float` は、引数 `X` のデータ型が整数または浮動小数点数であり、戻り値のデータ型もそれに対応して整数または浮動小数点数であることを意味します。

引数が指定範囲を超えるか、サポートされていないデータ型を使用すると、現在のSQL実行は失敗し、失敗回数が1増加しますのでご注意ください。
...
データ型判定関数は指定したフィールドのデータ型をチェックし、指定したデータ型に合致するかどうかを真偽値で示します。
...
`Term` をブール型に変換します。`Term` はブール型、整数型の0または1、文字列型の `true` または `false` のいずれかでなければなりません。
...
浮動小数点数はコンピュータ上で正確に格納できないため、`Decimals` が `Float` の小数桁数（先行ゼロ含む）を超える場合、`float2str` は `Float` の2進近似の10進表現を返すことがあります。
...
- `Term` が文字列の場合、少なくとも1つの数字文字を含み、先頭に `+` または `-` のいずれか1文字の接頭辞が付くことができ、先行ゼロは無視されます。数学的表記もサポートされます。  
...
UTF-16リトルエンディアンエンコード文字列はJSONオブジェクト内で正しく表示されない場合があります。EMQXでは通常バイナリデータとして扱われます。これを16進数文字列として読みやすく変換するには `bin2hexstr` 関数を使用してください。  
...
文字列関数は大文字・小文字変換、空白削除、部分文字列抽出、置換、エスケープ／アンエスケープなどに使用できます。

### ascii(Char: string) -> integer

文字 `Char` に対応するASCIIコードを返します。`Char` に複数文字が含まれる場合、最初の文字のコードのみ返します。例：
...
`String` 内で部分文字列 `SearchPattern` を検索し、`SearchPattern` より前の内容を削除して残りを返します。`SearchPattern` が見つからない場合は空文字列を返します。この関数は `find(String, SearchPattern, 'leading')` と同等です。
...
`trim/1` と同様ですが、文字列の先頭の空白文字のみ削除します。例：
...
`pad/2` と同様ですが、`Direction` でパディング方向を指定できます。`leading` は先頭にスペースを埋め、`trailing` は末尾に、`both` は両端に埋めます。

`both` 指定時、埋めるスペース数が奇数の場合は最後の1文字が末尾に埋められます。
...
ルールエンジンは `Char` が合法なグラフェムクラスタかどうかをチェックしないため、`Char` の文字数に関わらず1文字分として処理されます。例：
...
与えられた文字列から正規表現のキャプチャグループにマッチする部分を非グローバルに検索します。完全一致部分は含まず、キャプチャグループの内容のみを抽出します。

マッチがあればすべてのキャプチャグループをリストで返し、マッチがなければ空リストを返します。
...
`trim/1` と同様ですが、文字列の末尾の空白文字のみ削除します。例：
...
連続する複数の区切り文字は1つとして扱われず、分割結果に空文字列が含まれる場合があります。`split/2` はデフォルトで結果をトリムし空文字列を除外します。空文字列を残したい場合は `split(String, Separator, 'notrim')` を使用してください。
...
`Format` に従って書式化された文字列を返します。`Format` 文字列は通常の文字と書式制御シーケンスを含みます。

制御シーケンスの形式は一般的に `~F.P.PadModC` です。

`C` は制御シーケンスの種類を決定し、唯一必須の項目です。`F`, `P`, `Pad`, `Mod` はすべて任意です。詳細は https://www.erlang.org/doc/man/io.html#fwrite-1 を参照してください。
...
`String` の位置 `Start` から末尾までのすべての文字を返します。文字列の添字は0始まりで、例えば "hello" の位置0は "h" です。例：
...
`String` を `SeparatorList` の文字で分割した部分文字列のリストを返します。
...
- 16進エスケープコード：
  ...
  - `\xH...`：`H...` は1つ以上の16進数（0-9, A-F, a-f）で任意のUTF-32文字をエンコード可能
...
例：`SELECT map_to_redis_hset_args(payload.value) as hset_fields FROM t/1` のように使用し、Redisアクションのコマンドテンプレートで `HMSET name1 ${hset_fields}` のように利用します。
...
`Map` 内の指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。配列を使って複数のキーを指定し、ネストされたマップから値を取得することも可能です。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新されたマップを返します。`Key` が既に存在する場合は古い値を新しい値で置き換えます。配列を使って複数のキーを指定し、ネストされたマップにデータを挿入することも可能です。例：
...
配列 `Array` のN番目の要素を返します。`N` は `Array` の長さ以下でなければなりません。例：
...
`sublist/2` と同様ですが、`Start` で返し始める要素を指定できます。`Start` + `Length` が `Array` の長さを超える場合は全体を返します。例：
...
任意長の文字列 `String` の128ビット固定長MD5ハッシュ値を計算します。ハッシュ値は32桁の16進数文字列で返され、文字は小文字（a〜f）に固定されます。
...
任意長の文字列 `String` の160ビット固定長SHA-1ハッシュ値を計算します。ハッシュ値は40桁の16進数文字列で返され、文字は小文字（a〜f）に固定されます。
...
任意長の文字列 `String` の256ビット固定長SHA-2ハッシュ値を計算します。ハッシュ値は64桁の16進数文字列で返され、文字は小文字（a〜f）に固定されます。
...
注意：バイナリデータは直接JSONエンコードできません。`bin2hexstr` 関数を使って16進数文字列に変換してください。
...
`bitstring` 型は任意長のビット列を表し、8の倍数でなくても構いません。

つまり、すべての `binary` は `bitstring` ですが、逆は必ずしも真ではありません。

`bitstring` は長さが8の倍数でない場合、JSONなど外部形式に直接シリアライズできません。通常は整数などに変換される前の中間値として使われます。
...
バイト列 `Bin` の位置 `Start`（1始まり）から長さ `BitNum` のビットを抽出し、指定されたバイト順 `Endianness` と符号属性 `Signedness` に従い、指定データ型 `OutputType` に変換します。
...
任意のバイナリデータをMicrosoft SQL Serverのバイナリ型（`0x` プレフィックス付きのHEXエンコード文字列）に変換します。
...
EMQXはSparkplug Bメッセージのデコード・エンコード用の特殊関数（`sparkplug_decode` と `sparkplug_encode`）も備えています。詳細は [Sparkplug B](./sparkplug.md) をご覧ください。
...
Unix時間 `Time` を指定フォーマットの日時文字列に変換します。`Unit` は `Time` の時間単位、`Offset` は出力日時のタイムゾーンオフセット、`FormatString` は出力日時のフォーマットを表します。
...
現在時刻をMongoDBのISODate型または文字列として返します。MongoDB関連のアクションやSQLテストでのみサポートされます。SQLテストでは文字列（例：`ISODate("2024-02-23T15:00:00.123Z")`）を返します。その他の関数の入力としては文字列以外は現在サポートされていません。
...
`Value1` がnullの場合に `Value2` を返します。データフィールドがnullかどうかをチェックし、デフォルト値に置き換える際に便利です。

例えば、`coalesce(payload.value, 0)` は `payload.value` がnullでなければその値を返し、nullなら0を返します。これはSQLの `CASE WHEN is_null(payload.value) THEN 0 ELSE payload.value END` と同等ですが簡潔です。

...
2. EMQX 5.0以降、複雑なデータ変換に[jq構文](https://stedolan.github.io/jq/manual/)の利用が可能です。詳細は[jq関数](./rule-sql-jq.md)をご覧ください。
...
:::tip
浮動小数点数の絶対値計算には、`ceil` または `floor` 関数の使用を推奨します。
:::
...
変数 `Term` が未定義か判定します。値がJSONの `null` であっても割り当て済みとみなされます。
...
`Term` をブール型に変換します。`Term` はブール型、整数型の0または1、文字列型の `"true"` または `"false"` のいずれかである必要があります。
...
`Term` が文字列の場合、科学的記数法も使用可能です（例：`float('3.14e4')`）。浮動小数点数は最大16桁の有効数字をサポートします。文字列で表現された浮動小数点数の有効数字が16桁を超えると、変換時に丸め誤差が生じる可能性があります。
...
# 有効数字が16桁を超えると丸め誤差により異なる入力が同一出力になることがある
...
浮動小数点数はコンピュータ上で正確に保存できないため、`Decimals` が `Float` の小数桁数（先頭のゼロを含む）を超える場合、`float2str` は `Float` の2進近似の10進表現を返すことがあります。
...
UTF-16リトルエンディアンエンコードされた文字列はJSONオブジェクト内で正しく表示されないことがあります。EMQXでは通常バイナリデータとして扱われます。可読な16進数文字列に変換するには `bin2hexstr` 関数を使用してください。  
...
`String` 内で部分文字列 `SearchPattern` を検索し、`SearchPattern` より前の部分を削除して残りを返します。`SearchPattern` が見つからない場合は空文字列を返します。`find(String, SearchPattern, 'leading')` と同じ動作です。
...
`String` の末尾に空白を追加して指定長にパディングします。例：
...
`both` 指定時、追加する空白数が奇数の場合は末尾に多く空白が入ります。
...
ルールエンジンは `Char` の合法性をチェックしないため、`Char` が複数文字でも1文字分として扱われます。例：
...
正規表現 `Expression` にマッチした部分を文字列 `Replacement` に置換します。マッチなしの場合は元の文字列を返します。例：
...
正規表現のキャプチャグループを用いて文字列から部分抽出します。完全一致部分は含まず、キャプチャグループにマッチした部分のみリストで返します。マッチなしまたはキャプチャなしの場合は空リストを返します。
...
`String` の先頭に `Prefix` があれば削除し、なければ元の文字列を返します。例：
...
連続する複数の区切り文字は1つとして扱われません。そのため空文字列が結果に含まれることがあります。`split/2` はデフォルトで結果をトリムし空文字列を除外します。空文字列を残したい場合は `split(String, Separator, 'notrim')` を使用してください。

区切り文字は複数文字でも可能ですが、全体で1つの区切り文字として扱われます。複数の区切り文字を同時に指定したい場合は `tokens` 関数を使用してください。
...
`Format` に従い文字列をフォーマットして返します。`Format` 文字列は通常文字と制御シーケンスで構成されます。

制御シーケンスの形式は一般的に `~F.P.PadModC` です。

文字 `C` は制御シーケンスの種類を決定し、唯一必須の項目です。`F`、`P`、`Pad`、`Mod` はすべて任意です。詳細は https://www.erlang.org/doc/apps/stdlib/io.html#fwrite-1 をご参照ください。
...
`String` の先頭と末尾から空白文字（スペース、タブ、改ページ、改行など）を削除します。Unicode標準において `\r\n` はグラフェムクラスタとして扱われるため、`trim` はこれをまとめて削除します。例：
...
認識されないエスケープシーケンスや無効なUnicode文字の場合は例外をスローします。
...
`Map` 内の指定した `Key` の値を返します。`Key` が存在しない場合は `undefined` を返します。配列を指定するとネストしたマップから複数キーの値を取得できます。例：
...
`Map` に `Key` と対応する `Value` を挿入し、更新済みのマップを返します。`Key` が既に存在する場合は値を上書きします。配列を指定するとネストしたマップに複数キーの値を挿入できます。例：
...
任意長の文字列 `String` の128ビット固定長MD5ハッシュ値を計算し、32桁の16進小文字文字列で返します。例：
...
任意長の文字列 `String` の160ビット固定長SHA-1ハッシュ値を計算し、40桁の16進小文字文字列で返します。例：
...
任意長の文字列 `String` の256ビット固定長SHA-2ハッシュ値を計算し、64桁の16進小文字文字列で返します。例：
...
ルールエンジンはビット列操作用の関数を提供しています。例えば `subbits` はビット列から指定長のビットを抽出し、指定型に変換します。

:::tip

`binary` 型はバイト列を表し、1バイトは8ビットなので、`binary` のビット数は8の倍数です。`bitstring` 型は任意のビット数のビット列を表します。
...
バイト列 `Bin` の `Start` 位置（1始まり）から `BitNum` ビットを抽出し、指定のバイト順 `Endianness` と符号属性 `Signedness` に従い、指定型 `OutputType` に変換します。
...
- `urlsafe`: URLセーフBase64形式（`-` と `_` を使用）でデコードします。
...
- `urlsafe`: URLセーフBase64形式（`-` と `_` を使用）でエンコードします。
...
Microsoft SQL Serverの `CONVERT` 関数と組み合わせて、UTF-16リトルエンディアンエンコードされたUnicode文字列をUTF-8非対応のSQL Serverバージョンに書き込む際に利用可能です。
...
EMQXは指定したスキーマに従い、[Protobuf](https://developers.google.com/protocol-buffers) および [Avro](https://avro.apache.org/) データのエンコード・デコードを行う `schema_encode` と `schema_decode` 関数をサポートしています。詳細は[スキーマレジストリ](./schema-registry.md)をご参照ください。
...
EMQXはSparkplug Bメッセージのデコード・エンコード用関数（`spb_decode` と `spb_encode`）も備えています。詳細は[Sparkplug B](./sparkplug.md)をご覧ください。

## 日時変換関数

### date_to_unix_ts(Unit: string, FormatString: string, DateTimeString: string) -> integer

日時文字列 `DateTimeString` をフォーマット文字列 `FormatString` に従い解析し、指定単位 `Unit` のUnix時間に変換します。
...
現在時刻を秒単位のRFC3339形式日時文字列で返します。例：
...
`rfc3339_to_unix_ts/1` と同様ですが、`Unit` で返却単位を指定できます。例：
...
タイムゾーンオフセット文字列を秒数の整数に変換します。対応形式は以下の通りです：
...
秒単位のUnixタイムスタンプをシステムローカルタイムゾーンのRFC3339形式日時文字列に変換します。例：
...
データフィールドがnullかどうかをチェックし、デフォルト値に置き換えたい場合に便利です。

例えば、`coalesce(payload.value, 0)` は `payload.value` がnullでなければその値を、nullなら `0` を返します。

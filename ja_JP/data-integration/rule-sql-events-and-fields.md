...
EMQXのルールを使ってイベントトピックからデータを抽出し、クライアントのオンライン・オフラインやサブスクリプションなどのイベント通知を取得できます。イベントトピックは`"$events/"`で始まり、例えば`"$events/client/connected"`などがあり、ルールの`FROM`句で指定可能です。
...
後方互換性のため旧イベントトピックもサポートされていますが、新規設定ではネームスペース付きの新トピックの使用が推奨されます。以下は旧トピックと新トピックの対応表です。
...
例えば、`"$events/message/delivered"`イベントトピックから、パブリッシャーのIDとユーザー名、メッセージトピック、メッセージQoS、イベントが発生したEMQXノード、イベント発生時刻を抽出するには、以下のステートメントを使用します。
...
例えば、`"$events/message/acked"`イベントトピックから、パブリッシャーのIDとユーザー名、メッセージトピック、メッセージQoS、イベントが発生したEMQXノード、イベント発生時刻を抽出するには、以下のステートメントを使用します。<!--node部分の確認必要-->
...
例えば、`"$events/message/dropped"`イベントトピックから、破棄理由、メッセージトピック、メッセージQoS、イベントが発生したEMQXノード、イベント発生時刻を抽出するには、以下のステートメントを使用します。
...
例えば、`"$events/message/delivery_dropped"`イベントトピックから、パブリッシャーのIDとユーザー名、破棄理由、メッセージトピックとQoSを抽出するには、以下のステートメントを使用します。
...
| :-------------------- | :------------------------------------------------------- |
| `id`                  | MQTTメッセージID                                         |
| `reason`              | 破棄理由：<br/><br/>`queue_full`: メッセージ（QoS>0）キューが満杯<br/><br/>`no_local`: クライアントが自身のパブリッシュしたメッセージを受信不可<br/><br/>`expired`: メッセージまたはセッションの有効期限切れ<br/><br/>`qos0_msg`: メッセージ（QoS 0）キューが満杯 |
| `from_clientid`       | パブリッシャーのクライアントID                          |
| `from_username`       | パブリッシャーのユーザー名                              |
| `clientid`            | サブスクライバーのクライアントID                        |
| `username`            | サブスクライバーのユーザー名                            |
...
例えば、`"$events/client/connected"`イベントトピックから、クライアントID、ユーザー名、キープアライブ間隔、接続したMQTTクライアントがブリッジとして動作しているかどうかを抽出するには、以下のステートメントを使用します。
...
例えば、`"$events/client/disconnected"`イベントトピックから、クライアントID、ユーザー名、切断理由、切断時刻、イベントが発生したEMQXノードを抽出するには、以下のステートメントを使用します。
...
| フィールド           | 説明                                                     |
| :------------------ | :------------------------------------------------------- |
| `reason`            | 切断理由：<br/><br/>`normal`: クライアントが意図的に切断<br/><br/>`kicked`: EMQXがREST API経由で強制切断<br/><br/>`keepalive_timeout`: 指定されたキープアライブ時間の超過<br/><br/>`not_authorized`: 認可失敗<br/><br/>`tcp_closed`: ピアがネットワーク接続を閉じた<br/><br/>`discarded`: `clean_start`が`true`の別クライアントが同じClientIDで接続し、前接続が切断された<br/><br/>`takenover`: `clean_start`が`false`の別クライアントが同じClientIDで接続し、前接続を引き継いだ<br/><br/>`internal_error`: 不正なメッセージ形式やその他不明なエラー発生 |
| `clientid`          | クライアントID                                          |
| `username`          | クライアントのユーザー名                                |
| `peername`          | IPアドレスとポート番号                                  |
| `sockname`          | EMQXがリッスンしているIPアドレスとポート番号           |
| `connected_at`      | クライアント接続開始時刻（単位：ミリ秒）。現在のセッションが確立された時刻であり、切断イベントがどの接続セッションに属するかを判別するために利用されます。遅延した切断イベントが新しい接続状態を上書きしないようにします。 |
...
[^*]: MQTT v5.0プロトコルでは、リターンコードが理由コードに名称変更され、より多様なエラータイプを示す理由コードが追加されています（[Reason code and ACK - MQTT 5.0 new features](https://www.emqx.com/en/blog/mqtt5-new-features-reason-code-and-ack)）。
...
| `connection_rate_exceeded`     | 接続レート制限を超過                                   |
...
このイベントトピックは、端末のサブスクリプションが正常に解除された際にルールをトリガーします。
...
例えば、`"$events/sys/alarm_activated"`イベントトピックから、アラーム名、詳細、説明メッセージ、発動時刻を抽出するには、以下のステートメントを使用します。
...
例えば、`"$events/sys/alarm_deactivated"`イベントトピックから、アラーム名、詳細、説明メッセージ、発動タイムスタンプ、解除タイムスタンプを抽出するには、以下のSQLステートメントを使用します。
...
このイベントトピックは、EMQXが接続中のMQTTクライアントから`PINGREQ`パケットを受信した際にルールをトリガーできます。これはクライアントのハートビートが正常に受信されたことを示します。

主に診断やトラブルシューティングに使用されます。`$events/client/disconnected`はクライアントが切断された理由（例：`keepalive_timeout`）を示しますが、`"$events/client/ping"`イベントはEMQXが実際にクライアントからハートビートパケットを受信した直接的な証拠を提供し、クライアント側やネットワーク側の問題とブローカー側の問題を区別するのに役立ちます。

例えば、`"$events/client/ping"`イベントトピックから、クライアントID、ユーザー名、交渉されたキープアライブ間隔、イベント発生時刻、イベントが発生したEMQXノードを抽出するには、以下のステートメントを使用します。

...
[ルールエンジン構文](./rule-sql-syntax.md)のセクションで説明したように、`FROM`句でデータソースを指定し、対応するフィールドを`SELECT`や`where`句で参照できます。本セクションでは、[MQTTメッセージ](#mqtt-message)、[MQTTイベント](#mqtt-events)、および[データブリッジ](#data-bridges)のフィールドについて紹介します。
...
=======
EMQXルールを使ってメッセージのパブリッシュを処理する場合、`FROM`句でメッセージのトピックを指定する必要があります。

例えば、以下の文では、トピックパターン`t/#`にパブリッシュされたメッセージの`payload.msg`（`AS`句で`msg`にリネーム）、`clientid`、`username`、`payload`、`topic`、`qos`のフィールドを選択しています。
>>>>>>> origin/release-5.10
...
=======
EMQXルールを使ってイベントトピックからデータを抽出し、クライアントのオンライン・オフラインやサブスクリプションなどのイベント通知を受け取ることができます。イベントトピックは`"$events/"`で始まり、例えば`"$events/client/connected"`などがあり、ルールの`FROM`句で指定可能です。
...
| [$events/message/dropped](#message-dropped-when-routing-event-events-message-dropped) | ルーティング時メッセージ破棄  |
| [$events/message/delivery_dropped](#message-dropped-when-delivering-event-events-delivery-dropped) | 配信時メッセージ破棄          |
| [$events/client/connected](#connection-complete-event-events-client-connected) | 接続完了                     |
| [$events/client/disconnected](#disconnect-event-events-client-disconnected) | 切断                         |
| [$events/client/connack](#connection-acknowledge-event-events-client-connack) | 接続応答                     |
| [$events/auth/check_authz_complete](#authorization-check-complete-event-events-client-check-authz-complete) | 認可チェック完了              |
...
後方互換性のため旧イベントトピックもサポートされていますが、新規設定ではネームスペース付きの新しいトピック名の使用を推奨します。以下の表は旧トピックと新（ネームスペース付き）トピックの対応です。
>>>>>>> origin/release-5.10

| 旧イベントトピック名                    | 新イベントトピック名                         |
|:----------------------------------------|:--------------------------------------------|
...
=======
例えば、`"$events/message/delivered"`イベントトピックから、パブリッシャーのIDとユーザー名、メッセージトピック、メッセージQoS、イベントが発生したEMQXノード、イベント発生時刻のフィールドを抽出するには、以下の文を使用します。
>>>>>>> origin/release-5.10
...
例えば、`"$events/message/acked"`イベントトピックから、パブリッシャーのIDとユーザー名、メッセージトピック、メッセージQoS、イベントが発生したEMQXノード、イベント発生時刻のフィールドを抽出するには、以下の文を使用します。 <!--node部分の確認必要-->
>>>>>>> origin/release-5.10
...
### ルーティング時メッセージ破棄イベント ("$events/message/dropped")
...
=======
例えば、`"$events/message/dropped"`イベントトピックから、破棄理由、メッセージトピック、メッセージQoS、イベントが発生したEMQXノード、イベント発生時刻のフィールドを抽出するには、以下の文を使用します。
>>>>>>> origin/release-5.10
...
| :------------------ | :------------------------------------------------------- |
| `id`                | MQTTメッセージID                                         |
| `reason`            | 破棄理由：<br/><br/>`no_subscribers`: トピックにサブスクライブするクライアントが存在しない<br/><br/>`receive_maximum_exceeded`: `awaiting_rel`キューが満杯<br/><br/>`packet_identifier_inuse`: 未解放のパケットIDを持つQoS 2メッセージを受信したため |
| `clientid`          | パブリッシャーのクライアントID                          |
| `username`          | パブリッシャーのユーザー名                              |
| `payload`           | MQTTペイロード                                           |
| `peerhost`          | クライアントのIPアドレス                                |
...
### 配信時メッセージ破棄イベント ("$events/message/delivery_dropped")
...
=======
例えば、`"$events/message/delivery_dropped"`イベントトピックから、パブリッシャーのIDとユーザー名、破棄理由、メッセージトピック、QoSのフィールドを抽出するには、以下の文を使用します。
>>>>>>> origin/release-5.10
...
| :------------------ | :------------------------------------------------------- |
| `id`                | MQTTメッセージID                                         |
| `reason`            | 破棄理由：<br/><br/>`queue_full`: メッセージ（QoS>0）キューが満杯<br/><br/>`no_local`: クライアントは自身がパブリッシュしたメッセージを受信できない<br/><br/>`expired`: メッセージまたはセッションの有効期限切れ<br/><br/>`qos0_msg`: メッセージ（QoS 0）キューが満杯 |
| `from_clientid`     | パブリッシャーのクライアントID                          |
| `from_username`     | パブリッシャーのユーザー名                              |
| `clientid`          | サブスクライバーのクライアントID                        |
| `username`          | サブスクライバーのユーザー名                            |
...
例えば、`"$events/client/connected"`イベントトピックから、クライアントID、ユーザー名、キープアライブ間隔、接続クライアントがブリッジとして動作しているかどうかのフィールドを抽出するには、以下の文を使用します。
...
=======
例えば、`"$events/client/disconnected"`イベントトピックから、クライアントID、ユーザー名、切断理由、接続時刻、切断時刻、イベントが発生したEMQXノードのフィールドを抽出するには、以下の文を使用します。
>>>>>>> origin/release-5.10
...
| `node`              | イベントが発生したEMQXノード                             |
=======
| `reason`            | 切断理由：<br/><br/>`normal`: クライアントが意図的に切断<br/><br/>`kicked`: REST APIでEMQXが強制切断<br/><br/>`keepalive_timeout`: キープアライブ時間切れ<br/><br/>`not_authorized`: 認可失敗<br/><br/>`tcp_closed`: ピアがネットワーク接続を閉じた<br/><br/>`discarded`: 同一ClientIDで`clean_start=true`の別クライアントが接続し、前接続が切断<br/><br/>`takenover`: 同一ClientIDで`clean_start=false`の別クライアントが接続し、前接続を引き継ぎ<br/><br/>`internal_error`: 不正なメッセージ形式などの内部エラー |
| `clientid`          | クライアントID                                          |
| `username`          | クライアントのユーザー名                                |
| `peername`          | IPアドレスとポート番号                                  |
| `sockname`          | EMQXがリッスンしているIPアドレスとポート番号            |
| `connected_at`      | クライアント接続開始時刻（単位：ミリ秒）。このタイムスタンプは現在のセッション確立時刻を示し、切断イベントがどの接続セッションに属するかを識別します。遅延した切断イベントが新しい接続状態を上書きしないようにします。 |
| `disconnected_at`   | クライアント切断完了時刻（単位：ミリ秒）                |
| `disconn_props`     | DISCONNECTプロパティ（MQTT 5.0クライアントのみ）         |
| `timestamp`         | イベント発生時刻（単位：ミリ秒）                        |
| `node`              | イベントが発生したEMQXノード                            |
...
[^*]: MQTT v5.0プロトコルではリターンコードがリーズンコードに名称変更され、より多様なエラーを示すための理由コードが追加されました（[Reason code and ACK - MQTT 5.0 new features](https://www.emqx.com/en/blog/mqtt5-new-features-reason-code-and-ack)）。
>>>>>>> origin/release-5.10

以下はMQTT v3.1.1およびMQTT v5.0のリーズンコード一覧です。
...
このイベントトピックは、クライアントのサブスクライブ解除が成功した際にルールをトリガーできます。
...
このイベントトピックは、EMQXシステムアラームが発動した際にルールをトリガーできます。

例えば、`"$events/sys/alarm_activated"`イベントトピックから、アラーム名、詳細、説明メッセージ、発動時刻のフィールドを抽出するには、以下の文を使用します。
...
| ----------------- | :------------------------------------------------------- |
| `name`            | アラームの短い識別子（例：`"too_many_processes"`）      |
| `details`         | アラームの追加詳細を含むJSONオブジェクト（スキーマは固定されていない。例：`{"usage": "99%", "high_watermark": "80%"}`） |
| `message`         | アラームの説明メッセージ（例：`"99% process usage"`）   |
...
このイベントトピックは、EMQXシステムアラームが解除された際にルールをトリガーできます。

<<<<<<< HEAD
例えば、`"$events/sys/alarm_deactivated"`イベントトピックから、アラーム名、詳細、説明メッセージ、発動時刻、解除時刻のフィールドを抽出するには、以下のステートメントを使用します。
=======
例えば、`"$events/sys/alarm_deactivated"`イベントトピックから、アラーム名、詳細、説明メッセージ、発動時刻、解除時刻のフィールドを抽出するには、以下の文を使用します。
>>>>>>> origin/release-5.10
...
ルールでは、`$bridges/`で始まるトピックを使って、データブリッジによって送信されたメッセージやイベントを参照します。形式は以下の通りです。
>>>>>>> origin/release-5.10
...
=======
例えば、MQTTブリッジのイベントは`"$bridges/mqtt:*"`の形式で参照できます。MQTTデータブリッジ`my_mqtt_bridge`が送信するすべてのメッセージに対するルールを設定するには、以下の文を使用します。
>>>>>>> origin/release-5.10

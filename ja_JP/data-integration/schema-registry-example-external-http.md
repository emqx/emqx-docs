...
場合によっては、EMQXがネイティブにサポートしていないカスタムのエンコードやデコードロジックを適用する必要があります。EMQXでは、ルール内の `schema_encode` および `schema_decode` 関数を通じて外部HTTPサービスを呼び出し、その処理を委任することが可能です。
...
- `opts`：EMQXで設定可能な任意の文字列で、追加のオプションとしてHTTPサーバーにそのまま渡されます。

#### GETリクエスト

スキーマのメソッドが `GET` に設定されている場合、EMQXは同じフィールドをURLクエリパラメータとして送信します。

- `payload`：パディングなしのURLセーフBase64エンコード文字列。
...
| **Method**     | はい     | エンドポイント呼び出しに使用するHTTPメソッド。デフォルトは `POST`。クエリ文字列でリクエストフィールドを渡す必要がある場合のみ `GET` を使用します。              |
| **Params**     | いいえ   | すべてのリクエストの `opts` フィールドとして渡される任意の文字列。サービスへの追加オプションや設定値を送信するために使用します。                                |
| **Headers**    | いいえ   | すべてのリクエストに含まれるHTTPヘッダー。`content-type: application/json` ヘッダーはデフォルトで追加されます。認証トークンなど追加のヘッダーを含める場合は **Add** をクリックします。 |
...
def decode_payload(payload64):
    if request.method == "GET":
        # EMQXはGETでペイロードをパディングなしのURLセーフBase64で送信します。
...
```sh
# サーバーが同じディレクトリの `myapp.py` というファイル名であることを想定
...
3. 以下のパラメータで外部HTTPサーバースキーマを作成します。
   ...
   - **Method**：`POST` または `GET` を選択。`POST` がデフォルトです。外部サービスがクエリ文字列でリクエストフィールドを受け取る場合のみ `GET` を使用してください。
...
3. 作成したスキーマを用いて、以下のルールSQL文を記述します。
   ...
   `schema_encode('myhttp', payload)` と `schema_decode('myhttp', encoded)` はいずれも設定した外部HTTPサーバーを呼び出し、指定されたペイロードをエンコード／デコードします。
...
7. **Add** をクリックしてアクションをルールに追加します。
   ...
   このアクションにより、デコードされたメッセージがJSON形式で `external_http/out` トピックに送信されます。`${.}` はルールの出力全体の値に実行時に置き換わる変数プレースホルダーです。
...
6. WebSocket側で `external_http/out` トピックのメッセージを受信できることを確認します。例えば、ペイロードが `hello` の場合は以下のように表示されます。

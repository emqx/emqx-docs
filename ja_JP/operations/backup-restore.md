...
- [コアノード＋レプリカノード](../deploy/cluster/mria-introduction.md)モードが有効な場合、データのインポートはコアノードでのみ実行可能です。これは実際のインポート動作に影響しません。データはコアノードおよびレプリカノードを含むすべてのクラスターのノードにレプリケートされます。コアノードで操作することで正しいデータインポートが保証されます。

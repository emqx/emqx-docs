...
`Cmd`の値は`4`です。他のフィールドは車両ログインと同じ構造を持ちます。
...
各レポートは`Infos`配列に複数の情報項目を含むことができます。`Type`フィールドで情報の種類を区別します。
...
| `H_MaxTemp`           | 整数    | 最大水素系温度；有効範囲0～2400（オフセット40℃、-40℃～+200℃）；単位：0.1℃                 |
...
| `RemainingH2`         | 整数    | **gbt32960-2025のみ**：水素系の残量；単位：1 kg                                            |
...
| `Status`            | 整数    | 位置ステータス；全ステータスビットの整数値（プロトコル表15参照）                      |
| `CoordinateSystem`  | 整数    | **gbt32960-2025のみ**：座標系：`1`=WGS-84、`2`=GCJ-02                               |
| `Longitude`         | 整数    | 経度（度×10^6）；精度は百万分の一度                                                    |
| `Latitude`          | 整数    | 緯度（度×10^6）；精度は百万分の一度                                                    |
...
| `FaultDriveMotorList`       | 配列    | 駆動モーターの故障コードリスト                                                                       |
| `FaultEngineNum`            | 整数    | エンジンの故障総数；有効範囲0～252                                                                  |
| `FaultEngineList`           | 配列    | エンジンの故障コードリスト                                                                           |
...
| `FrameCellsIndex`    | 整数    | 本フレームで報告されるセルの開始インデックス（1始まり）；有効範囲1～65531。セル総数が200を超える場合はフレーム分割されます。 |
...
| `MinParallelUnitTotal`  | 整数    | 最小並列ユニット総数                                                                 |
...
| `Param`    | オブジェクト | コマンドパラメータ（コマンドにより異なり、空の場合は省略）                         |

...
NATS ゲートウェイはパブリッシュ／サブスクライブ操作に対する独自のアクセス制御を実装していません。トピックの権限管理は統合された [認可設定](../access-control/authz/authz.md) を使用してください。
...
3. 設定パラメータを入力します。各項目の詳細は [HTTP パスワード認証](../access-control/authn/http.md) を参照してください。
...
その他の認証方式については、[EMQX 認証器](../access-control/authn/authn.md#emqx-authenticators) のドキュメントを参照してください。

...
- **冗長モードサポート**：クライアントが `CONNECT verbose=true` で接続した場合に応答確認を有効化。
...
- **NATS ワイルドカードサブスクリプションのサポート**：MQTT 互換のトピック形式に自動変換されます。
- **Queue Group 共有サブスクリプションのサポート**：NATS の Queue Group サブスクリプションは MQTT の共有サブスクリプション形式に変換されます。
- **リクエスト／リプライモードのサポート**：
...
EMQX ダッシュボードから NATS ゲートウェイを素早く有効化するには：
...
デフォルト設定に加え、EMQX はさまざまな設定オプションを提供しており、特定のビジネスニーズに合わせて調整可能です。本節では、**ゲートウェイ** ページで利用可能なオプションの詳細を説明します。
...
2. **設定** タブで、ゲートウェイの接続パラメータ、マウントポイントプレフィックス、クライアント識別情報の上書きを設定できます。
   ...
   - **マウントポイント**：ゲートウェイを通過するすべてのトピックに自動的に付加される文字列プレフィックス。プロトコル間でトピックを分離するのに役立ちます。例：`nats/` を使うことで、クライアントが手動でプレフィックスを含める必要なくクロスプロトコルルーティングが可能になります。
   ...
   - **クライアント情報の上書き**：`CONNECT` パケットから認証情報を抽出する方法を定義します。
     ...
     認証が有効な場合は、`username` と `password` に正しいフィールドをマッピングして、適切に認証情報が処理されるようにしてください。

     :::

     - **ユーザー名**：`CONNECT` パケットの `user` フィールドにマッピング。
     - **パスワード**：`CONNECT` パケットの `pass` フィールドにマッピング。
     - **クライアントID**：`${generated}` を指定すると自動生成されます。特定のロジックでカスタマイズも可能です。
...
2. **リスナー追加** ダイアログで以下のオプションを設定します。
   ...
   **リスナー設定**
   ...
   - **プロキシプロトコルタイムアウト**：Proxy Protocol ヘッダー受信のタイムアウト。指定時間内にヘッダーを受信しない場合、接続は切断されます。デフォルトは 3 秒。
   ...
   **ピア検証設定**（SSL および WSS リスナーのみ適用）
   ...
   - **TLS キー**：TLS 秘密鍵ファイルのパスまたは内容。
   - **CA 証明書**：CA 証明書ファイルのパスまたは内容。
   - **ピア証明書検証の強制**：クライアント証明書検証を必須にするか。デフォルトは有効（true）。
...
NATS プロトコルゲートウェイの詳細およびユースケースについては、ブログ記事 [EMQX NATS Gateway: Enabling MQTT-NATS Bidirectional Interoperability](https://www.emqx.com/en/blog/emqx-nats-gateway) をご覧ください。

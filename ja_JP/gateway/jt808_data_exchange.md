...
ゲートウェイは **JT/T 808-2013** と **JT/T 808-2019** の両方のプロトコルバージョンをサポートしています。  
...
| メッセージシリアルNo | msg_sn    | word     | integer     |
...
パラメータIDに対応するデータ型により、JSON内のパラメータ値の型が決まります。
...
- 端末タイプにbit8が追加され、トレーラーの接続/対応を示します。
...
| 経路走行時間異常警報追加情報         | path_time_alarm  | -      | map                      |
...
- 速度超過警報追加情報（長さ1または5）は `overspeed_alarm` マップ内にあります。
...
- 出入区域/経路警報追加情報は `in_out_alarm` マップ内にあります。
...
- 経路走行時間異常警報追加情報は `path_time_alarm` マップ内にあります。
...
| フィールド名       | JSONキー   | 値の型 | JSON内の型 |
| :----------------: | :--------: | :----: | :--------: |
| 深いスリープ状態   | deep_sleep | 1ビット | integer    |
...
**補足**: 2019年版では新たにテキストタイプフィールドが追加され、flagのbit0–1の意味が変更されています。
...
**補足**: 2019年版では地域項目に夜間最大速度、地域名長、地域名が追加されています。
...
**補足**: 2019年版では夜間最大速度、地域名長、地域名が追加されています。
...
| 開始時刻                  | start_time     | string | string     |
| 終了時刻                  | end_time       | string | string     |
| 曲点総数                  | length         | word   | integer    |
| 曲点項目リスト            | points         | list   | 曲点のリスト |
| 曲点ID                    | point_id       | dword  | integer    |
| 区間ID                    | path_id        | dword  | integer    |
| 曲点緯度                  | point_lat      | dword  | integer    |
| 曲点経度                  | point_lng      | dword  | integer    |
...
**補足**: 2019年版では区間夜間最大速度、経路名長、経路名が追加されています。
...
**補足**: 2019年版では解像度フィールドに新たに `0x00`（最低画質圧縮）および `0xFF`（最高画質圧縮）が追加されています。
...
ベンダー定義メッセージ。メッセージ本文フォーマットはベンダーが定義。

...
::: tip
`base.hocon` でゲートウェイを設定するとノード単位の変更が必要ですが、ダッシュボードや REST API で設定するとクラスター全体に反映されます。
:::

LwM2M ゲートウェイは UDP および DTLS タイプのリスナーのみサポートしています。  
設定可能なパラメータの詳細は以下を参照してください： [Gateway Configuration - Listeners](https://docs.emqx.com/en/enterprise/v@EE_VERSION@/hocon/#V-gateway-S-gateway-lwm2m-S-lwm2m-listeners)
...
そのため、LwM2M ゲートウェイではこれらのメッセージモデルを互換させるためのメッセージフォーマットを作成しています。
...
このメッセージにはクライアントの情報や、エンドポイント名、ライフタイム、LwM2M バージョン、オブジェクトやオブジェクトインスタンスなどの機能情報が含まれます。
...
- `{?Binding}`: 列挙型、クライアントがサーバーとの通信に対応するバインディングタイプ。以下のいずれかです：  
  * `"U"`: UDP  
  * `"UQ"`: データキューイング付きUDP  
...
これは Register メッセージに似ていますが、初回登録後に送信されます。  
Update メッセージには、IPアドレスの変更や LwM2M オブジェクトでモデル化されたデータの更新など、クライアントの状態や機能の変更情報が含まれます。  
Update メッセージはクライアントの登録期間を延長する役割もあり、クライアントがまだ利用可能でアクティブであることをサーバーに知らせる手段です。
...
- `{?MsgType}`: 文字列、以下のいずれか：  
  ...
  - `"create"`: LwM2M Create  
  - `"delete"`: LwM2M Delete  
- `{?RequestData}`: JSON オブジェクト、`{?MsgType}` によって内容が異なり、以降のセクションで説明します。
...
- `{?ResponseData}`: JSON オブジェクト、コマンドのレスポンス内容。


#### Read（読み取り）

"Read" 操作は、リソースの値、リソースインスタンスの配列、オブジェクトインスタンス、またはオブジェクトのすべてのオブジェクトインスタンスの値にアクセスするために使用します。
...
- `{?ResourcePath}`: 文字列、要求されたリソースパス。以下の3つのシナリオがあります：  
  * オブジェクトIDのみ、例：`/3`。該当オブジェクト配下のすべてのインスタンスとリソースの値を読み取る。  
  * オブジェクトID/インスタンスID、例：`/3/0`。該当オブジェクトインスタンス配下のすべてのリソースの値を読み取る。  
...
- `{?ReadResponseData}`: JSON 配列、リクエストに対するリソース値の結果。
...
この操作は、特定のオブジェクトインスタンスにどのリソースがインスタンス化されているかを探索するために使われます。  
...
* オブジェクトIDのみ、例：`/3`。該当オブジェクト配下のすべてのインスタンス、リソース、属性を探索。  
* オブジェクトID/インスタンスID、例：`/3/0`。該当オブジェクトインスタンス配下のすべてのリソース、属性を探索。  
* フルパス（オブジェクトID/インスタンスID/リソースID）、例：`/3/0/1`。特定リソースのすべての属性を探索。
...
```
完全なパスは `{?BasePath}` と `"{ResourcePath}` の結合です。
...
"Execute" 操作は、LwM2M サーバーが特定のアクションを開始するために使用し、個々のリソースに対してのみ実行可能です。
...
この監視関係は、オブジェクト、オブジェクトインスタンス、リソースに対して "Observe" 操作を送信することで開始されます。  
...
Observe および Cancel Observe リクエストの **Payload** フォーマットは以下の通りです：
```json
{
  "reqID": {?ReqID},
  "msgType": {?MsgType},
...
"Notify" 操作は、有効な監視中に LwM2M クライアントから LwM2M サーバーに送信されます。  
...
変数：  
- `{?ReqID}`: 整数、リクエストID。リクエストとの対応付けに使用。  
- `{?ObserveSeqNum}`: 数値、CoAP メッセージ内の "Observe" オプションの値。  
- `{?ResponseCode}`: 文字列、LwM2M ステータスコード、例："2.01"、"4.00" など。  
- `{?ResponseMsg}`: 文字列、LwM2M レスポンスメッセージ、例："content"、"bad_request"。  
- `{?RequestPath}`: 文字列、リクエストの `path` フィールドと同じ。  
- `{?ResourcePath}`: 文字列、完全なリソースパス、例：`31024/11/1`。  
...
送信データがこの制限を超える場合、単一の CoAP パケットでの送信ができません。  
例えば、数百KBから数MBのファームウェアパッケージをデバイスにプッシュする場合や、多数のリソースを含むオブジェクトを読み取る場合などです。

この制限に対処するため、CoAP は [RFC 7959](https://datatracker.ietf.org/doc/html/rfc7959) でブロック転送機構を定義しています。  
...
- **Block1（サーバー → デバイス）**
  ...
  サーバーがデバイスに大きなデータを書き込む際（例：ファームウェア更新時）、EMQX はペイロードを複数の Block1 セグメントに分割し、順次デバイスに送信します。  
  例えば、256バイトのファームウェアペイロードを16バイトのブロックサイズで分割すると、16個のブロックに分割され順次送信されます。

- **Block2（デバイス → サーバー）**

  デバイスが単一パケットサイズ制限を超えるレスポンスを生成する場合（例：デバイスオブジェクト `/3/0` の読み取り時）、デバイスは複数の Block2 セグメントでレスポンスを送信します。  
...
適切に設定すると、UDP 上での大容量ペイロードの信頼性の高い転送を実現しつつ、MQTT アプリケーションには完全な透過性を保ちます。

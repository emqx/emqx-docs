...
EMQX 5.1 では、認証のために外部リソース（MySQL、PostgreSQL、MongoDB、Redis などのデータベース）にアクセスする場合や、HTTPS 経由でのパスワード認証を行う場合に TLS を有効にするオプションが提供されています。詳細は [TLS for External Resource Access](../guides/network/overview.md#tls-for-external-resource-access) を参照してください。

### プレースホルダー

MySQL、PostgreSQL、MongoDB、Redis、HTTP（外部リクエスト）、JWT（データ補間）などのバックエンドでサポートされるデータ補間は、従来の `%X` プレースホルダーから `${variable}` 形式に変更されました。
...
すべてのパスワードベースのプロバイダー（組み込みデータベース、MySQL、PostgreSQL、MongoDB、Redis）は、同一の `password_hash` オプションを持ち、同様の方法で設定されます。詳細は [Password Hashing](../guides/access-control/authn/authn.md#password-hashing) を参照してください。
...
- Mnesia は「組み込み」データベースと呼ばれるようになり、設定にユーザーレコードはありません。
...
- `user_id_type` は MQTT ユーザー識別子として `clientid` または `username` のどちらを使用するかを指定します。混在は許可されません。
...
- EMQX 4.4 で使用されていた SHA1 ハッシュはサポートされなくなりました。`algorithm` パラメータで `sha512` または `sha256` を選択してください。
...
- `database` は `cluster` タイプを除き引き続き使用可能です。
...
- `auth_cmd` は `cmd` に変更されました。Redis ハッシュデータ構造と `HGET`、`HMGET` コマンドのみサポートします。コマンド内で `${var}` 形式のプレースホルダーを使用してください。コマンドは少なくとも `password`（4.x 互換）または `password_hash` フィールドを取得し、オプションで `salt` と `is_superuser` フィールドを取得する必要があります。
- `super_cmd` は廃止されました。`cmd` で `is_superuser` フィールドを提供してください。クライアントにスーパーユーザー権限を与える場合は、Redis クエリコマンドに `is_superuser` フィールドを追加してください。
...
障害時に Redis に自動再接続するために `auto_reconnect` を使用できます。
...
- `auth_query` は `query` に変更。`${var}` 形式のプレースホルダーを使用してください。クエリは少なくとも `password` または `password_hash` カラムを取得し、オプションで `salt` と `is_superuser` カラムを取得する必要があります。
- `super_query` は廃止され、`is_superuser` カラムは `query` 内で提供します。クライアントにスーパーユーザー権限を与える場合は、認証 SQL 結果に `is_superuser` フィールドが含まれていることを確認してください。
...
障害時に MySQL に自動再接続するために `auto_reconnect` を使用できます。
...
- `type` は `mongo_type` に変更され、`single`、`rs`、`sharded` のいずれかを指定します。未知の値は使用不可。
- `server`
...
- `pool` は `pool_size` に変更。
- `ssl.*` は共通の SSL オプションに変更。詳細は [TLS for External Resource Access](../guides/network/overview.md#tls-for-external-resource-access) を参照。
- `auth_query.selector` は `filter` に変更。`filter` は文字列ではなく、完全なセレクタ構造体であり、`${var}` 形式のプレースホルダーを値に使用可能。
- `auth_query.salt_field` は `salt_field` に変更。
- `auth_query.super_field` は `is_superuser_field` に変更。
- `super_query` は廃止され、`filter` で取得されるドキュメントに `is_superuser_field` を含めてください。
...
`verify_claims` 内のセレクター値には `%X` ではなく `${var}` 形式のプレースホルダー（`${username}`、`${clientid}`）を使用してください。

追加パラメータ：

- `use_jwks`: JWKS からキーを取得するかどうか
- `algorithm`: 署名検証に使用するアルゴリズムタイプ（`public-key` または `hmac-based`）
...
EMQX 4.x は `public key`、`hmac secret`、`jwks` を同時にサポートしていましたが、EMQX 5.1 では一度に一つのアルゴリズムのみをグローバル設定で使用します。`use_jwks` と `algorithm` で利用可能なパラメータセットが決まります：
...
- `ssl.*` は共通の SSL オプションに変更。詳細は [TLS for External Resource Access](../guides/network/overview.md#tls-for-external-resource-access) を参照。
- `super_req` は廃止され、代わりにサービスレスポンスに `is_superuser` フィールドを含めてください。

4.4 とは異なり、`url`、`headers`、`body` パラメータはプレースホルダーを許容します。5.1 では `body` は文字列ではなくマップであり、POST リクエストでは JSON または X-WWW-Form-Urlencoded 形式でシリアライズされ、GET リクエストではクエリパラメータとして送信されます。

4.4 とは異なり、HTTP 認証は HTTP レスポンスステータスコード（2XX）を尊重し、レスポンスボディ内の JSON フィールド（`result`）から認証結果を判定します。
...
- データフォーマットおよび REST API が変更されました。4.x の ACL データは `./bin/emqx_ctl data export` コマンドでエクスポート可能です。ユーザーは 5.x 形式に変換し、対応する REST API `/authorization/sources/built_in_database/rules/{clients,users}` からインポートしてください。
...
JWT 認証時に取得されるクレームに基づく暗黙の ACL です。ACL クレーム内では `%X` ではなく `${variable}` プレースホルダーを使用してください。
...
4.4 とは異なり、認可結果は HTTP レスポンスステータスコードではなく、レスポンスボディ内の JSON フィールド（`result`）から判定されます。HTTP 認可は成功ステータスコード（2XX）のみを尊重します。
...
- `type` は `redis_type` に変更。
  ...
  - `cluster` タイプでは `database` オプションは廃止。
- `database` は `cluster` タイプを除き継続使用可能。
- `pool` は `pool_size` に変更。
- `password` はそのまま。
- `query_timeout` は廃止。
- `ssl.*` は共通の SSL オプションに変更。詳細は [TLS for External Resource Access](../guides/network/overview.md#tls-for-external-resource-access) を参照。
...
4.x のデータを継続利用する場合は手動でのマイグレーションが必要です。
...
- `access` フィールド名は `action` に、データ型は整数から文字列または列挙型に変更されました。
- `allow` フィールド名は `permission` に、データ型は整数から文字列または列挙型に変更されました。
...
EMQX 5.1 では、クエリは `permission, action, topic` のカラムを任意の順序で、ただし正確な名前で取得する必要があります。`IpAddr`、`Username`、`ClientId` の「誰が」部分はクエリの一部として含めることが推奨されます。
...
EMQX 5.1 では MongoDB データソースは許可ルールと拒否ルールの両方に対応可能です。以前はホワイトリストモードのみで、`acl_nomatch = deny` の設定が必要でした。ドキュメントは `permission`、`action`、`topics` フィールドを持つ個別ルールを含み、`topics` はトピックの配列である必要があります。詳細は [AuthZ-MongoDB](../guides/access-control/authz/mongodb.md) を参照してください。

...
- [#15753](https://github.com/emqx/emqx/pull/15753) リスナーの接続レート制限（`max_conn_rate` および `max_conn_burst`）は、アクセプター単位ではなくリスナー単位で適用されるようになり、5.9.0以前の動作に戻りました。
  ...
  そのため、バージョン5.9.0および5.9.1の設定は互換性がなく、同じ実効制限を維持するには、各リスナーに設定されたアクセプター数に応じて指定されたレート値をスケールアップする必要があります。
...
- [#15156](https://github.com/emqx/emqx/pull/15156) `dashboard.sso.oidc.issuer` フィールドに対する厳格なスキーマ検証を追加しました。このフィールドには有効なURLを指定する必要があります。以前は無効な設定でもAPIがエラーを返さず受け入れてしまい、EMQXの再起動に失敗してクラッシュ（`erl_crash.dump`）する可能性がありました。
...
- [#14765](https://github.com/emqx/emqx/pull/14765) SQL ServerコネクターにおけるNamed Instancesの使用に対する追加検証を行いました。  
  以前はユーザーが明示的にポートを指定したかどうかを判別できず、明示的に指定されていない場合は常にデフォルトポートを追加していました。

  Named Instancesの場合、ODBCドライバーで接続する際に明示的なポート指定が必要です。ドライバーは指定されたインスタンス名を無視して、そのポートで稼働しているインスタンスに接続します。
...
- [#14773](https://github.com/emqx/emqx/pull/14773) レート制限の設定オプションが変更されました。  
  ...
  - 非公開のエンドポイント `/configs/limiter` は削除されました。
...
- [#14957](https://github.com/emqx/emqx/pull/14957) プラグイン設定の更新方法が変更されました。  
  Dashboardを通じて行われる新しい設定更新時に、`on_config_changed` コールバックの結果が尊重されるようになりました。既にクラスターに保存されている設定に対しては引き続き無視されます。

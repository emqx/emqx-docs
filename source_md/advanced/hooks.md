...
当系统中不存在 **钩子(Hooks)** 模块时，整个事件处理的流程 *从事件(Event)的输入，到处理(Handler)，在到完成后的返回的结果(Result)* 对于系统外部而讲，都是不可见、且无法修改的。

而在这个过程中加入一个可挂载函数的点（一般称这个点为挂载点 - HookPoint，同时这个点的名称也是钩子的名称），让其允许外部可以挂载多个回调函数，形成一个调用链。最终达到对该事件处理的过程进行扩展和自定义的目的。


系统中常用到的认证插件则是按照该逻辑进行实现的。以最简单的 [emqx_auth_username](https://github.com/emqx/emqx-auth-username) 为例：

在只开启 `emqx_auth_username` 认证插件时，且设置[不允许匿名用户登录](#)，按照上图对事件的处理逻辑可知，此时认证模块的逻辑为：
...
- **回调链** 是具有传递性的，意思是指，链会将输入给链的入参输入给第一个回调函数，第一个回调函数的返回值会传递给第二个回调函数，直到最后一个函数，最后一个函数的返回值则为整个链的返回值。
- **回调链** 需要允许其上面的函数 *提前终止链* 和 *忽略本次操作*
    
    - **提前终止**：本函数执行完成后，直接终止链的执行。忽略链上后续所有的回调函数。例如：某个认证插件权限较高，认为某客户端允许登录后便不再检查其他认证插件，所以需要提前终止。
    - **忽略本次操作**：不修改链上的处理结果，直接透传给下一个回调函数。例如：存在多种认证情况时，某认证插件认为，此类客户端不属于我的认证范围，所以我不需要修改认证数据，直接将前一个函数的返回值传递给链上下一个函数。
...
挂载完成后，钩子会按优先级从大到小执行，同一优先级按挂载的先后顺序执行。所有官方插件挂载的钩子优先级都为 `0`

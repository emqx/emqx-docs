...
<ul>
  <li>`[]`: The default value, it allows *ALL* logins</li>
  <li>one: For example `{enable:true,backend:"built_in_database",mechanism="password_based"}`</li>
...
EMQX will drop old buffered messages under high memory pressure. The high memory threshold is defined in config `sysmon.os.sysmem_high_watermark`. NOTE: This config only works on Linux.
...
value of `kafka_headers` field before sending to Kafka.
...
`key_dispatch`: Hash Pulsar message key of the first message in a batch to a partition number.
...
**bridges.azure_event_hub_producer.$name.kafka.message.key**
  ...
  *Default*: `${.clientid}`
  ...
  Template to render Azure Event Hub message key. If the template is rendered into a NULL value (i.e. there is no such data field in Rule Engine context) then Azure Event Hub's `NULL` (but not empty string) is used.
...
**bridges.azure_event_hub_producer.$name.kafka.message.value**
  ...
  *Default*: `${.}`
  ...
  Template to render Azure Event Hub message value. If the template is rendered into a NULL value (i.e. there is no such data field in Rule Engine context) then Azure Event Hub's `NULL` (but not empty string) is used.
...
`random`: Randomly pick a partition for each message
`key_dispatch`: Hash Azure Event Hub message key to a partition number
...
The key-value pairs here will be combined with the
value of `kafka_headers` field before sending to Azure Event Hub.
...
**bridges.azure_event_hub_producer.$name.bootstrap_hosts**
  ...
  *Type*: `string`
  ...
  A comma separated list of Azure Event Hub Kafka `host[:port]` namespace endpoints to bootstrap the client.  Default port number is 9093.
...
`message_id`: the message ID assigned by GCP PubSub.
`publish_time`: message timestamp assigned by GCP PubSub.
`topic`: GCP PubSub topic.
`value`: the payload of the GCP PubSub message.  Omitted if there's no payload.
`attributes`: an object containing string key-value pairs.  Omitted if there are no attributes.
`ordering_key`: GCP PubSub message ordering key.  Omitted if there's none.

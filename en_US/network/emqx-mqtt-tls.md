...
This chapter will mainly introduce how to directly establish an SSL/TLS connection between the client and EMQX. For information on how to terminate TLS connections through a proxy or load balancer, please refer to [Cluster Load Balancing](../../en_US/deploy/cluster/lb.md).
...
To enable TLS authentication for clients, you need to configure the `ssl` listener that listens for incomming SSL connections. You need to add the `listeners.ssl` configuration items in the `emqx.conf` file and set its `ssl_options.verify` option to `verify_peer`.

The following example shows how `SSL` listener is configured on the default port `8883`:
...
SSL Client options, refer to: [broker:ssl_client_opts](../configuration/configuration-manual.md#broker-ssl-client-opts)

### Ciphers

As an SSL listener or SSL client, the ciphers can be configured by the following suites:

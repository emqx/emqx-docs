...
EMQX provides end-to-end tracing capabilities based on the OpenTelemetry standard. This allows you to monitor the entire lifecycle of MQTT messages and client activities within the EMQX cluster. This document details the various spans that EMQX generates, providing insight into the internal workings of the broker.

## Client Lifecycle Spans

These spans trace the primary lifecycle events of an MQTT client.

- **`client.connect`**: This is a root span that traces the client connection process. It starts when a client initiates a connection to the broker and ends when the connection is established or rejected.

- **`client.disconnect`**: This span traces the client disconnection process. It starts when a client sends a `DISCONNECT` packet or when the connection is closed for other reasons (e.g., network error, keep-alive timeout).

- **`client.subscribe`**: This span traces a client's subscription request. It covers the entire process of the broker receiving the `SUBSCRIBE` packet, processing the subscriptions, and sending a `SUBACK` packet.

- **`client.unsubscribe`**: This span traces a client's unsubscription request. It covers the process from receiving the `UNSUBSCRIBE` packet to sending the `UNSUBACK` packet.

## Authentication and Authorization Spans

These spans provide visibility into the security-related checks performed for a client.
...
- **`client.publish`**: This is a root span that traces a message published by a client to the broker. It starts when the broker receives the `PUBLISH` packet.

- **`message.route`**: A child of `client.publish`, this span traces the routing of the message within the broker to find matching subscribers.
...
- **`broker.publish`**: This span traces the broker preparing to publish a message to a subscriber. It is a child of `message.route` or `message.handle_forward`.
...
These spans trace the execution of the rule engine.

- **`broker.rule_engine.apply`**: Traces the rule engine processing a message to see if it matches any rules. This is a child of the `message.route` span.

- **`broker.rule_engine.action`**: Traces the execution of a specific action within the rule engine if a rule is matched. This is a child of `broker.rule_engine.apply`.

## Broker Internal Spans

These spans trace internal broker operations.
...
1.  **Trace Context Source (`follow_traceparent`)**: The first decision is where to get the initial trace context from. This is controlled by the `follow_traceparent` boolean switch.
    -   If `true` (the default), EMQX will try to extract trace context from the incoming request (e.g., from the `traceparent` user property in an MQTT packet). This allows you to link traces that originate from upstream instrumented applications.
    -   If `false`, EMQX will ignore any incoming trace context and always start a new trace.
...
3.  **Whitelist Rules**: If the trace is not already sampled by a remote parent, you can define specific rules to force sampling for certain clients or topics. This is the most direct way to ensure you are tracing the activity you care about.
    -   **By ClientID**: If a client's ID is in the ClientID whitelist, all its root-level activities (connect, subscribe, publish, etc.) will be traced.
    -   **By Topic**: If a message is published to a topic that matches a filter in the topic whitelist, the message's trace will be sampled.
...
4.  **Ratio-Based Sampling**: If no whitelist rule matches, the decision falls back to ratio-based sampling, which is controlled by the `sample_ratio` configuration.
    -   `sample_ratio`: You can configure this ratio (from `0.0` to `1.0`) to control the percentage of traces that are captured. A value of `1.0` means 100% of traces will be captured, while `0.0` means none will be (unless they match a whitelist rule).

5.  **Event Type Switches**: Even if a trace is selected by the ratio-based sampler, it will only be generated if the corresponding event type is enabled via its configuration switch. These switches act as a global on/off for categories of spans. The available switches are:
...
6.  **Message Trace Level**: For spans related to QoS acknowledgements (e.g., `PUBACK`, `PUBREC`), you can control their creation based on a QoS level using the `msg_trace_level` switch.
    -   `msg_trace_level`: This setting can be configured to a specific QoS level (0, 1, or 2) to control which acknowledgement spans are created based on the QoS of the original message. For example, if `msg_trace_level` is set to `1`, `PUBACK` spans will be created for QoS 1 messages. For QoS 2 messages, this setting will generate `PUBREC` spans, but not `PUBREL` or `PUBCOMP` spans. This helps to reduce the verbosity of traces for high-QoS message flows.
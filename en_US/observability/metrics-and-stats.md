...
| actions.messages | The number of messages handled by action executions. Because a single action execution may process a batch of messages, this value is greater than or equal to `actions.executed` and reflects the true per-message throughput of actions. |

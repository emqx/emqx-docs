...
| clientid_re (from e5.9.0) | No       | Regular expression for the client ID, e.g., `^client-[0-9]+$`                                                  |
| username_re (from e5.9.0) | No       | Regular expression for the username, e.g., `^user-[0-9]+$`                                                     |
| ipaddr (from e5.9.0)      | No       | IP address or subnet, e.g., `192.168.5.0/24`                                                                   |
| zone (from e5.9.0)        | No       | Zone name, e.g., `zone1`                                                                                       |
| zone_re (from e5.9.0)     | No       | Regular expression for the zone name, e.g., `^zone-[0-9]+$`                                                    |
| listener (from e5.9.0)    | No       | Listener name, e.g., `tcp:default`                                                                             |
| listener_re (from e5.9.0) | No       | Regular expression for the listener name, e.g., `^tcp:.*$`                                                     |

All present fields should match for a rule to be applied.

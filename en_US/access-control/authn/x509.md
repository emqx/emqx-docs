...
Use `cert_san.dns`, `cert_san.ip`, `cert_san.email`, and `cert_san.uri` in `mqtt.client_attrs_init` expressions. These variables are arrays, so use Variform functions such as `nth()` or `join_to_string()` to produce a client attribute value. For configuration details and examples, see [Initialize Client Attributes from Certificate Subject Alternative Names](../../client-attributes/client-attributes.md#initialize-client-attributes-from-certificate-subject-alternative-names).

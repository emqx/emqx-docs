...
::: warning mTLS Is Required When Mapping Peer Certificate Fields
...
Without mTLS, a client can connect without presenting a certificate and EMQX will derive an empty CN/DN, which becomes a blank username or client ID. A blank username typically bypasses or short-circuits the authorization checks that downstream rules rely on, and a blank client ID collides with every other unauthenticated client. With mTLS enforced, EMQX rejects the handshake before the mapping is applied.

**The same applies when terminating TLS at an upstream load balancer and forwarding peer cert info via PROXY protocol v2.** If you enable `proxy_protocol = true` on a TCP listener and trust the CN/DN that the LB injects:

- The load balancer must itself perform mTLS and only forward the verified CN/DN.

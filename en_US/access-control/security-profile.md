...
The `hardened` profile is planned to become the default in a future major release. New deployments should start with the `hardened` profile. Existing deployments can switch to `hardened` after reviewing the behavior changes listed below.
...
2. Verify that listeners and the Dashboard have explicit bind addresses when they must accept external connections.
3. Verify that every node has authentication configured, or that anonymous access is explicitly enabled where intended.
4. Change any Dashboard account that still uses the default password.
5. Set `EMQX_SECURITY_PROFILE=hardened` on all nodes and restart them one at a time.

...
- Bind the MQTT client ID to the authenticated identity whenever the authentication mechanism allows it. For example, verify a JWT `clientid` claim, map a certificate field with [`peer_cert_as_clientid`](./authn/x509.md#certificate-information-mapping), have the HTTP authenticator reject mismatches, or pair the authenticator with a [Client-Info](./authn/cinfo.md) rule. Without this binding:
  ...
  - An attacker holding any valid credential who knows a victim's client ID can take over the victim's session, because MQTT identifies and resumes a session by its client ID alone. Connecting with the victim's client ID kicks the victim offline (`DISCONNECT 0x8E`) and, with `Clean Start = 0`, resumes the victim's session — inheriting its existing subscriptions and receiving messages the attacker's own ACL would otherwise deny. EMQX authorizes a subscription when it is created and does not re-evaluate an inherited subscription against the resuming identity, so binding the client ID to the credential (so the takeover fails at connect time) is the effective control. Note that publishing is unaffected: it is authorized per message against the current identity.

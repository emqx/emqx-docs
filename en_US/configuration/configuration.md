...
- `CODE_LOADING_MODE`: Controls Erlang code loading. When `EMQX_FEATURES=ESSENTIAL` and `CODE_LOADING_MODE` is unset or empty, EMQX defaults it to `interactive`; otherwise, it defaults to `embedded`. Set it explicitly to `embedded` to preload Erlang modules at startup when using the `ESSENTIAL` preset.
...
- **Schema-less encode/decode functions**:
  ...
  - [base64_decode(Data, 'no_padding')](../data-integration/rule-sql-builtin-functions.md#base64-decode-data-string-bytes-string)  (since 6.0.2)
  - [base64_decode(Data, 'no_padding', 'urlsafe')](../data-integration/rule-sql-builtin-functions.md#base64-decode-data-string-bytes-string)  (since 6.0.2)

...
- `high_watermark_timezone` is the timezone used to determine the local day boundary when recording daily session high-watermark history. Use `"system"` to follow the node host's local timezone, or provide an explicit UTC offset such as `"+08:00"`. Default: `"system"`

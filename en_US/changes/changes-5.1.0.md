...
-   [#10841](https://github.com/emqx/emqx/pull/10841) Added a schema validation to ensure message key is not empty when "dispatching by key" is selected in Kafka and Pulsar Producer bridges.
...
-   [#11062](https://github.com/emqx/emqx/pull/11062) Renamed `log.file.to` to `log.file.path`.
...
-   [#10785](https://github.com/emqx/emqx/pull/10785) Ensured that `EMQX_LOG_DIR` is set by Windows boot script.
    The environment variable `EMQX_LOG_DIR` was missing in e5.0.4, caused EMQX Windows package fail to boot unless set by sysadmin.
...
-   [#10878](https://github.com/emqx/emqx/pull/10878) Rectified a vulnerability in the RabbitMQ bridge, which could potentially expose passwords to log files.
...
-   [#10902](https://github.com/emqx/emqx/pull/10902) Avoid syncing cluser.hocon file from the nodes runing a newer version than the self-node.
...
-   [#10998](https://github.com/emqx/emqx/pull/10998) Do not allow `batch_size` option for MongoDB bridge resource. MongoDB connector currently does not support batching, the `bath_size` config value is forced to be 1 if provided.
...
-   [#11100](https://github.com/emqx/emqx/pull/11100) Fixed problem when replicants were unable to connect to the core node due to timeout in `mria_lb:core_nodes()` call.

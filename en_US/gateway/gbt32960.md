...
As the concept of username and password is already defined in the connection message of the GB/T 32960 protocol, the GB/T 32960 supports a variety of authenticator types, such as:
...
GB/T 32960 gateway uses the information in the `login` command and use the `vin` code as `clientid` to generate the authentication fields for the client:

- Client ID: the `vin` code
- Username: the `vin` code

You can also use HTTP API to create a built-in database authentication for a GB/T 32960 gateway:
...
When no authenticator is enabled, all GB/T 32960 clients are allowed to log in.

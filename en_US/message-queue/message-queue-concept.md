...
Message Queue does not work for clients connected through a listener with a [mountpoint](../configuration/listener.md#mountpoint) configured. The mountpoint is prepended before EMQX matches the `$q/` prefix, so the subscription is treated as an ordinary subscription to the mounted literal topic, and no error is reported to the client.
:::

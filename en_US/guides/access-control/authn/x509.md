...
`peer_cert_as_username` and `peer_cert_as_clientid` are global `mqtt` settings: once set to anything other than `disabled`, the mapping applies to **every** listener that shares this configuration, not only the one TLS listener you have in mind. EMQX does not automatically require certificate verification when these are enabled, so you must enforce it yourself on each effective TLS listener that can serve these clients:

- `verify = verify_peer`
- `fail_if_no_peer_cert = true`
- `cacertfile` set to a CA bundle you control (do not trust public CAs that can issue certificates for arbitrary names)

A single listener that leaves the defaults (`verify = verify_none`, `fail_if_no_peer_cert = false`) is enough to reopen the hole for every client it accepts. Re-audit this whenever you add a listener or change a listener's TLS options.

The same requirement applies to any [`client_attrs_init`](../../../develop/client-attributes/client-attributes.md) expression that reads the certificate `cn` or `dn` field. Those values populate `client_attrs`, which are typically consumed by authorization templates such as `${client_attrs.X}`, so without enforced mTLS an attacker can set them from a self-signed certificate and gain access intended for another identity.
...
The mapping is also silently skipped when no peer certificate is available — on a plaintext listener, or on a TLS listener where the client presents no certificate because verification is not enforced. In that case the certificate-derived identity is left unset and EMQX falls back to the username and client ID from the MQTT `CONNECT` packet, quietly bypassing the identity binding you intended. Requiring `fail_if_no_peer_cert = true` closes this fallback by rejecting any client that does not present a certificate.

...
Starting from EMQX Enterprise 6.3.0, you can initialize client attributes from subject alternative names (SANs) in a client certificate when the TLS connection terminates at EMQX. The supported SAN types are DNS names, IPv4 and IPv6 addresses, email addresses, and URIs.

Use `cert_san.dns`, `cert_san.ip`, `cert_san.email`, and `cert_san.uri` in `mqtt.client_attrs_init` expressions. These variables are arrays, so use Variform functions such as `nth()` or `join_to_string()` to produce a client attribute value. For configuration details and examples, see [Initialize Client Attributes from Certificate Subject Alternative Names](../../../develop/client-attributes/client-attributes.md#initialize-client-attributes-from-certificate-subject-alternative-names).

`peer_cert_as_username` and `peer_cert_as_clientid` do not support SAN extraction. To use a SAN in authentication, authorization, or another feature that supports client attributes, first save the value as a client attribute.

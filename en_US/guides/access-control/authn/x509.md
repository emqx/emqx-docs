...
`peer_cert_as_username` and `peer_cert_as_clientid` are global `mqtt` settings. When either setting has a value other than `disabled`, the mapping applies to **every** listener that shares this configuration. EMQX does not automatically enable certificate verification for these settings, so every TLS listener that accepts these clients must enforce the following configuration:

- `verify = verify_peer`
- `fail_if_no_peer_cert = true`
- `cacertfile` set to a CA bundle you control (do not trust public CAs that can issue certificates for arbitrary names)

A single listener that retains the default settings (`verify = verify_none`, `fail_if_no_peer_cert = false`) exposes every client it accepts to the same risk. Review these settings whenever you add a listener or change its TLS options.

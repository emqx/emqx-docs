...
- If you map peer-certificate fields into the MQTT identity, the relevant global `mqtt` settings apply to every listener that shares this configuration. The mapping can set the username or client ID (`peer_cert_as_username` / `peer_cert_as_clientid`) or populate `client_attrs` through a [`client_attrs_init`](../../develop/client-attributes/client-attributes.md) expression that reads `cn` or `dn`. Every TLS listener that accepts these clients **must** enforce mTLS (`verify = verify_peer`, `fail_if_no_peer_cert = true`) with a CA bundle you control. EMQX does not enforce these settings automatically. If even one listener retains the `verify_none` / `fail_if_no_peer_cert = false` defaults, a client can present a self-signed certificate with an attacker-chosen CN/DN and impersonate any identity, including through forged values in authorization templates such as `${client_attrs.X}`. A client can also connect without a certificate and silently fall back to the username or client ID from the MQTT `CONNECT` packet. As an additional safeguard for the empty-username case, set `listeners.{type}.{name}.enable_authn = quick_deny_anonymous`. See [Certificate Information Mapping](./authn/x509.md#certificate-information-mapping).

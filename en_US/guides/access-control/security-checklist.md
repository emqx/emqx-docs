...
- For brokers exposed to untrusted networks, consider setting `authorization.deny_action = disconnect` (default: `ignore`) so that a client whose publish or subscribe is denied is disconnected rather than left to keep probing topics on the same connection. Combined with [Flapping Detect](./flapping-detect.md), clients that repeatedly reconnect and trigger denials are banned automatically. Because `deny_action` is a global setting that also disconnects legitimate clients attempting a denied operation, apply it where clients only access their authorized topics, and tune flapping-detection thresholds so that normal reconnection storms do not get clients banned. See [Authorization](./authz/authz.md).

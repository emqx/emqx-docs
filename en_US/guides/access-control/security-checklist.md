...
- If you map peer-certificate fields into the MQTT identity — as the username or client ID (`peer_cert_as_username` / `peer_cert_as_clientid`), or into `client_attrs` via a [`client_attrs_init`](../../develop/client-attributes/client-attributes.md) expression that reads `cn` or `dn` — remember these are global `mqtt` settings that apply to every listener sharing them, so **each** TLS listener that can serve these clients **must** enforce mTLS (`verify = verify_peer`, `fail_if_no_peer_cert = true`) with a CA bundle you control. EMQX does not enforce this for you: a single listener left at the `verify_none` / `fail_if_no_peer_cert = false` defaults lets a client present a self-signed certificate with an attacker-chosen CN/DN and impersonate any identity — including by feeding forged values into authorization templates such as `${client_attrs.X}` — or connect with no certificate at all and silently fall back to the `CONNECT` username/client ID. As an additional layer for the empty-username case, set `listeners.{type}.{name}.enable_authn = quick_deny_anonymous`. See [Certificate Information Mapping](./authn/x509.md#certificate-information-mapping).

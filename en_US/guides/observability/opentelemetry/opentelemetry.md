...
EMQX supports directly pushing telemetry data to the OpenTelemetry Collector via OTLP/gRPC, and then transferring, filtering, or converting the data through the Collector to any backend you want to integrate for storage and visualization, such as Jaeger and [Prometheus](../prometheus.md). Starting from EMQX 6.3.0, EMQX also supports exporting OpenTelemetry logs and traces directly to [Dynatrace](./dynatrace.md) through OTLP HTTP/protobuf with OAuth2 authentication.
...
::: tip
Starting from EMQX 6.3.0, OpenTelemetry is controlled by the `opentelemetry` feature gate. If you set `EMQX_FEATURES` manually, enabling `opentelemetry` also enables its required `dashboard` dependency. For more information, see [Feature Gates](../../../get-started/deploy/feature-gates.md).
:::

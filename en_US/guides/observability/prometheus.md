...
::: tip
Starting from EMQX 6.3.0, Prometheus metrics are controlled by the `metrics` feature gate. If you set `EMQX_FEATURES` manually, enabling `metrics` also enables its required `dashboard` dependency. Authentication and authorization are core capabilities and are not controlled by a feature gate. For more information, see [Feature Gates](../../get-started/deploy/feature-gates.md).
:::
...
1. Create an [API key](../api.md#authentication) with the `monitoring` scope in EMQX.

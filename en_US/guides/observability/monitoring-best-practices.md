...
- **Suggested actions:** Inspect connection, session, queue, and integration growth. Starting from EMQX 6.3.0, use [`emqx ctl session-top`](../cli.md#session-top) to identify sessions that retain the most MQTT payload bytes or have the longest message queues. Add capacity or reduce the source of growth before the operating system terminates the process. EMQX's built-in system-memory alarm defaults to 70%.

...
The following table describes the fields that can appear in audit log entries generated by Dashboard or REST API operations.
...
| http_request.query_string | Object | Optional. Parsed URL query parameters. This field is omitted when the request has no query parameters. Values under keys recognized as sensitive are redacted. |
| http_request.namespace | String | Optional. The resolved namespace targeted by the operation. The value is `global` for the global namespace. |
...
When a namespaced administrator targets a resource in their own namespace without a query parameter, the record omits `http_request.query_string` but still includes the resolved namespace in `http_request.namespace`.

Resolved target namespaces are recorded for namespaced operations involving authentication, authorization, connectors, bridges, rules, traces, data backup, A2A Registry, and Topic Metrics. Other endpoints that do not resolve a target namespace omit `http_request.namespace`.

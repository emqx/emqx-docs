...
Starting from EMQX 6.0.4, audit records include the non-empty query parameters of audited Dashboard and REST API requests in `http_request.query_string`. For data-backup operations, `http_request.namespace` records the resolved target namespace independently of whether the request includes a `namespace` query parameter. The following example shows a global administrator importing a backup into `ns2`:

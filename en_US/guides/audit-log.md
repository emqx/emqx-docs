...
EMQX Audit Log supports recording change-related operations from the [Dashboard](./dashboard/introduction.md), [REST API](../guides/api.md) and [CLI](./cli.md), such as Dashboard user logins and modifications to clients, access controls, and data integrations. However, read-only operations such as metrics retrieval and client list queries are not recorded.
...
| operate_id       | String  | The REST API path of the request, please refer to [REST API](../guides/api.md). |
...
| cmd         | String  | Specific command operation executed, for supported commands, please refer to [CLI](./cli.md). |

...
This plugin is available starting from EMQX 6.2.3. Install and start the plugin before using `maptab_lookup` in Rule SQL.

Mapping tables are loaded from JSON files through the `emqx ctl maptabs` CLI and stored in EMQX's replicated database. Rules query the tables with the `maptab_lookup` SQL function. For function signatures and SQL examples, see [Built-in SQL Functions](../../../../develop/data-integration/rule-sql-builtin-functions.md).

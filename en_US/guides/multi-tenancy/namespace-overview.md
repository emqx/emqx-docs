...
- **Admin User Isolation**
  ...
  - Starting from EMQX 6.0.4, namespaced administrators can [manage API keys within their own namespace](../api.md#manage-api-keys-as-a-namespaced-administrator). Global keys and keys in other namespaces remain inaccessible.
...
For details, see [Mitigate SSRF with Rule Engine Policy and Firewall Rules](../cluster/security.md#mitigate-ssrf-with-rule-engine-policy-and-firewall-rules).
...
Client ID and topic isolation are not enabled automatically. Configure them manually based on your trust model and business requirements. Starting from EMQX 6.0.3, rule trigger isolation is enabled by default, but it only limits which messages and events can trigger namespaced rules. It does not replace client ID or topic isolation.

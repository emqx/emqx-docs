...
For more details about the rate-limiting mechanism, see [Rate Limiting](../rate-limit.md).
...
Always check the corresponding [Swagger API documentation](../api.md) for detailed and up-to-date request and response endpoint schemas. These are served by the Dashboard listeners at `/api-docs`.
...
For command syntax, output, and error handling, see [`mt purge_ns`](../cli.md#mt).

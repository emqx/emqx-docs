...
Starting from EMQX 6.3.0, Prometheus pull endpoints require authentication by default. You can omit `basicAuth` only if you explicitly disable authentication by setting `prometheus.enable_basic_auth = false`. For all available endpoints and authentication options, see [Integrate with Prometheus](../../../../observability/prometheus.md#configure-pull-mode-integration).

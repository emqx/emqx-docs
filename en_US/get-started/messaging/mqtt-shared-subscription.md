...
EMQX also accepts the `$queue/` prefix as a shared subscription without a group name. This use is deprecated: when [Message Queue](../../develop/message-queue/message-queue-concept.md) is enabled, EMQX reserves the `$queue/` prefix for queue subscriptions, and the same topic filter no longer creates a shared subscription. Use `$share/<group-name>/` instead: a subscription to `$queue/t/1` is equivalent to `$share/$queue/t/1`.

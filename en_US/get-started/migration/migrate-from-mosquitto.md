...
Before importing data, configure the [password-based authentication](../../guides/access-control/authn/pwoverview.md) in EMQX using the [built-in database](../../guides/access-control/authn/mnesia.md) backend. You must use the specific settings below to match Mosquitto's default hashing mechanism.
...
Mosquitto uses bridges to forward messages and external scripts (Python/Node.js) for data processing. EMQX replaces these with the built-in [Rule Engine](../../develop/data-integration/rules.md) and [Data Integration](../../develop/data-integration/data-bridges.md).

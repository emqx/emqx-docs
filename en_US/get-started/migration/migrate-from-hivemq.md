...
For alternative discovery methods (etcd, Kubernetes, static files, etc.), see [Create and Manage Cluster](../../guides/cluster/create-cluster.md).
...
| :--- | :--- | :--- |
| **File Realm** (`credentials.xml`) | [**Built-in Database**](../../guides/access-control/authn/mnesia.md) | Export HiveMQ users and import via the EMQX REST API. |
| **SQL Realm** (JDBC) | [**MySQL**](../../guides/access-control/authn/mysql.md) / [**PostgreSQL**](../../guides/access-control/authn/postgresql.md) | Configure password-based authentication with a `mysql` or `postgresql` backend. Reuse existing user tables. |
| **LDAP Realm** / AD | [**LDAP**](../../guides/access-control/authn/ldap.md) | Configure password-based authentication with an LDAP backend. Map HiveMQ DN patterns to EMQX filter templates. |
| **OAuth / JWT** | [**JWT**](../../guides/access-control/authn/jwt.md) | Configure JWT authentication mechanism. Configure public keys or JWKS endpoint. |
| **HTTP / Webhooks** | [**HTTP Server**](../../guides/access-control/authn/http.md) | Configure password-based authentication with an HTTP backend to delegate credentials to your external auth service. |
| **X.509 Certs** | [**X.509**](../../guides/access-control/authn/x509.md) / [**mTLS**](../../guides/network/emqx-mqtt-tls.md#enable-ssl-tls-with-two-way-authentication) | Use `TLS` listener and mutual (two-way) authentication, reusing existing CA and client certificates. |
...
2. **Import:** Use the EMQX REST API to create users. EMQX supports bulk import of users with password hashes (e.g., bcrypt, pbkdf2). See [Importing Users](../../guides/access-control/authn/user_management.md#importing-users) for file format details.
...
HiveMQ uses a [fixed database schema](https://docs.hivemq.com/hivemq-enterprise-security-extension/latest/ese.html#table_users) for its SQL realm. In contrast, EMQX allows you to [define your own schema and queries](../../guides/access-control/authn/mysql.md). **This means you do not need to modify your existing MySQL or PostgreSQL database.** 
...
- [**File (`acl.conf`)**](../../guides/access-control/authz/file.md): `{allow, all, subscribe, ["device/${clientid}/#"]}.`
- [**Built-in Database**](../../guides/access-control/authz/mnesia.md): Configure rules via Dashboard or API based on Client ID, Username, or Topic.
...
For more information, refer to the [**Authorization**](../../guides/access-control/authz/authz.md) documentation.
...
- [**Data Integration Overview**](../../develop/data-integration/data-bridges.md)
- [**Rule Engine**](../../develop/data-integration/rules.md)
- [**Flow Designer**](../../develop/flow-designer/introduction.md)
...
Starting from EMQX 6.3.0, Prometheus scrape endpoints require authentication by default. Configure the scraper with an API key and secret key, or explicitly disable authentication. For details, see [Integrate with Prometheus](../../guides/observability/prometheus.md#authentication).
...
For Pushgateway configuration details, see [Integrate with Prometheus](../../guides/observability/prometheus.md#configure-push-mode-integration).
...
See [Logs](../../guides/observability/log.md) for configuring log levels, rotation, and formatters (text/JSON).
...
See [Log Trace](../../guides/observability/tracer.md) for advanced debugging.

...
| [CentOS/RHEL](./install-rhel.md)  | CentOS 7<br />Rocky Linux 8<br />Rocky Linux 9<br />Rocky Linux 10 | Yes | Yes |
...
Even if a cluster is not formed, EMQX will still listen on ports 4370 and 5370. These two ports are fixed and cannot be modified. The Offset is determined by the numeric suffix of the Name part in the node name (`Name@Host`). If there is no numeric suffix, the default is 0. For more information, refer to [Port Mapping](../../guides/cluster/security.md#port-mapping).
...
| ---------- | ------------------ | --------------------- | ------------------------ |
| `etc`      | Static config files | `./etc`               | `/etc/emqx`      |
| `data`     | Database and config | `./data`              | `/var/lib/emqx`     |
...
| --------- | ------------------- | ----------- | ------------------------------------------------------------ |
| bin       | Executables         | Read        | `emqx` and `emqx.cmd`: Executables of EMQX. For details, see [Command Line Interface](../../guides/cli.md). |

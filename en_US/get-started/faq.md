...
If it is confirmed that the above situation does not exist, yet the issue of message loss persists, you can use the [Log Trace](../guides/observability/tracer.md) to conduct further investigation.

## How to troubleshoot the cause of SSL/TLS connection failure?

Usually when the SSL/TLS connection handshake fails, EMQX will output the corresponding failure reason in the [Log](../guides/observability/log.md). The following are some common keywords in the log and their corresponding meanings:
...
You can also use the [Log Trace](../guides/observability/tracer.md) to trace all the logs related to the Client ID, IP, and topic you specify, and then you can analyze why the client disconnected based on these logs.
...
Different use cases require different tuning. Refer to [Performance Tuning](../guides/performance/tune.md) for tuning the system for general purposes.

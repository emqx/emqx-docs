...
The Queue Key Expression specifies how to extract the key used for message deduplication in Last-Value Semantics mode. This expression is evaluated against a message's data and follows the syntax of [Variform expressions](../../guides/configuration/configuration.md#variform-expressions).
...
All REST API operations require appropriate authentication and permissions. For detailed request and response schemas, refer to the "Message Queue" section in [REST API](../../guides/api.md).
...
- **Size-based limits**: You can optionally configure per-shard limits on:
  ...
  Note that these limits apply per shard in durable storage. For information on how to configure the number of shards, see [Number of Shards](../../guides/durability/managing-replication.md#number-of-shards). In addition, size limits do not account for the [replication factor](../../guides/durability/managing-replication.md#replication-factor); the actual physical storage used by a queue will be multiplied by the replication factor.

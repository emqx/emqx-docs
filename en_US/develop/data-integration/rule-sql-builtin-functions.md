...
Looks up a row in a mapping table managed by the [EMQX Mapping Tables Plugin](../../guides/extensions/plugin-catalog/6.2/emqx-maptabs.md). It returns the row's value map, or `undefined` when the table does not exist, the key does not exist, or the key type does not match.

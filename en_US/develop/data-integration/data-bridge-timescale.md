...
| ------------------------- | ------------------------------------------------------------ | --------------------- |
| **Application Name**      | Specifies the application name for PostgreSQL connections. The value appears in PostgreSQL activity views and logs. It must be 1 to 63 bytes long and contain only printable ASCII characters. Zero bytes are not allowed. | `emqx`                |

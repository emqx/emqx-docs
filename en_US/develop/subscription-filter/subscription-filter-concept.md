...
When [Authorization](../../guides/access-control/authz/authz.md) is enabled, EMQX validates the subscription topic against configured rules. The topic used for authorization is the **base topic filter** (the portion before the `?` delimiter). The filter expression is stripped before authorization is evaluated.
...
- [Shared Subscriptions](../../get-started/messaging/mqtt-shared-subscription.md): Distributes messages among a subscriber group for load balancing. Does not support content-based filtering.
- [Retained Messages](../../get-started/messaging/mqtt-retained-message.md): Stores the last message per topic for delivery to new subscribers. Retained message delivery is not affected by Subscription Filter expressions.
- [Topic Rewrite](../../get-started/messaging/mqtt-topic-rewrite.md): Rewrites topic strings before routing. Topic rewrite rules are applied before Subscription Filter evaluation.
- [Wildcard Subscription](../../get-started/messaging/mqtt-wildcard-subscription.md): Matches multiple topics using `+` and `#` wildcards. Wildcard topic filters can be combined with Subscription Filters.

...
For detailed instructions on creating and running an EMQX cluster, refer to [EMQX Cluster](../cluster/introduction.md).
...
- *Durable Storage (DS)*: a disk-based streaming database, used for write-heavy workloads and large volumes of data, such as messages sent to [durable sessions](../../guides/durability/durability_introduction.md) and [message queues](../message-queue/message-queue-concept.md). This database is designed for *consistency*.

These two database management systems have vastly different properties and provide different guarantees during network partition. This document focuses on *Mria* and behavior of EMQX without going into details about durability features, which are optional. For information regarding durable storage, please read the following document: [Durable Storage](./durable-storage.md).
...
To address this issue, EMQX (since version 4.4) introduced a new feature called "[Cluster Load Rebalancing](../../guides/cluster/rebalancing.md)". This feature allows the cluster to automatically rebalance the load by migrating the sessions from the overloaded nodes to the underloaded nodes.
...
1. The routing table contents are merged: nodes in the majority partition re-establish routes to the minority and vice versa.
1. Clients re-establish their presence in the global session registry.

For the log messages and alarms generated during partition recovery, see [Mria Logs and Alarms](../../guides/observability/mria-alarms.md).

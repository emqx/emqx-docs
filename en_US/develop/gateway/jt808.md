...
If EMQX is running in a cluster, configurations via Dashboard or HTTP API will be effective across the entire cluster. If you only wish to configure a single node, you can configure the gateway in [`base.hocon`](../../guides/configuration/configuration.md).
...
- For detailed HTTP API interface, refer to: [HTTP API - Gateway](../../guides/api.md)

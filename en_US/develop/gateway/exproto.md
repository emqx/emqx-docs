...
The ExProto gateway in EMQX can be configured and enabled through the Dashboard, HTTP API, or configuration file `base.hocon`. This section demonstrates how to enable ExProto gateway via the Dashboard.
...
If you are running EMQX in a cluster, the settings you made through the Dashboard or HTTP API will affect the whole cluster. If you only want to change the settings with one node, configure the gateway in [`base.hocon`](../../guides/configuration/configuration.md).

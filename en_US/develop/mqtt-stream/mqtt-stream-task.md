...
3. Configure the following options:
   ...
   - **Stream Key Expression**: Required. Defines the expression used to extract a key from each incoming message. The default value is `message.from`, which means the client ID of the message publisher. This field supports configuration using [Variform expressions](../../guides/configuration/configuration.md#variform-expressions).
...
The Stream Key Expression specifies how to extract the key used for message deduplication in Last-Value Semantics mode. This expression is evaluated against a message's data and follows the syntax of [Variform expressions](../../guides/configuration/configuration.md#variform-expressions).
...
### Stream Key Expression Examples
...
Assume the same messages are published to EMQX as in Example 1. When a client connects and subscribes to the stream, the following messages will be delivered:
| N | Sender | Topic | User Property `stream-key` |
|---|--------|-------|------------------------|
| 2 | `client1` | `t/2` | `keyB` |
| 4 | `client2` | `t/4` | `keyB` |
...
When a client connects and subscribes to the stream, all the messages will be delivered because the combination of `message.headers.properties.User-Property.stream-key` and `message.topic` is unique for each message:
...
All REST API operations require appropriate authentication and permissions. For detailed request and response schemas, refer to the "MQTT Stream" section in [REST API](../../guides/api.md).

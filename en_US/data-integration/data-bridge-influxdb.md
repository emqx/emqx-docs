...
4. In the **Configuration** step, configure the following information:
   ...
   - (InfluxDB v1 only) Toggle **Ping With Auth** to control whether EMQX includes connector credentials in `/ping` health-check requests. Enable this option if your InfluxDB service requires authentication on `/ping`. The default is disabled, which preserves the previous behavior and sends `/ping` without authentication.

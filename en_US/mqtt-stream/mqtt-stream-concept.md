...
Auto-creation is enabled by default for last-value streams and disabled for regular streams, but it only takes effect while the MQTT Streams feature is active. The feature itself is disabled by default (`streams.enable = false`). Once you enable it, a subscription to an unused stream name creates the stream. On deployments that accept untrusted clients, allow access only to specific pre-created streams, such as `$stream/events`, and deny all other subscriptions that match `$stream/#` or `$s/#`. Alternatively, disable auto-creation and create streams from the Dashboard or the REST API. See [Automatically Create Streams via Dashboard](./mqtt-stream-task.md#automatically-create-streams-via-dashboard).

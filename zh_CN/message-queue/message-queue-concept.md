...
EMQX 对队列订阅的授权基于完整主题，包括 `$queue/` 前缀和队列名称。请针对该完整主题编写授权规则。队列订阅还会投递该队列在订阅创建之前已存储的消息。
...
- 通配符规则永远不会匹配以 `$` 开头的主题。拒绝 `#` 的规则（包括默认 `acl.conf` 中的 `{eq, "#"}` 规则）不会拒绝 `$queue/orders/#`。

因此，一个被拒绝订阅 `#` 的客户端仍然可以订阅 `$queue/orders/#`，并接收该队列中的所有消息。请为 `$queue/` 命名空间添加显式规则，并使其严格程度不低于队列所摄取主题对应的规则：
...
该行为与[共享订阅](../messaging/mqtt-shared-subscription.md)不同。对于 `$share/<group>/t/#`，EMQX 会去掉前缀并对 `t/#` 授权；对于 `$queue/<name>/t/#`，EMQX 对完整主题授权。

### 自动创建会将摄取主题的控制权交给客户端

启用队列自动创建后，由发起订阅的客户端决定新队列的主题过滤器。EMQX 直接使用 `$queue/<name>/<topic_filter>` 中的过滤器创建队列，并不检查该客户端是否有权直接订阅该过滤器。被允许订阅 `$queue/+/#` 的客户端可以创建一个覆盖 `#` 的队列，从而读取整个主题空间。

最后值队列的自动创建默认启用。在接受不受信任客户端的部署中，请按上文限制 `$queue/` 命名空间，或禁用自动创建并改为通过 Dashboard 或 REST API 创建队列。参见[通过 Dashboard 自动创建队列](./message-queue-task.md#通过-dashboard-自动创建队列)。

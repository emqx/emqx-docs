...
自动创建对最后值队列默认启用，对常规队列默认禁用，但它仅在消息队列功能处于活动状态时生效。在默认的 `mq.enable = auto` 配置下，EMQX 仅在已存在至少一个队列时才激活该功能，因此订阅无法创建第一个队列。该功能激活后，或当 `mq.enable = true` 时，订阅一个尚未使用的队列名称即可创建该队列。在接受不受信任客户端的部署中，仅允许访问特定的预创建队列（例如 `$queue/orders`），并拒绝与 `$queue/#` 或 `$q/#` 匹配的所有其他订阅。或者，禁用自动创建并通过 Dashboard 或 REST API 创建队列。参见[通过 Dashboard 自动创建队列](./message-queue-task.md#通过-dashboard-自动创建队列)。

...
消息队列在可以存储或分发消息之前，必须显式声明/创建。您可以通过手动或自动的方式创建消息队列，自动创建的方法请参见[通过 Dashboard 自动创建队列](#通过-dashboard-自动创建队列)。
...
3. 在**创建队列**对话框中，配置以下选项：
   ...
   - **名称**： 指定队列的唯一名称。队列名称仅允许包含以下字符：
     ...
     客户端可以通过以下订阅格式消费队列中的消息：

     - `$queue/<name>`：当队列已存在时使用。
     - `$queue/<name>/<topic_filter>`：在订阅已存在队列时可选使用；当启用自动创建功能时，如果队列尚不存在，EMQX 将使用提供的 `<topic_filter>` 自动创建该队列。

   - **过滤主题**：输入主题或主题过滤器（例如 `t/1`）。该字段用于根据主题匹配决定哪些发布的消息将被写入该队列。一个队列将收集所有匹配该过滤器的消息。

     > 过滤主题属于队列的配置属性，但不定义队列的身份。
...
当客户端订阅带有 `$queue/` 前缀的主题时，消息队列可以自动创建。这使得队列能够在无需手动预先创建的情况下动态生成。

当启用自动创建功能时：

- 订阅 `$queue/<name>` 仅在队列已存在时有效。
- 订阅 `$queue/<name>/<topic_filter>` 时，如果队列尚不存在，EMQX 将使用提供的 `<topic_filter>` 自动创建该队列。

队列可以自动创建为**常规队列**或**最后值语义队列**。

::: tip

为了确保队列行为的正确性，只能启用**自动创建常规队列**或**自动创建最后值语义队列**其中之一，不能同时启用。

:::

### 自动创建最后值语义队列

此选项在**MQTT 配置**的**队列**选项卡中默认启用。启用后，EMQX 将自动创建具有最后值语义的队列，即仅保留每个键对应的最新消息。

1. 进入**管理** -> **MQTT 配置** -> **队列**选项卡。
2. 默认情况下， **启用自动创建队列** -> **最后值语义队列**选项已启用。您可以配置以下参数：
   - **[队列键表达式](#队列键表达式)**：必填项，用于定义如何从消息中提取唯一键（默认值为 `message.from`）。
   - **派发策略**：定义消息如何分发给订阅者（默认值为 `Random`）。
   - **数据保留期**：指定消息在队列中保留的时长。
3. 配置完成后，点击**保存更改**。

当客户端订阅 `$queue/my_queue/test` 时，如果 `my_queue` 尚不存在，EMQX 会自动创建一个名为 `my_queue` 的最后值语义队列，并使用 `test` 作为其过滤主题。该队列随后会出现在**队列**列表中。
...
1. 进入**管理** -> **MQTT 设置** -> **队列**选项卡。或者，也可以在**队列**页面点击右上角的**设置**按钮进入配置页面。

2. **队列**配置面板中包括以下选项：

   - **启用队列**：启用消息队列功能。

     > 该功能无法通过 Dashboard 禁用。若需禁用，请前往配置文件中手动修改。
   - **最大队列数量**：设置系统允许创建的最大队列数量。
   - **垃圾回收间隔**：队列中过期消息的清理周期，默认为 `1` 小时。
   - **常规队列保留周期**：普通队列中消息的最长保留时间，默认为 `7` 天。
   - **查找队列重试间隔**：当客户端订阅 `$queue/<name>` 且未找到对应队列时，该设置控制订阅端重试查找队列的频率。默认为 `10` 秒。
   - **启用自动创建队列**：当客户端订阅队列主题且不存在匹配队列时，启用自动创建队列功能。

     **自动创建队列类型**：指定自动创建的队列类型：

     - **最后值语义队列**（默认启用）：当客户端订阅 `$queue/<name>/<topic_filter>` 且不存在匹配队列时，EMQX 会自动创建一个启用“最后值语义”的队列。

       有关详细设置，请参见[自动创建最后值语义队列](#自动创建最后值语义队列)。
...
所有 REST API 操作都需要相应的身份认证和权限。有关详细的请求与响应数据结构，请参阅 [REST API](../admin/api.md) 文档中的“消息队列”章节。

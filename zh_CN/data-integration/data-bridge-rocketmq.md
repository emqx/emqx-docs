...
本节将通过 Dashboard 演示如何创建一条规则指定需要写入 RocketMQ 的消息或记录设备上下线状态。您需要为消息转发和设备上下线记录创建不同的规则。

以下步骤假定您在本地机器上同时运行 EMQX 和 RocketMQ。如果您在远程运行 RocketMQ 和 EMQX，请相应地调整设置。
...
6. 输入 RocketMQ 连接信息，**服务器地址**填写 `127.0.0.1:9876`，**RocketMQ 主题**填写 `TopicTest`，其他使用默认值即可。

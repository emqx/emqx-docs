...
规则引擎内置于 EMQX，是基于 SQL 的数据处理和分发核心组件。来自客户端的物联网数据将经过规则引擎，按照规则进行实时地提取、过滤、丰富和格式转换处理，之后，处理结果将输出到指定的数据桥接中。

您可以在[规则引擎](./rules.md)章节中找到有关规则引擎工作原理的详细信息。

### 基于 SQL 的数据处理

来自各种物联网设备和系统的数据源具有不同的数据类型和格式。规则 SQL 内置了丰富的函数，包括字符处理、数据类型转换函数、压缩/解压缩函数，可以灵活的进行复杂数据处理。有关基于 SQL 的规则的详细信息，请参阅 [SQL 语法与示列](./rule-sql-syntax.md)。

### 数据桥接

数据桥接是将 EMQX 与外部数据系统连接的关键组件。它实现了从 EMQX 到外部数据库、云平台或其他系统的数据无缝流动。您可以通过 EMQX Dashboard 配置数据桥接，指定数据转发目标和数据格式，以实现与外部系统的高效数据交换。

[支持的集成](#支持的集成)部分提供了设置不同数据桥接的具体说明链接。

...
物联网设备和系统产生的数据类型和格式多种多样。EMQX 配备了一款内置的、功能强大的基于 SQL 规则的引擎，这是处理和分发数据的核心组件。规则引擎具有广泛的功能，包括条件判断、字符操作、数据类型转换以及压缩/解压功能，能够实现复杂数据的灵活处理。
...
1. `${var}`：这种语法用于从规则的输出结果中提取变量，例如 `${topic}`。如果你想要提取嵌套的变量，可以使用 `.`， 例如 `${payload.temp}`。请注意，如果输出结果中不包含你想要提取的变量，你将得到 `undefined` 字符串。

2. `${.var}`：这种语法首先会尝试从规则的输出结果中提取变量，如果输出结果中不存在这个变量，它会尝试从对应的事件数据中提取，例如 `${.topic}`。同样也也支持使用 `.` 来提取嵌套的变量，例如 `${.payload.temp}`。如果规则输出结果和事件数据中都不存在你想要提取的变量，你将得到 `undefined` 字符串。你也可以使用 `${.}` 来提取规则输出结果和事件数据合并后的所有变量。
...
异步请求模式可以避免消息的发布订阅流程受到 Sink 执行速度的影响。但开启异步请求模式后，可能会存在订阅端收到消息了，但消息还未写入到外部数据提供情况。

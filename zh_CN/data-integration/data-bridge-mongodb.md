   ...
   ```
    ...
    配置有效载荷模板时需注意以下几点：
     ...
     - 不支持自动推导**值**的数据类型：
       ...
       - 时间戳、日期和时间类型，如不做特殊处理，则将被识别为数值或字符类型，如希望以日期和时间类型存储，需要在规则 SQL 中使用 `mongo_date` 函数对字段进行处理，参考 [时间与日期函数](./rule-sql-builtin-functions.md#时间与日期函数)。(注：此类 mongo_ 开头的函数的返回值仅用于 MongoDB 动作，不可于其他动作）

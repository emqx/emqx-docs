...
- **高性能和可扩展性**
  ...
  EMQX 的分布式架构和 MongoDB 的列式存储格式使数据量增加时无缝扩展变得容易。这确保了即使在大数据集下也能保持一致的性能和响应能力。随着您的物联网部署的增长，您的数据存储能力也可以轻松扩展。
...
- **成本效益**
  ...
  EMQX 和 MongoDB 都是开源解决方案，与专有解决方案相比，它们更具成本效益。这有助于降低物联网项目的总体拥有成本，并提高投资回报率。
...
本节演示了如何在 Dashboard 中创建 MongoDB 数据集成的规则来指定需要转发至 MongoDB 的数据并为规则添加触发的动作。以下示例假设您在同一台本地机器上运行 EMQX 和 MongoDB。如果您在远程运行 MongoDB 和 EMQX，请相应调整设置。
...
6. 根据情况选择**部署模式** 与 **SRV 记录**，此处选择 `single` 与不启用。

7. 输入 MongoDB 连接信息，**数据库名字**填写 `emqx_data`，**服务器地址**填写 `127.0.0.1:27017`，**用户名**填写 `admin`，**密码**填写 `public`。

8. **集合**中填写存储数据的集合，支持通过占位符 `${var_name}` 动态设置，本例中填入 `qx_messages`。其他配置项保持默认配置即可。

9. 配置 **有效载荷模板**，将 `clientid`、`topic`、`qos`、`timestamp`、`payload` 字段存储到 MongoDB 中，该模板将通过 MongoDB insert 命令执行，对应模板如下：
...
10. 高级配置（可选），详细请参考[高级配置](#高级配置)。

11. 在完成创建之前，您可以点击**测试连接**来测试 Sink 可以连接到 MongoDB 服务器。

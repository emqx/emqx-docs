...
本节介绍了在 EMQX 中创建 Cassandra Sink 之前需要做的准备工作，包括如何设置 Cassandra 服务器、创建 Keyspace 和 Table。
...
在创建到 Cassandra 的 Sink 之前，您必须先创建 Keyspace 和 Table。
...
本节将演示在 Dashboard 中创建一个 (.+) 连接器来存储所有 `t/#` 主题上的消息。以下示例假定您在本地机器上同时运行 EMQX 和 Cassandra。如果您在远程运行 Cassandra 和 EMQX，请相应地调整设置。

1. 转到 Dashboard **集成** -> **连接器** 页面。

2. 点击页面右上角的**创建**。

3. 在连接器类型中选择 **Cassandra**，点击**下一步**。
...
7. 高级配置（可选），根据情况配置同步/异步模式，队列与批量等参数，详细请参考[数据集成](./data-bridges.md)。
...
至此已经完成 Sink 创建，接下来将继续创建一条规则来指定需要写入的数据。
...
4. 点击**添加动作**，在下拉框中选择**使用 Sink 转发**选项，选择先前创建好的 Cassandra Sink 。
5. 点击最下方**创建**按钮完成规则创建。

...
| **健康检查间隔抖动** | 在基础健康检查间隔的基础上增加一个随机延迟（抖动），以减少集群中多个节点同时发起健康检查请求的可能性。这样可以避免超过 AWS Kinesis API 所设定的速率限制，并减少因临时限流导致的误报警。每次健康检查的实际间隔时间为：`interval + random(0..jitter)`。 | `15` 秒  |

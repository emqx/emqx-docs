...
5. 输入连接信息。
   - **服务器地址**：输入以逗号分隔的 RabbitMQ 节点列表。每个节点使用 `host[:port]` 格式，例如 `rmq1:5672,rmq2:5672`。如果连接某个节点失败，EMQX 会继续尝试列表中的下一个节点。不同的连接池 worker 从列表中的不同位置开始尝试，有助于将连接分布到已配置的节点。

     ::: tip
     从 EMQX 6.0.4 开始，可以配置多个 RabbitMQ 节点。故障转移发生在连接建立期间，不会在节点之间迁移已建立的 AMQP 连接。使用 `server` 和 `port` 指定单个节点的现有配置仍然兼容。
     :::

   - **Port**：输入未在**服务器地址**中指定端口的节点所使用的默认端口。默认值为 `5672`。
   - **用户名**：输入 `guest`。
   - **密码**：输入 `guest`。
...
5. 添加动作输出，为规则在被触发的情况下指定一个动作。详细的创建步骤，参考[添加 RabbitMQ Sink](#添加-rabbitmq-sink)。

6. 动作添加完成后，您可以看到新添加的 Sink 出现在**动作输出**栏下。点击**创建规则**页面上的**保存**按钮完成整个规则创建过程。

现在，您已成功创建了规则。您可以在**规则**页面看到新创建的规则，并在**动作 (Sink)** 标签页中查看新创建的 RabbitMQ Sink。

您还可以点击**集成** -> **Flow 设计器**来查看拓扑图，拓扑图可视化显示了主题 `t/#` 下的消息在经过规则 `my_rule` 解析后如何写入 RabbitMQ。

### 添加 RabbitMQ Sink

本节演示了为规则添加一个 RabbitMQ Sink，用于将规则处理结果写入 RabbitMQ。

1. 在**动作类型**下拉框中选择 `RabbitMQ`，保持**动作**下拉框为默认的`创建动作`选项，您也可以选择一个之前已经创建好的 RabbitMQ Sink。此示例将创建一个全新的 Sink 并添加到规则中。

2. 输入 Sink 名称，名称应为大/小写字母和数字的组合。

3. 在**连接器**下拉框中选择之前创建的 `my-rabbitmq` 连接器。您也可以点击下拉框旁边的创建按钮，在弹出框中快捷创建新的连接器，所需的配置参数按照参照[创建连接器](#创建连接器)。
...
4. 完成以下 Sink 配置：
   ...
   * **Headers 模版** 和 **属性模版**：使用模板定义自定义的 RabbitMQ Headers 和 Properties。详细信息请参见[设置 Headers 和属性模板](#设置-headers-和属性模版)。
...
7. 点击**创建**前，您可点击**测试连接**按钮确保 Sink 能连接到 RabbitMQ 服务器。

8. 点击**创建**按钮完成 Sink 创建。成功创建后，页面将返回到规则创建页面，新建的 Sink 将被添加到**动作输出**列表中。

#### 设置 Headers 和属性模板

从 EMQX 6.0 开始，在创建 RabbitMQ Sink 动作时，您可以定义自定义的 RabbitMQ 消息 Headers 和属性。这样可以通过将元数据直接附加到消息上，增强消息在 RabbitMQ 中的兼容性和路由灵活性。

这些字段可以使用规则 SQL 结果中的变量进行模板化（例如 `${payload.device_id}`）。Headers 和属性模板是可选的。如果您将这些字段留空，则不会向消息中添加额外的元数据。

##### 如何设置 Headers 模板

您可以添加一个或多个键值对作为 RabbitMQ Headers。这些 Headers 由用户自定义，可以携带自定义元数据，供 RabbitMQ 消费者解析。

- **键**：Header 名称，必须为字符串。
- **值**：与 Key 关联的值。可以使用静态字符串，也可以使用模板变量的动态值。

例如，要在 Header 中包含来自 MQTT payload 的 `device_id`：
...
RabbitMQ 支持一组标准的消息属性。EMQX 允许您定义这些属性，以提供消息级别的元数据，例如内容类型或关联 ID。

- **键**：从下列预定义的有效属性键中选择（无效的键将被忽略）。
- **值**：设置为静态值，或使用模板变量。
...
例如，要指定消息内容类型和 `app_id` （应用 ID），可以设置为：
...
- 一个从 MQTT payload 中提取的 Header `device_id`
- 一个静态值的属性 `app_id`
...
该配置确保转发到 RabbitMQ 的每条消息都包含：

- 用于消费者逻辑的自定义元数据（`Headers`）
- 用于消息处理或调试的标准化元数据（`属性`）

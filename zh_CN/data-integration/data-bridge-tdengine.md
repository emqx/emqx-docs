...
TDengine 数据集成是 EMQX 内置的功能。通过内置的[规则引擎](./rules.md)组件，该集成简化了从 EMQX 到 TDengine 的数据摄取过程，无需复杂编码。EMQX 通过规则引擎和数据桥将设备数据转发到 TDengine。通过 TDengine 数据桥接，MQTT 消息和客户端事件可以存储在 TDengine 中。此外，数据更新或在 TDengine 中的删除操作可以由事件触发，从而实现对设备在线状态和历史上下线事件的记录。
...
3. **数据摄取到 TDengine**：规则引擎中定义的规则触发动作将消息写入 TDengine。TDengine 数据桥提供 SQL 模板，允许灵活定义数据格式，将特定消息字段写入 TDengine 中相应的表和列。

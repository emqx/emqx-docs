...
6. 高级功能（可选）：根据情况配置同步/异步模式、批量模式。

7. 点击**创建**前，您可点击**测试连接**按钮确保能连接到 Apache IoTDB 服务器。
...
4. 在 SQL 编辑器中输入规则，例如我们希望将 `root/#` 主题的 MQTT 消息转发至 Apache IoTDB，可通过如下规则 SQL 实现：
  ...
  可以将以上结构的数据处理为数组，这将触发批量写入机制，整个数组通过批量一次性写入，能够有效提升写入性能：

  ```sql
  FOREACH
    payload.data_list as data
  DO
    data.measurement as measurement,

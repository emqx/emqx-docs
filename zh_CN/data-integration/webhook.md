...
5. 请求方法选择 `POST`，URL 设置为 `http://localhost:5000`。如需使用 OAuth2 保护 Webhook 请求，开启 **OAuth2 客户端凭证**并完成相关配置。详细信息参见[配置 OAuth2 客户端凭证认证](#配置-oauth2-客户端凭证认证)。点击 URL 输入框旁的**测试**按钮可以测试连接是否配置正确。

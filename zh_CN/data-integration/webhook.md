...
本页面全面介绍了 Webhook 相关信息，并附有实际使用说明。

## 工作原理

当客户端向特定主题发布消息或执行特定操作就会触发 Webhook，Webhook 兼容支持规则引擎所有的消息与事件。

你可以将 Webhook 配置为在以下情况触发，各事件的请求内容请参考 [SQL 数据源和字段](./rule-sql-events-and-fields.md)。

...
[GreptimeDB](https://github.com/GreptimeTeam/greptimedb) 是一个开源、分布式、云原生时序数据库，融合时序数据处理和分析能力。GreptimeDB 专为云而生，充分利用云的优势，如弹性、可扩展性和高可用性。EMQX 目前支持通过 Sink 的方式连接不同版本的 GreptimeDB, GreptimeCloud 以及 GreptimeDB 企业版。
...
本节介绍了在 EMQX 中创建 GreptimeDB Sink 之前需要做的准备工作，包括如何设置 GreptimeDB 服务器。
...
本节演示了如何在 Dashboard 中创建一个 GreptimeDB 连接器。以下示例假设您在同一台本地机器上运行 EMQX 和 GreptimeDB。如果您在远程运行 GreptimeDB 和 EMQX，请相应调整设置。

1. 转到 Dashboard **集成** -> **连接器**页面。

2. 点击页面右上角的**创建**。

3. 在**连接器类型**中选择 GreptimeDB，点击**下一步**。
...
至此您已经完成 Sink 创建流程。在 Sink 列表（集成 -> Sink ）中应出现 GreptimeDB Sink ，**资源状态**为`已连接`。
...
4. 添加动作，从**动作类型**下拉列表中选择 GreptimeDB，从**动作**下拉框中选择刚刚创建的连接器，点击**添加**按钮将其添加到规则中。
5. 点击最下方创建按钮完成规则创建。

至此您已经完成整个创建过程，可以前往 **集成** -> **Flow 设计器** 页面查看拓扑图，此时应当看到 `t/#` 主题的消息经过名为 `my_rule` 的规则处理，处理结果交由 GreptimeDB 进行存储。

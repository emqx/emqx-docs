...
本节介绍了在 EMQX 中创建 DynamoDB 数据集成 之前需要做的准备工作，包括安装 InfluxDB、创建数据表定义文件以及数据表。
...
本节演示了如何在 Dashboard 中创建 DynamoDB 数据集成的规则来指定需要转发至 DynamoDB 的数据并为规则添加触发的动作。您需要为消息转发和设备上下线记录创建两条不同的规则。
...
4. 点击右侧的**添加动作**按钮，为规则在被触发的情况下指定一个动作。在**动作类型**下拉框中选择 `DynamoDB`，保持**动作**下拉框为默认的`创建动作`选项，您也可以选择一个之前已经创建好的 DynamoDB Sink。此处我们创建一个全新的 Sink 并添加到规则中。
...
7. **模版**设置为默认值，即为空；模版为空时将会将整个消息转发给 DynamoDB，实际值为 JSON 模版数据。

8. 高级配置（可选），根据情况配置同步/异步模式，队列与批量等参数，详细请参考[ Sink 的特性](./data-bridges.md)。

9. 在完成创建之前，您可以点击**测试连接**来测试 Sink 可以连接到 DynamoDB 服务器。

...
## 创建 Google PubSub Sink 规则

本节演示了如何为 Google PubSub Sink 创建一条规则以指定需要转发至 GCP Pub/S 的数据。

1. 转到 Dashboard **集成** -> **规则页面**。ub
...
5. 点击右侧的**添加动作**按钮，为规则在被触发的情况下指定一个动作。在**动作类型**下拉框中选择 `Google PubSub`，保持**动作**下拉框为默认的“创建动作”选项，您也可以选择一个之前已经创建好的 Google PubSub Sink。此处我们创建一个全新的 Sink 并添加到规则中。

6. 输入名称，要求是大小写英文字母和数字组合。

7. 在**桥接角色**字段中，根据业务需求从下拉列表中选择`生产者`或`消费者`，并完成相应的配置。

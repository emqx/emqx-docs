...
5. 展开**高级设置**，根据需要配置可选设置。详细信息请参见[连接器高级设置](#连接器高级设置)。
6. 在点击**创建**之前，您可以点击**测试连接**以测试连接器是否能连接到 GCP Pub/Sub 服务器。
7. 点击底部的**创建**按钮完成连接器的创建。在弹出对话框中，您可以点击**返回连接器列表**或点击**创建规则**继续创建带有 GCP Pub/Sub 生产者 Sink 的规则，以指定要转发到 GCP Pub/Sub 的数据。详细步骤请参见[创建 GCP Pub/Sub 生产者 Sink 规则](#创建-gcp-pubsub-生产者-sink-规则)。
...
13. 展开**高级设置**，根据需要配置可选设置。详细信息请参见[高级设置](#高级设置)。
...
在添加 GCP Pub/Sub 消费者 Source 之前，您需要创建 GCP Pub/Sub 消费者连接器，以建立 EMQX 与 GCP Pub/Sub 之间的连接。

1. 转到 EMQX Dashboard，点击 **集成** -> **连接器**。
2. 在页面的右上角点击 **创建**，在连接器选择页面选择 **Google PubSub 消费者**，然后点击 **下一步**。
3. 输入连接器名称和描述，例如 `my-pubsubconsumer`。名称用于将 GCP Pub/Sub 消费者 Source 与连接器关联，并且必须在集群内唯一。
4. 在 **GCP 服务账户凭证** 中，上传您在 [创建服务账户凭证](#创建服务账户凭证) 中导出的 JSON 格式的服务账户凭证。
5. 展开**高级设置**，根据需要配置可选设置。详细信息请参见[连接器高级设置](#连接器高级设置)。
6. 在点击**创建**之前，您可以点击**测试连接**以测试连接器是否能连接到 GCP Pub/Sub 服务器。
7. 点击底部的**创建**按钮完成连接器的创建。在弹出对话框中，您可以点击**返回连接器列表**或点击**创建规则**继续创建带有 GCP Pub/Sub 消费者 Source 的规则，以消费来自 GCP Pub/Sub 的数据并转发到 EMQX 本地。详细步骤请参见[创建 GCP Pub/Sub 消费者 Source 规则](#创建-gcp-pubsub-消费者-source-规则)。
...
2. 填写消息重发布的配置：
   ...
   - **Payload**：设置生成转发消息 payload 的模板。留空表示转发规则输出结果。输入 `${.value}` 表示仅转发 GCP Pub/Sub 消息的 payload。
...
按照以下步骤，验证 GCP Pub/Sub 消费者 Source 能否消费 GCP Pub/Sub 消息，并将消息重新发布到 EMQX 的 MQTT 主题 `t/1`。

1. 使用 MQTTX CLI 订阅 EMQX 的 MQTT 主题 `t/1`：

   ```bash
   mqttx sub -t t/1 -v
   ```

2. 在 Google Cloud 控制台中，进入 **Pub/Sub** -> **主题**，点击主题 `my-iot-core`，然后发布以下消息：

   ```json
   {"msg":"hello GCP PubSub"}
   ```

3. 确认 MQTTX 在主题 `t/1` 上收到以下消息：
...
本节介绍 GCP Pub/Sub 连接器、生产者 Sink 和消费者 Source 的高级设置。

### 连接器高级设置

GCP Pub/Sub 生产者和消费者连接器使用相同的高级设置。

| 字段名称 | 描述 | 默认值 |
| --- | --- | --- |
| **HTTP 流水线** | 不等待每个响应即可连续发送的 HTTP 请求上限。设置为 `1` 时，发送下一个请求前必须等待当前请求的响应。 | `100` |
| **连接池大小** | 连接池中维护的连接数。 | `8` |
| **连接超时** | 等待 HTTP 连接建立的最长时间。 | `15` 秒 |
| **最大空闲时间** | HTTP 客户端尝试重新连接前允许无活动的最长时间。 | `10` 秒 |
| **最大重试次数** | 发送请求出错后的最大重试次数。 | `2` |
| **启动超时** | 创建连接器后，等待连接器达到健康状态的最长时间。 | `5` 秒 |
| **健康检查间隔** | 执行连接器健康检查的时间间隔。 | `15` 秒 |
| **健康检查超时** | 健康检查返回结果的最长时间。超时后，连接器将被视为已断开。 | `60` 秒 |

### 生产者 Sink 和消费者 Source 共同高级设置

生产者 Sink 和消费者 Source 使用以下共同高级设置。两者的**健康检查间隔**默认值不同。

| 字段名称 | 描述 | 生产者 Sink 默认值 | 消费者 Source 默认值 |
| --- | --- | --- | --- |
| **请求超期** | 从请求进入缓冲区开始计算的最长有效期。如果请求在该时间内仍停留在缓冲区，或发送后未收到响应或确认，则请求过期。 | `45` 秒 | `45` 秒 |
| **健康检查间隔** | 执行 Sink 或 Source 健康检查的时间间隔。 | `15` 秒 | `30` 秒 |
| **健康检查间隔抖动** | 在健康检查间隔上增加均匀分布的随机延迟，使共用连接器的 Action 和 Source 不会同时开始健康检查。 | `0` 毫秒 | `0` 毫秒 |
| **健康检查超时** | 健康检查返回结果的最长时间。超时后，Sink 或 Source 将被视为已断开。 | `60` 秒 | `60` 秒 |

### 生产者 Sink 特有高级设置

GCP Pub/Sub 生产者 Sink 提供以下特有高级设置。

| 字段名称 | 描述 | 默认值 |
| --- | --- | --- |
| **缓存池大小** | 将数据发送到 GCP Pub/Sub 之前，用于缓存和处理数据的缓冲工作进程数量。 | `16` |
| **分发策略** | 将未显式设置选择键的请求分配给缓冲工作进程的策略。`按客户端 ID` 将同一客户端的请求分配给同一工作进程；`随机` 将请求分散到不同工作进程。 | `按客户端 ID` |
| **缓存队列最大长度** | 每个缓冲工作进程可缓存的最大数据量。 | `256` MB |
| **最大批量请求大小** | 一个批次中的最大请求数。设置为 `1` 时禁用批处理。 | `1` |
| **请求模式** | 控制同步或异步发送请求。`异步` 模式下，EMQX 无需等待 GCP Pub/Sub 响应即可继续处理消息。 | `异步` |
| **请求飞行队列窗口** | 当**请求模式**为 `异步` 时，指定未收到响应前可发送的最大请求数。如果来自同一 MQTT 客户端的消息必须严格按顺序处理，请设置为 `1`。 | `100` |

### 消费者 Source 特有高级设置

GCP Pub/Sub 消费者 Source 提供以下特有高级设置。

| 字段名称 | 描述 | 默认值 |
| --- | --- | --- |
| **消息确认截止时间** | GCP Pub/Sub 等待 Source 确认收到消息的近似时长。超过该时长后，GCP Pub/Sub 可能重新投递消息。取值范围为 `10` 至 `600` 秒。 | `60` 秒 |

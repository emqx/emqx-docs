...
5. 点击右侧的**添加动作**按钮，为规则在被触发的情况下指定一个动作。在**动作**下拉框中选择`使用 Sink 转发`，该动作会将经规则处理的数据转发到 InfluxDB。

6. 点击 **Sink** 下拉框右侧的**+**按钮创建 Sink 。在**连接器类型**下拉框中选择 `InfluxDB`。

7. 在**创建连接器**页面配置以下信息：

   - 输入连接器名称，要求是大小写英文字母和数字的组合。
...
9. 点击**添加**完成 Sink 配置和创建。您将回到**添加动作**页面，在 **Sink** 下拉框中选择您创建好的 InfluxDB Sink ，点击**添加**以完成动作添加。
...
本节将深入介绍可用于 InfluxDB Sink 的高级配置选项。在 Dashboard 中配置 Sink 时，您可以根据您的特定需求展开**高级设置**，调整以下参数。

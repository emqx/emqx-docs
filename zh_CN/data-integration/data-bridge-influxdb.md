...
<<<<<<< HEAD
   ...
   - **连接器名称**：连接器的唯一名称，仅支持字母和数字，例如：`my_influxdb`。
   - **描述**（可选）：连接器的简要说明。
   - **服务器地址**：InfluxDB 服务器地址，例如 `127.0.0.1:8086`。如果使用 InfluxDB Cloud，请指定端口 `443`（例如 `{url}:443`），并启用 TLS。
   - **InfluxDB 版本**：选择正在使用的 InfluxDB 版本，支持 `v1`、`v2`（默认）和 `v3`。
   - **启用 TLS**：如果 InfluxDB 服务器需要使用 TLS 连接，请启用该选项。详细说明请参见[启用 TLS 加密访问外部资源](../network/overview.md#启用-tls-加密访问外部资源)。
   
   根据所选 InfluxDB 版本的不同，所需的配置项也有所差异，如下表所示。相关取值应与[安装和设置 InfluxDB](#安装和设置-influxdb) 中的设置保持一致。
   
   | 配置项         | InfluxDB v1   | InfluxDB v2 | InfluxDB v3 |
   | -------------- | ------------- | ----------- | ----------- |
   | 认证方式       | 用户名 / 密码 | Token       | Token       |
   | **Token**      | -             | 必填        | 必填        |
   | **用户名**     | 可选          | -           | -           |
   | **密码**       | 可选          | -           | -           |
   | **组织**       | -             | 必填        | -           |
   ...
   - 对于 **InfluxDB v1**，EMQX 会将数据直接写入指定的数据库，并使用可选的用户名和密码进行认证。
   - 对于 **InfluxDB v2**，EMQX 使用**组织** 和 **Bucket** 模型，所使用的 Token 必须具备向指定 Bucket 写入数据的权限。
   - 对于 **InfluxDB v3**，EMQX 使用基于数据库的模型，认证方式为 Token。
5. 在点击**创建**之前，可以先点击**测试连接**，验证 EMQX 是否能够成功连接到 InfluxDB 服务器。
...
=======
   ...
   - 根据情况选择 **InfluxDB 版本**。不同版本所需的认证配置项有所差异，如下表所示。对于 InfluxDB v2，请使用[安装和设置 InfluxDB](#安装和设置-influxdb) 中配置的组织、Bucket 和 Token 值；对于 InfluxDB v1，请填写数据库名称，以及（如已在 InfluxDB 服务器上配置）该数据库对应的用户名和密码。
     ...
     - 对于 InfluxDB v1，EMQX 会将数据直接写入指定的数据库，并使用可选的用户名和密码进行认证。
     - 对于 InfluxDB v2，EMQX 使用组织和 Bucket 模型，所使用的 Token 必须具备向指定 Bucket 写入数据的权限。
   
   - 切换 **Ping 请求携带认证**，控制 EMQX 在发送 `/ping` 健康检查请求时是否附带连接器认证信息。如果您的 InfluxDB 服务要求 `/ping` 也进行认证，请启用此项。默认为禁用，即保持原有行为，不在 `/ping` 请求中附带认证信息。
   
   - 设置是否启用 TLS。有关 TLS 连接选项的详细信息，请参阅[启用 TLS 加密访问外部资源](../network/overview.md#启用-tls-加密访问外部资源)。
...
13. 在点击**创建**之前，您可以点击**测试连接**，以测试 Sink 是否能够连接到 InfluxDB 服务器。

14. 点击**创建**完成 Sink 的创建。回到**创建规则**页面，您将看到新的 Sink 出现在**动作输出**页签下。

15. 在**创建规则**页面，验证配置的信息。点击**创建**按钮生成规则。

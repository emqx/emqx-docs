...
6. 点击 **添加** 在 **主题映射关系** 字段中至少添加一个 Kafka 到 MQTT 的主题映射。例如，对于此演示，在 **Kafka 主题名称** 中输入 `testtopic-out`，在 **MQTT 主题** 中输入 `t/1`。**MQTT Payload Template** 子字段指定了应使用的 MQTT Payload，可用于模板化的 Kafka 消息字段如下：
...
3. 高级设置（可选）：参见 **[高级配置](#高级配置)**。

4. 在点击 **创建** 之前，您可以点击**测试连接**来测试 Source 是否能连接到 Kafka 服务器。

...
Kafka 数据集成是 EMQX 企业版的功能。EMQX 企业版可以为您带来更全面的关键业务场景覆盖、更丰富的数据集成支持，更高的生产级可靠性保证以及 24/7 的全球技术支持，欢迎[免费试用](https://www.emqx.com/zh/try?product=enterprise)。
...
本页提供了关于 EMQX 和 Kafka 之间数据集成的全面介绍，以及如何创建和验证数据集成的实际指导。以下是本页涵盖的主题列表：

- [工作原理](#工作原理)
- [特性与优势](#特性与优势)
- 桥接准备
  - [前置准备](#前置准备)
  - [设置 Kafka 服务器](#设置-kafka-服务器)
  - [创建 Kafka 主题](#创建-kafka-主题)
- Kafka 生产者数据桥接
  - [创建 Kafka 生产者规则和数据桥接](#创建-kafka-生产者规则和数据桥接)
  - [测试 Kafka 生产者数据桥接和规则](#测试-kafka-生产者数据桥接和规则)
- Kafka 消费者数据桥接
  - [创建 Kafka 消费者数据桥接](#创建-kafka-消费者数据桥接)
  - [为消费者数据桥接创建规则（可选）](#为消费者数据桥接创建规则-可选)
  - [测试 Kafka 消费者数据桥和规则](#测试-kafka-消费者数据桥接和规则)
- [高级配置](#高级配置)
- [更多信息](#更多信息)
...
将数据流入或流出 Apache Kafka 涉及以两种角色分别创建与 Kafka 的数据桥接：生产者（向 Kafka 发送消息）和消费者（从 Kafka 接收消息）。EMQX 允许您以任意两种角色创建和配置数据桥接。假如您以生产者角色配置数据桥接，其工作流程如下：
...
3. **桥接到 Kafka**：规则引擎中定义的规则触发将消息转发到 Kafka 的动作。使用 Kafka 桥接功能，MQTT 主题被映射到预定义的 Kafka 主题，所有处理过的消息和数据被写入 Kafka 主题。
...
- **动态配置**：您可以在 Dashboard 中或配置文件中动态配置数据桥接。

这些特性增强了集成能力和灵活性，有助于您建立有效和稳健的物联网平台架构。您日益增长的物联网数据可以在稳定的网络连接下传输，并且可以进一步有效地存储和管理。

## 桥接准备

本节描述了在 EMQX Dashboard上创建 Kafka 数据桥之前需要完成的准备工作。
...
在 EMQX 中创建数据桥接之前应创建相关的 Kafka 主题。使用以下命令在 Kafka 中创建两个主题：`testtopic-in`（用于生产者角色）和 `testtopic-out`（用于消费者角色）。
...
本节演示了如何在 EMQX 中创建规则和数据桥以将 MQTT 数据从 EMQX 转发到 Kafka，以及如何测试规则和生产者数据桥接。

### 创建 Kafka 生产者规则和数据桥接

本节演示了如何在 EMQX 中创建规则，以处理来自源 MQTT 主题 `t/#` 的消息，并通过配置的 Kafka 数据桥接发送处理结果以产生数据到 Kafka 的 `testtopic-in` 主题。
...
4. 如果您想将主题 `t/#` 的 MQTT 消息转发到 Kafka，可以在 **SQL 编辑器** 中输入以下语句。
   ...
   注意：如果您想指定自己的 SQL 语法，请确保在 `SELECT` 部分包含了数据桥接所需的所有字段。
...
5. 点击 + **添加动作** 按钮来定义规则触发的动作。从下拉列表中选择 **通过数据桥转发**。通过这个动作，EMQX 会将规则处理的数据发送到数据桥接。

6. 点击 **数据桥接** 下拉框旁边的 **+** 图标来创建数据桥接。

7. 从 **数据桥类型** 下拉列表中选择 **Kafka**。填写所需字段（标有星号）。

8. 在 **桥接角色** 字段中，选择 **生产者**。

9. 为数据桥输入一个名称。名称应为大小写字母和数字的组合。
...
11. 配置数据桥选项。
    ...
    - **源 MQTT 主题**：您可以将此字段留空，这意味着 Kafka 只处理来自规则的数据。在本示例中，表示所有匹配主题 `t/#` 的 MQTT 消息都将被发送到 Kafka。如果您希望在不使用规则的情况下建立 Kafka 数据桥接，您可以直接输入一个 MQTT 主题作为数据来源。此字段支持通配符。
    ...
    - **消息的时间戳**：指定 Kafka 消息中此数据字段的字符串格式。
    - **压缩**：指定是否使用压缩算法压缩/解压 Kafka 消息中的记录。
    - **分区选择策略**：选择生产者向 Kafka 分区分发消息的方式。

12. 高级设置（可选）：请参阅 [高级配置](#高级配置)。

13. 点击 **添加** 按钮以完成数据桥配置。您将被重定向回 **添加动作** 页面。从 **数据桥接** 下拉列表中选择您刚刚创建的 Kafka 数据桥接。点击底部的 **添加** 按钮将此动作包含在规则中。

14. 回到 **创建规则** 页面，验证配置信息。点击 **创建** 按钮生成规则。

![Kafka_producer_bridge](./assets/Kafka_producer_bridge.png)

现在您已成功创建了规则和数据桥接。您可以点击 **集成** -> **Flow 设计器** 查看拓扑。可以看到，主题 `t/#` 下的消息在经过规则 `my_rule` 解析后被发送并保存到 Kafka。

### 测试 Kafka 生产者数据桥接和规则

为了测试 Kafka 生产者数据桥接和规则是否按照您的预期工作，您可以使用 [MQTTX](https://mqttx.app/zh) 来模拟客户端向 EMQX 发布 MQTT 消息。
...
2. 在 **数据桥接** 页面上点击数据桥接的名称查看统计信息。检查数据桥的运行状态，应该有一个新的传入消息和一个新的传出消息。
...
本节演示了如何创建数据桥接以接收来自 Kafka 的数据，以及如何选择性地创建规则来进一步处理数据，还包括如何测试规则和消费者数据桥接。

### 创建 Kafka 消费者数据桥接

本节展示了如何创建数据桥以接收来自 Kafka 的数据。通过数据桥接，Kafka 主题 `testtopic-out` 的消息将被转发到 EMQX 中的 MQTT 主题 `t/1`。

1. 进入 EMQX Dashboard，并点击 **集成** -> **数据桥接**。

2. 点击页面右上角的 **创建**。

3. 在 **创建数据桥接** 页面中，选择 **Kafka**，然后点击 **下一步**。

4. 在 **桥接角色** 字段中，选择 **消费者**。

5. 为数据桥输入一个名称。名称应为大小写字母和数字的组合。
...
7. **Key 编码模式** 和 **Value 编码模式**：选择 Kafka 消息键和消息值的编码模式。

8. 点击 **添加** 在 **主题映射关系** 字段中至少添加一个 Kafka 到 MQTT 的主题映射。例如，对于此演示，在 **Kafka 主题名称** 中输入 `testtopic-out`，在 **MQTT 主题** 中输入 `t/1`。**MQTT Payload Template** 子字段指定了应使用的 MQTT Payload，可用于模板化的 Kafka 消息字段如下：
...
9. **偏移重置策略**：选择当没有消费者偏移量或偏移量变得无效时，Kafka 消费者开始从 Kafka 主题分区读取的偏移量重置策略。

   - 如果您希望消费者从最新偏移量开始读取消息，跳过消费者启动前产生的消息，请选择 `latest`。
   - 如果您希望消费者从分区的开始读取消息，包括消费者启动前产生的消息，即读取主题中的所有历史数据，请选择 `earliest`。

10. 高级设置（可选）：参见 **[高级配置](#高级配置)**。

11. 在点击 **创建** 之前，您可以点击 **测试连接** 来测试桥是否能连接到 Kafka 服务器。

12. 点击 **创建**。您将被提供创建关联规则的选项。对于 Kafka 消费者数据桥接，不严格要求创建规则以进一步处理数据。如果您需要为数据桥接创建规则，请参见 [为 Kafka 消费者创建规则和数据桥接（可选）](https://chat.openai.com/c/c59d3d00-1a22-48c4-bdd9-82efe993fe5e#create-rule-for-consumer-data-bridge-optional)。

<img src="./assets/Kafka_consumer_bridge.png" alt="Kafka_consumer_bridge" style="zoom:67%;" />

### 为消费者数据桥接创建规则（可选）

本节演示了如何在 EMQX 中创建规则，以进一步处理由配置的 Kafka 数据桥接转发的消息，并将消息重新发布到 MQTT 主题。
...
4. 如果您想将从 Kafka 桥接 `$bridges/kafka_consumer:<bridgeName>` 转换的消息转发到 EMQX，请在 **SQL 编辑器** 中输入以下语句。
...
为了测试 Kafka 消费者数据桥接和规则是否如预期工作，您可以使用 [MQTTX](https://mqttx.app/zh) 模拟一个客户端订阅 EMQX 中的主题，并使用 Kafka 生产者向 Kafka 主题生产数据。然后，检查 Kafka 的数据是否被 EMQX 重新发布到客户端订阅的主题。
...
本节描述了一些高级配置选项，这些选项可以优化您的数据桥接性能，并根据您的特定场景定制操作。创建数据桥接时，您可以展开 **高级设置** 并根据您的业务需求配置以下设置。

| 字段                         | 描述                                                         | 推荐值    |
| ---------------------------- | ------------------------------------------------------------ | --------- |
| 元数据刷新最小间隔           | 客户端在刷新 Kafka 代理和主题元数据之前必须等待的最短时间间隔。将此值设置得太小可能会不必要地增加 Kafka 服务器的负载。 | `3`秒     |
| 元数据请求超时               | 数据桥接从 Kafka 请求元数据时的最大等待时长。                | `5`秒     |
...
| 同步查询超时时间（生产者）   | 在同步查询模式下，建立等待确认的最大等待时间。这确保及时完成消息传输，避免长时间等待。<br />仅当数据桥接的请求模式配置为 `同步` 时适用。 | `5`秒     |
...
| TCP Keepalive                | 此配置为 Kafka 桥接连接启用 TCP 保活机制，以维护持续连接的有效性，防止由长时间不活动导致的连接中断。该值应以逗号分隔的三个数字格式提供，格式为 `Idle, Interval, Probes`：<br />Idle：服务器发起保活探测前连接必须保持空闲的秒数。Linux 上的默认值是 7200 秒。<br />Interval：每个 TCP 保活探测之间的秒数。Linux 上的默认值是 75 秒。<br />Probes：在将连接视为关闭之前，发送的最大 TCP 保活探测次数（如果对端没有响应）。Linux 上的默认值是 9 次探测。<br />例如，如果您将值设置为 `240,30,5`，则意味着在 240 秒的空闲时间后将发送 TCP 保活探测，随后每 30 秒发送一次探测。如果连续 5 次探测尝试没有响应，连接将被标记为关闭。 | `none`    |
| 健康检查间隔                 | 检查数据桥接运行状态的时间间隔。                             | `15`秒    |

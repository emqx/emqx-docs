...
4. 当规则被触发并产生匹配数据后，Sink 会调用所配置的连接器，将数据以 JSON Lines 格式写入基础路径下的轮转文件。每条消息以一行 JSON 对象存储，便于标准工具或下游系统处理。
...
启用按时间轮转后，EMQX 会在文件扩展名前插入 `YYYYMMDDHH` 格式的时间戳。按小时轮转时，文件名包含对应小时，例如 `mqtt-trace-2026062413.log.1`。按天轮转时，小时部分固定为 `00`，例如 `mqtt-trace-2026062400.log.1`。文件名始终带有 `.N` 后缀。每个周期的文件集还包含 `.idx` 和 `.siz` 管理文件。

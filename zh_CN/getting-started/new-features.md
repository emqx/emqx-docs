...
与传统 MQTT 仅面向实时投递、依赖订阅者在线状态不同，消息流会持续捕获与主题过滤器匹配的消息并将其写入一个具名的持久化消息流中。每个消息流通过唯一的名称进行标识和管理，而不是通过主题过滤器进行标识。
...
与传统 MQTT 依赖订阅者在线不同，消息队列通过在服务器端缓存消息，实现发布者与订阅者的解耦。匹配配置主题过滤器的消息会被持久化存储到一个具名队列中。每个队列通过唯一名称进行标识和管理，而不是通过主题过滤器进行标识。
...
> 当启用消息队列功能后，`$queue/` 前缀将被保留用于消息队列订阅，不能再用于共享订阅（Shared Subscription）。

...
{% emqxce %}
配置项会在 EMQX 与扩展插件启动时读取并载入，EMQX 开源版尚不支持运行时更新配置（企业版支持这一能力），但由于扩展插件支持动态加载和卸载，因此可以在修改插件配置后重新加载插件来应用最新的配置。
{% endemqxce %}

{% emqxee %}
配置项会在 EMQX 与扩展插件启动时读取并载入，启动之后可以通过 Dashboard 与 REST API 更新部分配置，除此之外还可以在修改插件配置后重新加载插件来应用最新的配置。
{% endemqxee %}

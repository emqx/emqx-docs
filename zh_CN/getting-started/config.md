...
{% emqxce %}
EMQX 开源版尚不支持在运行时更新配置，如有相关需求，可以考虑迁移至 EMQX 企业版。但由于配置项会在 EMQX 与扩展插件启动时读取并载入，因此您可在修改插件配置后重新加载插件来应用最新的配置。
{% endemqxce %}

{% emqxee %}
EMQX 会在启动或插件启动时读取并载入配置项信息。后续如希望修改配置，可通过 Dashboard 或 REST API 在运行时更新配置，或在修改插件配置后重新加载插件来应用最新的配置。
{% endemqxee %}

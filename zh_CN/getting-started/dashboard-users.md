...
| `viewer` | 4 个通用类别 + 3 个 Dashboard 类别。`user_management` 和 `app_management` **不允许**分配给查看者。 | 4 个通用类别 + `mfa_management`。只读（GET）端点不受权限类别限制，因为 GET 请求在类别层之上被短路。 |

用户修改自己的密码、管理自己的 MFA 以及登出操作始终允许，与权限类别设置无关。

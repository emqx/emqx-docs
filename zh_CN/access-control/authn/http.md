...
- 从 EMQX v6.1.0 版本开始，您可以在响应体中设置一个可选的 `zone_override` 字段，用于在认证成功后覆盖客户端所属的 Zone。该值必须是已存在的 [Zone](../../configuration/configuration.md#zone-覆盖) 名称。该字段适用于根据认证结果动态应用不同 Zone 配置的场景，例如应用不同的 Zone 级别速率限制器。如果指定的 Zone 不存在，EMQX 将忽略该字段，并保留客户端原有的 Zone。
...
Body:
{
    ...
    "zone_override": "vip", // 可选 (自 v6.1.0 起)
    "acl": // 可选 (自 v5.8.0 起)
    [
        {
            "permission": "allow",
...
::: tip
如果监听器配置了 `parse_unit = frame`，则应用 `zone_override` 后，新 Zone 中的 `max_packet_size` 设置不会生效。
:::

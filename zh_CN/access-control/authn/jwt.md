...
3. 根据以下说明完成相关配置：
   ...
   - **调用条件**：一个 Variform 表达式，用于控制是否将此 JWT 认证器应用于客户端连接。该表达式会根据客户端属性（例如 `username`、`password`、`clientid` 和 `listener`）进行评估。如果表达式的结果为字符串 `"true"`，则会触发认证器。否则，认证器将被跳过。有关调用条件的更多信息，请参见[认证器调用条件](./authn.md#认证器调用条件)。

     示例：当同一认证链需要同时服务 JWT 客户端和密码认证客户端时，可以使用 `is_jwt(password)`。

     从 EMQX 6.2.3 开始，仅当密码在结构上是 JWT 时，`is_jwt(password)` 才返回 `true`。普通密码、未提供密码或格式错误的值会返回 `false`，因此 EMQX 会跳过 JWT 认证器，并由下一个认证器继续处理该客户端。如果结构有效的 JWT 未通过签名或声明校验，JWT 认证器仍会处理并拒绝该客户端。

...
在上述两种方式中，EMQX 均可使用配置的 `base_dn` 和 `filter` 从 LDAP 条目中提取额外属性，例如 `isSuperuser` 标志或 ACL 规则，用于判断用户是否为超级用户，或用于实施细粒度访问控制。详细内容，参考[]
...
4. 按照以下说明对数据源进行配置：
   ...
   - **高级设置**：设置并发连接数和连接超时前的等待时间。
     ...
     - **查询超时**（可选）：指定EMQX在查询超时之前的等待时间。默认值：`5` 秒。

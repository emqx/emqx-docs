...
- **本地密码比对**
  ...
  这种方式提供了更高的灵活性和可控性。它支持多种密码哈希算法，可以获取额外的用户属性，并可结合自定义认证逻辑或安全策略使用。
...
在上述两种方式中，EMQX 均可使用配置的 `base_dn` 和 `filter` 从 LDAP 条目中提取额外属性，例如 `isSuperuser` 标志或 ACL 规则，用于判断用户是否为超级用户，或用于实施细粒度访问控制。详细内容，参考[从 LDAP 获取 ACL 规则](#从-ldap-获取-acl-规则)。
...
4. 按照以下说明对数据源进行配置：
   ...
   - **启用 TLS**：如果要启用 TLS，请打开切换按钮。有关启用 TLS 的更多信息，请参见[网络和 TLS](../../network/overview.md)。

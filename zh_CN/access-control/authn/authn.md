...
身份认证是物联网应用的重要组成部分，可以帮助有效阻止非法客户端的连接。

为了向客户提供更好的安全性保障，EMQX 支持多种认证方式，如通过用户名/密码进行身份认证，JWT 认证，以及基于 MQTT 5.0 协议在客户端和服务之间通过质询/响应实现双向认证。
...
[JWT](https://jwt.io/) 是一种基于 Token 的认证机制，它不需要服务器来保留客户端的认证信息或会话信息。客户端可以在密码或用户名中携带 Token，EMQX JWT 认证器会基于预先配置的密钥或公钥对 JWT 签名进行验证。

 如果用户配置了 JWKs 服务器，JWT 认证器将使用从 JWKs 服务器查询到的公钥列表对 JWT 签名进行验证。在持有密钥的情况下用户可以为客户端批量签发认证信息，是最简便的认证方式。

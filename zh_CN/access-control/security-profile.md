...
- **网关监听器默认绑定回环地址。** 网关监听器在 `bind` 省略或仅指定端口时，只监听回环接口。
- **Dashboard HTTP 监听器默认绑定回环地址。** Dashboard HTTP 监听器在 `bind` 省略或仅指定端口时，只监听回环接口。配置显式的绑定地址以接受外部连接。

如需在不切换安全配置方案的情况下统一设置所有使用默认地址的监听器，请使用 `node.default_listener_address`。参见[默认监听地址](#默认监听地址)。
...
使用该配置项可以独立于安全配置方案控制监听器的暴露范围。例如，在保持 `hardened` 方案的同时接受外部 MQTT 连接：
...
| `all` | `0.0.0.0`，即所有 IPv4 接口。 |
| IP 地址 | 指定的地址，例如 `192.168.1.10` 或 `::1`。在大多数系统上 `::` 同时接受 IPv4 和 IPv6 连接，具体由操作系统的 `bindv6only` 设置决定。 |
| 主机名 | 在启动时解析，例如 `broker1.example.com`。 |

未设置该配置项时，由安全配置方案决定默认地址：`legacy` 绑定所有接口，`hardened` 绑定回环地址。

该配置项为节点级配置。EMQX 在启动时读取一次，因此修改后需要重启节点。也可以通过环境变量 `EMQX_NODE__DEFAULT_LISTENER_ADDRESS` 设置。

::: tip
官方 Docker 镜像设置了 `EMQX_NODE__DEFAULT_LISTENER_ADDRESS=all`，因为容器的回环接口无法通过发布的端口访问。这样在两种方案下，使用默认地址的监听器都保持可达，而是否对外暴露仍由 `docker run -p` 决定。
:::

...
- 在认证机制支持时，将 MQTT 客户端 ID 与认证身份绑定。例如，校验 JWT 的 `clientid` 声明、使用 [`peer_cert_as_clientid`](./authn/x509.md#证书信息映射) 映射证书字段、让 HTTP 认证器拒绝不匹配的请求，或将认证器与 [Client-Info](./authn/cinfo.md) 规则配合使用。缺少此类绑定时：
  ...
  - 由于 MQTT 仅凭客户端 ID 来标识和恢复会话，持有任意有效凭据且知道受害者客户端 ID 的攻击者可以接管受害者的会话：使用受害者的客户端 ID 连接会将受害者踢下线（`DISCONNECT 0x8E`），并在 `Clean Start = 0` 时恢复其会话，从而继承其已有订阅，收到攻击者自身 ACL 本应拒绝的消息。EMQX 在创建订阅时进行授权校验，而不会针对恢复会话的新身份重新校验被继承的订阅，因此将客户端 ID 与凭据绑定（使接管在连接阶段即失败）才是有效的防护手段。注意：发布不受影响，它会针对当前身份逐条消息进行授权。

...
- 在设计发往外部服务的请求时——包括 HTTP [认证](./authn/http.md)、HTTP [授权](./authz/http.md)，以及数据集成的连接器、桥接与动作——应将每个密钥放入 EMQX 能识别为敏感信息的字段或请求头中，使其值在日志、追踪和 API 响应中被脱敏为 `******`，而不是以明文打印。脱敏是依据字段名或请求头名进行的：对于放在 HTTP 请求头中的凭据，应使用标准的 `Authorization`（或 `Proxy-Authorization`）请求头，EMQX 会始终对其脱敏；对于放在其他配置字段中的密钥，应将字段命名为已识别的敏感键名，例如 `password`、`token`、`secret`、`secret_key` 或 `jwt`。自定义的请求头名（例如 `X-Api-Key`）或不符合约定的字段名不会被识别，其值可能在 `debug` 级别日志和错误信息中以明文出现。

...
- 尽可能将 secret 类型字段存储为 `file://` 引用，而不是直接写入明文值。对于所有标记为 secret 的字段（例如 SSL 密钥口令、桥接和连接器的密码、API Key），将其值设置为 `file:///path/to/secret`，EMQX 会在启动以及每次配置重载时从该文件读取实际值。这样可以避免明文机密出现在 `emqx.conf`、API 请求体、配置备份和版本控制中，降低共享或导出配置时泄露机密的风险。详见[从文件加载 Secret](../configuration/secret-from-file.md)。

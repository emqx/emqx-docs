...
- 对于暴露在不受信任网络中的 Broker，可考虑将 `authorization.deny_action` 设置为 `disconnect`（默认值为 `ignore`），使发布或订阅被拒绝的客户端被直接断开，而不是继续在同一连接上探测主题。结合[连接抖动检测](./flapping-detect.md)，反复重连并触发拒绝的客户端会被自动封禁。由于 `deny_action` 是全局配置，同样会断开尝试了被拒绝操作的合法客户端，因此应在客户端只访问其授权主题的部署中使用，并调整连接抖动检测阈值，避免正常重连风暴导致客户端被封禁。详见[授权](./authz/authz.md)。

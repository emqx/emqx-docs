...
本指南介绍如何使用 GPT-Realtime 模型提供的原生 WebRTC 方式快速构建一个实时语音智能体应用。

## 获取临时 API Key

要在浏览器中使用 WebRTC 方式连接 GPT-Realtime，我们需要先获取一个临时的 API Key。我们可以通过 OpenAI 的 REST API 获取该 Key：
...
以下代码展示了如何使用纯 WebRTC 方式连接 GPT-Realtime 模型，实现浏览器中的实时语音聊天功能：
...
上述代码除了创建 WebRTC 语音通道之外，还创建了一个 Data Channel 用于发送和接收 GPT-Realtime 模型的事件。这里将所有接收到的事件打印到了控制台，如果使用过程中出现收不到语音等问题，可以在控制台查看具体的错误信息。

## 给模型发送消息

GPT-Realtime 支持通过 WebRTC Data Channel 直接发送消息给模型：
...
首先我们需要启动 EMQX 并安装 MCP 桥接插件，然后启动一个 MCP Server 来模拟设备。具体步骤请参考[使用 EMQX MCP 桥接访问物联网设备](../../mcp-bridge/quick-start.md)。
...
在该函数中，我们需要处理 session.created 事件，以便在会话创建时发送 session.update 事件来启用 MCP 工具。MCP 服务器的地址填写 `https://your-emqx-host:port/mcp`，并且在系统提示词里告诉 GPT-Realtime 设备的 client ID 为 `abc123`：
...
::: tip 注意
GPT-Realtime 需要使用 Streamable HTTP 来访问 MCP 服务器，所以要访问 EMQX MCP 桥接插件暴露的 `/mcp` 路径，而不是 `/sse` 路径。
:::

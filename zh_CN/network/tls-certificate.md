...
基于路径的证书通过在监听器或连接器的 SSL 配置项中显式指定证书文件路径来进行配置，例如：
...
从 EMQX 6.1 开始，EMQX 引入了**托管证书**，这是一种集中式管理 TLS 证书文件的机制，可在多个组件之间复用。
...
托管证书可以通过 Dashboard 或 HTTP API 创建和管理，并存储在 EMQX 数据目录下的 `data/certs2/` 中。
...
除了 Dashboard，EMQX 还提供 HTTP API 用于管理 TLS 证书文件。通过 API 创建的托管证书与通过 Dashboard 创建的证书完全一致，并可通过相同方式被监听器和连接器引用。
...
EMQX 支持通过 ACME 协议自动签发和续期服务器 TLS 证书（例如使用 Let’s Encrypt）。

ACME 支持是可选的，仅适用于 EMQX 直接终止 TLS、且证书用于公网域名的部署场景。

ACME 需由 EMQX 管理员通过配置文件进行配置，无法通过 Dashboard 启用，也不适用于命名空间级用户。

ACME 仅用于服务器证书（MQTTS、HTTPS/Dashboard 和 WSS 监听器），不用于客户端证书。

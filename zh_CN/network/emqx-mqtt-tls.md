...
2. 选择名称为 **default**，类型为 **ssl** 的监听器，点击名称进行编辑。TLS/SSL 连接相关的参数如下：
   ...
   - **密钥文件密码**：如果密钥文件由密码保护，则需要输入密码。如果希望从文件而不是直接在配置中加载密码，可以使用 `file://<文件路径>` 形式；EMQX 将读取该文件的内容（去除末尾空白字符）作为密码。集群部署时，该文件必须在所有 EMQX 节点上都存在。详见[从文件加载机密](../configuration/secret-from-file.md)。

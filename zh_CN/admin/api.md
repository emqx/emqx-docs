...
2. 配置 API 密钥详细信息：
   ...
   - **权限模式**：管理员或查看者密钥可选择以下权限范围分配方式：
     - **角色默认权限**：使用所选角色的默认权限。角色默认权限发生变化时，新权限会自动生效。
     - **系统级权限**：仅授予 `system` 权限范围。
     - **自定义受限权限**：选择一个或多个非系统权限范围。如果将**权限范围**留空，密钥不能访问受权限范围保护的 API。
   - **权限范围**：选择**自定义受限权限**后显示。选择要授予的权限范围，参见 [API 权限范围](#api-权限范围)。
   - **备注**：可选，填写密钥的描述信息。
...
发布者密钥不显示**权限模式**字段，固定使用角色默认的 `publish` 权限范围。
...
在 EMQX 6.0.4 之前创建且使用混合权限范围列表的 API 密钥可以继续工作。在 Dashboard 中编辑此类密钥时，表单会显示兼容性警告，并要求在保存前选择**系统级权限**、**自定义受限权限**或**角色默认权限**。后续请求显式提交权限范围列表时，必须改为仅使用 `system`，或使用不包含 `system` 的列表。省略 `scopes` 字段时，更新请求保留已有设置，创建请求使用所选角色的默认权限；空列表 `[]` 仍表示拒绝所有业务端点。

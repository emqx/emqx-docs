...
从 EMQX 5.10.4 开始，除了基于角色的访问控制，EMQX 企业版还支持对 API Key 进行更细粒度的权限范围（Scope）控制。通过选择一个或多个 Scope，可以将 API Key 的访问限制在特定功能区域内。

创建或编辑 API Key 时，可在 Dashboard 中查看可用的权限范围及其描述。

##### 通过 Dashboard 配置权限范围

1. 进入 **系统** -> **API Key** 页面。
2. 点击 **创建** 或编辑已有的 API Key。
3. 在 **权限范围** 区域，选择需要允许的范围。
4. 点击 **保存**。

<!-- TODO: 添加 API Key 创建/编辑页面中权限范围区域的截图 -->

::: tip
- 如果不选择任何范围，API Key 将拥有其角色允许的所有资源访问权限。
- 选择特定范围后，API Key 只能访问对应的功能区域。
:::

...
::: warning 与监听器挂载点不兼容
对于通过配置了[挂载点](../configuration/listener.md#挂载点mountpoint)的监听器连接的客户端，消息流回放不生效。挂载点前缀会在 EMQX 匹配 `$stream/` 前缀之前被添加，因此对 `$stream/<name>` 的订阅会被当作对挂载后字面主题的普通订阅处理，且不会向客户端报告任何错误。消息写入同样受影响：发布的主题带有挂载点前缀，只有当消息流的主题过滤器包含该前缀时才能匹配。
:::

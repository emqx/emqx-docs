...
最后值消息流的自动创建默认启用。在接受不受信任客户端的部署中，仅允许访问特定的预创建消息流（例如 `$stream/events`），并拒绝与 `$stream/#` 或 `$s/#` 匹配的所有其他订阅。或者，禁用自动创建并通过 Dashboard 或 REST API 创建消息流。参见[通过 Dashboard 自动创建消息流](./mqtt-stream-task.md#通过-dashboard-自动创建消息流)。

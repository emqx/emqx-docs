...
::: warning 与监听器挂载点不兼容
对于通过配置了[挂载点](../configuration/listener.md#挂载点mountpoint)的监听器连接的客户端，消息流回放不生效。EMQX 会先应用挂载点，再匹配 `$stream/` 前缀，因此对 `$stream/<name>` 的订阅会被当作对挂载后字面主题的普通订阅处理，且不会向客户端报告任何错误。消息写入同样受影响：发布的主题带有挂载点前缀，只有当消息流的主题过滤器包含该前缀时才能匹配。
:::
...
- **MQTT 消息流**
  ...
  - **最后值流**：最后值消息流启用了[最后值语义](#最后值语义)。对于具有相同流键（key）的消息，新的消息会覆盖旧的消息，消息流中仅保留该 key 对应的最新一条消息。详情请参见[流键表达式](./mqtt-stream-task.md#流键表达式)。
...
EMQX 针对完整的消息流订阅主题过滤器执行授权，包括 `$stream/` 前缀和消息流名称。允许客户端订阅消息流后，该客户端还可能回放消息流在订阅创建前已存储的消息。

### 消息流订阅需要单独的规则

针对普通主题空间编写的规则不会覆盖对应的消息流订阅：

- 针对 `t/#` 的规则不适用于 `$stream/events/t/#`，EMQX 将两者视为不同的主题。
- 以 `#` 或 `+` 开头的授权主题过滤器不会匹配以 `$` 开头的订阅主题过滤器。拒绝 `#` 的规则（包括默认 `acl.conf` 中的 `{eq, "#"}` 规则）不会拒绝 `$stream/events/#`。

因此，被拒绝订阅 `#` 的客户端仍可以订阅 `$stream/events/#`，并将 `stream-offset` 设置为 `earliest`，以回放消息流中仍存储的所有消息。请为 `$stream/` 添加显式规则，并确保这些规则至少与消息流主题过滤器所匹配主题的规则同样严格：

```erlang
%% 允许某个消费者回放预先创建的 "events" 消息流，但不授予自动创建权限。
{allow, {username, "event_reader"}, subscribe, ["$stream/events"]}.

%% 拒绝其他所有消息流订阅，包括已废弃的前缀。
{deny, all, subscribe, ["$stream/#", "$s/#"]}.
```

只要仍有客户端使用已废弃的 `$s/` 前缀，就应在规则中一并包含该前缀。参见[已废弃的订阅前缀](#已废弃的订阅前缀)。

该行为与[共享订阅](../messaging/mqtt-shared-subscription.md)不同。对于 `$share/<group>/t/#`，EMQX 会去掉前缀并对 `t/#` 授权；对于 `$stream/<name>/t/#`，EMQX 对完整的订阅主题过滤器授权。

### 自动创建允许客户端指定主题过滤器

启用消息流自动创建后，由发起订阅的客户端决定新消息流的主题过滤器。EMQX 直接使用 `$stream/<name>/<topic_filter>` 中的 `<topic_filter>` 创建消息流。EMQX 不会单独检查该客户端是否有权直接订阅 `<topic_filter>`。

例如，获准订阅 `$stream/+/#` 的客户端可以订阅 `$stream/events/#`。如果 `events` 消息流不存在，EMQX 会创建该消息流，并将 `#` 用作其主题过滤器。该消息流随后会存储发布到所有非 `$` 主题的消息。因此，客户端可能回放来自其无权直接订阅的主题的消息。

自动创建对最后值消息流默认启用，对常规消息流默认禁用，但它仅在 MQTT 消息流功能处于活动状态时生效。该功能本身默认禁用（`streams.enable = false`）。启用该功能后，订阅一个尚未使用的消息流名称即可创建该消息流。在接受不受信任客户端的部署中，仅允许访问特定的预创建消息流（例如 `$stream/events`），并拒绝与 `$stream/#` 或 `$s/#` 匹配的所有其他订阅。或者，禁用自动创建并通过 Dashboard 或 REST API 创建消息流。参见[通过 Dashboard 自动创建消息流](./mqtt-stream-task.md#通过-dashboard-自动创建消息流)。

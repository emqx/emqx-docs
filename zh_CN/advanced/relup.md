...
使用版本热升级功能，用户可以在保持大多数客户端连接不断开的情况下，快速、安全地升级生产环境中正在运行的 EMQX，并避免了因重启服务导致的系统可用性降低。

热升级功能可能会导致少数连接断开，热升级过程的资源消耗跟版本之间代码变动的多少、在线客户端数量、系统消息速率等因素有关。请在系统负载较低的时段进行热升级操作。

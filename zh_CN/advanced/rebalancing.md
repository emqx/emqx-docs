...
MQTT 是有状态的长连接接入协议，因此连接建立后不会轻易断开。但在某些情况下，比如需要对集群节点进行升级、维护或扩容，此时就需要断开已有连接再进行重连，为便于用户的运维操作，EMQX 提供了节点疏散和集群负载重平衡功能。
...
| `--wait-takeover` | 正整数 | 所有连接断开后，等待客户端重连以接管会话的时间；单位： 秒，如不配置，则 |
...
该命令会以每秒 `30` 个连接的速度断开现有客户端，所有连接断开后将等待 `200` 秒，之后以每秒 `30` 个会话的速度迁移现有会话到 `emqx2@127.0.0.1` 与 `emqx3@127.0.0.1`。
...
| `conn_evict_rate` | 正整数 | 是         | 客户端每秒断开连接速度。                                     |
| `migrate_to`      | 正整数 | 是         | 目标节点列表，多个节点以逗号或空格分隔。                     |
| `wait_takeover`   | 正整数 | 否         | 所有连接断开后，等待客户端重连以接管会话的时间；单位： 秒    |
| `sess_evict_rate` | 正整数 | 否         | `wait-takeover` 之后每秒会话疏散速度。                       |
...
同样由于 MQTT 是有状态的长连接接入协议，已连接节点会保持连接状态。即使在节点扩容场景下，现有节点的连接也不会自动切换到新增节点中，因此如果没有大量的新客户端接入，新增节点会长时间处于负载不足的状态。此时，您需要手动将连接从高负载节点迁移到低负载节点，实现集群负载均衡。
...
如希望在 `emqx@127.0.0.1`、`emqx2@127.0.0.1` 与 `emqx3@127.0.0.1` 三个节点之间实现负载重平衡，可通过如下代码实现：

...
本页面基于在多种场景下进行的一系列基准测试，对 EMQX 的性能进行了详细分析。评估内容涵盖了 EMQX 在不同条件下的运行行为和能力表现，包括不同的服务质量（QoS）等级、消息负载大小、发布–订阅模型，以及 MQTT 消息桥接对系统性能的影响。

通过严谨的测试与精确的测量，这些结果为理解 EMQX 在真实生产环境中的性能表现提供了有价值的参考，并为用户优化 IoT 场景下的 EMQX 部署提供指导。

...
第二条警告列出了两种 Cookie 配置方式：在 `emqx.conf` 中设置 `node.cookie`，或设置 `EMQX_NODE__COOKIE`。从 EMQX 6.3.0 开始，两种配置都支持 `file://`，无需在配置文件或环境变量中直接写入 Cookie 值。更多信息，参见[从文件加载节点 Cookie](../guides/configuration/secret-from-file.md#从文件加载节点-cookie)。

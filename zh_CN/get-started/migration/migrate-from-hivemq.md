...
若需使用其他发现方式（如 etcd、Kubernetes 或静态文件），请参见[创建与管理集群](../../guides/cluster/create-cluster.md)。
...
| ---------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **文件领域（File Realm）** (`credentials.xml`) | [**内置数据库（Built-in Database）**](../../guides/access-control/authn/mnesia.md) | 从 HiveMQ 导出用户数据，并通过 EMQX REST API 导入。          |
| **SQL 领域（JDBC）**                           | [**MySQL**](../../guides/access-control/authn/mysql.md) / [**PostgreSQL**](../../guides/access-control/authn/postgresql.md) | 配置基于密码的认证机制，后端为 `mysql` 或 `postgresql`。可复用现有用户表结构。 |
| **LDAP 领域 / Active Directory**               | [**LDAP**](../../guides/access-control/authn/ldap.md)                  | 配置基于密码的认证机制，后端为 LDAP。将 HiveMQ 的 DN 模式映射为 EMQX 的筛选模板。 |
| **OAuth / JWT**                                | [**JWT**](../../guides/access-control/authn/jwt.md)                    | 配置 JWT 认证机制，指定公钥或 JWKS 端点。                    |
| **HTTP / Webhooks**                            | [**HTTP Server**](../../guides/access-control/authn/http.md)           | 配置基于密码的 HTTP 后端认证，将凭证验证委托给外部认证服务。 |
| **X.509 证书**                                 | [**X.509**](../../guides/access-control/authn/x509.md) / [**mTLS**](../../guides/network/emqx-mqtt-tls.md#enable-ssl-tls-with-two-way-authentication) | 使用 TLS 监听器和双向认证（mTLS），重用现有 CA 与客户端证书。 |
...
2. **导入到 EMQX：**
    使用 EMQX 的 REST API 创建用户。EMQX 支持导入包含哈希密码（如 bcrypt、pbkdf2 等）的用户数据。详情参见[导入认证数据](../../guides/access-control/authn/user_management.md#导入认证数据)。
...
HiveMQ 的 SQL 领域使用一个 [固定数据库模式](https://docs.hivemq.com/hivemq-enterprise-security-extension/latest/ese.html#table_users)。而 EMQX 允许您 [自定义数据库模式和查询语句](../../guides/access-control/authn/mysql.md)，**因此无需修改现有的 MySQL 或 PostgreSQL 数据库结构。**
...
- [**文件 ACL (`acl.conf`)**](../../guides/access-control/authz/file.md)：
   `{allow, all, subscribe, ["device/${clientid}/#"]}.`
- [**内置数据库（Mnesia）**](../../guides/access-control/authz/mnesia.md)：
   可通过 Dashboard 或 API 基于客户端 ID、用户名或主题配置访问规则。
- [**MySQL**](../../guides/access-control/authz/mysql.md)：
   `SELECT action, permission, topic, ipaddress, qos, retain FROM mqtt_acl WHERE clientid = ${clientid} AND ipaddress = ${peerhost};`
- [**PostgreSQL**](../../guides/access-control/authz/postgresql.md)：
   `SELECT action, permission, topic, ipaddress, qos, retain FROM mqtt_acl WHERE clientid = ${clientid} AND ipaddress = ${peerhost};`

更多详情请参见[**授权**](../../guides/access-control/authz/authz.md)文档。
...
- [**数据集成概述**](../../develop/data-integration/data-bridges.md)
- [**规则引擎**](../../develop/data-integration/rules.md)
- [**可视化 Flow 设计器**](../../develop/flow-designer/introduction.md)
...
从 EMQX 6.3.0 开始，Prometheus 抓取端点默认要求身份认证。请为抓取程序配置 API Key 和 Secret Key，或显式关闭身份认证。详情请参见[集成 Prometheus](../../guides/observability/prometheus.md#身份认证)。
...
有关 Pushgateway 的配置详情，请参见[集成 Prometheus](../../guides/observability/prometheus.md#配置-push-模式集成)。
...
请参考[日志](../../guides/observability/log.md)文档，了解日志级别、轮转策略和格式（text/JSON）的配置方法。
...
详细功能请参考[日志追踪](../../guides/observability/tracer.md)。

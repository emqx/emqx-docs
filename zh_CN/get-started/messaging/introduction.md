...
- [订阅过滤器](../../develop/subscription-filter/subscription-filter-concept.md)
- [消息队列](../../develop/message-queue/message-queue-concept.md)

### 订阅过滤器（高级功能）

[订阅过滤器](../../develop/subscription-filter/subscription-filter-concept.md)是 EMQX 6.2 引入的一项高级功能，在 MQTT 5.0 发布/订阅模型的基础上增加了订阅层面的内容过滤能力。通过在主题过滤器后附加过滤表达式，订阅方可指示 EMQX 只投递用户属性满足指定条件的消息。
...
[消息队列](../../develop/message-queue/message-queue-concept.md)是 EMQX 6.0 引入的一项高级功能，它在 MQTT 发布/订阅模型的基础上，提供了持久化和异步的消息排队能力。即使订阅者处于离线状态，消息也可以被存储。同时它还支持高级的消息分发策略以及通过最后值语义进行的消息压缩。

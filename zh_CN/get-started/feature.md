...
支持。你可以通过 **EMQX 企业版**的 [数据集成](../develop/data-integration/data-bridges.md) 功能实现消息的持久化，EMQX 支持多种关系型、非关系型以及时序数据库，你可以按需选择。

## EMQX 支持将 MQTT 消息转发到 Kafka 等消息队列吗？

支持。你可以通过 **EMQX 企业版**的 [数据集成](../develop/data-integration/data-bridges.md) 功能将消息转发给 Kafka、RabbitMQ 等消息队列。

## EMQX 支持将 MQTT 消息转发到其他 MQTT 服务吗？

支持。你可以通过 EMQX 的 [MQTT 桥接](../develop/data-integration/data-bridge-mqtt.md) 功能将消息转发给其他 MQTT 服务，包括部署在 AWS 和 Azure 等公有云上的 IoT Hub，以及 EMQX、Mosquitto 这类标准的 MQTT Broker。
...
详情可参考 [ExProto 协议网关](../develop/gateway/exproto.md)。

...
完整命令参考请参见 [license history](../../guides/cli.md#license-history)。
...
自然日的划分边界由配置项 `license.high_watermark_timezone` 决定。默认值为 `"system"`，即跟随节点所在主机的本地时区。您可以设置显式的 UTC 偏移量（如 `"+08:00"`），以确保跨地域节点的日期边界保持一致。详情请参见 [License 配置](../../guides/configuration/license.md)。

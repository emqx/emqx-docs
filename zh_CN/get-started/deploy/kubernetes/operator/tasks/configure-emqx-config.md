...
1. 将以下内容保存为 YAML 文件，并使用 `kubectl apply` 部署：
   ...
   ::: tip
   EMQX Operator 将监听器设置等大多数配置写入 [`base.hocon`](../../../../../guides/configuration/configuration.md#基础配置文件)，并通过 EMQX Configs API 在运行时应用变更，无需重启 Pod。Operator 将 Dashboard 监听器和节点设置等仅在 EMQX 启动时生效的配置写入 [`emqx.conf`](../../../../../guides/configuration/configuration.md#不可变配置文件)。修改此类设置会触发受控滚动更新。
   :::

...
EMQX 内置持久性功能所使用的数据库引擎将数据按照一个分层结构来组织。下图展示了持久存储数据库在 EMQX 集群中的分布方式：
...
代是按时间划分的逻辑分区。消息会根据时间段被切分到不同的代中。新消息写入当前代，而以前的代是只读的。EMQX 通过完全删除旧的代来清理旧的 MQTT 消息。旧 MQTT 消息的保留期限由 `durable_sessions.message_retention_period` 参数确定。
...
数据块是由分片 ID 与代 ID 唯一标识的物理数据分区。每个数据块是一个持久化的数据容器，内部包含一个或多个流（Stream）。同一数据块内的所有数据使用相同的编码格式，并且写入操作是原子的（Atomic），无需额外的元数据管理。

**示例：**`shard 2, gen 3` 表示“分片 2、第 3 代”的一个独立数据块，用于承载该分片在该代中的一个或多个流及其数据。

### 流 (Stream)

流是每个 Slab 内部的逻辑批处理与序列化单元。Stream 将主题相似的**主题–时间戳–值（Topic–Timestamp–Value, TTV）**三元组进行分组，使数据能够按时间顺序以确定性方式读取。流可以包含多个主题的消息，不同的存储布局可以采用不同的策略将主题映射到流中。

持久会话以批量方式从流中获取消息，批量大小可以通过 `durable_sessions.batch_size` 参数调整。

### 主题–时间戳–值（Topic–Timestamp–Value, TTV）

TTV 是最小的数据存储单元，表示一条 MQTT 记录。每个 TTV 包含以下字段：

...
启用会话持久化后，EMQX 会将持久会话的元数据以及发送到持久会话的 MQTT 消息保存到磁盘。因此，必须为 EMQX 部署足够的存储容量。EMQX 数据目录必须使用本地文件系统，因为[内嵌持久存储后端](../design/durable-storage.md#内嵌后端)不支持 NFS、SMB/CIFS 等网络文件系统。为获得最佳吞吐量，请使用固态硬盘（SSD）存储。

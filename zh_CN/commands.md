...
emqttd 消息服务器提供了'./bin/emqttd_ctl'的管理命令行。
...
| broker        | 查询 emqttd 消息服务器描述、版本、启动时间     |
| ------------- | ---------------------------------------------- |
...
cluster 命令集群多个 emqttd 消息服务器节点(进程):
...
sessions 命令查询 MQTT 连接会话。emqttd 消息服务器会为每个连接创建会话，clean_session 标记 true，创建临时(transient)会话；clean_session 标记为 false，创建持久会话(persistent)。
...
plugins 命令用于加载、卸载、查询插件应用。emqttd 消息服务器通过插件扩展认证、定制功能，插件置于 plugins/目录下。
...
bridges 命令用于在多台 emqttd 服务器节点间创建桥接:

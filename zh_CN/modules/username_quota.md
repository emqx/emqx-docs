...
::: tip 提示
从企业版 4.4.38 版本开始，用户名配额限制模块的跨节点数据同步已由 Mnesia 复制重构为「每节点 ETS 表 + 专用 RPC 通道」的流式同步，降低了内存占用以及集群节点变动时的资源消耗，并为数据同步与处理路径增加了过载保护。相应地，此前已废弃的 `refresh_username_tab_interval` 配置项（即 Dashboard 中的**刷新用户名表的间隔时间**选项）已被移除。
:::
...
![user-quota-create](./assets/username-quota-create-zh.png)

## 管理用户名配额

在**模块**页面中，找到您已添加的**用户名配额限制**模块。点击右侧的**管理**。

![user-quota-usage](./assets/username-quota-usage-zh.png)

### 查看使用详情

在**使用详情**标签页，您可以看到 EMQX 集群中的当前用户名及其使用的会话数。

- 点击会话数右侧的**查看**按钮，可以查看当前用户名的会话列表。
- 点击**断开所有会话**按钮，可强制断开该用户名下的所有客户端连接。
- 若需按会话数量排序用户名列表，可打开**会话数**旁边的**排序**开关。

### 配置配额限制

点击**参数设置**标签页，您可以编辑用户名会话配额限制的设置。你可以通过此配置控制每个用户名允许的并发会话数。

### 删除用户名配额模块

如需删除**用户名配额** 模块，点击页面右上角的**删除**按钮。

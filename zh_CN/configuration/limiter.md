...
速率限制器自 EMQX 5.0 引入，用于限制连接速率，以及客户端发布和接收消息、传输数据和发送 `SUBSCRIBE` 报文的速率。有关速率限制器及其工作原理的更多信息，请参见[速率限制](../rate-limit/rate-limit.md)。
...
订阅报文速率限制分别应用于每个客户端连接。该限流器统计报文数量，而不是主题过滤器数量。达到限制后，EMQX 不处理该报文，也不创建其中的订阅，但会保持客户端连接。对于 MQTT 5.0，EMQX 返回 `SUBACK`，其中每个主题过滤器对应的原因码均为 `Quota Exceeded`（0x97）。对于 MQTT 3.x，EMQX 在 `SUBACK` 中为每个主题过滤器返回失败返回码（0x80）。

`subscribes_rate` 的默认值为 `infinity`，即不限制订阅报文速率。为托管命名空间配置订阅报文速率限制后，该配置会覆盖命名空间内客户端所使用的监听器级订阅报文速率限制。

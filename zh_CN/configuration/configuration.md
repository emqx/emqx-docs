...
- **无模式编码/解码函数**：
  ...
  - [base64_decode(Data, 'no_padding')](../data-integration/rule-sql-builtin-functions.md#base64-decode-data-string-bytes-string)  (自 6.0.2 起)
  - [base64_decode(Data, 'no_padding', 'urlsafe')](../data-integration/rule-sql-builtin-functions.md#base64-decode-data-string-bytes-string)  (自 6.0.2 起)
...
- **数据提取函数**：
  ...
  - `is_jwt(Data)`（从 6.2.3 起）：仅当 `Data` 在结构上是 JWS Compact Serialization 形式的 JWT 时返回 `true`。该值必须包含 3 个以点号分隔且可进行 Base64URL 解码的片段，解码后的 header 必须是包含 `alg` 字段的 JSON 对象。该函数不会校验签名，也不会检查 payload。对于 `undefined`、`null`、空字符串、包含 5 个片段的 JWE Token 和格式错误的值，该函数返回 `false`。

...
<!-- for a more detailed description of each log level, see [Log and observability - Log](https://docs.emqx.com/en/enterprise/latest/observability/log.html). -->

You can configure EMQX logs with Dashboard or configuration files. To configure with EMQX Dashboard, you can click **Management** -> **Logging** on the left navigation menu to configure.

:::tip

Most configuration items listed here also can be configured with Dashboard, and their Dashboard UI fields are also introduced on this page.
...
This section introduces how to configure logging behaviour for EMQX.
...
EMQX's log output directory is determined by the environment variable `EMQX_LOG_DIR` which is set to `/var/log/emqx` if installed via RPM or DEB packages.
Othereise, the log directory is `log` in the EMQX installation directory.

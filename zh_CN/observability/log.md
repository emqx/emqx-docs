...
您可通过 EMQX Dashboard 或者配置文件修改日志配置。比如，如果您想要将级别为 warning 的日志输出到日志文件和控制台，您可以在 `emqx.conf` 文件中修改 `log` 下的配置项，参见下面示例。重启节点后配置生效。想了解更多通过配置文件修改日志配置，详见 [Logs](../configuration/logs.md)。
...
下拉页面，继续配置日志处理进程过载保护和日志限流保护功能选项：

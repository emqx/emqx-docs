...
系统的默认日志输出可以通过环境变量 `EMQX_DEFAULT_LOG_HANDLER` 来配置，该环境变量接受以下设置：
...
- **启用日志输出**：单击切换开关以启用控制台日志输出。
- **日志级别**：选择要记录的最低日志级别。可选值为 `debug`、`info`、`notice`、`warning`、`error`、`critical`、`alert` 和 `emergency`。默认值为 `warning`。
- **日志格式类型**：选择日志格式。可选值为 `text` 和 `json`，分别表示自由文本日志和结构化日志。默认值为 `text`。
...
- **时间偏移量**：设置格式化日志时间戳时使用的时间偏移量。输入 `system` 可使用本地系统的时区偏移量，输入 `utc` 可使用 UTC，也可以输入 `+-[hh]:[mm]` 格式的固定偏移量，例如 `-02:00` 或 `+00:00`。默认值为 `system`。JSON 日志的时间戳采用 Unix 纪元格式，因此此设置不影响 JSON 日志。
- **Payload 编码**：选择日志条目中 Payload 数据的编码方式。可选值为：
  - `text`：使用文本编码，适用于基于文本的协议。Payload 采用 JSON 编码时，建议选择此值。
  - `hex`：使用十六进制编码，适用于自定义二进制协议。
  - `hidden`：将 Payload 替换为 `******`。
...
- **启用日志输出**：单击切换开关以启用文件日志输出。
- **日志文件名字**：输入日志文件的路径和名称。默认值为 `${EMQX_LOG_DIR}/emqx.log`，其中 `${EMQX_LOG_DIR}` 表示 EMQX 日志目录。
- **最大日志文件数**：设置轮换日志文件的最大数量。默认值为 `10`。
- **日志文件轮换大小**：设置单个日志文件开始轮换前的最大大小。输入数值并选择 `KB`、`MB` 或 `GB`。默认值为 `50 MB`。关闭开关后，该值变为 `infinity`，日志文件不再按大小轮换。
- **日志级别**：选择要记录的最低日志级别。可选值为 `debug`、`info`、`notice`、`warning`、`error`、`critical`、`alert` 和 `emergency`。默认值为 `warning`。
- **日志格式类型**：选择日志格式。可选值为 `text` 和 `json`，分别表示自由文本日志和结构化日志。默认值为 `text`。
- **时间戳格式**：从下拉列表中选择日志时间戳格式。可选值为：
  - `auto`: 根据所使用的日志格式类型自动确定时间戳格式。对于文本格式类型，使用 `rfc3339` 格式；对于 JSON 格式类型，则使用 `epoch`格式。

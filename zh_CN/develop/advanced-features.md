...
- [消息队列](./message-queue/message-queue-concept.md) 通过持久化队列语义扩展 MQTT 发布/订阅模式，实现可靠的异步消息传递。
- [MQTT over WebSocket](./connect-emqx/mqtt-over-websocket.md) 支持 MQTT 客户端（包括基于浏览器的应用）通过 WebSocket 连接到 EMQX。
- [MQTT over QUIC](./mqtt-over-quic/introduction.md) 介绍了 EMQX 中的这一开创性功能，以及如何在 EMQX 中启用该功能。

...
::: warning 与监听器挂载点不兼容
对于通过带有[挂载点](../../guides/configuration/listener.md#挂载点mountpoint)的监听器连接的客户端，消息队列不生效。EMQX 会先应用挂载点，再匹配 `$queue/` 前缀，因此该订阅会被当作对挂载后字面主题的普通订阅处理。此过程不会向客户端报告任何错误。
:::
...
虽然 MQTT 通过[共享订阅](../../get-started/messaging/mqtt-shared-subscription.md)（如 `$share/{group}/topic`）提供了一些类队列功能，但仍存在以下限制：
...
- **消息存储 DB**：存储所有入队消息，构建于 EMQX 的[持久化存储](../../guides/durability/durability_introduction.md#持久存储的架构)之上。
...
- [共享订阅](../../get-started/messaging/mqtt-shared-subscription.md)：将消息分发给多个订阅者，但在没有客户端在线时不会保留消息。
- [保留消息](../../get-started/messaging/mqtt-retained-message.md)：保存某个主题的最新消息，但仅在新客户端订阅该主题时发送一次。
- [MQTT 持久会话](../../guides/durability/durability_introduction.md)：在客户端断线重连后，保留其会话状态（订阅信息及 QoS 1/2 消息）。
...
该行为与[共享订阅](../../get-started/messaging/mqtt-shared-subscription.md)不同。对于 `$share/<group>/t/#`，EMQX 会去掉前缀并对 `t/#` 授权；对于 `$queue/<name>/t/#`，EMQX 对完整的订阅主题过滤器授权。
...
自动创建对最后值队列默认启用，对常规队列默认禁用，但仅在消息队列功能处于活动状态时生效。在默认的 `mq.enable = auto` 配置下，EMQX 仅在至少已有一个队列时才激活消息队列功能，因此订阅无法创建第一个队列。在接受不受信任客户端的部署中，仅允许访问特定的预创建队列（例如 `$queue/orders`），并拒绝与 `$queue/#` 或 `$q/#` 匹配的所有其他订阅。或者，禁用自动创建并通过 Dashboard 或 REST API 创建队列。参见[通过 Dashboard 自动创建队列](./message-queue-task.md#通过-dashboard-自动创建队列)。

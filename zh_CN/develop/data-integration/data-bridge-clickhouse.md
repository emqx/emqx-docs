...
9. 从 EMQX 6.3.1 开始，**批量值分隔符**配置仅为兼容性保留，不再生效。启用[批量模式](./data-bridges.md)时，EMQX 会根据 SQL 格式自动推导分隔符。

10. 在 **SQL 模板**中输入以下命令：

    ::: warning 重要提示

    从 EMQX 6.3.1 开始，EMQX 会在创建 Sink 时解析 SQL 模板，根据 SQL 上下文转义占位符值，并拒绝不支持的语法。模板必须是单条 ClickHouse `INSERT` 语句，并使用 `VALUES`、`FORMAT Values` 或 `FORMAT JSONCompactEachRow`。占位符可用于值位置和字符串字面量中。不支持 SQL 注释、附加语句或在标识符中使用占位符。
...
12. **高级设置（可选）**：具体可阅读[ 高级配置](#高级配置)。
13. 点击**创建**前，您可点击**测试连接**按钮确保能连接到 ClickHouse 服务器。
14. 点击**创建**按钮完成 Sink 的创建，创建成功后页面将回到创建规则，新的 Sink 将添加到规则动作中。
15. 回到规则创建页面，点击**创建**按钮完成整个规则创建。

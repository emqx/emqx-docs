...
在 [EMQX Mapping Tables 插件](../../guides/extensions/plugin-catalog/6.3/emqx-maptabs.md)管理的 mapping table 中查找一行数据。查找成功时返回该行的值映射；如果表不存在、Key 不存在或 Key 类型不匹配，则返回 `undefined`。

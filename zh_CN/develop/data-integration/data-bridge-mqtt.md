...
- **MQTT 服务地址**：目标 MQTT 服务的地址和端口，例如：`broker.emqx.io:1883`。
  - 支持 `host:port`、`[IPv6]:port`、`mqtt://host:port`、`mqtt://[IPv6]:port`、`mqtts://host:port` 和 `mqtts://[IPv6]:port` 格式。如果省略端口，EMQX 使用默认 MQTT 端口 `1883`。不支持其他 URI scheme。
  - TLS 需在连接器中单独配置。地址中的 scheme 与 TLS 设置的关系如下：
    - 显式使用 `mqtts://` 时必须启用 TLS。否则，**测试连接**会失败，连接器也无法启动。
    - 使用 `mqtt://` 但启用 TLS 时，TLS 设置优先。EMQX 将使用 TLS 连接并记录一条警告。
    - 地址未包含 scheme 时，是否使用 TLS 由 TLS 设置决定。
...
5. 填写连接相关配置：
   ...
   - **MQTT 服务地址**：仅支持 MQTT over TCP/TLS。输入 MQTT 服务地址，例如 `broker.emqx.io:1883`、`[::1]:1883` 或 `mqtt://broker.emqx.io:1883`。

   - **启用 TLS**：连接到启用了 TLS 的 MQTT 监听器时，启用此选项。如果 MQTT 服务地址显式使用 `mqtts://`，必须启用此选项。地址未包含 scheme 时，是否使用 TLS 由此选项决定。
   ...
   - **静态客户端 ID 映射表**：此部分允许为连接器配置静态 Client ID，以确保连接稳定，特别是在连接 Azure IoT Hubs 等服务时。更多配置方法参见[配置静态客户端 ID](#配置静态客户端-id)。
...
2. 对于每个条目，填写以下字段：
   - **节点名称**：指定客户端 ID 所分配的节点，例如 `emqx@10.0.0.1`。
...
你也可以在配置文件中为每个节点单独定义 `static_clientids` 参数。
...
9. 其他配置使用默认值，点击**创建**按钮完成 Sink 的创建，创建成功后将回到创建规则页面，新的 Sink 将添加到规则动作中。

10. 在规则创建页面，点击**创建**按钮完成整个规则创建。
...
8. 配置 Source 信息，完成消息从外部 MQTT 服务订阅到 EMQX 的配置：
   ...
   - **禁止本地转发**：如果使用同一个连接器向 MQTT Source 所订阅的主题发布消息，启用此选项可防止远端 MQTT Broker 将这些消息转发回该 Source。此选项默认关闭，仅在连接器使用 MQTT 5.0 时生效。
   - **保留发布时 Retain 标志**：启用后，上游 MQTT Broker 会保留转发消息原始的 `retain` 标志；关闭后，上游 MQTT Broker 会清除该标志。此选项默认启用，仅在连接器使用 MQTT 5.0 时生效。
...
9. 其他配置使用默认值，点击**创建**按钮完成 Source 的创建，将 Source 添加到规则数据输入中。同时你可以看到规则 SQL 已经变为：
   ...
   | retain                        | 消息是否携带从上游 MQTT Broker 接收的 `retain` 标志。关闭**保留发布时 Retain 标志**后，该值为 `false` |
   | qos                           | 消息服务质量                                                 |
...
2. 填写消息重发布配置：
   ...
   - **Retain**：选择 `true` 或 `false` 确认是否以保留消息方式发布消息，也可以输入占位符从其他字段中设置保留消息标志位，此处我们选择 `false`。
     ...
     - 您也可以输入 `${retain}`，表示跟随 MQTT Source 接收到的 `retain` 标志。使用 MQTT 5.0 连接时，需要在 Source 中启用**保留发布时 Retain 标志**，以保留上游消息的 `retain` 标志。
   - **消息模板**：用于生成转发消息 Payload 的模板，默认留空表示转发规则输出结果。此处可以输入 `${payload}` 表示仅转发 Payload。

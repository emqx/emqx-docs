...
8. 配置 **SQL 模板**。如需转发指定主题的消息，可使用以下 SQL 语句插入数据。
   ...
   从 EMQX 6.3.1 开始，EMQX 会在创建 Sink 时解析 SQL 模板，根据 SQL 上下文转义占位符值，并拒绝不支持的语法。模板必须是单条 Microsoft SQL Server `INSERT INTO ... VALUES` 语句，且只能配置一行值。占位符可用于值位置和字符串字面量中。不支持 SQL 注释、附加语句、配置多行值或在标识符中使用占位符。

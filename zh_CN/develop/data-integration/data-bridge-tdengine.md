...
8. 配置 **SQL 模板**，可使用如下 SQL 完成数据插入，并支持通过 CSV 文件批量设置，详细请参考[批量设置](#批量设置)。
   ...
   从 EMQX 6.3.1 开始，EMQX 会在创建 Sink 时解析 SQL 模板，根据 SQL 上下文转义占位符值，并拒绝不支持的语法。模板必须是单条 TDengine `INSERT` 语句。模板支持多表 `VALUES` 插入和 `USING ... TAGS` 子句。目标表名称不使用引号或使用反引号时可包含占位符，例如 `test_${clientid}`；EMQX 会将完整目标名称渲染为一个带引号的标识符。不支持 SQL 注释、`FILE` 输入或附加语句。

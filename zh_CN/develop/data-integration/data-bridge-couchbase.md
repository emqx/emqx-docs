...
11. **高级设置（可选）**：请参阅[高级配置](#高级配置)。

12. 在点击**创建**之前，您可以点击**测试连接**按钮，确保能够连接到 Couchbase 服务器。

13. 点击**创建**按钮完成 Sink 配置。在**创建规则**页面，您将在**动作输出**选项卡下看到新的 Sink。

14. 在**创建规则**页面，验证配置信息并点击**创建**按钮生成规则。您创建的规则将显示在规则列表中，**状态**应为已连接。

...
4. 配置连接到 Kafka 所需的参数：
   - **主机列表**：输入 `127.0.0.1:9092`。本示例假设 EMQX 和 Kafka 在同一台机器上运行。如果它们在不同的机器上运行，请输入实际的 Kafka broker 地址。从 EMQX 5.10.5 开始，可以使用方括号指定 IPv6 地址，例如 `[::1]:9092`。

   - **IP 地址族**：从 EMQX 5.10.5 开始，选择 EMQX 连接 Kafka broker 时使用的 IP 地址族：

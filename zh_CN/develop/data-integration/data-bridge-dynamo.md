...
本节介绍了在 EMQX 中创建 DynamoDB 数据集成之前需要完成的准备工作，包括选择凭证获取方式、安装 DynamoDB 以及创建数据表。
...
从 EMQX 5.10.5 开始，DynamoDB 连接器支持以下凭证获取方式。请根据 EMQX 的部署环境选择一种方式：

- **手动配置访问密钥**：在连接器中填写 **连接访问 ID** 和 **连接访问密钥**（即 AWS Access Key ID 和 AWS Secret Access Key），并确保该密钥具备目标 DynamoDB 资源访问权限。此方式适用于本地部署、非 AWS 环境，以及不使用 ECS 任务角色或 EC2 实例角色的部署环境。
- **自动获取临时凭证**：如果 EMQX 以 Amazon ECS 任务方式运行或部署在 Amazon EC2 实例上，请配置具备目标 DynamoDB 资源访问权限的 ECS 任务角色或 EC2 实例角色，并在连接器中将**连接访问 ID**和**连接访问密钥**留空。EMQX 将从 ECS 任务角色或 EC2 实例元数据中获取临时凭证，并在凭证过期前自动刷新。

::: warning 重要提示

必须同时填写或同时留空**连接访问 ID**和**连接访问密钥**。只填写其中一项会导致连接器配置无效。
...
3. 在 **配置** 步骤，配置以下信息：
   ...
   - **连接访问 ID**和**连接访问密钥**：在本地 DynamoDB 示例中，分别填写 `root` 和 `public`。如需使用 ECS 任务角色或 EC2 实例角色，请将这两项留空。详情请参见[选择凭证获取方式](#选择凭证获取方式)。
4. 点击**创建**按钮完成连接器创建。
...
8. 配置以下设置：
   ...
   - **消息模版**：留空，表示将整个消息转发给 DynamoDB，实际值为 JSON 模版数据。
     ...
     如果在模板中使用未定义的占位符变量，您可以切换**未定义变量作为 NULL** 开关（位于**消息模板**上方）来定义规则引擎的行为：
...
10. **高级配置（可选）**：根据情况配置同步/异步模式，队列与批量等参数，详细请参考 [Sink 的特性](./data-bridges.md)。

11. 在完成创建之前，您可以点击**测试连接**来测试 Sink 可以连接到 DynamoDB 服务器。

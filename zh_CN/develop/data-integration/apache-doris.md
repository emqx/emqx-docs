...
EMQX 中的 Apahe Doris 数据集成支持 Apache Doris 2.1.7 及之后版本。
...
8. 配置 **SQL 模板**，使用以下 SQL 插入数据。未启用批量模式时，Apache Doris 使用[预处理 SQL](./data-bridges.md#sql-预处理)，占位符不应包含在引号内，SQL 末尾不要带分号（`;`）。

   ::: warning 重要提示

   从 EMQX 6.3.1 开始，启用批量模式时，EMQX 会在创建 Sink 时解析 SQL 模板，使用与 Doris 兼容的语法转义文本和二进制占位符值，并拒绝不支持的模板。模板必须是单条 Apache Doris `INSERT INTO ... VALUES` 语句，且只能配置一行值。占位符可作为完整值，也可用于普通或原始字符串字面量中。不支持 SQL 注释、动态标识符、配置多行值、`INSERT SELECT`、行别名或 `ON DUPLICATE KEY UPDATE`。

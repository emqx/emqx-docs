   ...
   ```
    ...
    配置有效载荷模板时需注意以下几点：
     ...
     - 不支持自动推导**值**的数据类型：
       ...
       - 时间戳、日期和时间类型，如不做特殊处理，将被识别为数值或字符类型，如希望以日期和时间类型存储，需要在规则 SQL 中使用[专用于 MongoDB 的时间函数](./rule-sql-builtin-functions.md#专用于-mongodb-的时间函数)对字段进行处理。（注：此类函数的返回值仅用于 MongoDB 动作，不可用于其他动作。）

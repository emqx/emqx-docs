...
对于通过配置了[挂载点](../../../guides/configuration/listener.md#挂载点mountpoint)的监听器连接的 Agent，A2A Registry 不生效。唯一的例外是挂载点恰好为一个主题层级，例如 `acme/`，EMQX 会将其解析为 `$a2a` 主题的命名空间前缀。使用其他形式的挂载点时，挂载后的主题不再符合 `$a2a/v1/` 主题规范：Agent Card 会作为普通保留消息存储，不会被收录、校验或附加在线状态标注。此过程不会向客户端报告任何错误。
:::

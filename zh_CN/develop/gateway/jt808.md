...
在当前的实现中，它有以下限制：
- 基于 TCP 传输。
- 仅支持 JT/T 808 2011，2013 和 2019。暂时不支持 JT/T 808 2021。
- 无法使用 SMS 方式发送终端注册和终端注销消息。
- 无法使用 EMQX 内建的认证器系统，需要配置终端注册/终端接入鉴权的的 HTTP 服务地址。

## 启用 JT/T 808 网关

您可以通过 Dashboard，REST API 或配置文件 `base.hocon` 启用和配置 JT/T 808 网关。
...
如果 EMQX 在集群中运行，通过 Dashboard 或 REST API 的配置将在整个集群中生效。如果您只希望针对单个节点进行设置，可以使用配置文件 `base.hocon` 配置网关。
...
您也可以通过 REST API 或 `base.hocon` 来启用和配置 JT/T 808 网关，例如：
...
通过 Dashboard 或者 REST API 管理则会在整个集群中生效。
:::
...
- **解析未知消息 ID**：是否解析非标准协议中消息ID。
    - 设置为 `true` 时，网关会将这条消息内容以 base64 的方式编码后发送出来。默认值为 `true`。
    - 设置为 `false` 时，网关会忽略该消息、或者断开客户端连接。这取决于**忽略不支持的帧**配置。
- **字符串编码**：设置网关在解析设备上报字符串以及封装并下发字符串到设备时所采用的的字符串编码格式。
  - 设置为 `utf8` 时，网关将使用 `UTF-8` 编码解析字符串内容。默认值为 `UTF-8`。
  - 设置为 `gbk` 时，网关将使用 `GBK` 编码解析字符串，并在消息发布到 EMQX 前，将其统一转换为 UTF-8 编码格式。
...
- 详细 REST API 接口参考：[REST API - 网关](../../guides/api.md)

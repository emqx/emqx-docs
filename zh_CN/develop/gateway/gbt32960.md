...
从 EMQX 5.4 开始，您可以通过 Dashboard、REST API 和配置文件 `base.hocon` 来配置和启用 GB/T 32960 网关。

::: tip

如果您在集群中运行 EMQX，则通过 Dashboard 或 REST API 所做的设置将影响整个集群。如果您只想更改一个节点的设置，请使用 [`base.hocon`](../../guides/configuration/configuration.md) 进行配置。
...
关于 GB/T 32960 命令与 MQTT 消息之间的转换细节，请参阅 [GB/T 32960 数据交换格式](./gbt32960_data_exchange.md)。
...
- **代理协议**：设置当 EMQX 部署在[负载均衡器](../../guides/cluster/lb.md)后，是否开启 Proxy Protocol V1/2。默认值：`false`。
...
GB/T 32960 网关仅支持 [HTTP Server 认证](../../guides/access-control/authn/http.md)。网关使用 `login` 命令中的信息，并使用 `vin` 代码作为 `clientid` 来为客户端生成认证字段：

- 客户端 ID：`vin` 代码
- 用户名：`vin` 代码

您可以通过 REST API 或 `base.hocon` 为 GB/T 32960 网关创建 HTTP 认证：

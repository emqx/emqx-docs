...
网关内无独立的发布订阅的权限控制，其对主题的权限控制需要统一在[授权](../../guides/access-control/authz/authz.md)中管理。
...
2. 在弹出的**添加监听器**页面中进行以下配置：
   ...
   **双向认证**开关默认开启。您需要配置相关的 TLS 证书，TLS 密钥和 CA 证书信息，可以通过输入文件内容或使用**选择文件**按钮上传。详细信息请参阅 [开启 SSL/TLS 连接](../../guides/network/emqx-mqtt-tls.md)。
...
从 EMQX 6.3.0 开始，`proxy_address_header` 和 `proxy_port_header` 均默认为 `""`。只有显式配置转发请求头名称后，NATS WebSocket 监听器才会使用请求头值，否则使用 TCP 对端地址和端口。

如果 NATS 监听器位于受信任代理之后，并且该代理会覆盖转发请求头，可在 `base.hocon` 中配置请求头名称。例如：
...
对于 WSS 监听器，请改用 `gateway.nats.listeners.wss.<listener-name>.websocket`。EMQX 使用每个已配置请求头中第一个（最左侧的）条目。如果请求头不存在或值无效，EMQX 使用对应的 TCP 对端地址或端口。
...
2. 点击 **+ 添加认证**，选择 **Password-Based** 作为**认证方式**，并选择 **HTTP 服务**作为**数据源**，点击**下一步**。然后在**配置参数**中，您可以设置身份验证规则。对于每一项配置的具体解释，您可以参考[使用 HTTP 服务进行密码认证](../../guides/access-control/authn/http.md)。
...
```
其他类型的认证器的配置格式参考每种 [EMQX 认证器](../../guides/access-control/authn/authn.md#emqx-认证器)的使用文档。

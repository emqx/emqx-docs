...
从 EMQX 6.0.4 开始，新建网关监听器的名称长度必须为 1 至 64 字节，以 ASCII 字母或数字开头，并且只能包含 ASCII 字母、数字、连字符（`-`）和下划线（`_`）。如果名称不符合上述要求，REST API 将返回 HTTP `400` 响应，错误码为 `BAD_REQUEST`。升级前已存在且名称超过 64 字节的网关监听器仍可更新配置或删除，但不能重命名。

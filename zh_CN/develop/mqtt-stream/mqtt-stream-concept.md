...
::: warning 与监听器挂载点不兼容
对于通过配置了[挂载点](../../guides/configuration/listener.md#挂载点mountpoint)的监听器连接的客户端，消息流回放不生效。EMQX 会先应用挂载点，再匹配 `$stream/` 前缀，因此对 `$stream/<name>` 的订阅会被当作对挂载后字面主题的普通订阅处理，且不会向客户端报告任何错误。消息写入同样受影响：发布的主题带有挂载点前缀，只有当消息流的主题过滤器包含该前缀时才能匹配。
:::
...
该行为与[共享订阅](../../get-started/messaging/mqtt-shared-subscription.md)不同。对于 `$share/<group>/t/#`，EMQX 会去掉前缀并对 `t/#` 授权；对于 `$stream/<name>/t/#`，EMQX 对完整的订阅主题过滤器授权。
...
自动创建对最后值消息流默认启用，对常规消息流默认禁用，但仅在 MQTT 消息流功能启用时生效。MQTT 消息流功能默认禁用（`streams.enable = false`）。在接受不受信任客户端的部署中，仅允许访问特定的预创建消息流（例如 `$stream/events`），并拒绝与 `$stream/#` 或 `$s/#` 匹配的所有其他订阅。或者，禁用自动创建并通过 Dashboard 或 REST API 创建消息流。参见[通过 Dashboard 自动创建消息流](./mqtt-stream-task.md#通过-dashboard-自动创建消息流)。

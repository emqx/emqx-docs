...
::: warning 与监听器挂载点不兼容
对于通过带有[挂载点](../../guides/configuration/listener.md#挂载点mountpoint)的监听器连接的客户端，文件传输不生效。EMQX 会先应用挂载点，再匹配 `$file/` 和 `$file-async/` 命令前缀，因此文件传输命令会作为普通消息路由到挂载后的字面主题。对 `$file-response/` 响应主题的订阅也会被添加挂载点，因此客户端无法收到 EMQX 发出的命令结果消息。此过程不会向客户端报告任何错误。
:::

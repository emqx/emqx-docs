...
从 EMQX 6.0.4 开始，命名空间 Dashboard 用户和 API 密钥无法使用文件传输端点列出文件或下载本地存储的文件，无论其角色或权限范围如何。请使用全局 Dashboard 用户身份或全局 API 密钥进行认证。在 S3 存储场景中，只有通过 EMQX 列出文件时需要 EMQX 认证；通过返回的预签名 URL 访问文件由 S3 控制。

...
会话实现的选择取决于会话过期间隔和 `durable_sessions.enable` 配置参数，该参数可以全局设置或按 [zone](../guides/configuration/configuration.md#zone-override) 设置。基于以下标准可以选择实现方式：
...
持久存储通过在 EMQX 集群内的多个节点间一致地复制会话元数据和 MQTT 消息，提供了强大的持久性和高可用性。可配置的[复制因子 (replication factor) ](../guides/durability/managing-replication.md#replication-factor)确定每条消息或会话的副本数量，使用户能够根据具体需求定制持久性和性能之间的平衡。
...
消息根据客户端进行隔离，并根据发布者的客户端 ID 存储在分片中。分片数量在 EMQX 初始启动时由 [n_shards](../guides/durability/managing-replication.md#number-of-shards) 配置参数决定。分片也是复制的单位。每个分片会根据 `durable_storage.messages.replication_factor` 在不同节点间进行一致性复制，确保每个副本中的消息集是相同的。
...
- [管理数据副本](../guides/durability/managing-replication.md)
- [配置和管理会话持久化](../guides/durability/management.md)

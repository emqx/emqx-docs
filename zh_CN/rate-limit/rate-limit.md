...
| subscribes_rate           | 订阅速率                               | 单个客户端连接在配置的时间间隔内可发送的最大 `SUBSCRIBE` 报文数量。 | 不处理该报文，也不创建其中的订阅；在 `SUBACK` 中为每个主题过滤器返回失败码。 |
| subscribes_burst          | 订阅突发速率                           | 单个客户端连接在突发情况下可额外发送的 `SUBSCRIBE` 报文数量。  | 同上               |
...
- 单个客户端每分钟最多发送 120 个 `SUBSCRIBE` 报文，并允许每 10 秒突发发送最多 10 个额外报文。
- 数据的最大发布速率为每个客户端每秒 1MB。
- 每 60 分钟允许在短时间内突发发送最多 100 兆字节。

### 订阅报文速率限制器

订阅报文速率限制分别应用于每个客户端连接。

该限流器统计 `SUBSCRIBE` 报文数量，而不是报文中主题过滤器的数量。包含多个主题过滤器的一个 `SUBSCRIBE` 报文只消耗一个配额。达到限制后，EMQX 不处理该报文，也不创建其中的订阅，但会保持客户端连接。对于 MQTT 5.0，EMQX 返回 `SUBACK`，其中每个主题过滤器对应的原因码均为 `Quota Exceeded`（0x97）。对于 MQTT 3.x，EMQX 在 `SUBACK` 中为每个主题过滤器返回失败返回码（0x80）。

`subscribes_rate` 的默认值为 `infinity`，即不限制订阅报文速率。为托管命名空间配置订阅报文速率限制后，该配置会覆盖命名空间内客户端所使用的监听器级订阅报文速率限制。详情请参阅[配置与管理命名空间](../multi-tenancy/configure-manage-namespace.md#命名空间的速率限制器)。
...
当 QoS 1 或 QoS 2 消息因达到投递速率限制而无法立即投递时，EMQX 会将该消息与后续 QoS 1 和 QoS 2 消息一起入队。这样可避免投递速率限制器本身导致后续消息越过先前消息。

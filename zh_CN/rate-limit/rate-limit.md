...
| subscribes_rate           | 订阅速率                               | 单个客户端发送 `SUBSCRIBE` 报文的数量。                        | 使用包含 `Quota Exceeded` 原因码（0x97）的 `SUBACK` 拒绝该报文 |
| subscribes_burst          | 订阅突发速率                           | 单个客户端在突发情况下可额外发送的 `SUBSCRIBE` 报文数量。      | 同上               |
...
该限流器统计 `SUBSCRIBE` 报文数量，而不是报文中主题过滤器的数量。包含多个主题过滤器的一个 `SUBSCRIBE` 报文只消耗一个配额。达到限制后，EMQX 不处理该报文，也不创建其中的订阅。EMQX 会保持客户端连接，并返回 `SUBACK`，其中每个主题过滤器对应的原因码均为 `Quota Exceeded`（0x97）。

...
- [#12711](https://github.com/emqx/emqx/pull/12711) 实现了消息验证功能。通过此功能，一旦为某些主题过滤器配置了验证，发布的消息将进行配置的检查。如果未通过验证，消息将被丢弃，根据配置客户端可能会被断开连接。

- [#12863](https://github.com/emqx/emqx/pull/12863) 现在可以通过在创建追踪模式时将格式化参数设置为 "json"，将追踪日志条目格式化为 JSON 对象。

- [#12827](https://github.com/emqx/emqx/pull/12827) 现在可以使用新的规则 ID 追踪过滤器以及客户端 ID 过滤器来追踪规则。为测试目的，还可以使用新的 HTTP API 端点（rules/:id/test）来人工应用规则，并在呈现后可选地停止其操作。
...
- [#12898](https://github.com/emqx/emqx/pull/12898) IoTDB 数据集成支持 iotdb 1.3.0 和批量插入（batch_size/batch_time）选项。

- [#12899](https://github.com/emqx/emqx/pull/12899) 添加了对命名空间和键调度策略的支持。
...
- [#12871](https://github.com/emqx/emqx/pull/12871) 修复了被撤离节点的启动过程。以前，如果一个节点被撤离并在没有停止撤离的情况下停止，它将无法重新启动。

...
- 现在 Kafka 消费组里的 MQTT 主题字段支持包含占位符的模板了。

  比如 Kafka 消费到消息里 key 为 "a"，配置的 MQTT 主题为 "topic/${key}"，那么在消息转发的是，MQTT 主题会被替换为 "topic/a"。

- 现在 “消息重发布” 动作支持在添加 MQTT 属性和用户属性了。

  “消息重发布” 动作支持了两个新的字段："MQTT Properties" 和 "User Property Value"，都是键值对的格式，并且键和值都支持占位符。

...
- 改进从旧版本（4.2或更早）导入数据时的告警日志。
  ...
  在此改进之前，如果从旧版本（4.2及之前）导入到4.4版本，由于缺少认证类型，内置认证部分的数据将会被丢弃，
  但用户无法从日志中清楚地获知失败原因。
  在此修改之后，将会在 EMQX 日志中提示用户改用命令行工具做数据导入，并指定认证类型：
...
- 修复当 TDEngine SQL 语句带有换行符时创建规则失败的问题。
...
- 修复 HTTP 接口 `/load_rebalance/:node/start` 返回的错误信息没有正确编码的问题。

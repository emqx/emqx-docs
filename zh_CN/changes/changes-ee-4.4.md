...
- 更平滑的全局垃圾回收。
  ...
  EMQX 会周期性地对所有进程进行垃圾回收（由 `node.global_gc_interval` 配置项控制，默认为 15 分钟），以避免在极端情况下，默认的垃圾回收策略无法及时回收堆外二进制内存，导致内存占用过高，但 CPU 使用率会因此出现周期性的明显波动。新的全局垃圾回收机制会在每次全局垃圾回收时，分批次对进程进行垃圾回收，从而降低 CPU 使用率的波动。此优化仅在 `node.global_gc_interval` 配置为大于一分钟时生效。

### 修复

- 修复在从旧版本热升级到 4.4.28 或 4.4.29 时部分连接会断开的问题。

  从旧版本热升级到 4.4.28 或 4.4.29 的过程中，可能会出现部分客户端连接断开的问题，断线的客户端数量与当前节点上的消息量正相关。

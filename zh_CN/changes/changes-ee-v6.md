...
EMQX 6.1.0 引入了消息流、增强的命名空间能力、新增数据集成以及集中式证书管理。

**消息流**

EMQX 消息流提供了一种基于主题过滤器的持久化消息集合，并支持显式的生命周期管理。所有匹配消息流主题过滤器的消息都会自动追加到流中，从而支持具备顺序保证的消息消费，并允许多个消费者同时读取。
...
- [#16326](https://github.com/emqx/emqx/pull/16326) 实现了消息流。消息流是一种通过主题过滤器标识的持久化消息集合。消息流具有显式的生命周期管理，任何与消息流主题过滤器匹配的已发布消息都会自动追加到该消息流中。消息流支持具备顺序保证的消息消费，并且允许对同一消息流进行多次消费。

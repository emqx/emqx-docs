...
由于 Stomp 协议的连接消息中已经定义了用户名和密码的概念，GB/T 32960 支持多种认证器类型，例如：
...
GB/T 32960 网关使用 `login` 命令中的信息，并使用 `vin` 代码作为 `clientid` 来为客户端生成认证字段：

- 客户端 ID：`vin` 代码
- 用户名：`vin` 代码

您还可以使用 HTTP API 为 GB/T 32960 网关创建内置数据库认证：
...
当没有启用任何认证器时，所有 GB/T 32960 客户端都被允许登录。

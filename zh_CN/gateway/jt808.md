...
您可以通过 Dashboard，REST API 或配置文件 `emqx.conf` 启用和配置 JT/T 808 网关。
...
如果 EMQX 在集群中运行，通过 Dashboard 或 REST API 的配置将在整个集群中生效。如果您只希望针对单个节点进行设置，可以使用配置文件 `emqx.conf` 配置网关。
...
您也可以通过 REST API 或 `emqx.conf` 来启用和配置 JT/T 808 网关，例如：

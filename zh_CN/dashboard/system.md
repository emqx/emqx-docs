...
在 Dashboard 中创建用户时，**使用角色默认权限**默认开启。启用后，用户会自动获得其角色和所属范围（全局或命名空间）的默认权限。以后角色的默认权限发生变化时，新权限也会自动生效，无需再次编辑用户。

关闭**使用角色默认权限**后，界面会显示**权限范围**字段，可为用户显式分配权限范围。未选择时，不授予任何权限范围。

**命名空间**选项默认关闭。对于全局管理员，保持关闭会创建全局用户；打开后选择一个命名空间，可创建命名空间用户。用户的所属范围决定所选角色适用的默认权限。
...
在 EMQX 6.0.4 之前创建且使用混合权限范围列表的用户可以继续工作。下次自定义权限范围列表时，该列表必须仅包含等同管理员权限的范围，或仅包含其他权限范围。使用角色默认权限或不授予任何权限范围时，不受此限制。
...
2. 配置用户：
   ...
   - **密码**：用户登录密码。
   - **角色**：选择**管理员**或**查看者**。
   - **使用角色默认权限**：默认开启。保持开启将使用所选角色和所属范围（全局或命名空间）的默认权限；关闭后可显式分配权限范围。

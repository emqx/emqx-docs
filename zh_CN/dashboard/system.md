...
- **自定义受限权限**：从角色可用的非管理权限范围中选择，例如 `connections`、`publish`、`data_integration`、`monitoring` 和 `mfa_management`。如果将权限范围列表留空，用户不能访问受权限范围保护的 API。

如需创建命名空间用户，打开**命名空间**开关并选择一个命名空间。命名空间用户不使用 3 选 1 的**权限模式**字段，而是默认开启**使用角色默认权限**。关闭该开关后，可从该角色在命名空间中允许持有的全部权限范围中选择。如果将**权限范围**留空，用户不能访问受权限范围保护的 API。
...
<!-- TODO: 将 ./assets/user_scopes.png 替换为更新后的裁剪截图，展示全局用户的“权限模式”字段。 -->
...
在 EMQX 6.0.4 之前创建且使用混合权限范围列表的用户可以继续工作。在 Dashboard 中编辑此类全局用户时，表单会显示兼容性警告，并要求在保存前选择**管理权限范围**、**自定义受限权限**或**角色默认权限**。显式权限范围列表必须仅包含等同管理员权限的范围，或仅包含其他权限范围。使用角色默认权限或不授予任何权限范围时，不受此限制。
...
2. 配置用户：
   ...
   - **使用角色默认权限**：打开**命名空间**后显示，并默认开启。保持开启将使用所选命名空间角色的默认权限；关闭后可显式分配权限范围。
   - **权限范围**：关闭**使用角色默认权限**后显示。选择要授予的权限范围；未选择时，不授予任何权限范围。
3. 点击**创建**完成用户配置。

...
如果您允许委派管理员管理命名空间范围内的资源，建议在可用版本中优先启用 `rule_engine.ssrf` 来校验规则引擎管理的出站目标；如果还需要运行时网络边界，再增加主机级出站访问控制，例如 `iptables` 或 `nftables`。参见[结合规则引擎策略与防火墙规则缓解 SSRF](../deploy/cluster/security.md)。

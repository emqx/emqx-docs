...
在 Dashboard 中创建用户时，可以启用**使用角色默认权限**，也可以在**权限范围**中选择具体范围。通过 REST API 创建用户时，省略 `scopes` 字段会应用角色的默认权限范围：

- **管理员**：拥有全部权限范围，包括上述 4 个登录专属权限范围。
- **查看者**：拥有全部通用 API 密钥权限范围；`mfa_management` 仅在显式分配时才会被授予。

通过 REST API 更新用户时，省略 `scopes` 将保留用户已存储的权限范围设置。
...
| 请求值 | 创建用户 | 更新用户 |
| --- | --- | --- |
| 省略字段 | 应用角色的默认权限范围。 | 保留用户已存储的权限范围设置。如果请求同时更改角色，已存储的权限范围必须适用于新角色。 |
| `"unset"` | 使用角色的隐式默认权限范围，不存储显式列表。 | 清除显式列表，恢复角色的隐式默认权限范围。 |
| 与角色默认值相同的列表 | 按 `"unset"` 处理，列表顺序不影响比较结果。 | 按 `"unset"` 处理，列表顺序不影响比较结果。 |
| 空列表 `[]` | 拒绝访问所有受权限范围控制的 API 区域。 | 将已存储的设置替换为拒绝访问所有受权限范围控制的 API 区域。 |
| 其他显式列表 | 校验并存储该列表。 | 校验并存储该列表。 |

使用 `"unset"` 可以使用户的权限范围在 EMQX 升级并新增权限范围后继续与角色默认值保持一致。对于没有显式权限范围列表的用户，API 可以返回 `"unset"`。API 客户端执行读取、修改、写回操作时，可以原样回传该值，例如只修改用户备注时。

::: warning 等同管理员权限的范围必须单独使用

以下权限范围等同管理员权限（EMQX 校验错误消息中称为 `privilege scopes`）：
...
从 EMQX 6.0.4 开始，全局 Dashboard 用户的显式权限范围列表不能将上述任何等同管理员权限的范围与其他权限范围组合。创建或更新请求将返回 HTTP 400。需要等同管理员权限时，仅使用上述权限范围；需要受限访问时，仅使用其他权限范围。`mfa_management` 不会授予等同管理员的权限。

在 EMQX 6.0.4 之前创建且使用混合权限范围列表的用户可以继续工作。后续请求显式提交权限范围列表时，该列表必须仅包含等同管理员权限的范围，或仅包含其他权限范围。省略 `scopes` 字段、使用 `"unset"`、使用与角色默认值相同的列表或使用空列表 `[]` 时，不会按显式混合列表处理。

此互斥规则不适用于命名空间 Dashboard 管理员。命名空间管理员的权限范围组合仍受命名空间角色兼容性和端点级授权控制。

:::

#### 角色变更与权限范围兼容性

变更用户角色时，EMQX 会检查该用户当前的权限范围是否与新角色兼容。如果不兼容，请求将返回 HTTP 400。要解决此问题，请在同一请求中提供一个对新角色有效的 `scopes` 列表。
...
- 权限范围**不能被自定义**，始终使用隐式的完整管理员权限范围。通过用户 API 更新其他字段时，可以省略 `scopes` 字段、使用 `"unset"`，或使用与管理员角色默认值相同的列表。
- 描述和密码**可以**正常修改。

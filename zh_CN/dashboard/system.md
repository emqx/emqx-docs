...
如果您允许委派管理员管理命名空间范围内的资源，应限制 EMQX 节点的出站网络访问，避免管理功能访问任意内部或外部端点。例如，可使用主机级防火墙规则（如 `iptables` 或 `nftables`）仅放行经批准的目标地址。参见[使用防火墙规则缓解 SSRF](../deploy/cluster/security.md)。

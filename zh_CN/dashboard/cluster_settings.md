...
<!-- XXX: UI 文案草稿及截图待替换
以下地址信息相关文案依据 emqx/emqx#18609 已认可的 Dashboard 设计编写，发布前需核对实际列表和编辑表单。请替换 `./assets/config-listener-list.png`，展示仅指定端口的绑定，并展开地址信息提示，使配置值、解析地址和地址来源清晰可见。获得新截图前保留现有图片，完成替换后删除此说明。
-->

监听器列表显示配置中的绑定值。通过监听地址旁的信息图标，可以查看：
...
通过编辑表单中监听地址旁的信息图标，可以查看地址信息。输入框保留配置值，例如仅指定端口时仍显示 `1883`，不会自动填入解析后的 IP。查看提示不会修改配置。

<!-- XXX: 截图待补充
核对实际编辑表单后，新增 `./assets/config-listener-edit-bind-info.png`。展示仅指定端口的绑定，并展开地址信息提示，使配置值和解析地址同时可见。请使用实际 UI 截图，不保留设计讨论中的红框标注。发布前用截图替换此说明。
-->

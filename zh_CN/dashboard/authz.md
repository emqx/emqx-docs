...
EMQX 提供了强大的权限控制能力，而 EMQX Dashboard 提供了开箱即用的授权管理功能，通过可视化页面来快速配置 EMQX 的授权机制，且无需编写代码或手动编辑配置文件，实现对 MQTT 客户端连接后的发布和订阅操作进行权限控制，实现各个级别与各类场景下的安全配置。

在左侧的访问控制菜单下，用户可以进入到授权页面，授权页面中用户可以快速创建和管理创建好的各类授权的配置资源。

## 创建授权

在授权页面下的右上角，点击 `创建` 按钮，即可进入到授权的页面。创建一个授权需要选择一种数据源来配置和存储权限数据。
...
使用外置数据库来存储授权规则，支持选择主流数据库包括：`MySQL`、`PostgreSQL`、`MongoDB`、`Redis` 等。
...
选择外置数据库的话，需要配置能访问到的数据库地址，数据库名称，用户名密码，以及认证相关配置，填写如何从数据库中获取授权数据的 SQL 语句或其它查询语句等。当发布和订阅操作时，会从数据库中查询相关的授权数据来判断是否能通过权限规则。以 MySQL 为例：

![image](./assets/authz-mysql-config.png)

更多关于 MySQL 或其它外置数据库的配置详情，请访问 [MySQL](../security/authz/mysql.md) 或查看使用其它数据库。
...
当授权配置创建完成后，我们可以在授权列表中查看和管理。列表中我们可以查看授权的数据源，数据源状态，比如外置数据库没有正常部署成功和连接到的话，可看到目前数据源状态为已断开。鼠标悬浮到该字段的话，可以查看更多 EMQX 集群内所有节点连接到该数据源的状态。点击是否启用开关，可以快速开启和关闭该授权配置。
...
当需要修改授权配置的数据源时，可以点击列表页中的 `设置`，在设置页面可以修改配置信息，如需要修改 ACL 的权限规则内容时，或当外置数据库的一些连接信息发生或查询语句发生变化时。

...
1. 如果需要持久化 Docker 容器中生成的数据 ，请将以下目录挂载到容器外部，这样即使容器被删除数据也不会丢失：
   ...
   即使容器默认将日志输出到控制台，也应挂载 `/opt/emqx/log`。正常运行时该目录为空，但节点异常终止时 Erlang 虚拟机会在此写入崩溃转储文件。未挂载该目录时，该文件会随容器一起丢失。该目录必须对容器内的 `emqx` 用户（UID 1000）可写。详情请参见 [Docker 中的崩溃转储文件](../configuration/logs.md#docker-中的崩溃转储文件)。

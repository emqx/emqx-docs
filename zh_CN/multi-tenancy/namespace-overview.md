...
- **管理员用户隔离**
  ...
  如果允许委派管理员配置命名空间范围内的资源，建议在可用版本中优先启用 `rule_engine.ssrf` 来校验规则引擎管理的出站目标；如果还需要在运行时限制 Broker 主机可访问的地址范围，再通过 `iptables` 或 `nftables` 等主机级防火墙规则限制出站访问。参见[结合规则引擎策略与防火墙规则缓解 SSRF](../deploy/cluster/security.md)。
...
命名空间级管理访问的设计目标是受信任的内部委派，而不是为彼此不受信任的租户提供强隔离边界。

如果您启用了该功能，建议在可用版本中优先启用 `rule_engine.ssrf` 作为规则引擎管理的出站目标的第一道防线；如果部署还需要运行时网络边界，再在 EMQX 所在主机上实施出站网络访问控制。尤其建议：

- 仅允许访问已批准的业务端点，例如部署中使用的身份提供商、Webhook 接收端或连接器后端。
...
关于何时仅启用 `rule_engine.ssrf` 通常就足够、何时还应增加 `iptables` 或 `nftables` 规则，参见[结合规则引擎策略与防火墙规则缓解 SSRF](../deploy/cluster/security.md)。

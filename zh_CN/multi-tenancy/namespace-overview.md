...
- **客户端 ID 覆盖**
  ...
  当不同命名空间下的客户端之间互不信任时（例如每个命名空间对应一个外部客户或独立组织），**必须**配置 `mqtt.clientid_override`，将命名空间附加到客户端 ID 上，实现客户端 ID 隔离。

  MQTT broker 在整个集群范围内维护单一的客户端 ID 空间：当新连接使用了已有会话的客户端 ID 时，broker 会断开已有会话。如果没有客户端 ID 隔离，恶意客户端可以在自己的命名空间中使用与其他租户客户端相同的客户端 ID，从而持续将其踢下线、劫持其持久会话，或对该租户造成拒绝服务。仅依靠身份认证和访问控制并不能阻止此类攻击 —— 两端客户端在各自命名空间内均能通过认证，而会话抢占发生在连接层，早于主题级 ACL 生效。

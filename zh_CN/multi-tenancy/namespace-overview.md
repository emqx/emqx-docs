...
- **管理员用户隔离**
  ...
  如果允许委派管理员配置命名空间范围内的资源，应将 EMQX 节点的出站网络访问限制为仅可访问经批准的目标地址。这有助于在管理员不应通过 Broker 所在主机访问任意服务的场景中降低 SSRF 暴露面。例如，可使用主机级防火墙规则（如 `iptables` 或 `nftables`）。参见[使用防火墙规则缓解 SSRF](../deploy/cluster/security.md)。
...
如果您启用了该功能，应在 EMQX 所在主机上实施出站网络访问控制，确保 Broker 只能访问部署实际需要的外部服务。尤其建议：
...
关于基于 `iptables` 或 `nftables` 的示例做法，参见[使用防火墙规则缓解 SSRF](../deploy/cluster/security.md)。

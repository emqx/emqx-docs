...
::: tip 面向不受信任网络的 Broker 的安全建议

对于暴露在不受信任网络或公网中的 Broker，建议将 `deny_action` 从默认值 `ignore` 改为 `disconnect`。这样，发布或订阅被拒绝的客户端会被直接断开连接，而不能继续在同一连接上探测其他主题。配合[连接抖动检测](../flapping-detect.md)，反复重连并触发拒绝的客户端会被自动封禁一段时间。

注意：`deny_action` 是全局授权配置，无法按监听器单独设置。由于 `disconnect` 同样会断开尝试了被拒绝操作的合法客户端，此设置适用于客户端只会访问其授权主题的部署场景；同时应合理调整连接抖动检测的阈值，避免正常的重连风暴导致客户端被封禁。

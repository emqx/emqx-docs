...
- 对于暴露在不受信任网络中的 Broker，建议设置 `authorization.deny_action = disconnect`（默认值为 `ignore`），使发布或订阅被拒绝的客户端被直接断开连接，而不能继续在同一连接上探测其他主题。配合[连接抖动检测](./flapping-detect.md)，反复重连并触发拒绝的客户端会被自动封禁。由于 `deny_action` 是全局授权配置，也会断开尝试了被拒绝操作的合法客户端，此设置适用于客户端只会访问其授权主题的部署场景，并应调整连接抖动检测阈值，避免正常重连风暴导致客户端被封禁。详见[授权](./authz/authz.md)。

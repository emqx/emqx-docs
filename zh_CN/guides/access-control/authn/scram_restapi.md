...
- **客户端属性**：可选，您可以使用 `client_attrs` 字段指定[客户端属性](../../../develop/client-attributes/client-attributes.md)。键和值都必须为字符串。
...
5. 按照以下说明设置数据源：
   ...
   - **URL**：输入 HTTP 服务的 URL 地址。URL 主机不支持占位符，请使用固定主机名或 IP 地址。

   - **调用条件**：一个 Variform 表达式，用于控制是否将此 HTTP 服务认证器应用于客户端连接。该表达式会根据客户端的属性（例如 `username`、`clientid`、`listener` 等）进行评估。如果表达式的结果为字符串 `"true"`，则会触发认证器。否则，认证器将被跳过。有关调用条件的更多信息，请参见[认证器调用条件](./authn.md#认证器调用条件)。

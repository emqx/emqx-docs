...
如果需要在同一认证链中同时使用 JWT 认证器和密码认证器，可以为 JWT 认证器配置以下调用条件：
...
对于 MQTT 客户端，认证后端可以在认证成功结果中返回 `clientid_override`。该字段必须是包含完整替代客户端 ID 的非空字符串。EMQX 在认证后、打开客户端会话前应用该值。如果该字段不存在或值为空，EMQX 将继续使用此前确定的客户端 ID。

当替代客户端 ID 依赖仅在认证后可用的可信信息（例如认证后端返回的租户标识）时，请使用此机制。在多租户部署中，可以将租户标识纳入有效客户端 ID，避免不同租户使用相同原始客户端 ID 时发生会话冲突。如果 EMQX 可以在认证前根据连接信息生成替代客户端 ID，请改用 `mqtt.clientid_override`。有关多租户部署中如何选择机制、具体执行顺序和失败处理行为，参见[客户端 ID 隔离](../../multi-tenancy/namespace-global-settings.md#客户端-id-隔离)。

一个 MQTT 连接只应使用一种客户端 ID 覆盖机制。如果同时配置两种机制，认证结果覆盖会在稍后执行，并替换 `mqtt.clientid_override` 生成的客户端 ID。

::: warning

从 EMQX 6.3.1 开始，网关协议不支持通过认证结果覆盖客户端 ID。对于通过网关接入的客户端，EMQX 会忽略 `clientid_override`，保留由网关协议确定的客户端 ID，并记录包含 `gateway_authn_clientid_override_not_supported` 的警告日志。

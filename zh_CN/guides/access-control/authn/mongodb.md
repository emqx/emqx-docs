...
添加用户名为 `emqx_u`、密码为 `public`、盐值为 `salt_foo123`、散列方式为 `sha256` 且超级用户标志为 `true` 的用户示例：
...
2. 在**客户端认证**页面，点击**+ 创建**。
3. 依次选择**认证方式**为 `Password-Based`，**数据源**为 `MongoDB`，点击**下一步**进入**配置参数**页签：
...
4. 根据以下说明配置数据源：
   ...
   - 认证加密算法相关的配置：
     ...
     - **Client ID 覆盖字段名**：指定 MongoDB 查询结果中的字段名，该字段用于覆盖 MQTT 客户端连接的 Client ID。通过此字段，用户可以根据认证结果动态地分配唯一的 Client ID，以避免因多个客户端使用相同 ID 而导致的会话抢占问题（如实现基于租户或用户名的多租户隔离）。
     - **salt 字段名**：指定 MongoDB 数据中盐的字段。
   - **调用条件**：一个 Variform 表达式，用于控制是否将此 MongoDB 认证器应用于客户端连接。该表达式会根据客户端的属性（例如 `username`、`clientid`、`listener` 等）进行评估。如果表达式的结果为字符串 `"true"`，则会触发认证器。否则，认证器将被跳过。有关调用条件的更多信息，请参见[认证器调用条件](./authn.md#认证器调用条件)。
   - **启用TLS**：如果要启用TLS，请打开切换按钮。有关启用 TLS 的更多信息，请参见[网络和 TLS](../../network/overview.md)。
   - **查询 Filter**：MongoDB filter，用于过滤数据，支持[认证占位符](./authn.md#认证占位符)。
   - **高级设置**：配置以下可选连接设置：
     - **认证源**：指定存储用户认证凭据的 MongoDB 数据库。例如，如果用户创建在 `admin` 数据库中，请填写 `admin`。
     - **使用旧协议**：选择 EMQX 如何使用 MongoDB 旧协议。`auto` 表示由 EMQX 自动检测服务器支持的协议，`true` 表示使用旧协议，`false` 表示不使用旧协议。默认值为 `auto`。
     - **连接池大小**：指定从 EMQX 节点到 MongoDB 服务器的并发连接数。默认值为 `8`。
     - **连接超时**：指定建立连接的超时时间。支持的单位包括毫秒、秒、分钟和小时。默认值为 `20` 秒。

...
从 EMQX 6.3.1 开始，新建 MQTT 监听器的名称必须符合以下要求：

- 长度为 1 至 64 字节。
- 以 ASCII 字母或数字开头。
- 只能包含 ASCII 字母、数字、连字符（`-`）和下划线（`_`）。

如果名称不符合上述要求，EMQX 将拒绝创建监听器。升级前已存在且名称超过 64 字节的 MQTT 监听器仍可更新配置或删除，但不能重命名。
...
支持的取值及启动行为参见[默认监听地址](../access-control/security-profile.md#默认监听地址)。官方 Docker 镜像会设置自己的默认值，参见 [Docker 中的监听地址](../../get-started/deploy/install-docker.md#docker-中的监听地址)。
...
有关 MQTT over WebSocket 的工作原理及其典型使用场景的概述，请参阅 [MQTT over WebSocket](../../develop/connect-emqx/mqtt-over-websocket.md)。
...
查看 `listen_on` 确认配置的绑定值，查看 `resolved_address` 确认解析后的 IP，并通过 `resolved_address_from` 了解地址来源。同时检查 `running`，确认监听器是否正在运行：已停止的监听器仍可返回解析地址。各字段的含义（包括 `resolved_address` 为空时的含义）参见[监听地址信息](../cli.md#监听地址信息)。

### 通过 REST API 查询监听器

如需通过 REST API 检查单个监听器，使用 `GET /api/v5/listeners/:id`，例如 `GET /api/v5/listeners/tcp:default`。响应返回处理该请求的节点上的地址信息。请求需按要求进行 [API 认证](../api.md#认证)。
...
- 如果前面有代理，但代理是将自身观察到的地址**追加**到入站 `X-Forwarded-For` 请求头之后，而不是覆盖或去除它（大多数代理默认为追加行为，例如 NGINX 的 `$proxy_add_x_forwarded_for`），那么 EMQX 读取的最左侧条目仍然是客户端提供的值，源 IP 依然可以被伪造。应将代理配置为使用其观察到的地址覆盖该请求头，或改用 [Proxy Protocol](../cluster/lb.md)，或将上述配置项设置为 `""`。
...
| 功能 | 主题前缀 |
| --- | --- |
| [延迟发布](../../get-started/messaging/mqtt-delayed-publish.md) | `$delayed/` |
| [文件传输](../../develop/file-transfer/introduction.md) | `$file/`、`$file-async/`、`$file-response/` |
| [消息队列](../../develop/message-queue/message-queue-concept.md) | `$queue/` |
| [MQTT 消息流](../../develop/mqtt-stream/mqtt-stream-concept.md) | `$stream/` |
| [集群连接](../../develop/cluster-linking/introduction.md) | `$LINK/` |
...
[共享订阅](../../get-started/messaging/mqtt-shared-subscription.md)（`$share/{group}/`）以及[排他订阅](../../get-started/messaging/mqtt-exclusive-subscription.md)（`$exclusive/`）是例外：它们可以与挂载点配合使用。EMQX 会先解析这些订阅前缀，然后再应用挂载点，因此挂载点只会被添加到内部的实际主题过滤器上。例如，通过配置了挂载点 `mp/` 的监听器订阅 `$share/g/t`，会以主题 `mp/t` 加入共享订阅组 `g`。

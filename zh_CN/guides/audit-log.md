...
EMQX 审计日志支持记录来自 [Dashboard](./dashboard/introduction.md) 、[REST API](../guides/api.md) 以及 [CLI](./cli.md) 和控制台的变更性操作，例如 Dashboard 用户登录，对客户端、访问控制以及数据集成等资源的修改。而指标获取、客户端列表查询等只读操作则不会被记录。
...
![EMQX 审计日志列表](./dashboard/assets/audit-log-list.png)
...
| operate_id       | 字符 | 请求的 REST API 路径，请参考 [REST API](../guides/api.md)。   |

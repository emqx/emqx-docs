...
从 EMQX 6.0.3 开始，在启用命名空间角色的部署中，日志追踪访问按命名空间进行隔离：

- 命名空间用户在 `GET /trace` 响应中仅能看到属于其命名空间的追踪记录。
- 以下单条追踪操作在追踪记录属于其他命名空间时返回 `404 Not Found`，不会泄露跨命名空间追踪是否存在：
  - `PUT /trace/:name/stop`
  - `GET /trace/:name/download`
  - `GET /trace/:name/log`
  - `GET /trace/:name/log_detail`
  - `DELETE /trace/:name`
- 批量删除操作（`DELETE /trace`）仅限全局管理员使用，命名空间用户将收到 `403 Forbidden`。

全局管理员可查看和管理所有命名空间的追踪记录。
...
该追踪类型可以通过[测试规则](../../develop/data-integration/rule-get-started.md#测试规则)操作自动创建和管理。测试规则时，EMQX 会自动生成一个追踪任务，并在测试停止后自动删除。

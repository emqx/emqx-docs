...
`emqx_mria_lag` 表示复制节点分片落后于上游核心节点分片的事务数量，不是以秒为单位的时长。写入突增期间的短时峰值可能是正常现象。当该值持续高于代表性峰值流量期间观察到的最大值，或者该值与 Mria 队列指标持续呈正增长趋势时，应触发告警。按节点和 `shard` 对告警分组，因为某个分片可能异常，而其他分片仍能正常复制。有关各项 Mria 指标的详细信息，请参阅[集群监控和故障排查](../../develop/cluster/mria-introduction.md#集群监控和故障排查)。
...
- **建议措施：**检查连接、会话、队列和集成的增长情况。从 EMQX 6.3.0 开始，可以使用 [`emqx ctl session-top`](../cli.md#session-top) 识别保留 MQTT 消息载荷字节数最多或消息队列最长的会话。在操作系统终止进程之前，增加容量或消除增长来源。EMQX 内置系统内存告警的默认阈值为 70%。
...
3. **准备维护流程。**
   ...
   在冗余仍然可用时，使用 warning 告警安排维护。更改集群前，请确认备份可用、其余节点能够承载负载，并且告警系统运行正常。相关流程包括[备份与恢复](../backup-restore.md)、[节点疏散和集群负载重平衡](../cluster/rebalancing.md)以及 [EMQX Enterprise 滚动升级](../../get-started/deploy/rolling-upgrades.md)。

...
| actions.messages | 动作执行所处理的消息总数。由于单次动作执行可能批量处理多条消息，该值大于或等于 `actions.executed`。 |
...
你也可以通过 API 获取统计和指标。在 API 页面左侧导航栏中点击 **Metrics**，即可执行对应的 API 请求。关于如何使用 EMQX API，请参见 [REST API](../api.md)。

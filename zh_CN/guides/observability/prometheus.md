...
::: tip
从 EMQX 6.3.0 开始，Prometheus 指标由 `metrics` 功能门控控制。如果手动设置 `EMQX_FEATURES`，启用 `metrics` 时也会自动启用其依赖的 `dashboard`。认证和授权属于核心能力，不受功能门控控制。更多信息请参见[功能门控](../../get-started/deploy/feature-gates.md)。
:::
...
1. 在 EMQX 中创建具有 `monitoring` scope 的 [API 密钥](../api.md#创建-api-密钥)。

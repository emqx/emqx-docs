...
在节点信息区域点击右侧的**查看节点列表**，或点击顶部的**节点**页签，即可访问节点列表页面。该页面列出 EMQX 集群中的所有节点，并显示各节点的名称、状态、运行时长、版本、连接数、角色、安全配置模式、功能预设、Erlang 进程数、内存使用量和 CPU 负载。安全配置模式和功能预设在节点启动时确定。对比不同节点的这两列，可以发现集群中不一致的启动设置。

停止的节点和运行 6.3 之前版本的节点不会报告安全配置模式或功能预设。点击页面右上角的**刷新**按钮，可以更新节点信息。

![节点列表](./assets/node_list.png)

### 节点详情

在节点列表中，仅可以查看节点的部分基础信息，如需查看更多节点的完整信息，可以**名称**列中的节点名，进入到节点的详情页面。在详情页面中，您可以看到以下信息：

- 在**节点信息**卡片中，可以查看节点角色、安全配置模式、功能预设、最大文件描述符数、系统路径和日志路径。仅当启用文件日志时，才会显示日志路径。

- 在**节点统计**卡片中，可以看到一些当前节点的统计数据，包括：连接数，订阅数，主题数，保留消息数，会话数和共享订阅数等。数据统计的数值分为两部分，通过斜杠分割，左边的数据为实时的数据展示，右边为高水位线数据，即当前数据达到过的最大值。


![节点详情](./assets/node-detail.png)

...
::: warning 重要提示
SSRF 策略不校验其他连接器类型、连接器启停操作、连接器删除操作或运行时出站连接。如果 HTTP 或 MQTT 连接器的目标地址在连接器创建后被策略禁止，仍可重新启用该连接器。如果需要保护其他连接器类型、使策略变更应用于已保存的连接器配置、防御 DNS rebinding 或其他运行时地址变化，请使用 `iptables`、`nftables` 等主机级出站访问控制。完整指导请参见[结合规则引擎策略与防火墙规则防御 SSRF](../cluster/security.md#结合规则引擎策略与防火墙规则防御-ssrf)。
:::
...
<!-- XXX: 监听地址信息及截图待替换
Dashboard 尚未显示 `resolved_address` 或 `resolved_address_from`。UI 实现后，补充监听器列表如何展示配置中的绑定值及各节点的地址信息，并替换 `./assets/config-listener-list.png`，使新截图展示仅指定端口的绑定和实际实现的地址信息视图。获得新截图前保留现有图片，完成替换后删除此说明。
-->
...
从 EMQX 6.3.1 开始，新建监听器的名称长度必须为 1 至 64 字节，以 ASCII 字母或数字开头，并且只能包含 ASCII 字母、数字、连字符（`-`）和下划线（`_`）。如果名称不符合上述要求，EMQX 将拒绝创建监听器。详情参见[监听器名称要求](../configuration/listener.md#监听器名称要求)。
...
完成添加一个监听器后，可以在列表中看到该监听器。点击监听器名称进入到编辑页面，在该页面下可以修改该监听器的配置或删除一个监听器。可以修改监听地址，但监听器名称和类型不可修改。

<!-- XXX: 编辑表单地址信息及截图待补充
地址信息 UI 实现后，补充其在编辑表单中的行为说明，并新增 `./assets/config-listener-edit-bind-info.png`。截图应同时展示仅指定端口的绑定和实际实现的地址信息视图。请使用实际 UI 截图，不保留设计讨论中的红框标注。发布前用操作说明和截图替换此说明。
-->

...
从 EMQX 6.0.4 开始，新建监听器的名称长度必须为 1 至 64 字节，以 ASCII 字母或数字开头，并且只能包含 ASCII 字母、数字、连字符（`-`）和下划线（`_`）。如果名称不符合上述要求，EMQX 将拒绝创建监听器。详情参见[监听器名称要求](../configuration/listener.md#监听器名称要求)。

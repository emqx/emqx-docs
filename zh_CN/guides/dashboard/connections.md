...
您可以使用 [MQTTX](https://mqttx.app/zh) 作为客户端连接到 EMQX 进行发布订阅。也可使用[各语言提供的客户端库](../../develop/connect-emqx/introduction.md)快速实现客户端连接至 EMQX。在**客户端**页面，您可以查看到当前连接至服务器或会话尚未过期的客户端及某个客户端连接的详细信息和指标统计。
...
其中，客户端的 IP 地址显示为其 IP 与连接 EMQX 时所用端口的组合。
...
在客户端列表中点击某个客户端 ID，可进入该客户端的详情页面，查看其连接信息、会话状态及运行指标。页面顶部提供刷新和清除会话操作，用于手动更新连接数据或重置会话。

**客户端信息**页签在列表页基础信息之上，补充展示了更详细的连接属性，例如使用的 MQTT 协议版本、清除会话配置，以及（对已断开连接）最近一次断开时间。右侧的**会话信息**区域展示了会话相关数据，包括会话过期间隔、创建时间、堆内存占用、订阅数量、消息队列长度、飞行窗口大小以及 QoS 2 消息接收队列长度。

**指标**页签按流量、报文和消息三个维度展示客户端的运行统计信息，便于查看连接的实时或累计使用情况。

**当前订阅**页签列出了该客户端当前订阅的主题。用户可以通过**新建订阅**添加主题订阅，或在列表中对指定主题执行取消订阅操作。

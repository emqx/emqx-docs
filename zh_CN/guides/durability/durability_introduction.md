...
![MQTT 保留会话](../../develop/assets/session-persistence-list.png)
...
持久会话依赖持久存储来保存会话状态和消息。要了解该存储层的结构及其工作原理，请参阅[持久存储设计](../../develop/design/durable-storage.md)中的 *架构：后端与存储层次结构* 章节。
...
启用会话持久化后，EMQX 会将持久会话的元数据以及发送到持久会话的 MQTT 消息保存到磁盘。因此，必须为 EMQX 部署足够的存储容量。EMQX 数据目录必须使用本地文件系统，因为[内嵌持久存储后端](../../develop/design/durable-storage.md#内嵌后端)不支持 NFS、SMB/CIFS 等网络文件系统。为获得最佳吞吐量，请使用固态硬盘（SSD）存储。
...
如需深入了解 MQTT 持久会话背后的设计原理，请参阅[持久存储设计](../../develop/design/durable-storage.md)。

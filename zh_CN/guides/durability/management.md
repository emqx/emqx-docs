...
自 EMQX 6.0.2 起，持久存储引入了[数据库组](../../develop/design/durable-storage.md#持久存储数据库组)，用于支持节点级别的资源治理。数据库组允许对多个持久存储数据库进行统一治理，并共享资源使用限制，而不会改变其逻辑数据模型。

默认情况下，每个持久存储数据库都属于一个以该数据库自身命名的数据库组，并且每个这样的数据库组只包含这一个数据库，从而保持与早期版本一致的行为。

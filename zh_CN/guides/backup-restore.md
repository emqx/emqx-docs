...
- 与命名空间关联的内置数据库认证凭证和授权规则不能按单个命名空间导出或导入。如需备份或恢复这些记录，请使用全局备份。全局备份会统一处理所有命名空间的相关记录。
...
- 通过 Dashboard 或 `POST /api/v5/data/import` 导入数据时，集群中所有运行节点的主版本号和次版本号必须相同，补丁版本号可以不同。如果节点版本不满足此要求，或无法获取某个节点的版本，EMQX 会在导入开始前拒绝请求，并返回 HTTP `400` 响应，错误码为 `BAD_REQUEST`。错误消息会列出每个节点及其 `major.minor` 版本；无法获取版本时显示为 `unknown`。重试前，请确保 EMQX 能够获取所有运行节点的版本，并且这些节点的主版本号和次版本号相同。
- 如果启用了[核心节点 + 复制节点](../develop/cluster/mria-introduction.md)模式，数据导入只能在核心节点上进行。这不会影响实际的导入行为，因为数据将被复制到所有集群节点，包括核心节点和复制节点。在核心节点上进行操作可以确保正确导入数据。
- 数据文件不能被重命名。
...
从 EMQX 6.0.4 开始，将备份导入具体命名空间时，EMQX 仅应用该命名空间支持的配置根。如果备份中的命名空间配置包含认证、授权、ExHook、监听器等集群级配置根，EMQX 会跳过这些配置根，不会将其写入全局配置。全局导入仍会应用集群级配置。

EMQX 会记录 `data_import_skipped_global_roots` 警告，其中列出被跳过的配置根，并继续导入同一备份中该命名空间支持的配置根。跳过这些集群级配置根不会导致整个导入失败。通过 CLI 导入时，命令输出也会列出被跳过的配置根；通过 Dashboard 或 REST API 导入时，可在 EMQX 日志中查看该警告。
...
全局管理员可以管理全局范围或具体[命名空间](./multi-tenancy/namespace-overview.md)下的备份文件。命名空间管理员可以管理和下载所属命名空间下的备份文件，但不能访问全局范围或其他命名空间下的备份文件。
...
2. 如果您是全局管理员，在**命名空间**选择器中选择**全局**或具体命名空间。页面会加载选定范围内的备份文件。选择具体命名空间后，确认列表上方的提示中显示了目标命名空间。

   命名空间管理员不会看到此选择器。EMQX 会将其备份操作限制在所属命名空间内。

3. 如需导出数据，点击**创建**。全局管理员只能在**全局**视图中创建备份。全局管理员选择具体命名空间后，**创建**按钮不可用。命名空间管理员可以为所属命名空间创建备份。
...
4. 如需将备份文件添加到选定范围，点击**上传**。上传文件不会恢复其中的数据。上传到具体命名空间时，成功消息中会显示目标命名空间。上传成功后，确认文件已显示在备份文件列表中。

5. 在**操作**列中点击以下按钮，管理备份文件：

   - **下载**：将备份文件下载到本地。
   - **删除**：从选定范围中删除备份文件。
   - **恢复**：将备份文件导入选定范围。如果选择了具体命名空间，请在确认恢复前核对确认对话框中的目标命名空间。恢复成功后，确认成功消息中显示了目标命名空间。

在具体命名空间视图中，上传、下载、删除和恢复操作均作用于该命名空间。全局管理员可以在此视图中管理和恢复备份文件，但不能创建备份。
...
全局管理员未传入 `namespace` 时，操作作用于全局范围内的备份文件。对于命名空间调用方，EMQX 会忽略该参数，并将操作限制在调用方所属的命名空间内。

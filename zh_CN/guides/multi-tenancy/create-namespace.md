...
从 EMQX 6.3.0 开始，这两种创建方式均不能使用 `multi_tenancy.deny_namespaces` 列表中的名称。默认列表和配置方法请参见[禁止使用的命名空间名称](./namespace-global-settings.md#禁止使用的命名空间名称)。
...
如果名称在 `multi_tenancy.deny_namespaces` 列表中，EMQX 将拒绝请求并返回 HTTP 400。

## 自动创建命名空间

在 EMQX 中，命名空间也可以在客户端连接时自动创建。

自动创建命名空间并非直接基于 `client_attrs.tns` 字段本身，而是依赖于**[命名空间来源](./namespace-global-settings.md#命名空间来源)**配置。当客户端连接到 EMQX 时，系统会根据已配置的命名空间来源规则，从客户端连接信息中生成 `tns`（租户命名空间）属性。如果解析出的名称在 `multi_tenancy.deny_namespaces` 列表中，EMQX 将拒绝连接并返回 `not_authorized`。否则，在允许自动创建且对应命名空间不存在时，EMQX 将自动创建该命名空间。

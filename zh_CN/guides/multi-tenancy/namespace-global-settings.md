...
EMQX 不会使用命名空间和原始客户端 ID 两个独立字段共同标识会话，而是在全局范围内仅使用一个有效客户端 ID 标识会话。为避免不同命名空间中的客户端使用相同原始客户端 ID 时发生冲突，客户端 ID 隔离会生成全局唯一的有效客户端 ID，通常将命名空间作为前缀添加到原始客户端 ID。客户端仍发送原始客户端 ID，EMQX 在内部使用覆盖后的 ID 标识会话。

这些客户端 ID 覆盖机制仅适用于 MQTT 客户端连接。从 EMQX 6.3.1 开始，网关协议会保留由协议确定的客户端 ID，并忽略认证后端返回的 `clientid_override`。

...
该插件从 EMQX 6.3.0 开始可用。请先安装并启动该插件，再在 Rule SQL 中使用 `maptab_lookup`。

Mapping table 通过 `emqx ctl maptabs` CLI 从 JSON 文件加载，并存储在 EMQX 的复制数据库中。规则可使用 `maptab_lookup` SQL 函数查询这些表。函数签名和 SQL 示例参见[内置 SQL 函数](../../../../develop/data-integration/rule-sql-builtin-functions.md)。

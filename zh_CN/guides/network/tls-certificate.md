...
从 EMQX 6.1.4 开始，如果仍有配置引用证书包，则无法删除该证书包。

1. 点击**删除**。Dashboard 将打开**确认删除?**对话框。
2. 点击**取消**关闭对话框，证书包不会被删除；或点击**确定**继续。
3. 点击**确定**后，将出现以下结果之一：
   - 如果没有依赖项，证书包将被删除。
   - 如果存在一个或多个依赖配置，EMQX 将拒绝删除。**证书包正在使用中**对话框会列出引用该证书包的配置。点击**前往页面**，更新或删除这些配置后再重新删除证书包；或点击**取消**关闭对话框，证书包不会被删除。
...
从 EMQX 6.1.4 开始，如果所有命名空间中有任意配置引用托管证书包，EMQX 将拒绝删除整个证书包或包内的单个文件。请求将返回 `400 BAD_REQUEST` 错误。
...
如需删除证书包中的单个文件，请使用相同的端点，并通过 `kind` 查询参数指定文件。依赖检查同时适用于整个证书包和包内的单个文件。

`force_delete` 查询参数已从 API schema 中移除，无法再绕过依赖检查。如果现有客户端仍发送 `force_delete=true`，EMQX 将忽略该参数，并按普通删除请求处理。
...
重新发起删除请求前，请更新或删除 `referencing_configs` 中列出的所有配置，使这些配置不再引用该证书包。

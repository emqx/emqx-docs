...
| 函数名 | 函数功能 |        参数         | 返回值 |
| -------- | -------------- |--------------- | --------------------------- |
| `format_date` | 获取时间，并按指定格式输出时间字符串 | 1. 时间精度 <br />2. 时间偏移 <br />3. 时间格式字符串 | 字符串 |
| `format_date` | 将整数时间戳按指定格式输出时间字符串 | 1. 时间精度 <br />2. 时间偏移 <br />3. 时间格式字符串 <br />4. 整数时间戳 | 字符串 |
| `date_to_unix_ts` | 使用指定格式解析时间字符串 | 1. 时间精度 <br />2. 时间偏移 <br />3. 时间格式字符串 <br />4. 字符串 | 整数 |
...
用户可以通过该字符串指定输出时间的时区，
比如东八时区可以用"+08:00"来表示，如果输入为空，则使用系统默认时区

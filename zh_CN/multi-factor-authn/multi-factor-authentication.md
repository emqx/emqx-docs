...
从 EMQX 5.10.4 开始，管理员可以为通过 SSO（如 SAML、OIDC、LDAP）登录的用户强制启用 MFA。这确保了 SSO 用户也受到双因素认证的保护。

### 为 SSO 后端启用强制 MFA

要求通过特定 SSO 后端登录的所有用户启用 MFA：

1. 在 Dashboard 中，进入 **系统** -> **SSO**。
2. 选择 SSO 后端（如 SAML、OIDC）。
3. 启用 **强制 MFA** 开关。
4. 点击 **保存**。

<!-- TODO: 添加 SSO 后端配置页面中强制 MFA 开关的截图 -->

### SSO MFA 工作流程

当 SSO 后端启用 `force_mfa` 后：

1. **首次登录**：SSO 认证成功后，用户需要使用认证器应用（如 Google Authenticator 或 Authy）扫描二维码来绑定 TOTP。用户必须输入有效的 TOTP 验证码完成绑定。
2. **后续登录**：SSO 认证成功后，用户需要输入 TOTP 验证码才能访问 Dashboard。
3. **管理员豁免**：管理员可以为特定用户豁免 MFA（参见下方 [管理 SSO 用户的 MFA](#管理-sso-用户的-mfa)）。被豁免的用户即使在 `force_mfa` 启用时也可以无需 TOTP 登录。

### 管理 SSO 用户的 MFA

管理员可以通过 Dashboard 管理 SSO 用户的 MFA：

#### 禁用 MFA（豁免用户）

豁免某个 SSO 用户的 MFA：

1. 在 Dashboard 中，进入 **系统** -> **用户**。
2. 找到 SSO 用户，点击 **MFA 设置**。
3. 点击 **禁用**。

<!-- TODO: 添加 MFA 设置对话框中禁用按钮的截图 -->

该用户将不再需要输入 TOTP 验证码，即使其 SSO 后端启用了 `force_mfa`。

#### 重置 MFA（重新绑定 TOTP）

如果用户丢失了认证器应用的访问权限（如手机丢失），管理员可以重置其 MFA：

1. 在 Dashboard 中，进入 **系统** -> **用户**。
2. 找到 SSO 用户，点击 **MFA 设置**。
3. 点击 **重置**。

<!-- TODO: 添加 MFA 设置对话框中重置按钮的截图 -->

下次启用了 `force_mfa` 的 SSO 登录时，用户将被提示重新扫描二维码设置 TOTP。

#### 重新启用 MFA（取消豁免）

取消用户的 MFA 豁免，要求其在下次登录时重新设置 TOTP：

1. 在 Dashboard 中，进入 **系统** -> **用户**。
2. 找到 SSO 用户，点击 **MFA 设置**。
3. 点击 **启用**。

<!-- TODO: 添加 MFA 设置对话框中启用按钮的截图 -->

用户将在下次 SSO 登录时被提示设置 TOTP。

...
其中，关于每个认证插件的配置及用法，可参考 `扩展插件 (Plugins)`_ 关于认证部分。


.. note:: auth 插件可以同时启动个。每次检查的时候，按照优先级从高到低依次检查，同一优先级的，先启动插件的先检查。(内置默认的 acl.conf 优先级为-1，各个插件默认为0)
...
.. NOTE::
    ...
    HTTP 发布接口采用 `Basic <https://en.wikipedia.org/wiki/Basic_access_authentication>`_ 认证

...
Erlang/OTP最初是爱立信为开发电信设备与系统设计的编程语言平台，电信设备(路由器、接入网关、...)典型设计是通过背板连接主控板卡与多块业务板卡的分布式系统。

Erlang/OPT语言平台的分布式系统，由分布互联的Erlang运行系统组成，每个Erlang运行系统被称为节点(Node)，节点(Node)间通过消息传递的方式通信::
...
Erlang节点由唯一的节点名称标识，节点间通过该名称进行通信寻址。

节点(Node)
----------

例如在本机启动四个Erlang节点，节点名称分别为:
...
Erlang节点间通过一个设置相同的cookie进行相互连接认证。

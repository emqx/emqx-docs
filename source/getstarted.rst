...
MQTT是一个设计得非常出色的传输层协议，在移动消息、物联网、车联网、智能硬件甚至能源勘探等领域有着广泛的应用。1个字节报头、2个字节心跳、QoS支持的消息等设计，非常适合在低带宽、不可靠网络、嵌入式设备上应用。

不同的应用场景有不同的系统要求，用户使用emqttd消息服务器前，应该按自己的应用进行测试，而不是简单的连接压力测试。例如下述不同应用的主要侧重:

1. Android消息推送: 推送主题设计与消息广播。

2. 移动即时消息应用: 消息收发确认与离线处理。
  
3. 智能硬件应用: 控制消息的往返时延(latency)。

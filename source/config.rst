...
配置 Kubernates API 服务器列表，以 ',' 进行分隔:

.. code-block:: properties

    cluster.k8s.apiserver = http://10.110.111.204:8080

配置帮助查找集群中的 EMQ 节点的服务名称:
...
配置 MQTT/WebSocket/SSL 每秒最大创建连接数:

.. code-block:: properties

    listener.wss.external.max_conn_rate = 1000

配置 MQTT/WebSocket/SSL 连接的速率限制:

.. code-block:: properties

    ## listener.wss.external.rate_limit = 1024,4096

配置 MQTT/WebSocket/SSL 监听器下的连接所使用的 Zone:

.. code-block:: properties

    listener.wss.external.zone = external

配置 MQTT/WebSocket/SSL 监听器的挂载点:

.. code-block:: properties

    ## listener.wss.external.mountpoint = devicebound/

配置 MQTT/WebSocket/SSL 监听器的访问控制规则:
